...
assert.match(circleRecalculated.nextRun.pc.realm,/^[2-5]서클$/,'Circle must be recalculated only after inherited axes are applied');assert.equal(circleRecalculated.nextRun.creation.fateStart.origin.realm,circleRecalculated.nextRun.pc.realm,'persisted Origin and materialized PC must share the final evaluation');assert.equal(circleRecalculated.receipt.cost,6,'progressive stat costs must be 1 + 2 + 3');
