...
assert.deepEqual(filterTurnHookChoices('지도를 확인한다.',{importance:'routine',scene:[{kind:'narration',text:'지도에는 숲, 항구, 성채가 표시되어 있고 한 곳으로 향해야 한다.'}],state_delta:{},choices:['숲으로 간다.','항구로 간다.','성채로 간다.']}),['숲으로 간다.','항구로 간다.','성채로 간다.'],'scene-grounded destination alternatives survive without decision vocabulary');
...
assert.deepEqual(filterTurnHookChoices('목검과 방패를 선택한다.',{importance:'routine',event_progress:{event_instance_id:'assessment#1',active_beat:'stance'},scene:[{kind:'narration',text:'선택한 장비가 놓였다.'}],state_delta:{},choices:['자세를 다시 준비한다.','장비를 정리한다.','잠시 기다린다.']}),[],'event_progress alone does not promote a procedural follow-up menu into a player decision');
