...
coincidentChoiceTurn={scene:[{kind:'dialogue',speaker_key:'artemis',text:'훈련을 계속할지 잠시 쉴지 고르겠나?'}],state_delta:{advance_minutes:60,items_add:['미검증 훈련 보상']},choices:['계속 훈련한다','잠시 쉰다','질문한다'],event_progress:null};
coincidentChoiceTurn.time_execution=choiceExecution(coincidentChoiceTurn,{minutes:60,completed:[],interrupted:'action_1'});
const validOrdinaryChoiceAtSchedule=applySceneMomentumTimeFloor({action:'2시간 동안 훈련한다.',saveState:coincidentScheduleSave},coincidentChoiceTurn,'game');
assert.equal(validOrdinaryChoiceAtSchedule.reconciliationReason,'schedule-boundary','an exact required schedule outranks a valid ordinary choice receipt at the same minute');
assert.deepEqual(coincidentChoiceTurn.choices,[],'a valid but schedule-unowned choice receipt cannot suppress the exact required schedule');
assert.deepEqual(coincidentChoiceTurn.state_delta.items_add,[],'effects owned by neither the interrupted action nor the schedule fail closed');
assert.equal(validOrdinaryChoiceAtSchedule.runtimeChoicesTrusted,false,'the rejected ordinary receipt cannot create runtime choice authority at the schedule boundary');
coincidentChoiceTurn={scene:[{kind:'dialogue',speaker_key:'artemis',text:'기사과 필수 수업이 시작된다. 참석하겠나?'}],state_delta:{advance_minutes:60},choices:['수업에 참석한다','사정을 설명한다','교관에게 묻는다'],event_progress:null};
coincidentChoiceTurn.time_execution=choiceExecution(coincidentChoiceTurn,{minutes:60,completed:[],interrupted:'action_1',boundaryEventId:'coincident-class'});
const scheduleOwnedChoiceReceipt=applySceneMomentumTimeFloor({action:'2시간 동안 훈련한다.',saveState:coincidentScheduleSave},coincidentChoiceTurn,'game');
assert.deepEqual(coincidentChoiceTurn.choices,['수업에 참석한다','사정을 설명한다','교관에게 묻는다'],'a choice receipt structurally owned by the exact schedule retains its player decision');
assert.equal(scheduleOwnedChoiceReceipt.runtimeChoicesTrusted,true,'an exact schedule-owned choice remains authoritative at runtime');
coincidentChoiceTurn={scene:[{kind:'dialogue',speaker_key:'artemis',text:'기사과 필수 수업이 시작된다. 참석하겠나?'}],state_delta:{advance_minutes:60},choices:['수업에 참석한다','사정을 설명한다','교관에게 묻는다'],event_progress:null};
coincidentChoiceTurn.time_execution=choiceExecution(coincidentChoiceTurn,{minutes:60,completed:[],interrupted:'action_1',boundaryEventId:'coincident-class',boundaryKind:'schedule'});
const verifiedScheduleReceiptChoices=applySceneMomentumTimeFloor({action:'2시간 동안 훈련한다.',saveState:coincidentScheduleSave},coincidentChoiceTurn,'game');
assert.deepEqual(coincidentChoiceTurn.choices,['수업에 참석한다','사정을 설명한다','교관에게 묻는다'],'a verified schedule-kind receipt retains the decision it structurally locates');
assert.equal(verifiedScheduleReceiptChoices.runtimeChoicesTrusted,true,'schedule-kind receipt choices remain authoritative during runtime synthesis');
assert.deepEqual(runtimeSynthesisTurn(coincidentChoiceTurn,verifiedScheduleReceiptChoices).choices,['수업에 참석한다','사정을 설명한다','교관에게 묻는다'],'runtime synthesis cannot drop choices owned by a verified schedule receipt');
coincidentChoiceTurn={scene:[{kind:'dialogue',speaker_key:'artemis',text:'훈련을 마쳤으니 기사과 필수 수업에 참석하겠나?'}],state_delta:{advance_minutes:60},choices:['수업에 참석한다','사정을 설명한다','교관에게 묻는다'],event_progress:null};
coincidentChoiceTurn.time_execution=choiceExecution(coincidentChoiceTurn,{minutes:60,completed:[],interrupted:'action_1',boundaryEventId:'coincident-class',boundaryKind:'schedule'});
const contradictoryScheduleReceipt=applySceneMomentumTimeFloor({action:'2시간 동안 훈련한다.',saveState:coincidentScheduleSave},coincidentChoiceTurn,'game');
assert.doesNotMatch(JSON.stringify(coincidentChoiceTurn.scene),/훈련을 마쳤으니/,'a schedule-owned decision does not trust narration that contradicts its incomplete action clause');
assert.deepEqual(coincidentChoiceTurn.choices,['수업에 참석한다','사정을 설명한다','교관에게 묻는다'],'rejecting schedule narration does not discard its receipt-owned choices');
assert.equal(contradictoryScheduleReceipt.runtimeSceneTrusted,false,'contradictory schedule narration cannot enter runtime authority');
assert.deepEqual(runtimeSynthesisTurn(coincidentChoiceTurn,contradictoryScheduleReceipt).choices,['수업에 참석한다','사정을 설명한다','교관에게 묻는다'],'receipt-owned schedule choices survive independent narration sanitization');
coincidentChoiceTurn={scene:[{kind:'narration',text:'한 시간 훈련을 모두 마쳤다.'},{kind:'narration',text:'기사과 필수 수업이 시작되었다.'},{kind:'dialogue',speaker_key:'artemis',text:'훈련을 계속할지 고르겠나?'}],state_delta:{advance_minutes:60,active_events_add:['coincident-class'],scheduled_events_remove:['coincident-class']},choices:['계속 훈련한다','잠시 쉰다','질문한다'],event_progress:{event_instance_id:'coincident-class',active_beat:'start',completed_beats:[]}};
coincidentChoiceTurn.time_execution=choiceExecution(coincidentChoiceTurn,{minutes:60,completed:[],interrupted:'action_1'});
const verifiedScheduleWithMixedCompletion=applySceneMomentumTimeFloor({action:'2시간 동안 훈련한다.',saveState:coincidentScheduleSave},coincidentChoiceTurn,'game');
assert.equal(verifiedScheduleWithMixedCompletion.reconciliationReason,'schedule-boundary','a valid ordinary choice receipt still yields to its independently verified schedule');
assert.equal(coincidentChoiceTurn.event_progress?.event_instance_id,'coincident-class','the independently verified schedule progress survives unrelated false completion prose');
assert.deepEqual(coincidentChoiceTurn.state_delta.active_events_add,['coincident-class'],'the verified nonterminal schedule remains active');
assert.deepEqual(coincidentChoiceTurn.state_delta.scheduled_events_remove,['coincident-class'],'the verified started schedule may leave the pending bucket');
...
turn={scene:[{kind:'narration',text:'10분 동안 기본 자세를 반복했다.'},{kind:'dialogue',speaker_key:'artemis',text:'훈련을 마쳤다. 이제 완료 보상을 고르겠나?'}],state_delta:{advance_minutes:10,items_add:['미완료 훈련 보상']},choices:['보상을 받는다','다음 훈련을 고른다','쉰다']};
turn.time_execution=choiceExecution(turn,{minutes:10,completed:[],interrupted:'action_1'});
const contradictoryValidPrompt=applySceneMomentumTimeFloor({action:'1시간 동안 훈련한다.',saveState:{pc:{name:'카인'},world:{date:'1285-03-01',time:'09:00'},scheduleContext:{due:[],upcoming:[]}}},turn,'game');
assert.deepEqual(turn.state_delta.items_add,[],'a valid interruption receipt still drops effects from its incomplete action');
assert.doesNotMatch(JSON.stringify(turn.scene),/훈련을 마쳤다|완료 보상/,'a prompt contradicting the receipt-owned incomplete clause is removed from the visible decision scene');
...
turn={scene:[{kind:'narration',text:'10분 동안 기본 자세를 반복했다.'},{kind:'dialogue',speaker_key:null,speaker_name:'훈련장 경비병',text:'나는 훈련을 마쳤다. 이제 무엇을 하겠나?'}],state_delta:{advance_minutes:10},choices:['계속 훈련한다','경비병에게 묻는다','쉰다']};
turn.time_execution=choiceExecution(turn,{minutes:10,completed:[],interrupted:'action_1'});
const minorNpcOwnedCompletionPrompt=applySceneMomentumTimeFloor({action:'1시간 동안 훈련한다.',saveState:{pc:{name:'카인'},world:{date:'1285-03-01',time:'09:00'},scheduleContext:{due:[],upcoming:[]}}},turn,'game');
assert.match(JSON.stringify(turn.scene),/나는 훈련을 마쳤다/,'first-person completion in schema-valid minor-NPC dialogue remains owned by speaker_name');
assert.equal(minorNpcOwnedCompletionPrompt.runtimeSceneTrusted,true,'an unregistered NPC speaker does not become a PC completion claim');
turn={scene:[{kind:'dialogue',speaker_key:'pc',speaker_name:'카인',text:'나는 훈련을 마쳤다. 다음 훈련을 계속할까?'}],state_delta:{advance_minutes:10},choices:['계속 훈련한다','그만둔다','쉰다']};
turn.time_execution=choiceExecution(turn,{minutes:10,completed:[],interrupted:'action_1'});
const playerOwnedCompletionPrompt=applySceneMomentumTimeFloor({action:'1시간 동안 훈련한다.',saveState:{pc:{name:'카인'},world:{date:'1285-03-01',time:'09:00'},scheduleContext:{due:[],upcoming:[]}}},turn,'game');
assert.doesNotMatch(JSON.stringify(turn.scene),/나는 훈련을 마쳤다/,'the player actor identity cannot hide its own false completion claim behind dialogue ownership');
assert.equal(playerOwnedCompletionPrompt.runtimeSceneTrusted,false,'a contradictory player-owned prompt fails closed against the structured receipt');
...
assert.match(JSON.stringify(turn.scene),/지금 어떻게 대응하겠나/,'a valid interruption prompt that does not claim completion remains visible');
assert.equal(alignedValidPrompt.runtimeSceneTrusted,true,'a receipt-consistent interruption prompt remains trusted');
turn={scene:[{kind:'dialogue',speaker_key:'artemis',text:'훈련을 마쳤으니 이제 무엇을 하겠나?'}],state_delta:{advance_minutes:10},choices:['계속 훈련한다','쉰다','교관에게 묻는다']};
turn.time_execution=choiceExecution(turn,{minutes:10,completed:[],interrupted:'action_1'});
const sameQuestionCompletionPrompt=applySceneMomentumTimeFloor({action:'1시간 동안 훈련한다.',saveState:{pc:{name:'카인'},world:{date:'1285-03-01',time:'09:00'},scheduleContext:{due:[],upcoming:[]}}},turn,'game');
assert.doesNotMatch(JSON.stringify(turn.scene),/훈련을 마쳤으니/,'a completion claim inside the same question sentence is checked without its interrogative punctuation');
assert.equal(sameQuestionCompletionPrompt.runtimeSceneTrusted,false,'a same-sentence completion contradiction cannot enter runtime scene authority');
turn={scene:[{kind:'dialogue',speaker_key:'artemis',text:'붉은 약병과 푸른 약병 중 어느 것을 고르겠나?'}],state_delta:{advance_minutes:10},choices:['첫 번째','두 번째','다시 살핀다']};
...
const capSchedule={id:'cap-class',title:'기사과 다음 날 필수 수업',date:'1285-03-02',time:'09:00',kind:'academic',status:'scheduled'},capScheduleSave={pc:knightPc,world:{date:'1285-03-01',time:'09:00'},scheduledEvents:[capSchedule],scheduleContext:{due:[],upcoming:[capSchedule]}};
turn={scene:[{kind:'dialogue',speaker_key:'artemis',text:'수업에 참석하겠나?'}],choices:['참석한다','계속 기다린다','묻는다'],state_delta:{advance_minutes:1440}};
const capScheduleIntent=applySceneMomentumTimeFloor({action:'25시간 기다리고 8시간 잔다',saveState:capScheduleSave},turn,'game');
assert.equal(capScheduleIntent.reconciliationReason,'turn-limit','the canonical one-turn cap remains authoritative when a schedule starts at the exact same minute');
assert.equal(deriveTimedActionRuntime({},capScheduleIntent,'25시간 기다리고 8시간 잔다',turn,'game').remaining_minutes,540,'an exact-cap schedule cannot discard the resumable committed remainder');
turn={scene:[{kind:'narration',text:'하루의 진행 한계에 도달했고 기사과 필수 수업 시각도 되었다.'}],choices:[],state_delta:{advance_minutes:1440}};
const capScheduleWithoutChoiceIntent=applySceneMomentumTimeFloor({action:'25시간 기다리고 8시간 잔다',saveState:capScheduleSave},turn,'game');
assert.equal(capScheduleWithoutChoiceIntent.reconciliationReason,'turn-limit','turn-limit ownership does not depend on the model returning choices at the exact schedule tie');
assert.equal(deriveTimedActionRuntime({},capScheduleWithoutChoiceIntent,'25시간 기다리고 8시간 잔다',turn,'game').remaining_minutes,540,'the no-choice exact-cap tie also preserves the committed remainder');
const capConsequence={id:'consequence:cap-warning',status:'deferred',event_consequence:{version:'1.0',event_name:'상한 경보',target_bucket:'active',reason:'정확히 하루 뒤 경보가 발현한다',secret_level:0,due_at:'1285-03-02T09:00',expires_at:'1285-03-05T09:00'}},capConsequenceScene=[{kind:'narration',text:'하루가 지나자 상한 경보가 울렸다.'}],capConsequenceLifecycle={selected_id:capConsequence.id,status:'resolved',attribution_safe:true,preserved_delta:{hooks_update:[{id:capConsequence.id,status:'resolved'}]}};
turn={scene:[...capConsequenceScene],choices:[],state_delta:{advance_minutes:1980,hooks_update:[{id:capConsequence.id,status:'resolved'}]}};
const capConsequenceIntent=applySceneMomentumTimeFloor({action:'25시간 기다리고 8시간 잔다',saveState:{pc:knightPc,world:{date:'1285-03-01',time:'09:00'},hooks:[capConsequence],scheduleContext:{due:[],upcoming:[]},scheduledEvents:[]}},turn,'game',capConsequenceLifecycle,capConsequenceScene);
assert.equal(capConsequenceIntent.reconciliationReason,'turn-limit','the canonical one-turn cap outranks a resolved consequence at the exact same minute');
assert.equal(turn.state_delta.advance_minutes,1440,'an overlong model timeline is reduced to the canonical cap even when the consequence resolves there');
assert.equal(deriveTimedActionRuntime({},capConsequenceIntent,'25시간 기다리고 8시간 잔다',turn,'game').remaining_minutes,540,'an exact-cap consequence cannot discard the resumable committed remainder');
assert.deepEqual(turn.state_delta.hooks_update,[{id:capConsequence.id,status:'resolved'}],'turn-limit precedence preserves the independently verified consequence lifecycle');
assert.deepEqual(runtimeSynthesisTurn(turn,capConsequenceIntent).scene,capConsequenceScene,'runtime state retains the exact-cap verified consequence manifestation');
...
const consequenceOwnedFalsePrompt=[{kind:'dialogue',speaker_key:'emily',text:'훈련을 마쳤으니 흔들리는 봉인을 먼저 확인할까?'}];
turn={scene:[...consequenceOwnedFalsePrompt],state_delta:{advance_minutes:60,hooks_update:[{id:coincidentConsequenceHook.id,status:'resolved'}]},choices:['봉인을 확인한다','에밀리를 돕는다','물러난다'],event_progress:null};
turn.time_execution=choiceExecution(turn,{minutes:60,completed:[],interrupted:'action_1',boundaryEventId:coincidentConsequenceHook.id});
...
const safeConsequenceRow={kind:'narration',text:'동시에 봉인이 흔들리기 시작했다.'},mixedConsequenceRows=[safeConsequenceRow,...consequenceSecondFalsePrompt];
turn={scene:[...mixedConsequenceRows],state_delta:{advance_minutes:60,hooks_update:[{id:coincidentConsequenceHook.id,status:'resolved'}]},choices:['봉인을 확인한다','에밀리를 돕는다','물러난다'],event_progress:null};
turn.time_execution=choiceExecution(turn,{minutes:60,completed:['action_1'],interrupted:'action_2',decisionCompletions:['action_2'],boundaryEventId:coincidentConsequenceHook.id,boundaryKind:'consequence'});
const mixedSafeConsequenceIntent=applySceneMomentumTimeFloor({action:'1시간 훈련하고 1시간 훈련한다.',saveState:{world:{date:'1285-03-01',time:'09:00'},hooks:[coincidentConsequenceHook],scheduleContext:{due:[],upcoming:[]},scheduledEvents:[]}},turn,'game',{selected_id:coincidentConsequenceHook.id,status:'resolved',attribution_safe:true},mixedConsequenceRows);
const mixedSafeRuntime=runtimeSynthesisTurn(turn,mixedSafeConsequenceIntent);
assert.deepEqual(mixedSafeRuntime.scene,[safeConsequenceRow],'runtime state preserves sanitized safe consequence rows while dropping only the contradictory selected row');
assert.doesNotMatch(JSON.stringify(mixedSafeRuntime.scene),/두 번째 훈련을 마쳤으니/,'safe consequence runtime state excludes the rejected completion prompt');
...
const unownedConsequenceChoiceIntent=applySceneMomentumTimeFloor({action:'2시간 동안 훈련한다.',saveState:{world:{date:'1285-03-01',time:'09:00'},hooks:[coincidentConsequenceHook],scheduleContext:{due:[],upcoming:[]},scheduledEvents:[]}},turn,'game',{selected_id:coincidentConsequenceHook.id,status:'resolved',attribution_safe:true},verifiedConsequenceScene);
assert.deepEqual(turn.choices,[],'an ordinary valid choice receipt cannot lend unrelated choices to a coincident consequence');
assert.equal(unownedConsequenceChoiceIntent.runtimeChoicesTrusted,false,'consequence-unowned choices cannot enter awaiting-player runtime state');
const mixedConsequencePrompt=[{kind:'dialogue',speaker_key:'emily',text:'훈련을 마쳤고 동시에 봉인이 흔들렸다. 어느 쪽을 확인할까?'}];
turn={scene:[...mixedConsequencePrompt],state_delta:{advance_minutes:60,hooks_update:[{id:coincidentConsequenceHook.id,status:'resolved'}]},choices:['봉인을 확인한다','에밀리를 돕는다','물러난다'],event_progress:null};
const mixedConsequenceIntent=applySceneMomentumTimeFloor({action:'2시간 동안 훈련한다.',saveState:{world:{date:'1285-03-01',time:'09:00'},hooks:[coincidentConsequenceHook],scheduleContext:{due:[],upcoming:[]},scheduledEvents:[]}},turn,'game',{selected_id:coincidentConsequenceHook.id,status:'resolved',attribution_safe:true},mixedConsequencePrompt);
assert.doesNotMatch(JSON.stringify(turn.scene),/훈련을 마쳤고/,'a consequence scene mixed with an unowned action-completion claim is fail-closed to deterministic boundary context');
assert.doesNotMatch(JSON.stringify(runtimeSynthesisTurn(turn,mixedConsequenceIntent).scene),/훈련을 마쳤고/,'mixed unowned completion narration cannot persist through consequence runtime state');
