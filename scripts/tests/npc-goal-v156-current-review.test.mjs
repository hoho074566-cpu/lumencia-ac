...
test('structured format requires and preserves the TPP execution ownership receipt',()=>{
  ...
  const patched=patchGoalV2StructuredFormat({instructions:'base',text:{format}}),receipt={version:'1.0',plan_used:true,boundary_kind:'choice',boundary_minutes:60,completed_clause_ids:['action_1'],interrupted_clause_id:'action_2',decision_scene_index:0,decision_completion_clause_ids:[],boundary_event_id:null,effect_owners:[],scalar_contributions:[]};
  ...
  assert.ok(patched.text.format.schema.properties.time_execution.required.includes('decision_completion_clause_ids'),'strict output requires clause ownership for decision completion claims');
