...
assert.match(proactive.params.input,/"kind":"npc-goal-initiative"/);
assert.match(proactive.params.input,/"desire":"PC에게 실기 평가 결과를 직접 확인한다\."/);
assert.match(proactive.params.input,/"next_action":"평가 결과를 먼저 묻는다\."/,'the bounded next action must reach the selected Goal Tick as data');
assert.doesNotMatch(proactive.params.input,/MODE=goal-tick|ORDER=USER_ACTION_FIRST|GUARDS=/,'Goal Tick must not become a writer-facing execution checklist');
assert.match(proactive.params.instructions,/새로운 PC 의도·목표·대사·감정/,'player sovereignty must survive as one consolidated hard invariant');
...
assert.match(maximumGuarded.params.input,/"kind":"npc-goal-initiative"/,'the selected initiative fact must survive the minimum authority budget');
assert.doesNotMatch(maximumGuarded.params.input,/ORDER=USER_ACTION_FIRST|GUARDS=/,'maximum-length goal fields must not restore the removed narrative checklist');
...
assert.match(pressured.params.input,/"kind":"npc-goal-initiative"/,'Goal Tick authority must survive the minimum adaptive routine budget as compact data');
assert.ok(pressured.params.input.endsWith('주변을 살펴본다.'),'the committed ending of a long user action must remain the final authority');
assert.doesNotMatch(pressured.params.input,/ROLE=SCENE_WRITER|DEPTH>DISTANCE/,'the removed composition tail must stay absent under pressure');
