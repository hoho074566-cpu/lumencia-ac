...
assert.match(context,/buildHardExecutionFacts/);
assert.match(context,/classifySceneIntent/);
assert.match(context,/compactDirectorFactSignal/);
...
assert.doesNotMatch(context,/===== SCENE MOMENTUM HF1 =====|===== SCENE PURPOSE V1 =====|===== EXPLICIT SCENE EXIT CONDITION V1 =====|===== STRONGER TURN HOOK V1 =====/);
assert.doesNotMatch(context,/위 행동까지만 처리하고 PC의 다음 행동을 정하지 마라/);
assert.match(context,/일상적 신체 동작·필요한 이동·즉각 결과/);
assert.match(context,/Compress routine process; give important moments enough space/);
assert.doesNotMatch(context,/ROUTINE: DISTANCE > DEPTH|IMPORTANT SCENE: DEPTH > DISTANCE/);
