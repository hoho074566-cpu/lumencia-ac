...
const structuredPatchMatch = routerSource.match(/const STATE_RECEIPT_CONTRACT = String\.raw`[\s\S]*?\n}\n\nfunction installResponsesRouter\(\)/);
...
assert.match(patched.instructions, /\[HARD STATE RECEIPTS\]/, 'growth fields must be factual post-Scene receipts');
assert.doesNotMatch(patched.instructions, /Trait은 100 진척|서로 다른 이정표/, 'internal progression recipes must not choreograph Writer prose');
...
const minimumSave = JSON.parse(minimumText).hard_facts;
...
assert.equal(minimumSave.pc.awakening_candidates.trait['공명 감각'].progress, 8, 'awakening progress must survive the minimum route');
assert.equal(minimumSave.pc.awakening_candidates.trait['공명 감각'].milestones, 0, 'awakening milestone count must survive the minimum route');
...
const relevantAbilityFacts = JSON.parse(relevantAbilityRoute.params.input.split('===== THIN SCENE PACKET — CURRENT FACTS =====\n')[1].split('\n\n=====')[0]).hard_facts.pc;
assert.equal(relevantAbilityFacts.traits[directlyMentionedOverflowTrait].description, '특성 정의 9', 'directly relevant Trait definition must survive routing');
assert.equal(relevantAbilityFacts.authorities[directlyMentionedOverflowAuthority].description, '권능 정의 9', 'directly relevant Authority definition must survive routing');
assert.ok(Object.keys(relevantAbilityFacts.traits).length <= 8, 'relevance selection must preserve the existing Trait bound');
assert.ok(Object.keys(relevantAbilityFacts.authorities).length <= 8, 'relevance selection must preserve the existing Authority bound');
...
const maximalMinimumText = maximalGrowthRoute.params.input.split('===== THIN SCENE PACKET — CURRENT FACTS =====\n')[1].split('\n\n=====')[0];
const maximalMinimum = JSON.parse(maximalMinimumText).hard_facts.pc;
assert.ok(Object.prototype.hasOwnProperty.call(maximalMinimum.traits, directlyMentionedTrait), 'directly mentioned Trait must outrank unmentioned entries under pressure');
assert.ok(Object.prototype.hasOwnProperty.call(maximalMinimum.awakening_candidates.authority, directlyMentionedAwakening), 'directly mentioned awakening candidate must survive pressure compaction');
