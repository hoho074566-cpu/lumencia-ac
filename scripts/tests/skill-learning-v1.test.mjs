...
assert.match(contextRouter, /skill_candidates:boundedSkillCandidates/, 'thin hard facts must retain bounded learning candidate identities and progress only');
...
assert.doesNotMatch(router, /basis 없는 진척은 금지|\[SKILL LEARNING V1\]/, 'skill-learning quality rules must stay in hard validation instead of controlling Writer composition');
assert.match(router, /replaceStructuredEffectRows\(data\.turn,'skill_experience',mode==='auto'\?\[\]:filterExistingSkillExperience\(structuredEffectRows/, 'adapter must freeze AUTO experience, block legacy candidate leakage, and keep accepted ownership row-aligned');
...
const thinPc=(result)=>JSON.parse(result.params.input.split('===== THIN SCENE PACKET — HARD FACTS =====\n')[1].split('\n\n=====')[0]).canonical_facts.pc;
const minimumPc = thinPc(denseRouted);
assert.equal(minimumPc.skill_candidates['반월 보법'].progress, 8, 'active candidate progress must survive the mandatory minimum block');
assert.equal(minimumPc.skills['대검술'].grade, 'D', 'existing skill names must survive the mandatory minimum to prevent duplicate candidates');
...
const pressurePc = thinPc(pressureRouted);
assert.ok(Object.keys(pressurePc.skills).length <= 8, 'pressure routing must retain only a bounded relevant skill-name set');
assert.ok(Object.keys(pressurePc.skill_candidates).length <= 5, 'pressure routing must retain only bounded active candidates');
assert.ok(Object.values(pressurePc.skill_candidates).every((row) => Object.keys(row).length === 1 && Object.prototype.hasOwnProperty.call(row, 'progress')), 'mandatory candidates must retain progress without duplicating basis, timestamps, or history');
