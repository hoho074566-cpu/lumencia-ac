...
assert.match(app,/world: save\.world, creation: save\.creation, fateBook: save\.fateBook, pc: save\.pc/,'base compact request must carry persistent creation and Fate Book state');
...
const runtimePayload=Function('save',`${compactStateStableSource}; return compactStateStable();`)({id:'runtime-test',version:6,turnNumber:0,world:{},creation:commoner.creation,fateBook:{version:1,records:[]},pc:commoner.pc,usage:{},qualityTelemetry:{}});
...
assert.deepEqual(runtimePayload.fateBook,{version:1,records:[]},'deployed stable runtime request must carry persistent Fate Book state');
