...
const routerRules = context.match(/const CANON_KERNEL = String\.raw`([\s\S]*?)`;/)?.[1] || '';
const novelContract = context.match(/const MINIMAL_WRITER_CONTRACT = String\.raw`([\s\S]*?)`;/)?.[1] || '';
assert.match(novelContract, /writing the next scene of serialized fantasy fiction/);
assert.match(novelContract, /let NPCs, time, and the world move naturally/);
assert.match(novelContract, /never invent a new player intention, dialogue, emotion, or meaningful decision/);
assert.match(novelContract, /Compress routine process/);
assert.match(novelContract, /characters as people, not as functions/);
assert.match(routerRules, /System facts are authoritative/);
assert.match(routerRules, /USER ACTION is the player's exact chosen intent/);
assert.ok(routerRules.length + novelContract.length <= 1400, 'R2 production guidance must stay minimal');
