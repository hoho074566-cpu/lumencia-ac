...
assert.doesNotMatch(routed.params.input, /GUARDS=|TURN_PLAN=/, 'confirmed world facts must route as data instead of a narrative execution checklist');
...
assert.match(pressureRouted.params.input, /"kind":"public-world-result"/, 'the compact fact kind must survive minimum routing pressure');
assert.match(pressureRouted.params.input, /"fact":"마법과 정오 연구회 일정 종료가 공개적으로 확인됨"/, 'the exact confirmed fact must survive minimum routing pressure');
