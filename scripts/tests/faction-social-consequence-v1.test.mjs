...
assert.doesNotMatch(routed.params.input,/student_council|blue_knights|knight_department/,'unrelated faction state must stay out of the thin packet');
...
const denseRouted=routeOpenAIParams(
  {instructions,input:'===== TURN OPTIONS =====\nnormal\n===== AUTHORITATIVE SAVE_STATE =====\n{}'},
  {incoming:{action:`백장미회의 오래된 평판을 확인한다. ${'긴 행동 '.repeat(1200)}`.slice(0,5000),saveState:{turnNumber:11,world:{location:'academy'},sceneRuntime:{participants:['lucia'],faction_social:denseFactionState},npcInnerStates:{},routerFeedback:{routerVersion:'p3-pr01r-thin-scene-packet-v1',profile:'routine-17k-v154',lastInputTokens:100000}},recentTurns:[]},mode:'game'},
);
...
const thinOptional=(result)=>JSON.parse(result.params.input.split('===== THIN SCENE PACKET — SCENE / CHARACTER / MEMORY =====\n')[1].split('\n\n=====')[0]);
const minimumFactionSocial=thinOptional(denseRouted).relevant_faction_state;
...
assert.ok(thinOptional(indirectRouted).relevant_faction_state.reputations.white_rose,'most recently discussed older faction must survive an indirect important-turn follow-up despite two newer stored factions and broad save keywords');
