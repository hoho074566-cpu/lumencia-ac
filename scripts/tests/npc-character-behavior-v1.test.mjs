...
assert.match(routed.params.instructions, /MINIMAL WRITER CONTRACT/);
assert.doesNotMatch(routed.params.input, /CHARACTER-DRIVEN NPC BEHAVIOR|판단|반응 방향|state_delta\.memories_add/,'behavior interpretation must not become a prose directive');
assert.match(routed.params.input, /"character_packets":\[/);
