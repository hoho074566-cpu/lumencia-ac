// LUMENSIA MOBILE V1.5.3 runtime patch loader
...
// a small, auditable V1.5.3 delta at boot. If required source markers disappear, boot stops visibly.
...
function portraitCandidatesV153(key, expression = 'default') {
  ...
  // V1.5.3: portrait/default까지 실패했을 때만 전신 DEFAULT를 마지막 안전망으로 사용.
...
function latestWorldRecordV153() {
  return [...(save.renderedTurns || [])].reverse().find((record) => record?.turn && !record?.meta) || null;
}

function canAutoFlowV153() {
  const latest = latestWorldRecordV153();
...
function canContinueV153() {
  const latest = latestWorldRecordV153();
  return Boolean(latest && !busy && !metaModeOnce);
}

function renderFlowControlsV153() {
  let wrap = $('flowControlsV153');
  const latest = latestWorldRecordV153();

  if (!wrap) {
    wrap = document.createElement('section');
    wrap.id = 'flowControlsV153';
    wrap.className = 'flow-controls-v153';

    const auto = document.createElement('button');
    auto.id = 'autoFlowBtnV153';
    auto.type = 'button';
    auto.className = 'flow-btn-v153 auto';
    auto.textContent = '▶ 자동 진행';
    auto.addEventListener('click', () => sendAction('', 'auto'));

    const cont = document.createElement('button');
    cont.id = 'continueBtnV153';
    cont.type = 'button';
    cont.className = 'flow-btn-v153 continue';
  ...
  const auto = $('autoFlowBtnV153');
  const cont = $('continueBtnV153');
  if (auto) {
    auto.disabled = !canAutoFlowV153();
...
function installAutoFlowGestureV153() {
  if (!$('v153FlowStyle')) {
    const style = document.createElement('style');
    style.id = 'v153FlowStyle';
    style.textContent = `
      .flow-controls-v153{display:flex;gap:8px;padding:8px 12px 14px;align-items:center;justify-content:flex-end}
      .flow-controls-v153.hidden{display:none}
      .flow-btn-v153{appearance:none;border:1px solid rgba(148,163,184,.26);border-radius:999px;background:rgba(30,41,59,.82);color:#e5e7eb;padding:9px 13px;font-size:12px;font-weight:800;letter-spacing:-.01em}
      .flow-btn-v153.continue{border-color:rgba(217,184,108,.38);color:#f4d995}
      .flow-btn-v153:disabled{opacity:.34;filter:saturate(.4)}
      #autoPullHintV153{position:fixed;z-index:98;left:50%;bottom:104px;transform:translate(-50%,18px);opacity:0;pointer-events:none;padding:7px 11px;border-radius:999px;background:rgba(15,23,42,.94);border:1px solid rgba(148,163,184,.24);font-size:11px;font-weight:800;color:#dbeafe;transition:opacity .12s ease,transform .12s ease}
      #autoPullHintV153.show{opacity:1;transform:translate(-50%,0)}
      #autoPullHintV153.ready{color:#fde68a;border-color:rgba(245,158,11,.5)}
    `;
    document.head.append(style);
  }

  let hint = $('autoPullHintV153');
  if (!hint) {
    hint = document.createElement('div');
    hint.id = 'autoPullHintV153';
    document.body.append(hint);
  }

  actionInput.addEventListener('input', renderFlowControlsV153);
  choicesEl.addEventListener('click', () => setTimeout(renderFlowControlsV153, 0));
  ...
  window.addEventListener('touchstart', (event) => {
    ...
    if (!touch || !atBottom() || !canAutoFlowV153()) return reset();
    eligible = true;
  ...
  window.addEventListener('touchend', () => {
    const fire = eligible && pull >= AUTO_GESTURE_PX && canAutoFlowV153();
    reset();
    if (fire) sendAction('', 'auto');
  }, { passive: true });

  window.addEventListener('touchcancel', reset, { passive: true });
  renderFlowControlsV153();
}

function renderChoicesV153(choices) {
  choicesEl.innerHTML = '';
...
function updateStatusV153(route) {
...
function mergeContinuationIntoRecentV153(turn) {
...
function accumulateUsageV153(usage) {
  if (!usage) return;
...
function applyRuntimeStateV153(data, isContinue = false) {
...
async function sendActionV153(action, requestedMode = null) {
  action = String(action || '').trim();
  const requested = ['auto', 'continue'].includes(requestedMode) ? requestedMode : null;
  if (busy) return;
  if (requested === 'auto' && !canAutoFlowV153()) {
    renderFlowControlsV153();
    return;
  }
  if (requested === 'continue' && !canContinueV153()) {
    renderFlowControlsV153();
    return;
  }
  ...
  const apiAction = isMeta
    ...
    : isAuto
      ? FLOW_AUTO_ACTION_V153
      : isContinue
        ? FLOW_CONTINUE_ACTION_V153
        : action;
  ...
  try {
    ...
    if (settings.demoMode) {
      ...
      data.route = { ...(data.route || {}), input_mode: inputMode, adapter_version: 'demo-v153' };
      if (isContinue && data.turn) {
        data.turn.state_delta = zeroStateDeltaV153();
      ...
      const res = await fetch('/api/chat-v153', {
        method: 'POST',
    ...
    if (!isMeta && !isContinue) {
      ...
      save.recentTurns.push({
        action: isAuto ? FLOW_AUTO_ACTION_V153 : apiAction,
      ...
      mergeContinuationIntoRecentV153(data.turn);
    }

    if (!isMeta) applyRuntimeStateV153(data, isContinue);

    save.renderedTurns.push(record);
    save.renderedTurns = save.renderedTurns.slice(-80);
    accumulateUsageV153(data.usage);
...
async function checkHealthV153() {
  try {
...
function replaceOnce(source, needle, replacement, label) {
  ...
  if (first < 0) throw new Error(`V1.5.3 patch marker missing: ${label}`);
  if (source.indexOf(needle, first + needle.length) >= 0) throw new Error(`V1.5.3 patch marker duplicated: ${label}`);
...
function replaceRegexOnce(source, regex, replacement, label) {
  ...
  if (matches.length !== 1) throw new Error(`V1.5.3 regex marker ${label}: expected 1, got ${matches.length}`);
...
function showBootError(error, phase = 'patch') {
  ...
  box.textContent = `LUMENSIA V1.5.3 ${phase} 실패\n${error?.message || error}\nBase ${BASE_APP_SHA.slice(0, 10)}`;
...
async function boot() {
  ...
  try {
    ...
    source = replaceRegexOnce(
      ...
      `${renameFunction(portraitCandidatesV153, 'portraitCandidatesV153', 'portraitCandidates')}\n\nfunction assetUrl`,
      'portraitCandidates'
    );
    ...
    source = replaceRegexOnce(
      ...
      `${renameFunction(compactStateV153, 'compactStateV153', 'compactState')}\n\nasync function sendAction`,
    ...
    source = replaceRegexOnce(
      ...
      `${renameFunction(renderChoicesV153, 'renderChoicesV153', 'renderChoices')}\n\nfunction updateStatus`,
      'renderChoices'
    );
    ...
    source = replaceRegexOnce(
      ...
      `${renameFunction(updateStatusV153, 'updateStatusV153', 'updateStatus')}\n\nfunction renderInfo`,
      'updateStatus'
    );

    const helperSource = [
      `const AUTO_GESTURE_PX = ${AUTO_GESTURE_PX};`,
      `const FLOW_AUTO_ACTION_V153 = ${JSON.stringify(FLOW_AUTO_ACTION)};`,
      `const FLOW_CONTINUE_ACTION_V153 = ${JSON.stringify(FLOW_CONTINUE_ACTION)};`,
      latestWorldRecordV153.toString(),
      canAutoFlowV153.toString(),
      canContinueV153.toString(),
      renderFlowControlsV153.toString(),
      installAutoFlowGestureV153.toString(),
      mergeContinuationIntoRecentV153.toString(),
      accumulateUsageV153.toString(),
      zeroStateDeltaV153.toString(),
      compactStateV153.toString(),
      applyRuntimeStateV153.toString(),
    ].join('\n\n');
    ...
    source = replaceRegexOnce(
      ...
      `${renameFunction(sendActionV153, 'sendActionV153', 'sendAction')}\n\nfunction demoResponse`,
      'sendAction'
    );
    ...
    source = replaceRegexOnce(
      ...
      `${renameFunction(checkHealthV153, 'checkHealthV153', 'checkHealth')}\n\n// ===== characters-v2 asset audit: BEGIN =====`,
      'checkHealth'
    );
    ...
    source = replaceOnce(
      ...
      'refreshScheduleContext(); updateForceTerraButton(); checkHealth(); installAutoFlowGestureV153(); renderAll(); renderFlowControlsV153();',
      'boot hook'
    );
  } catch (error) {
