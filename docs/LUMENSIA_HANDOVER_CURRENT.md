...
- Initial exact Preview on `4a24a77...` compressed `밖으로 간다` through corridor/stairs to the true A-building exterior, but still returned three generic destination suggestions. A later affected rerun preserved a real Isabel question correctly, then exposed generic room suggestions retained only by scene importance. The local remediation now keeps choices only with structured failure, combat, event/schedule, decision-language, or direct NPC question evidence; importance alone is insufficient, routine travel choices are removed, and `마법과 건물` is not misclassified as combat.
...
- Its two P2s are also handled: declarative `알겠어.` / `그 부탁은 이미 처리했어.` no longer match direct requests, and the labeled Director/Schedule tail compacts dynamically so dense 3,900- and 5,200-character fixed-authority cases remain inside the profile input limit.
...
> `docs/LUMENSIA_HANDOVER_CURRENT.md`와 `docs/IMPLEMENTATION_PROGRESS.md`를 먼저 읽고 Lumensia 프로젝트를 그대로 이어가라. 새 프로젝트가 아니다. PR #39는 exact reviewed head `ba64f9f...`에서 main `4516cc1...`로 병합됐고 production Vercel/health까지 통과했다. HF1/HF2/HF3, 12-case acceptance, Scene Purpose, Scene Exit 진단을 다시 하지 않는다. 현재 PR #40 branch `codex/stronger-turn-hook-v1`; initial head `4a24a77...`는 Vercel PASS 후 live room->exterior compression을 통과했지만 generic three-choice suggestions를 노출했다. Local remediation은 structured failure/combat/event/high-importance/decision/direct-NPC evidence가 없는 choices를 제거하며 focused regressions가 통과한다. clean-LF full rerun, commit/push, fresh exact gates/review, affected Preview를 진행한다. Event Consequence는 이 PR에 섞지 않는다.`
