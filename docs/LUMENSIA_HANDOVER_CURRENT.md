...
- Overall: `2 / 9 PR merged`; Terminal: `2 / 9 processed`; separate bounded insertion: `1 DEFERRED`.
...
- P2-PR03 Background Persistence + Character-Dependent Start — **IN_PROGRESS** at PR #74 from current main. Scope is bounded to persistent background visibility, starting-route variation, NPC first impression, same-event/different-meaning, PC-strength awareness, and evaluation adjustment. First exact-head review found one P1: stable runtime request compaction omitted `creation`, so the server could not receive the persisted background. This is **SAME_ROOT** and correction `1/1` adds `creation` to base/stable compact payloads plus a runtime-produced-request regression; fresh corrected-head gates/review are pending. It must not add rumor propagation, faction intelligence, a global epistemic graph, or correct PR #69.
...
- NEXT ACTION: implement **P2-PR03 — Background Persistence + Character-Dependent Start** on this branch, run its focused/required regression and hosted exact-head gates, then apply the bounded merge/defer rule. Do not return to PR #72 or Phase 1 deferred PR #66/#68/#69.
