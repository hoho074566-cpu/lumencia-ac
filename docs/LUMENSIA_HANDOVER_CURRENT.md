...
- Current local correction head `add85ff44da7bfeddb83f31b6aca65fea9dfb024` requires learning-specific visible-scene evidence even for combat, rejects ambiguous setting nouns such as `훈련장` and map `지도`, and compacts mandatory candidates to exact identity plus progress only. Permanent ordinary-combat/setting-noun regressions and true 80/48-character maximal identity pressure pass; the authoritative clean-LF full PR check passes on this code head.
...
- Current corrected code head: `add85ff44da7bfeddb83f31b6aca65fea9dfb024` on `codex/skill-learning-v1`, based directly on the verified PR #47 merge.
...
- A normal player-owned GAME turn may accept at most two learning rows, each +1..15. Each row requires a bounded independent skill name, concrete `basis`, causal `reason`, positive player learning intent or combat, and learning-specific visible scene-side training/instruction/correction/practice/insight evidence. Bare combat/failure words, model-authored metadata, negated learning text, training-ground/map nouns, existing-skill names, spacing aliases, generic placeholders, basis-less claims, ordinary movement, duplicate rows, and excess candidates are rejected.
...
- Permanent coverage is `scripts/tests/skill-learning-v1.test.mjs`. It includes self-authored/negated evidence rejection, ordinary-combat and ambiguous setting-noun rejection, positive combat insight, legacy experience isolation, the exact 22→37 live regression, and maximum-width context pressure. Dedicated/affected suites and the authoritative clean-LF full PR check pass on `add85ff...`; the full check must be rerun once more on the final docs head.
- The user opened this correction cycle after the `83377b8...` hosted review. Its two local fix/review rounds are consumed: learning-specific scene evidence plus maximal mandatory-context compaction, then Korean setting-noun ambiguity closure and regression review. The substantive second review found no remaining local P0/P1 blocker; no third automatic remediation is permitted in this cycle.
...
4. Commit this docs checkpoint, rerun the authoritative clean-LF full PR check on that exact head, inspect the exact diff, and perform no third automatic fix/review iteration. If a P0/P1 remains, report FAIL and stop.
5. Push/update PR #48, then require fresh exact-head Safety, Vercel, and Codex P0/P1=0 with current main/merge-base/no-conflict revalidation. The prior `75d7f7c...` and `83377b8...` review results are superseded only after the new exact-head review completes.
6. On the new Exact Preview, verify ordinary combat/failure and ambiguous setting nouns create no learning, real training still creates bounded progress, AUTO/META/CONTINUE preserve learning state, the former 22/100 case advances by at most 15, and 100 unlocks exactly once at F grade.
7. If every gate and affected Preview case passes, report the Skill Learning V1 protected-path PR ready for the user's human merge. Never merge it from Codex.

---

# NEW CHAT START INSTRUCTION

> `docs/LUMENSIA_HANDOVER_CURRENT.md`와 `docs/IMPLEMENTATION_PROGRESS.md`를 먼저 읽고 Lumensia 프로젝트를 그대로 이어가라. 새 프로젝트가 아니다. PR #47은 reviewed head `866c991...`에서 main `54327ea...`로 merge됐고 merge tree가 reviewed tree와 정확히 같다. 완료된 HF1/HF2/HF3, 12-case acceptance, Scene Purpose/Exit/Turn Hook, Event Consequence, Goal Tick, Off-screen Progression, Scene Novelty, NPC↔NPC Relationship V1, Faction Social V1을 다시 분석하지 않는다. PR #48은 open/not-merged이고 현재 branch는 `codex/skill-learning-v1`, corrected code head는 `add85ff...`이다. 이전 `75d7f7c...` evidence/legacy bypass와 `83377b8...` ordinary-combat evidence/max-context findings은 superseded됐다. 활성 범위는 learning-specific scene evidence, legacy skill_experience 격리, 기존 `pc.skillCandidates` 누적, 100점 단일 F등급 습득, AUTO/META/CONTINUE 동결, bounded mandatory context, UI 표시뿐이다. 새 save root/migration/endpoint/model call, canonical `app.js` 변경, 즉흥 각성, 기존 스킬 재생성은 범위 밖이다. 이 교정 주기의 두 로컬 수정 라운드도 소진됐으므로 final docs-head clean-LF/full diff, fresh exact-head hosted review, corrected Exact Preview 뒤 protected-path human merge로 진행한다.`
