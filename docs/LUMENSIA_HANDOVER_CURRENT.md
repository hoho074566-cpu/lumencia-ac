...
- Bounded correction budget `1/1` is consumed under the shared progression-integrity/UI contract. The correction uses an explicit player-visible Fate Affinity allowlist; reconciles the latest stored spend ledger before validation and every write; retains full Character Ending reward authority; writes relationship history in the existing string schema; and exposes deterministic previewed Origin candidates so a purchased reroll selects the actual next-life result. Dedicated STAB-02, STAB-01, Fate Start/Origin/Background/Personal Story, Context Router/authority-tail, Novel/UI, CONTINUE/core, syntax/static, `git diff --check`, and full `scripts/lumensia-pr-check.mjs origin/main HEAD` all PASS locally after correction.
- **CURRENT MERGE GATE:** STAB-01 **MERGED / POST-MERGE PASS**. STAB-02 correction is locally complete but **not READY** until its corrected exact head is published and receives new Safety, Vercel, and fresh exact-head P0=0/P1=0. Protected `app.js`, `app-runtime.js`, and `api/**` changes require human merge.
- **NEXT ACTION:** publish the single corrected PR #83 head, rerun exact-head Safety/Vercel/fresh review without further correction, then require human merge only if P0=0/P1=0 and repository integrity remains PASS. Keep every deferred branch and STAB-01 P2 backlog untouched.
