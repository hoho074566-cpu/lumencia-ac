...
9. Commit/push the PR #59 second-correction docs checkpoint after code `9d9709b`, verify the new exact head keeps base/merge-base `9458633a...`, behind 0, no conflict, and Vercel PASS, then request a fresh direct exact-head review. If the exact head remains P0/P1=0 with no blocking P2, select the next legacy target from a fresh read-only inventory. If a P0/P1 or gameplay-blocking P2 remains, keep `MERGE_GATE: FAIL` and hand off because the Codex-local two-remediation cap is exhausted; do not hide it with a Phase 4 wording patch.
