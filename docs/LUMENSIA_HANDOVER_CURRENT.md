...
- PR #51 is open from `codex/combat-growth-v2`. Current code/test checkpoint: `e21b2070d5f6caa8aacb84e5dc1e803cdcaba47d`; initial implementation head was `5866bced8963e2dcdd70c0899c3717dddb6a3b88`.
...
- The user explicitly resumed correction 5/5. Code checkpoint `e21b207...` permits exactly one intervening dialogue only when the speaker is an instructor and the dialogue directly addresses the player; the final-application and immediately following cause narrations still provide the actual evidence. Non-instructor dialogue, unaddressed instructor dialogue, two dialogue rows, wider gaps, and NPC performers inside the cause body are rejected. Player-owned cause narration remains accepted. Focused and authoritative clean-LF full regression pass; the new exact-head hosted cycle and affected Preview rerun are pending.
