...
- Current local correction head `108713fd121356b647eaf02536714e06a1a5c6ff` requires both a player-owned active learning/combat action and visible learning evidence attributable to the player. NPC-only training/combat, observation-only, refused training, a PC observer with an NPC performer, and `NPC가 직접` emphasis are rejected; observation followed by the PC's own direct practice remains valid. Permanent attribution regressions, the adversarial phrase matrix, syntax checks, `git diff --check`, and the authoritative clean-LF full PR check pass on this code head.
...
- Current corrected code head: `108713fd121356b647eaf02536714e06a1a5c6ff` on `codex/skill-learning-v1`, based directly on the verified PR #47 merge.
...
- Permanent coverage is `scripts/tests/skill-learning-v1.test.mjs`. It includes self-authored/negated evidence rejection, ordinary-combat and ambiguous setting-noun rejection, NPC observation/refusal/sole-performer rejection, observation-then-direct-practice acceptance, positive combat insight, legacy experience isolation, the exact 22→37 live regression, and maximum-width context pressure. Dedicated/affected suites and the authoritative clean-LF full PR check pass on `108713f...`; the full check must be rerun once more on the final docs head.
- The user opened a new correction cycle after the `a0d76f5...` hosted attribution P1. Its two local implementation/review rounds are now consumed: actor attribution and permanent false-positive cases, then observer/performer ordering plus NPC combat/direct-emphasis hardening. The substantive second review and adversarial matrix found no remaining local P0/P1 blocker; no third automatic remediation is permitted in this cycle.
...
5. Push/update PR #48, then require fresh exact-head Safety, Vercel, and Codex P0/P1=0 with current main/merge-base/no-conflict revalidation. The prior `a0d76f5...` P1 is superseded only after the new exact-head review completes.
6. On the new Exact Preview, first verify watching/refusing an NPC's training creates no PC learning, then verify the PC's direct practice creates bounded progress and CONTINUE/META preserve that exact value. The prior ordinary-combat/setting-noun, 22→37, 100→single-F, and no-repeat cases remain permanent/local coverage and previously passed exact Preview acceptance.
