...
- PR #44: **open / protected-path / human-merge only**. Code checkpoint `6eada327ea91afecdb3fa9655f917ddb65781f7b` implements bounded visible-term repetition tracking and passes the authoritative clean-LF full check plus focused/affected regressions. A final docs checkpoint, fresh exact-head hosted gates/review, and affected Exact Preview acceptance are still required.
...
- When active novelty authority consumes input budget, only the middle of long USER ACTION text is compacted by the same amount. The beginning, committed ending, mandatory schedule payload, Purpose, Exit, Turn Hook, and minimum `.76` budget remain present.
...
3. Continue only PR #44 / Deterministic Scene Novelty V1 on `codex/deterministic-scene-novelty-v1`; code checkpoint `6eada32...` is the initial exact candidate.
...
5. Revalidate current main/merge-base/no-conflict and run the affected Preview sequence `본다` → `돌아다닌다` → `본다`, followed by explicit recap, direct-question, and CONTINUE boundary cases.
