...
# 0. RELATIONSHIP THRESHOLDS V1 — AUTHORITATIVE CURRENT TASK

- PR #67 NPC Significance Evaluator V1 exact head `3293e1c11569e5531518ae7e05918a54693c2bb9` was human-merged. Verified latest main is merge commit `9f2274abeef7f34531c8d0240f66ed39293b9eef`; the active branch `codex/relationship-thresholds-v1` was created directly from that main.
- PR #66 Setup -> Payoff Memory V1 remains explicitly **deferred** at `fbf2935c1229b8f16bed404b48c3cdf0abb95809`. Keep it open and unmerged; do not close it, mutate its Preview, fix lifecycle P1 `3871077298`, or use its branch as a base.
- V1 reuses existing `relationships`, `relationship_changes`, canonical Router/Core model call, and `npcInnerStates`. The model may return `threshold_signal=trust_opened|trust_withdrawn|hostility_opened|hostility_eased` only when cumulative relationship meaning and the current grounded event qualitatively change later behavior; ordinary changes remain `none`.
- Deterministic code owns only the canonical allowlist, registered-NPC ownership, bounded relationship direction, entry/release hysteresis, one transition per NPC per turn, required concrete `followup`, and META/AUTO/CONTINUE freeze. A numeric boundary creates eligibility only; it never auto-activates a threshold or forces dialogue, an event, or a player choice.
- Accepted state is nested under the existing `npcInnerStates[npc].relationship_thresholds` root and routed back as later semantic context. No new save root, lifecycle phase, parser, Korean wording regex, model call, generic relationship/NPC simulation engine, reputation/faction/social-graph expansion, or NPC Significance redesign exists.
- Dedicated Relationship Threshold tests and relevant Context Router, authority-tail, NPC Relationship/Significance/Motivation, Scene Orchestration, CONTINUE, time-floor, save-migration, and core-invariant suites pass. Full `node scripts/lumensia-pr-check.mjs origin/main HEAD` passes on the working tree, including every repository test and architecture check.
- The single bounded structural correction budget is consumed (1/1): empty threshold context initially occupied the minimum routed authority budget and displaced the required USER ACTION suffix. The corrected form omits that optional minimum field when no relationship context exists. A later source-contract compatibility adjustment restored the unchanged `localNpcUpdates(incoming0,runtimeTurn)` call boundary and merges the validated receipt immediately afterward; it changed no behavior or correction scope.
- Substantive second review found P0=0/P1=0 locally. NEXT ACTION: commit/publish the exact implementation head, open the protected main-target PR, verify Safety/Vercel/current-main integrity, and request a fresh exact-head review. Exact Preview is not required unless hosted review exposes a real-play semantic uncertainty; do not create unnecessary player-choice mutation. If the fresh review reports a distinct structural P1, stop without another correction. If all gates pass, leave the PR merge-ready for human merge and fix the next Phase 1 action to **Knowledge Boundaries V1**.
...
2. Continue **Relationship Thresholds V1** only on `codex/relationship-thresholds-v1`, based directly on verified main `9f2274abe...` after human-merged PR #67 exact head `3293e1c...`.
3. Keep scope to the existing model-owned `relationship_changes.threshold_signal` semantic receipt and existing `npcInnerStates` persistence. Deterministic code owns only canonical ownership/direction/bounds/hysteresis/cap/freeze invariants; numeric eligibility never forces narrative behavior.
4. Preserve one canonical model call, stable Router/Core/Director/Orchestration, `store:false`, prompt cache/retention, Context Router budgets, canon, NPC autonomy, and player sovereignty. Add no wording regex/parser, new save root, lifecycle/state engine, generic relationship simulator, reputation/faction graph, or NPC Significance redesign.
5. Focused/relevant/full local regression and substantive P0/P1 review pass. Publish the exact checkpoint and require main-target Safety, Vercel, current-main integrity, and fresh exact-head review. The one bounded correction is consumed (1/1); any distinct structural P1 stops the task without another patch.
6. If all gates are green, leave the protected PR human-merge-ready and set the next action to **Knowledge Boundaries V1**. Do not add another Phase 1 feature; after Knowledge Boundaries V1, FREEZE Phase 1.

---

# NEW CHAT START INSTRUCTION

> Read both progress documents first. PR #66 Setup -> Payoff Memory V1 remains deferred at `fbf2935c...`; do not touch, merge, close, Preview-mutate, fix P1 `3871077298`, or base work on it. PR #67 exact head `3293e1c...` was human-merged and latest verified main is `9f2274abe...`. Continue Relationship Thresholds V1 only on `codex/relationship-thresholds-v1`: the canonical model supplies a bounded semantic `threshold_signal`, deterministic code validates hard boundaries, and accepted state stays under existing `npcInnerStates`. Focused/relevant/full local checks pass. Publish and require main-target Safety/Vercel/current-main/fresh exact-head review. Correction budget is consumed (1/1); any distinct structural P1 stops without another patch. If green, leave human-merge-ready and proceed next to Knowledge Boundaries V1, then Phase 1 FREEZE.
