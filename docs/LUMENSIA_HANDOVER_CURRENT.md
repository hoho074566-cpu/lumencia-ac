...
- PR #82 STAB-01 is **MERGED**. Its merged exact head `d589f7939a1296d966eeb46fc7c5174f8b9a00a2` equals the READY head; latest main is `89fbc5b94d74a3e1927615993e5c7da88b84a4ff`. STAB-01 is not being reimplemented.
...
- Active work is **STAB-BASE — Async Run Ownership & Commit Boundary** on `codex/stab-base-async-run-commit-boundary`, created cleanly from latest main `89fbc5b94d74a3e1927615993e5c7da88b84a4ff`. No PR #83/#84 code or commit was reused. Published implementation checkpoint is `b7f7d73bd69812c5830ce982865d0f114ae0b56a`.
- Scope is limited to four hard boundaries: start-time run identity capture with pre-commit ownership rejection; paired run/Fate persistence with a specialized rollback journal; legacy Fate migration write-before-delete; and one canonical versioned Ending-handler marker shared by the stable loader. Loader patch failure now stops visibly instead of importing the `/api/chat` fallback.
- The paired commit owns only `lumensia.save.v1` plus `lumensia.fate-book.v1`. It is not a generic transaction, lifecycle, distributed-ledger, event-sourcing, or meta-state engine. Inheritance/Next Life is not implemented or expanded in this branch.
- Local validation PASS at the code checkpoint: dedicated async ownership/commit failure/recovery/loader fixtures; existing Fate/Ending; free, paste, Fate Start, Origin and Background; CONTINUE/core; long USER ACTION and Context Router authority-tail/context-pressure; offline cache; syntax and `git diff --check`; full `node scripts/lumensia-pr-check.mjs`.
- Migration failure preserves embedded authority until external Fate persistence and subsequent run persistence succeed. A late old-run response is rejected before staged mutation, and a paired-persistence failure restores both the prior in-memory authorities and stored values.
- Browser/Preview remains on the established bounded-unavailable fallback; no repeated recovery attempt is part of this pass. Hosted Safety, Vercel, and fresh exact-head review are pending until the final code+docs head is published.
- Phase state: `PHASE_2_IMPLEMENTATION_PASS: COMPLETE`; `PHASE_2_STATUS: STABILIZATION IN PROGRESS / PRODUCT COMPLETION NOT COMPLETE`; STAB-BASE correction budget `0/1` used.
- **CURRENT MERGE GATE:** pending final exact-head Safety, Vercel, fresh P0=0/P1=0, and repository-integrity checks. Protected `app.js`/`app-runtime.js` remain human-merge-only.
- **NEXT ACTION:** publish the final code+docs STAB-BASE head on a new PR, run hosted gates and fresh exact-head review, then either report the exact human-merge head or stop with `PHASE_2_STATUS: BLOCKED` under the user-defined structural stop rule.
...
1. PR #82 STAB-01 is merged as latest main `89fbc5b94d74a3e1927615993e5c7da88b84a4ff` from exact READY head `d589f7939a1296d966eeb46fc7c5174f8b9a00a2`; do not reimplement it.
...
3. Continue only STAB-BASE on `codex/stab-base-async-run-commit-boundary`, created from exact latest main. Published implementation checkpoint `b7f7d73bd69812c5830ce982865d0f114ae0b56a`; focused/full local checks PASS.
4. Publish the final code+docs head and require exact-head Safety, Vercel, fresh P0=0/P1=0, base/merge/conflict and tree-identity gates. If PASS, report the exact protected-runtime head for human merge.
5. The only correction budget is one bounded pass for the same async ownership/commit-boundary root. A new lifecycle/distributed/meta architecture or separate structural P1 ends stabilization with `PHASE_2_STATUS: BLOCKED`.
6. Keep PR #76/#79, PR #66/#68/#69/#78, and obsolete/superseded PR #72 untouched. Do not retry browser recovery under the bounded fallback.

---

# NEW CHAT START INSTRUCTION

> Read both progress documents first. Latest main `89fbc5b94d74a3e1927615993e5c7da88b84a4ff` contains merged STAB-01 exact READY head `d589f7939a1296d966eeb46fc7c5174f8b9a00a2`. PR #83/#84 are terminal BLOCKED and frozen at `f0bef9f...` / `e83f6ec...`; do not touch or reuse them. Continue only STAB-BASE on `codex/stab-base-async-run-commit-boundary`, published implementation checkpoint `b7f7d73bd69812c5830ce982865d0f114ae0b56a`, then run exact-head Safety/Vercel/fresh P0=0/P1=0 plus repository-integrity gates. One bounded correction is allowed only for the same async ownership/commit root; a new structural architecture requirement ends Phase 2 as BLOCKED.
