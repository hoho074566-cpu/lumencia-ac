...
- PR #53 is open from `codex/event-director-v3-result-surfacing` against exact base `4bbe3fb...`. Code/test checkpoint `5845cda6b7a5aaa09fc56b2cb8a4f609cf5a2f2d` passes focused tests and the authoritative clean-LF full repository regression.
...
- Next gates are a docs checkpoint, exact-head Safety/Vercel, current mergeability, fresh Codex P0/P1=0, and targeted Exact Preview acceptance. Because `api/**` is protected, PR #53 remains human-merge only.
...
3. Confirm PR #53 exact head includes code checkpoint `5845cda...` plus the current docs checkpoint and remains based on current main with no conflict/behind state.
4. Require GitHub Safety Gate, Vercel Ready, Merge Readiness, and a fresh exact-head Codex review with P0/P1=0. Run the authoritative clean-LF full regression again after every head change.
5. On the exact Preview, create/retain one eligible public non-PC completion result, then verify: a safe observe/explore/wait turn surfaces exactly one plausible visible trace; the confirmed completion is not enriched with an invented outcome; the selected result does not create a new active event; the next eligible turn does not repeat it. Separately verify direct question, combat/active event or reachable schedule, present-NPC Goal Tick, disabled background simulation, and AUTO/META/CONTINUE suppression. Do not inspect protection tokens, cookies, or local storage or bypass Preview protection.
6. Only after every hosted/review/Preview gate passes, report PR #53 ready for a person's merge. Never merge this protected-path PR from Codex.

---

# NEW CHAT START INSTRUCTION

> `docs/LUMENSIA_HANDOVER_CURRENT.md`와 `docs/IMPLEMENTATION_PROGRESS.md`를 먼저 읽고 Lumensia 프로젝트를 그대로 이어가라. 새 프로젝트가 아니다. PR #52는 reviewed head `10de71a...`에서 main `4bbe3fb...`로 merge됐고 merge tree가 reviewed tree와 정확히 같다. 완료된 HF1/HF2/HF3부터 Living World V1까지 다시 분석하지 않는다. 현재 branch는 `codex/event-director-v3-result-surfacing`, open PR은 #53, code checkpoint는 `5845cda...`이며 이 뒤 docs checkpoint를 포함한 exact head를 다시 확인한다. Event Director V3는 Living World의 최신 공개 `종료 확정` 1건만 existing Director beat로 화면에 전달하며 due consequence/direct question/combat/player stop/active event/reachable schedule/present Goal Tick/freeze priority를 유지한다. 최대 한 번 재시도하고 visible title+completion evidence만 checkpoint를 닫는다. Focused와 clean-LF full regression은 통과했다. 다음은 docs push → exact-head Safety/Vercel/Merge Readiness/fresh Codex review → targeted Exact Preview acceptance다. 새 schema/save root/migration/endpoint/serverless function/canonical `api/chat.js`/`app.js` change/second model call은 금지한다. 보호된 Preview의 token/cookie/local storage를 읽거나 protection을 우회하지 않는다. `api/**` 보호 경로이므로 최종 병합은 사람만 수행한다.`
