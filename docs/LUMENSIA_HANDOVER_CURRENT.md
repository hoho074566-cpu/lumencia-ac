...
- Current blocker: correction 4/5 passes focused and authoritative clean-LF full regression locally. Publish the docs checkpoint, then require exact-head Safety/Vercel/fresh Codex review and rerun the affected Exact Preview correction case. If the same split-row rejection recurs, stop under condition 1 instead of automatically using correction 5/5.
...
3. Confirm PR #51 live branch tip includes correction-4 code checkpoint `5235c6c...`; the local focused and authoritative clean-LF full checks already pass.
4. Publish the docs checkpoint, then require GitHub Safety/Vercel/fresh Codex review on that exact head.
5. Rerun the affected Exact Preview correction case through the already-authorized browser path. Do not extract a protection token or bypass Preview protection. If the same split-row rejection recurs, stop under condition 1 instead of consuming correction 5/5 automatically.
...
> `docs/LUMENSIA_HANDOVER_CURRENT.md`와 `docs/IMPLEMENTATION_PROGRESS.md`를 먼저 읽고 Lumensia 프로젝트를 그대로 이어가라. 새 프로젝트가 아니다. PR #50은 exact reviewed head `1eb316b...`에서 main `88ce7b4...`로 merge됐고 merge tree가 reviewed tree와 정확히 같다. 완료된 HF1/HF2/HF3부터 Awakening / Talent Evolution V1까지 다시 분석하지 않는다. 현재 branch는 `codex/combat-growth-v2`, open PR은 #51, correction-4 code/test checkpoint는 `5235c6c...`이며 live branch tip을 다시 확인한다. Correction 4/5는 최대 두 개의 바로 인접한 narration row만 묶어 final application + immediate cause를 판정하고 instructor TTL 및 combined third-party attribution을 모두 요구한다. Intervening row, named-NPC cause ownership, wider window는 영구 회귀로 거부되며 focused와 authoritative clean-LF full regression이 통과했다. 다음은 docs checkpoint push → exact-head Safety/Vercel/fresh Codex review → affected Exact Preview rerun이다. 같은 split-row 거부가 재발하면 자동 correction 5/5로 넘어가지 말고 조건 1에 따라 중단한다. 새 schema/save root/migration/endpoint/serverless function/canonical `api/chat.js`/`app.js` change/second model call은 금지한다. 보호된 Preview의 token/cookie/local storage를 읽거나 protection을 우회하지 말고 동일 설정 요구를 반복하지 않는다. 보호 경로이므로 최종 병합은 사람만 수행한다.`
