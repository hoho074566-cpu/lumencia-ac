...
- Active work is fresh **STAB-02R — Canonical Meta Progression / Inheritance / Next Life** on `codex/stab-02r-fresh-canonical-meta-next-life`, created directly from main `c04ff7398ddd559ba12b409a22b9968725a8048c`. No PR #83/#84 production code, commit, or branch was reused; only their documented invariants and failure corpus were referenced.
- Code checkpoint `56a8703c167bd45db23c7aba9546b1b8c322ab4d` uses immutable historical Ending earned receipts plus immutable historical purchase receipts as canonical accounting. Current registry/allowlists affect only new eligibility. Applied run benefit carries the exact canonical receipt marker and cannot survive an orphaned/mismatched import.
- Import accepts equality, subset, or safe superset only. Divergent earned/spent ledgers and malformed/orphan benefit state fail closed without partial mutation; stale saves retain newer canonical meta progression; every valid state maintains `spent <= earned`.
- Same-device purchase validation/read/commit is serialized by one named Web Lock. Fate + Inheritance + run are written through a STAB-BASE-specific three-key rollback journal, and async turn commits re-read canonical meta under the same lock before commit. Old-run results cannot enter a newly committed Next Life.
- Next Life supports progressive Stats, Talents, Starting Resources, Fate Affinity, Origin reroll and paid region/occupation locks. Incompatible locks fail loudly. Realm/Circle is not a purchase kind and is recalculated only after final inherited axes/talents are materialized. Free/paste/Fate Start and existing Origin/Background/Personal Story paths remain intact.
- Focused/affected/full local validation PASS: first discovery/dedupe; historical Ending/NPC retirement; stale/malformed/divergent imports; receipt/benefit consistency; same-device race; old-run rejection; Origin compatibility; Realm/Circle recalculation; actual Next Life application; STAB-01/STAB-BASE and free/paste/Fate Start regressions; long USER ACTION/authority-tail/context-pressure; syntax; offline cache; `git diff --check`; full repository check.
- No generic transaction/meta-state/distributed-ledger/CRDT/event-sourcing/conflict-solver/parser/lifecycle/model-call architecture was added. Browser/Preview remains on the established bounded-unavailable fallback without retry.
- Phase state: `PHASE_2_IMPLEMENTATION_PASS: COMPLETE`; `PHASE_2_STATUS: STABILIZATION IN PROGRESS / PRODUCT COMPLETION NOT COMPLETE`; STAB-02R correction budget `0/1` used.
- **CURRENT MERGE GATE:** pending final code+docs publication, Safety, Vercel, fresh exact-head P0=0/P1=0, and remote/local repository-integrity checks. Protected runtime remains human-merge-only.
- **NEXT ACTION:** publish the final STAB-02R code+docs head, open one new main-target PR, run exact-head hosted gates and fresh review, then report only the validated exact head for human merge. A new structural P1 requiring distributed ownership, generic meta lifecycle, or a different persistence authority model ends Phase 2 as BLOCKED.
...
1. PR #85 STAB-BASE is merged at main `c04ff7398ddd559ba12b409a22b9968725a8048c` from exact READY head `4a7d492b2296b2ead96bdd26eadf151c7c015e7c`; post-merge verification and all prior hosted gates remain PASS.
...
3. Continue only PR #86 on fresh STAB-02R branch `codex/stab-02r-fresh-canonical-meta-next-life`, based directly on main `c04ff7398ddd559ba12b409a22b9968725a8048c`. Prior exact `d46f4e3...` review found one P1 because new Fate Affinity purchases used the full asset/character registry; the reroll-limit P2 remains non-blocking backlog and must not be changed.
4. The explicitly authorized targeted eligibility correction `1/1` reuses the existing `DIRECTOR_NPC_DEPT` / server `DIRECTOR_ROSTER` public gameplay roster for new purchases only. Existing historical affinity receipts/accounting remain independent from current eligibility. Correction-inclusive focused/affected/full checks and loader identity PASS; publish the final code+docs head and require new exact-head Safety, Vercel, fresh P0=0/P1=0 plus base/merge/tree/worktree integrity.
5. If those gates PASS, report only that exact protected-runtime head for human merge. The targeted correction is exhausted; any fresh P1 ends Phase 2 with `PHASE_2_STATUS: BLOCKED` without another correction.
6. After human merge only, verify latest main and run the final Character Creation → Fate Start → Origin → Background → NPC/World Reaction → Personal Story → World Consequence → Ending/Dead Ending → Fate Book → first-discovery reward → Inheritance → Next Life loop. Do not add new Phase 2 feature discovery.
7. Keep PR #76/#79, PR #66/#68/#69/#78, and obsolete/superseded PR #72 untouched. Browser/Preview remains on the bounded-unavailable fallback without retry.

---

# NEW CHAT START INSTRUCTION

> Read both progress documents first. Latest main `c04ff7398ddd559ba12b409a22b9968725a8048c` contains merged STAB-BASE exact READY head `4a7d492b2296b2ead96bdd26eadf151c7c015e7c`. PR #83/#84 are terminal BLOCKED and frozen at `f0bef9f...` / `e83f6ec...`; do not touch or reuse them. Continue only PR #86 on fresh STAB-02R branch `codex/stab-02r-fresh-canonical-meta-next-life`. Prior review P1 is addressed by targeted eligibility correction `1/1`: new Fate Affinity purchases use the existing public gameplay Director roster instead of the asset manifest, while historical receipts remain intact. Do not fix the reroll-limit P2. Local focused/full checks pass; publish final code+docs, then rerun Safety/Vercel/fresh P0=0/P1=0 and repository integrity. Any fresh P1 ends Phase 2 as BLOCKED.
