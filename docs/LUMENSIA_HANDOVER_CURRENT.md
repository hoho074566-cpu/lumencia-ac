...
- Current corrected code head: `1922f3c05c7c5ab19a0c13c15f60d367ebefe43d` on `codex/skill-learning-v1`, based directly on the verified PR #47 merge.
...
- A normal player-owned GAME turn may accept at most two learning rows, each +1..15. Each row requires a bounded independent skill name, concrete `basis`, causal `reason`, positive player learning intent or combat, and visible scene-side training/instruction/correction/practice/combat-learning evidence. Model-authored metadata cannot authenticate itself and negated learning text is removed before classification. Existing-skill names, spacing aliases, generic placeholders, basis-less claims, ordinary movement, duplicate rows, and excess candidates are rejected.
...
- Dense `.76` routing retains up to 24 bounded existing skill names and all eight bounded active candidates in the mandatory minimum while omitting candidate history there; the 6,840-character routine input ceiling remains enforced.
- Permanent coverage is `scripts/tests/skill-learning-v1.test.mjs`. It now includes self-authored/negated evidence rejection, legacy experience isolation, and the exact 22→37 live regression. Dedicated/affected suites and the authoritative clean-LF full PR check pass on `1922f3c...`; the full check must be rerun once more on the final docs head.
- The user opened a new correction cycle after prior hosted/live failures. Its two corrective review rounds are consumed: independent evidence/legacy-path isolation, then negation/scene anchoring plus AUTO experience freeze. The substantive second review found no remaining local P0/P1 blocker; no third automatic remediation is permitted in this cycle.
...
4. Commit the correction docs checkpoint, rerun the authoritative clean-LF full PR check on that exact head, inspect the exact diff, and perform no third automatic fix/review iteration. If a P0/P1 remains, report FAIL and stop.
5. Push/update PR #48, then require fresh exact-head Safety, Vercel, and Codex P0/P1=0 with current main/merge-base/no-conflict revalidation. Ignore the superseded `75d7f7c...` P1 only after a new exact-head review completes.
6. On the new Exact Preview, verify ordinary movement creates no learning, real training creates bounded progress, AUTO/META/CONTINUE preserve learning state, existing skills are not duplicated, the former 22/100 case advances by at most 15, and 100 unlocks exactly once at F grade.
