...
Phase 4 first cleanup PR #58 reached exact docs head `9458633a9657c53c7e875a5cf26538d5f38a0cac`. Vercel deployment `7zQFv7yvTLZyJ3Ct97n3LmraP6Jw` is Ready, direct fresh review comment `5431906772` found no major issue, and the stack compare has `base_commit == merge_base_commit == 3fde9714fa4835278b40d94b29c263394b001573`, behind 0. Local merge-tree and repository readiness report no conflict, while GitHub's separate `mergeable` field was temporarily inconsistent; preserve that as a hosted recheck item instead of claiming it green. PR #58 changes protected `api/**` and remains human-merge-only.

The second Phase 4 cleanup is isolated on `codex/time-plan-parser-phase4-single-completion`. Code checkpoint `0913a612d1a2dfa35cb25e849588c16b8583245b` removes the same narration-completion authority for eligible single-clause plans before a player choice. Missing/invalid receipts keep the returned decision minute; valid receipts preserve the completed clause and its owned effects. TPP-ineligible concurrent timing still uses the legacy fallback, explicitly proving that the cleanup did not expand beyond TPP authority. No parser, regex, feature, schedule/consequence rule, or reconciliation policy was added. Focused Phase 3/time-floor tests, syntax, `git diff --check`, and full `node scripts/lumensia-pr-check.mjs 9458633a... HEAD` pass. The full check includes the repository-owned HF1 integration suite; one separately typed obsolete test filename did not exist and is not counted as a test result.

Fresh exact-head review on initial PR #59 head `94856d0cbe37271cd5b8a119eb69a2e9ccd5de54` found P0=0 / P1=0 and one P2 that is blocking under the saved Lumensia rule: a missing single-action receipt held the returned minute but did not enter shortening reconciliation, so unowned rewards, location changes, and completion prose could survive. Correction `ed6d921e206506f2f124be6595285c9bec8f0f55` treats that returned choice minute as the structural decision boundary only when no authoritative schedule, consequence, or turn-limit boundary has already been reached. It therefore removes unowned completion effects/prose while preserving the existing external-boundary priority. The Director/consequence regression now supplies its pre-response Director occurrence and visible metadata instead of relying on unowned returned state. Focused time-floor, Phase 3, Event Consequence, HF1 integration, syntax, diff, and full repository regression pass. This is the first repository-authorized remediation for the current exact-head review cycle.

Fresh direct review `5035759966` on exact docs head `96b6a996730f2be17bcb8fbc00d72dbf0c89d21e` again found P0=0 / P1=0, but identified three P2s that are blocking under the saved gameplay-corruption rule: an exact-time external schedule/consequence did not force removal of unowned action effects; an unverified single-action completion sentence could survive when it also contained the displayed choice question; and an invalid receipt tied with a verified schedule could win arbitration and clear legitimate schedule state. The second correction is code checkpoint `9d9709bc2884098975350a0515852d95933f17f9`. It keeps schedule/consequence boundary authority separate from action-completion ownership, reconciles unverified exact-boundary effects even when the clock already equals the floor, restores only the verified schedule lifecycle, and preserves player choices without trusting an unverified completion prompt. No natural-language expression, regex, parser feature, or new reconciliation policy was added. Permanent regressions cover all three findings. Focused time-floor, Phase 3, Event Consequence, Event Progress, HF1 integration, core-invariant suites and the full `node scripts/lumensia-pr-check.mjs 9458633a... HEAD` pass; `git diff --check` is clean. This is the second and final Codex-local remediation authorized by `AGENTS.md` for this review cycle.

Exact docs head `030df74ad2998c4d4d59d8b4fcd819f21b7a3d39` is pushed and clean. Vercel deployment `ECudKwaJmpGikoNYovUtsUAiBqDm` passed. GitHub reports PR #59 open and mergeable; compare has `base_commit == merge_base_commit == 9458633a9657c53c7e875a5cf26538d5f38a0cac`, behind 0, and the local merge-tree is conflict-free. Fresh direct review comment `5432234403` reviewed `030df74ad2` and found no major issue, so current direct P0/P1=0 with no gameplay-blocking P2. Stacked main-only Safety remains unavailable and is not synthesized as PASS. PR #59 changes protected `api/**` and remains human-merge-only.

The required final docs-head review superseded that result. Exact head `37cd4368ce24d094c6536d962e7b1e7eb1b4ffc1` passed Vercel and retained base=merge-base `9458633a...`, behind 0 and mergeability, but fresh review `5035881462` found P0=0 / P1=2. First, a missing/invalid single-action receipt at an exactly coincident verified consequence can preserve the consequence scene while clearing its real player choices. Second, an ordinary sanitized decision preserves visible choices in the response but `runtimeSceneTrusted:false` causes runtime synthesis to discard the awaiting-player signal, allowing AUTO to pass a decision the user still sees. These are player-sovereignty/runtime-state failures, not wording variants. The current Codex-local cycle already used both repository-authorized remediations; do not add a third automatic patch. Keep `MERGE_GATE: FAIL` and hand off for a fresh person/trusted-authorized structural cycle. The next design must carry sanitized choices/awaiting-player state independently from untrusted completion narration and must preserve verified consequence choices independently from action-effect ownership.

The user explicitly opened a fresh correction task for those two P1s. Structural code checkpoint `136c072a745a84b8d8eb55a4ecccd86c09a126d7` separates player-choice runtime authority from narration trust. Reconciliation now preserves the bounded choices shown at an attributed consequence boundary, emits `runtimeChoicesTrusted` only for an applied decision, verified schedule choice, or attributed consequence choice, and lets runtime synthesis carry those sanitized choices plus the incomplete-boundary marker even while completion narration remains untrusted. This lets `localSceneRuntime` persist `unresolved_question`/awaiting-player state and prevents AUTO from crossing the displayed choice. Action effects still require structural clause ownership; verified consequence lifecycle and scene ownership remain separate. No wording/regex/parser coverage, feature, or new gameplay policy was added. Permanent regressions reproduce both review cases. Scene Momentum time-floor, TPP Phase 3, Event Consequence, Event Progress, Turn Hook, Scene Exit, HF1 integration, core invariants, syntax/diff checks, and full `node scripts/lumensia-pr-check.mjs 9458633a... HEAD` pass. A substantive second review found no routing, one-call, `store:false`, cache, canon, schedule/consequence priority, freeze, or player-sovereignty regression.

Fresh review `5036074986` on exact head `084ebcc47b359f04fa3350f189f38b0882eb6a5b` found P0=0 / P1=0 and one player-sovereignty P2: a response that ran past a resolved consequence could keep choices created after the rewound boundary. A delayed prior-head review `5035920744` had reported the schedule analogue: an unsurfaced schedule rewind could retain later-scene choices. Remediation checkpoint `16307a3693fec3889e98d13a8bad43f02264ccf0` closes both with one structural rule. Schedule choices survive only when the model-returned minute exactly equals the schedule boundary and that boundary is visible or structurally identified; attributed consequence choices survive only when the returned minute exactly equals the consequence boundary. Rewound later choices are cleared and never receive runtime authority. Permanent positive/negative regressions cover exact consequence choices, overrun consequence choices, exact structured schedule choices, and unsurfaced schedule rewind choices. Focused decision/event suites and full repository regression pass; no wording inference or policy expansion was added. This is remediation 1/2 of the fresh user-authorized Codex-local task.

Fresh review `5036112001` on exact head `ae594b2cd0fb7a30b98ae2faa20735c43d6d1dc4` found P0=0 / P1=0 and one remaining player-sovereignty P2: when choices were returned exactly at an upcoming schedule minute but the schedule itself was neither visible nor structurally started, the ordinary decision boundary could still win. Remediation checkpoint `c973de0a90f9e53432fc2aec2af4f136d8b00c2e` makes an in-profile exact required schedule authoritative before decision reconciliation whenever the response has choices. The existing `scheduleChoiceAtBoundary` guard then preserves choices only if the schedule was actually visible or structurally identified; otherwise the unrelated choices and prerequisite scene are discarded. No-choice coincident action completion remains unchanged. A permanent regression covers an unsurfaced exact class boundary with unrelated room choices. Focused decision/event/invariant suites and full repository regression pass after second scope review. This is remediation 2/2 of the current Codex-local task.

Exact docs head `6b3a937bbea2565dc1d9bbc521cf2282c5a9d64f` is pushed and clean. Vercel deployment `AQDvPCtEWYTDdDcqUMjmFCwFZSqh` is Ready. Compare against Phase 4 base has `base_commit == merge_base_commit == 9458633a9657c53c7e875a5cf26538d5f38a0cac`, behind 0, and GitHub reports PR #59 open/mergeable. The stacked PR has no main-target Safety result; none is synthesized as PASS.

Fresh direct review `5036179846` on that exact head reports P0=0 / P1=0 but two P2 findings that are blocking under the saved player-sovereignty/schedule-integrity rule. First, an ordinary valid `boundary_kind:choice` receipt can tie an unsurfaced exact required schedule and still leave unrelated choices trusted, so the required schedule never surfaces or starts. Second, a structurally valid interruption receipt can leave an action incomplete while its choice prompt falsely says that action completed; the contradictory prompt then receives runtime trust even though its unowned effects are removed. These are structural arbitration/receipt-consistency failures, not new Korean wording or regex coverage requests.

Both repository-authorized Codex-local remediations for this implementation task are exhausted. Do not add a third automatic code patch in this task. PR #59 remains `MERGE_GATE: FAIL` and must not merge. The next fresh person/trusted-authorized correction task must (1) give an exact required schedule priority over an ordinary valid choice receipt unless the receipt/choice is structurally attributable to that schedule boundary, and (2) validate choice-prompt completion claims against the receipt's completed/interrupted clause set before granting runtime choice/scene authority. Preserve action/clause/effect ownership, external boundary priority, and the one-call/routing/canon/freeze invariants; do not add wording-specific regexes.

The user started that fresh correction task. Code checkpoint `2ad8112aa1caa1b7340cf3aefca95672d65209a4` closes both review findings structurally. A valid ordinary choice receipt now loses a same-minute authoritative schedule/consequence tie unless its verified `boundary_event_id` owns that exact external boundary; an event-owned choice remains available. Separately, a valid choice receipt's completed-clause set is compared with the existing completion classifier for its selected prompt. If the prompt claims completion of a receipt-owned incomplete clause, only that prompt narration loses scene trust and is replaced by the existing deterministic boundary text; bounded choices remain independently player-owned. Receipt-consistent interruption prompts remain visible and trusted. No new natural-language expression, regex, parser coverage, feature, or gameplay policy was added.

Permanent regressions cover an ordinary valid choice receipt at an unsurfaced exact schedule, the positive schedule-owned receipt case, a valid interrupted receipt with a contradictory completion prompt, and a receipt-consistent interruption prompt. Scene Momentum time-floor, TPP Phase 1/2/3, Event Consequence, Event Progress, Turn Hook, Scene Exit, HF1 integration, core invariants, syntax/static checks, `git diff --check`, and full `node scripts/lumensia-pr-check.mjs 9458633a... HEAD` pass. The substantive second review found no change to consequence-owned choices, completed-prefix effect ownership, one canonical call, stable routing, `store:false`, prompt cache/retention, canon, schedules, or META/AUTO/CONTINUE behavior.

Fresh exact-head review `5036302175` on `9ce03c28952132864dae19d4b74aaffca676ce4f` found P1=1 plus four gameplay-blocking P2s. The P1 was a canonical 1,440-minute cap tied with an exact schedule: schedule reconciliation erased the resumable remainder. The P2s were consequence runtime reintroducing a rejected completion prompt, same-question completion claims escaping the question guard, independently verified schedule lifecycle being cleared by unrelated completion prose, and consequence-unowned choices surviving an exact consequence tie. Delayed prior-head review `5036235132` additionally showed that safe missing/invalid-receipt decision context was being discarded and mixed consequence/action prose could remain trusted.

Remediation 2/2 for this fresh task is code checkpoint `9306aff7c3cd09d3b1f32b2a04b8862a216369de`. Canonical turn-limit ownership now defeats an exact schedule tie with or without choices and preserves the timed-action remainder. Existing completion classification is applied to receipt-owned incomplete clause IDs after removing only terminal question punctuation; no wording pattern was added. Safe missing-receipt prompts remain, contradictory prompts lose scene authority, and mixed consequence rows are fail-closed before visible/runtime preservation. Verified schedule lifecycle survives independently from false action prose. Exact consequence choices survive only when the receipt owns that consequence or the sanitized retained consequence scene contains the selected prompt; ordinary unrelated choices are removed. The existing deterministic consequence boundary is used when every raw row is unsafe.

Permanent regressions cover all five current-head findings plus both delayed context/consequence cases and positive controls. Focused Scene Momentum/TPP/Event/Turn Hook/Scene Exit/HF1/core suites, syntax/static, `git diff --check`, full repository regression, and the substantive second review pass. This is the second and final Codex-local remediation in the current implementation task. The next exact-head review is authoritative; if it leaves any P0/P1 or gameplay-blocking P2, keep `MERGE_GATE: FAIL` and hand off rather than adding a third patch.

Exact docs head `6de11a72b24a611bfcab2e503cfd68e9cab848c8` is pushed and clean. Vercel deployment `DEB1siw3y7yPDzjySPzQwJ5LsZKX` is Ready. Compare retains `base_commit == merge_base_commit == 9458633a9657c53c7e875a5cf26538d5f38a0cac`, behind 0, and GitHub reports PR #59 open/mergeable. Stacked main-only Safety is absent and is not synthesized as PASS.

Fresh direct review `5036378635` on that exact head reports P0=0 / P1=0 but three P2 findings that block under the saved player-sovereignty/world-state rule. First, converting every prompt to narration for contradiction detection can misattribute an NPC's explicit first-person completion (`나는 훈련을 마쳤다`) to the PC and erase safe NPC-owned context. Second, a structurally authenticated Director choice can retain its choices/progress but lose receipt-owned Director metadata in `runtimeSynthesisTurn` when narration is untrusted. Third, a missing-receipt choice that continues the exact prior `sceneRuntime.eventProgress` instance can write `event_progress:null`, causing merge to delete authoritative prior progress and completed beats.

Both Codex-local remediations for this implementation task are exhausted. PR #59 remains `MERGE_GATE: FAIL` and must not merge. A fresh person/trusted-authorized structural task must begin from these three cases: preserve source-speaker ownership while evaluating completion contradictions; carry verified Director metadata independently from rejected narration; and represent “discard unowned returned progress mutation while preserving authoritative prior progress” distinctly from explicit progress deletion. Do not add wording-specific regexes or broaden Phase 4 into new gameplay policy.

The user started that fresh structural task. Code checkpoint `a0e55cb7a1d46ca551a3e8ba62a963100bb2c115` closes all three ownership cases without adding parser wording coverage. Choice extraction carries the original source row into contradiction classification, so keyed NPC dialogue retains ownership of explicit first-person completion while subjectless claims about an incomplete PC clause remain fail-closed. Receipt-verified Director metadata is transported to runtime independently from narration trust. Missing-contract reconciliation distinguishes a rejected same-instance progress mutation from an explicit deletion by omitting the untrusted runtime field, preserving the authoritative prior event identity, active beat, and completed beats.

Permanent regressions cover positive and negative speaker ownership, rejected Director narration with retained receipt-owned metadata, and prior-progress preservation without accepting the returned mutation. Focused Scene Momentum/TPP/Event/Turn Hook/Scene Exit/HF1/core suites, syntax/static, `git diff --check`, the substantive second review, and full `node scripts/lumensia-pr-check.mjs 9458633a... HEAD` pass. Schedule/consequence priority, explicit deletion behavior, one canonical call, stable routing, `store:false`, cache/retention, canon, player sovereignty, and META/AUTO/CONTINUE remain unchanged. The new docs exact HEAD still needs push, Vercel, exact-stack verification, and a fresh direct P0/P1 plus gameplay-blocking P2 review; stacked Safety is absent and must not be synthesized.

Before that fresh request, the repository already contained a later review `5036449743` of prior docs exact `0090900d7e2ce3d8e8d9c36317667a4c35bd1b4b`. It found P1=1 and one gameplay-blocking P2: verified `boundary_kind:schedule` receipts lost their structurally located attendance choices, and a completion statement owned by the first of two same-type clauses was reassigned to the incomplete second clause. Remediation 2/2 code `d8a8bede4fae63e5728d7ddfb3d84f3123e998f2` transports choices owned by a verified schedule/consequence receipt independently from narration trust and consumes same-type completion claims against already completed clauses before testing incomplete ones. Contradictory schedule prose remains sanitized, and no wording regex/parser feature was added.

Permanent regressions cover both findings and the contradictory-schedule positive control. All focused suites, syntax/static, `git diff --check`, substantive scope review, and full repository regression pass. Both Codex-local remediations for this fresh task are now used. Push the docs exact HEAD, verify Vercel and exact stack state, and request one authoritative fresh review. If any P0/P1 or gameplay-blocking P2 remains, keep `MERGE_GATE: FAIL` and hand off; do not add a third patch or merge PR #59.

Exact docs head `09bcd880ffd0d830aa625810f7b44b1dc80f748b` passed Vercel deployment `DL6a7DDAH76m2LrVYmPPACPfGfzd`. Compare retains exact base/merge-base `9458633a9657c53c7e875a5cf26538d5f38a0cac`, behind 0, and GitHub reports PR #59 open/mergeable. Fresh direct review `5036544315` reports P0=0/P1=0 but one gameplay-blocking P2: aggregate same-type claim counts cannot distinguish a prompt accurately describing completed `action_1` from one falsely claiming interrupted `action_2` completed.

This is the same repeated-clause root cause after remediation 2/2. Wording-free clause identity cannot be recovered from narration once two clauses share an action type; adding Korean ordinal/position regex would violate the locked Phase 4 cleanup policy. PR #59 remains `MERGE_GATE: FAIL` and must not merge. A fresh structural Phase 3 task must attach bounded `action_N` ownership to decision/completion claims in the canonical response and reconcile by that ID. Preserve choices independently and add both “first completed” and false “second completed” permanent cases. Do not run Preview as a merge gate or add another Phase 4 wording patch while this blocker remains.

The user started that fresh structural task. Code checkpoint `3b3a5468e90777651da51baac452f92deb0d94e5` adds required `decision_completion_clause_ids` to the existing `time_execution` receipt. The canonical call now identifies exactly which `action_N` a displayed decision prompt claims completed; the validator accepts only unique IDs from the canonical plan and rejects missing, invented, or no-choice claim sets. Decision reconciliation compares those IDs directly with `completed_clause_ids`, so a claim owned by interrupted `action_2` loses only its untrusted prompt narration while its bounded choices and awaiting-player state remain. An accurate `action_1` claim remains visible. The consequence path also removes the exact selected source row by structural row identity, preventing rejected prompt narration from re-entering a verified consequence scene. No Korean ordinal expression, completion regex, parser coverage, feature, or new gameplay policy was added.

Permanent regressions cover accurate first-clause completion, false second-clause completion, ordinary and consequence-owned choices, runtime awaiting-player preservation, strict schema transport, missing/invented/no-choice claim sets, and consequence runtime non-reintroduction. TPP Phase 1/2/3, Scene Momentum time-floor, Event Consequence, Event Progress, Turn Hook, Scene Exit, HF1 unit/integration, core invariants, syntax/static, `git diff --check`, the substantive second review, and full `node scripts/lumensia-pr-check.mjs 9458633a... HEAD` pass. The change does not alter one canonical call, stable routing, `store:false`, prompt cache/retention, Context Router budgets, canon, schedule/consequence priority, player sovereignty, or META/AUTO/CONTINUE. Next require a docs exact HEAD push, Vercel, exact-stack verification, and fresh direct exact-head P0/P1 plus gameplay-blocking P2 review. Stacked main-only Safety remains absent and must not be synthesized.
...
8. Preserve PR #58 exact head `9458633a...`, Vercel Ready, direct P0/P1=0, and its clean local/compare stack evidence. Recheck the inconsistent GitHub `mergeable` field only when evaluating that PR's final gate; do not merge it from Codex.
9. PR #59 structural code checkpoint `3b3a5468...` attaches decision completion claims to canonical `action_N` IDs. Exact-head review `5036652816` on docs head `181d6800075910806e3cb6e4a091e1ef322efecb` then found one P1 and two gameplay-blocking P2s: an exact 1,440-minute consequence tie could defeat the canonical turn limit and lose 540 resumable minutes; a schema-valid minor NPC identified only by `speaker_name` could be misread as the PC; and one contradictory consequence row could erase independently safe consequence context from runtime. Structural remediation `3e11906` closes all three without adding wording/ordinal regex: turn-limit authority wins the exact consequence tie while independently verified lifecycle effects remain, dialogue ownership accepts `speaker_key` or non-PC `speaker_name`, and runtime keeps the already-sanitized safe consequence rows rather than globally rejecting them. Positive/negative speaker ownership, exact-cap resumability, and mixed safe/unsafe consequence regressions were added. Focused suites and full `node scripts/lumensia-pr-check.mjs 9458633a... HEAD` pass after substantive second review. Next push this docs checkpoint, verify Vercel and exact base/merge-base `9458633a...`, behind 0/no conflict, then request a fresh exact-head review. Keep `MERGE_GATE: FAIL` until review and required Exact Preview acceptance pass. Do not merge; PR #59 changes protected `api/**` and is human-merge-only.
10. Phase 4 remains **legacy-cleanup only**: prove one TPP-authoritatively replaced fallback with permanent regression, remove exactly that path, rerun focused/full regression, and move to the next target. Add no feature, natural-language coverage, regex coverage, or new reconciliation policy. A removal regression must first be classified as evidence that legacy is still required or Phase 3 is incomplete; do not patch Phase 3 design flaws inside Phase 4. Keep the `owner_kind:world` P2 outside this cleanup.

---

# NEW CHAT START INSTRUCTION

> `docs/LUMENSIA_HANDOVER_CURRENT.md`와 `docs/IMPLEMENTATION_PROGRESS.md`를 먼저 읽고 Lumensia 프로젝트를 그대로 이어가라. 새 프로젝트가 아니다. 완료된 HF1/HF2/HF3부터 Event Director V3까지 다시 분석하지 않는다. Main은 PR #53 merge `1018d8c...`이고, PR #54의 wording-parser 사례는 TPP corpus로 보존하며 새 regex patch를 추가하지 않는다. 현재 branch는 protected stacked `codex/time-plan-parser-phase4-single-completion`, PR #59, exact base `9458633a...`이다. `decision_completion_clause_ids` 구조 보강 뒤 fresh review `5036652816`의 exact-cap consequence, minor-NPC `speaker_name`, mixed safe consequence runtime 문제는 structural code checkpoint `3e11906`에서 닫았고 focused/full regression과 second review가 통과했다. 다음은 docs exact HEAD commit/push → Vercel/exact stack comparison → fresh direct exact-head review다. P0/P1 또는 gameplay-blocking P2가 나오면 repository-authorized 범위에서 structured receipt/validator의 일반 규칙으로 교정하고, zero면 Exact Preview의 accurate-first/false-second, minor-NPC ownership, exact-cap resumability, safe consequence context를 포함한 필요한 PR #59 acceptance를 실행한다. Stacked PR에 없는 main-only Safety를 PASS로 합성하지 않는다. one canonical call, stable routing, `store:false`, prompt cache/retention, budgets, canon, player sovereignty, freeze, 1440 cap을 보존한다. `api/**` 보호 경로는 사람만 병합한다.`
