...
- The first Codex review of `5866bce...` reported P0/P1=0 and three P2 hardening notes. This checkpoint closes all three inside the new helper/tests only: English third-person NPC growth is rejected, overlapping skill names cannot inherit each other's evidence, and combat-plus-training actions still require exact `resolution_log` membership.
...
- Initial-head Codex review found no P0/P1. Its three P2 notes are closed with permanent regressions for English NPC-only attribution, overlapping exact skill names, and mixed combat/training resolution bypass; no router/core/schema change is needed for the closure.
...
> `docs/LUMENSIA_HANDOVER_CURRENT.md`와 `docs/IMPLEMENTATION_PROGRESS.md`를 먼저 읽고 Lumensia 프로젝트를 그대로 이어가라. 새 프로젝트가 아니다. PR #50은 exact reviewed head `1eb316b...`에서 main `88ce7b4...`로 merge됐고 merge tree가 reviewed tree와 정확히 같다. 완료된 HF1/HF2/HF3부터 Awakening / Talent Evolution V1까지 다시 분석하지 않는다. 현재 branch는 `codex/combat-growth-v2`, open PR은 #51, initial code/test head는 `5866bce...`이다. 이 후보는 existing `stat_progress`, `skill_experience`, `resolution_log`를 사용해 player-owned current-scene learning evidence, per-ability evidence tier, grade-aware bounded gain, exact combat ability matching, deliberate-training support, zero/negative rejection, META/AUTO/CONTINUE freeze, pre-Momentum filtering을 추가한다. Initial-head Codex는 P0/P1=0과 P2 세 건을 보고했고, 현재 checkpoint는 English NPC attribution, overlapping skill identity, mixed combat/training resolution bypass를 helper/test 안에서 모두 닫는다. 새 schema/save root/migration/endpoint/serverless function/canonical `api/chat.js`/`app.js` change/second model call은 없고 correction count는 0/5다. 다음은 새 exact-head clean-LF full check/diff/second review, Safety/Vercel/fresh Codex review, Exact Preview health/basic-training/mere-use/failure-correction/NPC-only/question-only/freeze acceptance다. 새 통제 가능한 P0/P1은 exact-head→원인→최소 수정→focused/full→새 head→Safety/Vercel→fresh review 순서로 자동 계속한다. 같은 유형 재발, 여러 핵심 시스템 확장, canon/player sovereignty/one-call/stable-routing 영향, 5회 소진, 또는 상충하는 안전 지침일 때만 중단한다. 보호 경로이므로 최종 병합은 사람만 수행한다.`
