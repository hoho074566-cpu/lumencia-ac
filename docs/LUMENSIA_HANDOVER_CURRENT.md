...
- Initial exact Preview on `4a24a77...` compressed `밖으로 간다` through corridor/stairs to the true A-building exterior, but still returned three generic destination suggestions. The local remediation now keeps choices only with structured failure, combat, event/schedule, high-importance, decision-language, or direct NPC question evidence; routine travel choices are removed and `마법과 건물` is not misclassified as combat.
