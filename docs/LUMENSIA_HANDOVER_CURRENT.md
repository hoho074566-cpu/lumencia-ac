...
1. Commit/push the correction docs checkpoint on PR #60. Verify its new exact HEAD against stacked base `9458633a9657c53c7e875a5cf26538d5f38a0cac`: base/merge-base equality, behind 0, no conflict, and Vercel PASS.
2. Request the mandatory fresh direct exact-current-HEAD Codex review. The previous `5b64b8d...` review cannot approve the correction HEAD. Do not synthesize main-target-only Safety as PASS on the stacked PR.
3. If direct P0/P1=0, run only the representative Exact Preview Narrative Time Policy corpus: ordinary explicit training prose, compressed meal/travel/evening sequence, long-duration compression, required-schedule interruption, direct time question, routine action non-fragmentation, meaningful NPC-question STOP, and CONTINUE/META/AUTO/player-sovereignty/one-call invariants. Do not explore natural-language permutations.
4. Fix only current-scope P0/P1 within repository-authorized remediation. Add no wording-specific time regex, parser semantics, additional model call, TPP rewrite, new gameplay policy, P2/P3 hardening, or further time-engine phase.
5. Because `api/lib/context-router.js` is protected, this PR remains human-merge-only. After closure, update both progress documents, read the latest Narrative Engine NEXT ACTION, and continue there rather than further refining time precision.
