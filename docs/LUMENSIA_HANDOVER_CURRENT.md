...
- Current code head: `85fef8008eb0c03a30615f99e9a408526a6b9bd7` on `codex/skill-learning-v1`, based directly on the verified PR #47 merge.
...
- A normal player-owned GAME turn may accept at most two learning rows, each +1..15. Each row requires a bounded independent skill name, concrete `basis`, causal `reason`, and visible training/instruction/correction/practice/combat-learning evidence. Existing-skill names, spacing aliases, generic placeholders, basis-less claims, ordinary movement, duplicate rows, and excess candidates are rejected.
...
- META and CONTINUE emit an empty learning delta. AUTO normalizes the bounded state but cannot progress PC learning without a player action. Accepted learning is resolved before Scene Momentum so rejected model rows cannot fake State Delta.
...
- Permanent coverage is `scripts/tests/skill-learning-v1.test.mjs`. Dedicated and affected Context Router, authority-tail, Scene Momentum, Goal V2 legacy-parser, CONTINUE, core-invariant, and save-key tests pass. The clean-LF full PR check passed on the initial code checkpoint; it must be rerun on the final docs head before publication.
- Both allowed local hardening rounds are used: latest-authoritative alias normalization, then maximal skill/candidate minimum-context bounds. The final substantive review found no P0/P1 blocker; additional automatic remediation is not permitted in this cycle.
...
4. Rerun the authoritative clean-LF full PR check on the final docs head, inspect the exact diff, and perform no third automatic fix/review iteration. If a P0/P1 remains, report FAIL and stop.
5. Push the exact head and open/update the focused protected-path PR, then require fresh exact-head Safety, Vercel, and Codex P0/P1=0 with current main/merge-base/no-conflict revalidation.
6. On the Exact Preview, verify real training creates bounded candidate progress, ordinary movement creates none, AUTO/META/CONTINUE preserve growth state, existing skills are not duplicated, and a near-complete candidate unlocks once at F grade.
...
> `docs/LUMENSIA_HANDOVER_CURRENT.md`와 `docs/IMPLEMENTATION_PROGRESS.md`를 먼저 읽고 Lumensia 프로젝트를 그대로 이어가라. 새 프로젝트가 아니다. PR #47은 reviewed head `866c991...`에서 main `54327ea...`로 merge됐고 merge tree가 reviewed tree와 정확히 같다. 완료된 HF1/HF2/HF3, 12-case acceptance, Scene Purpose/Exit/Turn Hook, Event Consequence, Goal Tick, Off-screen Progression, Scene Novelty, NPC↔NPC Relationship V1, Faction Social V1을 다시 분석하지 않는다. 현재 branch는 `codex/skill-learning-v1`, code head는 `85fef80...`이며 활성 범위는 기존 `pc.skillCandidates` 기반의 증거 제한 학습 누적, 후보/이력/문맥 상한, 100점 F등급 습득, AUTO/META/CONTINUE 동결, UI 표시뿐이다. 새 save root/migration/endpoint/model call, canonical `app.js` 변경, 즉흥 각성, 기존 스킬 재생성은 범위 밖이다. 두 로컬 수정 라운드는 이미 소진됐으므로 final clean-LF/full diff/hosted review/Exact Preview 뒤 protected-path human merge로 진행한다.`
