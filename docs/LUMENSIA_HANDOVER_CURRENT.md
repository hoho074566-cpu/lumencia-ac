...
- PR #51 is open from `codex/combat-growth-v2`. Current code/test checkpoint: `451679f8509c6318b3294cd4aac9a22028637426`; initial implementation head was `5866bced8963e2dcdd70c0899c3717dddb6a3b88`.
...
- Dedicated Combat Growth plus affected Skill Learning, Awakening, Scene Momentum, Orchestration, Context Router, CONTINUE, core, debug, and save suites pass. Correction code checkpoint `451679f...` also passes the authoritative dependency-free clean-LF full repository check.
- Reviews of `5866bce...`, `c715ec5...`, and `7b63bab...` reported P0/P1=0. `d125b65...` closed the accumulated review P2s; corrections remain inside `lib/combat-growth.js` and its dedicated tests.
...
- Current code/test checkpoint: `451679f8509c6318b3294cd4aac9a22028637426` on `codex/combat-growth-v2`, based directly on the verified PR #50 merge. PR #51 is open and unmerged; confirm the live branch tip before any hosted decision because a docs-only checkpoint follows this code commit.
...
- `scripts/tests/combat-growth-v2.test.mjs` plus all affected growth/router/momentum/orchestration/continue/core/debug/save suites pass. Code checkpoint `451679f...` also passes `git diff --check` and the dependency-free authoritative clean-LF full PR check.
...
- The user-authorized P0/P1 correction allowance applies: up to five exact-head correction cycles for a new controllable issue type. Stop only for repeated same-type recurrence, cross-core expansion, canon/player-sovereignty/one-call/stable-routing impact, five exhausted cycles, or conflicting unsafe guidance. Current count for this PR is **2/5**; `57085b0...` covered the exact imperative wording and `451679f...` covers the adjacent natural narration form while preserving named-NPC rejection.
- Current blocker: correction 2/5 still failed the exact Preview when successful application was narrated as `엉키지 않은 채 진입을 끝냈다`. Automatic correction is stopped under condition 1. If the user explicitly resumes, first design a structural helper-local predicate around player-owned deliberate action + visible instructor guidance + adjacent applied final repetition/cause identification, with explicit NPC-subject rejection, instead of appending more success synonyms; then restart the exact-head protocol.
...
3. Confirm PR #51 live branch tip and read the repeated same-type Preview blocker above. Automatic correction is stopped under user condition 1; do not append another wording-specific success synonym without explicit user direction.
4. If the user explicitly resumes, design the smallest structural helper-local evidence predicate for player-owned deliberate action + visible instructor guidance + adjacent applied final repetition/cause identification, while preserving explicit named-NPC rejection. Avoid canonical core, schema, save, routing, sovereignty, or model-call changes.
5. Then run focused/full regression, publish a new exact head, require GitHub Safety/Vercel/fresh Codex review, and rerun the affected Preview cases. Do not extract a protection token or bypass Preview protection.
6. Only after every gate and affected Preview case passes, report PR #51 ready for the user's human merge. Never merge this protected-path PR from Codex.

---

# NEW CHAT START INSTRUCTION

> `docs/LUMENSIA_HANDOVER_CURRENT.md`와 `docs/IMPLEMENTATION_PROGRESS.md`를 먼저 읽고 Lumensia 프로젝트를 그대로 이어가라. 새 프로젝트가 아니다. PR #50은 exact reviewed head `1eb316b...`에서 main `88ce7b4...`로 merge됐고 merge tree가 reviewed tree와 정확히 같다. 완료된 HF1/HF2/HF3부터 Awakening / Talent Evolution V1까지 다시 분석하지 않는다. 현재 branch는 `codex/combat-growth-v2`, open PR은 #51, correction code/test checkpoint는 `451679f...`이며 live branch tip을 다시 확인한다. 이 후보는 existing `stat_progress`, `skill_experience`, `resolution_log`를 사용해 player-owned current-scene learning evidence, per-ability evidence tier, grade-aware bounded gain, exact combat ability matching, deliberate-training support, zero/negative rejection, META/AUTO/CONTINUE freeze, pre-Momentum filtering을 추가한다. Preview에서 basic training `D 0→1`과 mere-use `1→1`은 통과했지만 failure-plus-visible-correction은 세 번 `evidence_tier:0`으로 거부됐다. Correction 1/5 `57085b0...`은 exact imperative wording을 인정했고 correction 2/5 `451679f...`은 adjacent instructor guidance→successful PC repetition을 연결했다. Exact code/docs head `d980b33...`은 Vercel 배포 후 `이번에는` deliberate training, 교관의 골반/체중 지적, 그리고 `마지막 반복에서는 지시한 각도와 순서를 적용해 발이 엉키지 않은 채 진입을 끝냈다`까지 생성했지만 다시 `신체 1→1`, `evidence_tier:0`, `rejected_stat_count:1`이었다. 같은 유형 반복 재발이므로 사용자 자동 중단 조건 1에 따라 추가 코드 수정은 멈춘다. 사용자가 명시적으로 재개하면 성공 동의어를 더하는 대신 player-owned deliberate action + visible instructor guidance + adjacent applied repetition/cause identification + explicit NPC-subject rejection을 구조적으로 판정하는 helper-local 최소안을 먼저 설계한다. 새 schema/save root/migration/endpoint/serverless function/canonical `api/chat.js`/`app.js` change/second model call은 없다. 보호된 Preview의 token/cookie/local storage를 읽거나 protection을 우회하지 말고 동일 설정 요구를 반복하지 않는다. 보호 경로이므로 최종 병합은 사람만 수행한다.`
