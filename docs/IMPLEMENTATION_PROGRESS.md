...
- Latest hosted Adaptive Time Scale V2 head: `44ba95731b366e1f68b7dd970efdfcb4d669fc06`; Safety #401, Vercel, current-base mergeability, full local regression, and the future-clock Preview case pass. Its ranged-schedule Preview case exposed a visible-noon/11:40 mismatch; the user explicitly resumed the same-type structural correction, which is locally complete and pending commit/push.
...
- Focused/affected suites and the full repository regression pass after the current fresh-review correction.
...
- Adapter/health candidate `0.8.6` adds `adaptive_time_scale_v2`, selected profile, and version telemetry without a save/schema/root change.
...
- Dedicated and affected Scene Momentum, time-floor, Scene Exit/Purpose/Hook, Orchestration, authority-tail, core invariant, NPC Motivation, and Event Director tests pass. Exact code checkpoint `ed5d2b0...` passes the full repository check in a clean-LF worktree.
...
## NEXT ACTION
1. Commit and push the surfaced-schedule structural correction on top of hosted head `44ba957...`, then confirm the new exact head is current against main `1018d8c...`.
...
3. On the new Exact Preview, rerun the exact `11:00 → 충분한 훈련 → 정오 종/일정 선택` case first and require `12:00`, then a future-only schedule mention and ordinary sleep/shortening shared paths. Deterministic tests remain authoritative for due-consequence routing; rerun the full matrix only if a shared-path regression appears.
...
- Current candidate: Adaptive Time Scale V2 in PR #54 on `codex/adaptive-time-scale-v2`; hosted head `44ba957...` passed Safety #401/Vercel/current-base mergeability and future-clock acceptance, but its ranged-schedule case exposed a visible noon / 11:40 clock mismatch. The explicitly resumed structural surfaced-boundary correction and full regression pass locally; commit/push, a new exact-head hosted/fresh-review cycle, and affected Preview reruns remain.
