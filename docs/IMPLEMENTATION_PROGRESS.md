...
- Current blocker is hosted/live validation only: local structural implementation and all regressions pass. The new exact-head authority cycle and Preview confirmation remain.
...
## NEXT ACTION
1. Commit and push this structural correction docs checkpoint, then confirm the new exact live branch head against code checkpoint `717e85c...`.
2. Require GitHub Safety, Vercel Ready, and a fresh exact-head Codex review. Revalidate main, merge-base, no conflict, one-call/routing/schema/save invariants.
3. On the Exact Preview, rerun a fresh failure→instructor guidance→applied final repetition/cause case and confirm bounded growth; then verify NPC-only/question-only and META/AUTO/CONTINUE freeze. Do not extract a protection token or bypass Preview protection.
...
- Current candidate: Combat Growth V2 in open PR #51 on `codex/combat-growth-v2`; explicit-resume correction checkpoint `717e85c...` replaces completion-word enumeration with a bounded structural instructor-guidance→applied-final-repetition/cause predicate. The exact failed live response passes permanently, while missing-guidance and named-NPC variants remain rejected. Focused/clean-LF full tests pass and correction count is 3/5. New exact-head hosted/fresh-review and Preview acceptance remain; there is still no router/core/schema change.
