...
- Current blocker: correction 4/5 passes focused and authoritative clean-LF full regression locally. Publish the docs checkpoint, require exact-head Safety/Vercel/fresh Codex review, and rerun the affected Preview correction case. If the same split-row rejection recurs, stop under condition 1 instead of automatically consuming correction 5/5.
...
## NEXT ACTION
1. Confirm PR #51 live branch tip includes correction-4 code checkpoint `5235c6c...`; focused and authoritative clean-LF full regression already pass.
2. Publish the docs checkpoint, then require GitHub Safety/Vercel/fresh Codex review on that exact head.
3. Rerun the affected Exact Preview correction case. Do not extract a protection token or bypass Preview protection; if the same split-row rejection recurs, stop under condition 1 rather than automatically consuming correction 5/5.
4. Only if every hosted gate, fresh review, and affected Preview case passes, report PR #51 ready for the user's human merge. Codex must not merge it.
...
- Current candidate: Combat Growth V2 in open PR #51 on `codex/combat-growth-v2`; user-authorized correction 4/5 checkpoint `5235c6c...` adds only the immediate two-narration-row final-application/cause bundle with combined attribution, and focused plus authoritative clean-LF full regression pass. Exact-head Safety/Vercel/fresh review and the affected Preview rerun are next; there is still no router/core/schema change.
