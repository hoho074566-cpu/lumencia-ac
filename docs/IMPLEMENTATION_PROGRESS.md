...
- Working branch: `codex/time-plan-parser-phase2`, stacked on reviewed Phase 1 head `2ad5e28321b274c36a20a9ae52c1141d55d6d384`.
...
- Correction code head `5f7fbc5ebaa16ab74c28b2f92fabe238c084d647` closes the prior six P1s plus fresh exact-head review `5029322166` P1=4 at shared structured boundaries: decimals/date tokens/sleep subtype are preserved, incomplete connectors and unresolved `-면`/`-할 때` conditions are noncommitted, fixed past-date elapsed status survives gated clocks, the clock period taxonomy now matches legacy `낮` semantics, bare sleep verbs require lexical boundaries, and terminal deadlines remain fallback-only. Exact reproductions plus positive successor/date-particle cases are permanent tests. Phase 2/Phase 1, Adaptive Time Scale, Scene Momentum correctness/time-floor, syntax/static, `git diff --check`, and full repository regression pass. One destination-substring P2 remains deferred as non-blocking. Push, Vercel, stack mergeability, and fresh exact-current-HEAD P0/P1 review are pending; any P0/P1 starts the next correction immediately and cycle counts cannot stop closure. The main-only Safety workflow has no stacked run. Schedule/consequence arbitration, mutation, narration, boundary reconciliation, API routing, one-call behavior, saves, canon, cache, and budgets remain unchanged.
...
2. Publish PR #56 correction code head `5f7fbc5...` plus this progress checkpoint, then require Vercel, current-stack mergeability, and fresh exact-current-HEAD direct P0/P1=0. Do not start Phase 3 or merge before those gates.
3. Any new P0/P1 starts the next correction immediately at structured clause ownership/normalization boundaries; cycle counts are not a stop. Do not resume PR #54 wording-specific patches or promote non-blocking Phase 1 P2 cases prematurely.
4. After Phase 2 exact-current-HEAD P0/P1=0 and all available gates pass, record the checkpoint and start the separate Phase 3 branch. Main-only Safety remains pending until stack resolution.
5. Once Phase 2 is stable, start Phase 3 separately for plan-based schedule/consequence/decision boundary reconciliation. Preserve completed prefix effects, strip only unfinished suffix effects, and align narration with authoritative time. Phase 4 legacy cleanup waits for full permanent regression and Exact Preview evidence.
...
- Current active candidate: protected stacked TPP Phase 2 PR #56 at local correction code head `5f7fbc5...`, based on reviewed Phase 1 exact head `2ad5e283...` / PR #55. Ten known review P1s are locally closed and focused/full regression pass; push, Vercel, current-stack mergeability, and fresh exact-current-HEAD review are pending. `MERGE_GATE: FAIL`; Phase 3 has not started.
