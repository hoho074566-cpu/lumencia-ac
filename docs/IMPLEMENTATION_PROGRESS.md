...
## NEXT ACTION
1. Commit and push this PR #51 docs checkpoint, then confirm the new exact live branch head and its tree against correction code checkpoint `451679f...`.
...
- Current candidate: Combat Growth V2 in open PR #51 on `codex/combat-growth-v2`; correction code checkpoint `451679f...` recognizes both explicit applied-instruction wording and the natural adjacent instructor-guidance→successful-PC-repetition form without transferring an explicitly named NPC's success. Both live responses are permanent regressions, focused/clean-LF full tests pass, and correction count is 2/5. New exact-head hosted gates/fresh review and affected Preview acceptance remain; there is still no router/core/schema change.
