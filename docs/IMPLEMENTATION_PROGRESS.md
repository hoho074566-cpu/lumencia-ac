...
- The correction tags raw hook rows with the same non-serializing exact-source identity used by growth validators, carries it only when the selected consequence row is deterministically rewritten, and remaps/drops receipt owners in the same lifecycle replacement operation. Runtime-synthesized expiry rows inherit no model ownership. Decision reconciliation now keeps emotion rows only for NPC keys present in retained decision dialogue; post-boundary/unretained speakers still fail closed. Permanent regressions cover selected-consequence reordering and retained-vs-unretained decision emotions. Event Consequence, Phase 3/time-floor/HF1 focused suites and the full repository check pass.
- NEXT ACTION: commit/push this correction and docs checkpoint, require new exact-head Vercel and fresh direct P0/P1=0, then run the prepared Exact Preview Phase 3 boundary corpus. Continue any new P0/P1 structurally; do not add narration regex patches.
