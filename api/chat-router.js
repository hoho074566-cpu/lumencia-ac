...
function prefixEffectsFromSegments(turn,prefixSegments=[],{travelPrefix=false,travelDestination='',completedPrefixActionTypes=[],completedPrefixClauseIndexes=[],structuredExecution=false}={}){
  ...
  for(const [field,value] of Object.entries(delta)){if(!Array.isArray(value)||blockedArrays.has(field)||field==='npc_state_updates')continue;const kept=value.filter(row=>prefixEffectMatchesCompletedClauses(row,segments,completedPrefixActionTypes));if(kept.length)preservedDelta[field]=kept;}
...
function turnBeforePlayerChoice(turn){
  ...
  return{...turn,scene_title:'',scene_summary:'',scene:scene.slice(0,decisionIndex),choices:[],_choice_prompt_text:String(scene[decisionIndex]?.text||''),_decision_evidence_ordered:true};
}
...
function applySceneMomentumTimeFloor(incoming,turn,mode='game',consequenceLifecycle=null,consequenceVisibleScene=[]){
  ...
  const decisionPlan=intent?.structuredExecutionPlan||intent?.structuredDecisionPlan,terminalClause=array(decisionPlan?.clauses).at(-1),completionIntent=terminalClause?{...intent,completionActionType:String(terminalClause.action_type||'')}:intent,completionEvidence=timedActionCompletionEvidence(turn,completionIntent,incoming?.action||'',incoming?.saveState?.pc?.name||''),decisionEvidenceTurn=hasMeaningfulStop?turnBeforePlayerChoice(turn):turn,completionBeforeDecision=hasMeaningfulStop?timedActionCompletionEvidence(decisionEvidenceTurn,completionIntent,incoming?.action||'',incoming?.saveState?.pc?.name||''):completionEvidence,completedBeforeChoice=!hasMeaningfulStop||completionBeforeDecision,rawDecisionBoundary=structuredDecisionBoundaryMinutes(decisionEvidenceTurn,decisionPlan,current,hasMeaningfulStop,completionBeforeDecision),decisionBoundary=rawDecisionBoundary==null?null:Math.min(profileMax,Math.max(0,Number(rawDecisionBoundary)||0)),earlierInterruptionBeforeConsequence=Boolean(consequenceWithinProfile&&current<consequenceBoundary&&(structuredInterruption||(hasMeaningfulStop&&!completedBeforeChoice)));
