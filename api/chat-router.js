// LUMENSIA MOBILE V1.5.6 — Stable Router + Event Director V2.1 + Living World V1
// External API version: 0.8.4
...
export default async function handler(req,res){
  ...
  try{
    ...
    if(mode==='meta'){if(data.turn?.state_delta){data.turn.state_delta.stat_progress=[];data.turn.state_delta.skill_experience=[];data.turn.state_delta.skill_learning=[];data.turn.state_delta.awakening_progress=[];data.turn.state_delta.talent_evolution=[];}const pipeline={pipeline:'meta-full-stable-v156',stages:1,qa_result:'SKIP',rewrite_applied:false,background_sim:false,context_router:telemetry,event_director_v2:telemetry?.event_director_v2||null,scene_orchestration:telemetry?.scene_orchestration||null,scene_orchestration_v1:true,npc_motivation_v1:true,npc_goal_v2:true,relationship_reason_v1:true,faction_social_v1:true,combat_growth_v2:true,skill_learning_v1:true,awakening_talent_v1:true};data.pipeline=pipeline;setAdapterRoute(data,mode,pipeline,telemetry);return res.status(200).json(data);}
    ...
    const combatGrowthTelemetry=compactCombatGrowthTelemetry(combatGrowthState);
    ...
    const pipeline={pipeline:incoming0.qualityPipeline===false?'single-writer-stable-v156-hf1':'single-pass-q3-stable-v156-hf1',stages:1,qa_result:incoming0.qualityPipeline===false?'SKIP':'LOCAL_GUARD',rewrite_applied:false,background_sim:false,background_local:incoming0.backgroundSim!==false,offscreen_progression:offscreenProgression.telemetry,offscreen_progression_v2:true,living_world_v1:true,combat_engine:isCombatLike(incoming.action),combat_growth:combatGrowthTelemetry,combat_growth_v2:true,runtime_synthesized:true,continuation_beats:array(sceneRuntime.remaining_beats).length,context_router:telemetry,event_director_v2:telemetry?.event_director_v2||null,scene_momentum:sceneMomentum,scene_momentum_v1:true,scene_novelty:sceneNovelty,scene_novelty_v1:true,scene_purpose:sceneRuntime.purpose||null,scene_purpose_v1:true,scene_exit_condition:sceneRuntime.exit_condition||null,scene_exit_condition_v1:true,turn_hook:sceneRuntime.turn_hook||null,turn_hook_v1:true,scene_orchestration:sceneRuntime.orchestration||null,scene_orchestration_v1:true,event_consequence:consequenceLifecycle,event_consequence_v1:true,npc_motivation_v1:true,npc_goal_v2:true,npc_goal_tick:sceneRuntime.goal_tick||null,npc_goal_tick_v1:true,relationship_reason_v1:true,faction_social:factionSocialTelemetry,faction_social_v1:true,skill_learning:skillLearningTelemetry,skill_learning_v1:true,awakening_talent:awakeningTalentTelemetry,awakening_talent_v1:true,note:'V1.5.6 Scene Momentum Recovery HF1 keeps one core model call while restoring semantic action compression, deterministic State Delta/stall tracking, NPC initiative, downtime skip, and meaningful stop points. Living World V1 records bounded public off-screen schedule starts and propagates only explicitly confirmed completions to absent known NPC state and the background digest. Multi-System Scene Orchestration V1 selects one primary driver and at most one causal secondary response while treating relationship, faction, growth, off-screen, and novelty systems as effects. Combat Growth V2 accepts only PC-attributed, evidence-backed stat and existing-skill adaptation, applies grade-aware caps, and freezes META/AUTO/CONTINUE. Skill Learning V1 persists bounded candidates; Awakening / Talent Evolution V1 keeps rare growth behind milestone and mythic-source gates.'};
