...
function structuredTextClaimsIncompleteCompletion(text='',sourceRow={},plan={},executionAuthority=null,intent={},action='',actorName=''){
  ...
  const source=object(sourceRow),speakerOwnsFirstPerson=String(source.kind||'')==='dialogue'&&Boolean(String(source.speaker_key||'').trim()),clauses=array(plan?.clauses).map((clause,position)=>({...object(clause),_id:String(clause?.clause_id||`action_${Number(clause?.index||position+1)}`)})),segments=String(prompt).split(/(?<=[.!?。！？])|\n+/).map(value=>value.trim()).filter(Boolean),candidates=[...new Set(segments.flatMap(value=>[value,value.replace(/[?？]+$/u,'').trim()]).filter(Boolean))],promptTurn=(value)=>({scene:[{...source,text:value}],choices:[]}),claimCounts=new Map(),completedCounts=new Map();
...
function applySceneMomentumTimeFloor(incoming,turn,mode='game',consequenceLifecycle=null,consequenceVisibleScene=[],runtimeAuthority={}){
  ...
  const scheduleTiedAtTurnLimit=Boolean(turnLimitReached&&scheduleBoundary===1440),reachedScheduledBoundary=!scheduleTiedAtTurnLimit&&(surfacedScheduledBoundary||crossedScheduledBoundary||hasMeaningfulStop&&scheduleBoundary!=null&&scheduleBoundary<=profileMax&&current===scheduleBoundary);
