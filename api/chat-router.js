...
function prefixNpcStateUpdate(row,segments=[]){
  ...
  const supported=new Set(['location','status','current_goal','long_term_goal','short_term_goal','obstacle','next_activity','next_location','last_seen']),kept={npc_key:key};
...
function prefixEffectsFromSegments(turn,prefixSegments=[],{travelPrefix=false,travelDestination='',completedPrefixActionTypes=[],completedPrefixClauseIndexes=[],structuredExecution=false}={}){
  ...
  for(const [field,value] of Object.entries(delta)){if(!Array.isArray(value)||blockedArrays.has(field)||field==='npc_state_updates')continue;const kept=value.filter(row=>prefixEffectMatches(row,segments));if(kept.length)preservedDelta[field]=kept;}
...
function applySceneMomentumTimeFloor(incoming,turn,mode='game',consequenceLifecycle=null,consequenceVisibleScene=[]){
  ...
  if(preserveChoiceDirectorState){turn.event_progress=trustedChoiceDirectorProgress;turn.state_delta.active_events_add=mergePreservedRows(turn.state_delta.active_events_add,trustedChoiceDirectorActiveAdds);if(trustedChoiceDirector)turn.director=trustedChoiceDirector;}
