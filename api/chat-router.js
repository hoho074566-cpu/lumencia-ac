...
function prefixNpcScheduleMatches(row,segments=[],registry=CHARACTER_REGISTRY){
  const key=String(row?.npc_key||row?.key||'').trim(),labels=[key,String(registry?.[key]||'').trim().toLowerCase()].filter(value=>value.length>=2),attributed=array(segments).filter(segment=>labels.some(label=>String(segment||'').toLowerCase().includes(label)));
  return Boolean(labels.length&&attributed.length&&consequenceNpcScheduleMatches(row,attributed));
}
...
function turnBeforePlayerChoice(turn){
  ...
  const item=object(scene[decisionIndex]),source=String(item.text||''),parts=[...source.matchAll(/[^.!?。！？\n]+[.!?。！？]?/gu)].map(match=>({text:String(match[0]||'').trim(),index:match.index??0,question:/[?？]/.test(match[0]),score:choicePromptScore(match[0],choices)})).filter(row=>row.text),questionParts=parts.filter(row=>row.question),promptPart=[...(questionParts.length?questionParts:parts)].sort((left,right)=>right.score-left.score||right.index-left.index)[0]||null,prefix=promptPart?source.slice(0,promptPart.index).trim():'',evidenceScene=[...scene.slice(0,decisionIndex),...(prefix?[{...item,text:prefix}]:[])];
  return{...turn,scene_title:'',scene_summary:'',scene:evidenceScene,choices:[],_choice_prompt_text:String(promptPart?.text||source),_decision_evidence_ordered:true};
}
