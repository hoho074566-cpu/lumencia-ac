...
function prefixEffectsFromSegments(turn,prefixSegments=[],{travelPrefix=false,travelDestination='',completedPrefixActionTypes=[],completedPrefixClauseIndexes=[],structuredExecution=false}={}){
  ...
  const npcStateUpdates=array(delta.npc_state_updates).filter(row=>consequenceEffectMatches(row,segments)),npcScheduleUpdates=array(delta.npc_schedule_updates).filter(row=>consequenceNpcScheduleMatches(row,segments)),verifiedTravelLocation=travelPrefix&&travelDestination&&travelDestinationReachedForReconciliation(delta.new_location,travelDestination),newLocation=travelPrefix&&(verifiedTravelLocation||consequenceEffectMatches(delta.new_location,segments))?String(delta.new_location||'').trim():'',pcStatus=consequenceEffectMatches(delta.pc_status,segments)?String(delta.pc_status||'').trim():'';
...
function structuredPrefixEffectsForShortening(turn,plan={},applied=0){
  ...
  const elapsed=Math.max(0,Number(applied)||0),requireUpperCompletion=turn?._boundary_evidence_unordered===true,prefix=array(plan.clauses).slice(0,-1).filter(clause=>{const startMin=Number(clause?.start_min_minutes),startMax=Number(clause?.start_max_minutes),completeMin=Number(clause?.complete_min_minutes),completeMax=Number(clause?.complete_max_minutes);return[startMin,startMax,completeMin,completeMax].every(Number.isFinite)&&completeMax>startMax&&(requireUpperCompletion?completeMax:completeMin)<=elapsed;});
...
function turnBeforePlayerChoice(turn){
  const scene=array(turn?.scene),choices=array(turn?.choices),candidates=scene.map((item,index)=>({index,question:/[?？]/.test(String(item?.text||'')),dialogue:String(item?.kind||'')==='dialogue',score:choicePromptScore(item?.text,choices)})).filter(row=>row.question||row.dialogue),scored=[...candidates].sort((left,right)=>right.score-left.score||right.index-left.index),questionIndexes=candidates.filter(row=>row.question).map(row=>row.index),dialogueIndexes=candidates.filter(row=>row.dialogue).map(row=>row.index),decisionIndex=scored[0]?.score>0?scored[0].index:questionIndexes.at(0)??dialogueIndexes.at(-1)??scene.length;
  return{...turn,scene_title:'',scene_summary:'',scene:scene.slice(0,decisionIndex),choices:[],_choice_prompt_text:String(scene[decisionIndex]?.text||'')};
}
function turnBeforeVisibleBoundary(turn,matchesBoundary){
  const kept=[];
  for(const item of array(turn?.scene)){
    const segments=String(item?.text||'').split(/(?<=[.!?。！？])|\n+/).map(value=>value.trim()).filter(Boolean),before=[];
    for(const segment of segments){if(matchesBoundary(segment))return{...turn,scene_title:'',scene_summary:'',scene:[...kept,...(before.length?[{...object(item),text:before.join(' ')}]:[])],choices:[],_boundary_evidence_unordered:false};before.push(segment);}
...
function turnBeforeScheduleBoundary(turn,boundaryRows=[]){
  const rows=array(boundaryRows);return turnBeforeVisibleBoundary(turn,segment=>rows.some(row=>scheduleBoundaryOccurred({scene:[{kind:'narration',text:segment}]},row)));
}
function turnBeforeConsequenceBoundary(turn,visibleScene=[]){
  ...
  return turnBeforeVisibleBoundary(turn,segment=>{const text=String(segment||'').trim().toLowerCase();return targets.some(target=>text.includes(target)||target.includes(text));});
}
