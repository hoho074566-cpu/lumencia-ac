...
function prefixEffectsFromSegments(turn,prefixSegments=[],{travelPrefix=false,completedPrefixActionTypes=[],completedPrefixClauseIndexes=[],structuredExecution=false}={}){
  const segments=array(prefixSegments).map(value=>String(value||'').trim().toLowerCase()).filter(Boolean),empty={preserved_delta:{},npc_state_updates:[],new_location:'',pc_status:'',completed_prefix_action_types:completedPrefixActionTypes,completed_prefix_clause_indexes:completedPrefixClauseIndexes,structured_execution:structuredExecution};if(!segments.length)return empty;
  const delta=object(turn?.state_delta),blockedArrays=new Set(['active_events_add','active_events_remove','completed_events_add','scheduled_events_add','scheduled_events_remove','scheduled_events_complete','hooks_add','hooks_update','delayed_consequences_add','npc_schedule_updates']),preservedDelta={};
  for(const [field,value] of Object.entries(delta)){if(!Array.isArray(value)||blockedArrays.has(field)||field==='npc_state_updates')continue;const kept=value.filter(row=>consequenceEffectMatches(row,segments));if(kept.length)preservedDelta[field]=kept;}
  if(Number(delta.fatigue_delta||0)!==0&&segments.some(segment=>/(?:피로|지쳤|지침|탈진|기력|힘이\s*빠)/.test(segment)))preservedDelta.fatigue_delta=Number(delta.fatigue_delta);
  if(Number(delta.gold_delta||0)!==0&&segments.some(segment=>/(?:보상|상금|지급|금화|골드|돈)/.test(segment)))preservedDelta.gold_delta=Number(delta.gold_delta);
  const npcStateUpdates=array(delta.npc_state_updates).filter(row=>consequenceEffectMatches(row,segments)),newLocation=travelPrefix&&consequenceEffectMatches(delta.new_location,segments)?String(delta.new_location||'').trim():'',pcStatus=consequenceEffectMatches(delta.pc_status,segments)?String(delta.pc_status||'').trim():'';
  return{preserved_delta:preservedDelta,npc_state_updates:npcStateUpdates,new_location:newLocation,pc_status:pcStatus,completed_prefix_action_types:completedPrefixActionTypes,completed_prefix_clause_indexes:completedPrefixClauseIndexes,structured_execution:structuredExecution};
}
function structuredPrefixEffectsForShortening(turn,plan={},applied=0){
  if(!plan?.eligible||!Array.isArray(plan.clauses)||plan.clauses.length<2)return null;
  const planTypes=array(plan.clauses).map(clause=>String(clause?.action_type||''));if(new Set(planTypes).size!==planTypes.length)return null;
  const elapsed=Math.max(0,Number(applied)||0),prefix=array(plan.clauses).slice(0,-1).filter(clause=>{const startMin=Number(clause?.start_min_minutes),startMax=Number(clause?.start_max_minutes),completeMin=Number(clause?.complete_min_minutes),completeMax=Number(clause?.complete_max_minutes);return[startMin,startMax,completeMin,completeMax].every(Number.isFinite)&&completeMax>startMax&&completeMin<=elapsed;});
  if(!prefix.length)return prefixEffectsFromSegments(turn,[],{structuredExecution:true});
  const segments=[turn?.scene_title,turn?.scene_summary,...array(turn?.scene).filter(item=>String(item?.kind||'')!=='dialogue').map(item=>item?.text)].filter(Boolean).flatMap(value=>String(value).toLowerCase().split(/(?<=[.!?。！？])|\n+/)).map(value=>value.trim()).filter(Boolean),hypothetical=/(?:예정|계획|하려|할\s*(?:예정|계획)|아직|않았|못했|미완료|[?？])/;
  const actionCue={training:/(?:훈련|연습|수련|단련)/,'class-attendance':/(?:수업|강의|세미나|실습|오리엔테이션|교육|입학식)/,meal:/(?:식사|아침|점심|저녁|밥|만찬)/,dialogue:/(?:대화|이야기|상담|논의|면담|회의|브리핑|협상)/,sleep:/(?:잠|수면|숙면)/,rest:/(?:휴식|쉬었|쉼)/,wait:/(?:기다림|대기|기다렸)/,travel:/(?:이동|도착|나가|들어가|향했|다다랐|닿았)/},modifier=String.raw`(?:(?:모두|전부|완전히|남김없이)\s*)?`,done=String.raw`(?:마쳤|마치고|끝냈|끝내고|완료했|완료하고|마무리했|마무리하고)`,ended=String.raw`(?:끝났|종료되|종료됐|완료되|완료됐)`,completionByKind={training:new RegExp(`(?:훈련|연습|수련|단련)(?:(?:을|를)\\s*${modifier}${done}|(?:이|가)\\s*${modifier}${ended})`),'class-attendance':new RegExp(`(?:수업|강의|세미나|실습|오리엔테이션|교육|입학식)(?:(?:을|를)\\s*${modifier}(?:${done}|수료했)|(?:이|가)\\s*${modifier}${ended})`),meal:new RegExp(`(?:식사|아침|점심|저녁|밥|만찬)(?:(?:을|를)\\s*${modifier}(?:마쳤|마치고|끝냈|다\\s*먹었|비웠|해치웠)|(?:이|가)\\s*${modifier}(?:끝났|완료됐))`),dialogue:new RegExp(`(?:대화|이야기|상담|논의|면담|회의|브리핑|협상)(?:(?:을|를)\\s*${modifier}${done}|(?:이|가)\\s*${modifier}${ended})`),sleep:new RegExp(`(?:잠|수면|숙면)(?:(?:을|를)\\s*${modifier}(?:마쳤|완료했)|(?:이|가)\\s*${modifier}(?:끝났|완료됐))|잠에서\\s*깨어|눈을\\s*떴|푹\\s*잤`),rest:new RegExp(`(?:휴식|쉬기)(?:(?:을|를)\\s*${modifier}(?:마쳤|완료했)|(?:이|가)\\s*${modifier}(?:끝났|완료됐))|충분히\\s*쉬었`),wait:new RegExp(`(?:기다림|대기)(?:(?:을|를)\\s*${modifier}(?:마쳤|완료했)|(?:이|가)\\s*${modifier}(?:끝났|완료됐))`),travel:new RegExp(`(?:이동|여정)(?:(?:을|를)\\s*${modifier}(?:마쳤|완료했)|(?:이|가)\\s*${modifier}(?:끝났|완료됐))|(?:도착했|다다랐|닿았|나왔|들어왔)`)};
  const matchedTypes=[],matchedIndexes=[],matchedSegments=[],planClauses=array(plan.clauses);
  for(const clause of prefix){const kind=String(clause?.action_type||''),cue=actionCue[kind],completion=completionByKind[kind],planPosition=planClauses.findIndex(row=>Number(row?.index)===Number(clause?.index));if(!cue||!completion||planClauses.filter(row=>String(row?.action_type||'')===kind).length>1)continue;const destination=String(clause?.destination||'').trim().toLowerCase(),laterCues=planClauses.slice(planPosition+1).map(row=>actionCue[String(row?.action_type||'')]).filter(Boolean),rows=[];for(const segment of segments){const actionAt=segment.search(cue);if(actionAt<0)continue;let boundary=segment.length;for(const laterCue of laterCues){const relative=segment.slice(actionAt+1).search(laterCue);if(relative>=0)boundary=Math.min(boundary,actionAt+1+relative);}const fragment=segment.slice(0,boundary).trim();if(!fragment||hypothetical.test(fragment)||!completion.test(fragment)||!completionSegmentAttributedToPc(fragment,actionAt,kind,String(plan.actor_name||''))||(kind==='travel'&&destination&&!travelDestinationReachedForReconciliation(fragment,destination)))continue;rows.push(fragment);}if(!rows.length)continue;matchedTypes.push(kind);matchedIndexes.push(Number(clause.index));matchedSegments.push(...rows);}
  const uniqueSegments=[...new Set(matchedSegments)],travelPrefix=prefix.some(clause=>matchedTypes.includes(String(clause?.action_type||''))&&String(clause?.action_type||'')==='travel');
  return prefixEffectsFromSegments(turn,uniqueSegments,{travelPrefix,completedPrefixActionTypes:[...new Set(matchedTypes)],completedPrefixClauseIndexes:matchedIndexes,structuredExecution:true});
}
function structuredDecisionBoundaryMinutes(turn,plan={},current=0,hasMeaningfulStop=false,terminalCompletion=false){
  if(!hasMeaningfulStop||terminalCompletion||!plan?.eligible||!Array.isArray(plan.clauses)||plan.clauses.length<2)return null;
  const effects=structuredPrefixEffectsForShortening(turn,plan,current);if(!effects)return null;const matched=new Set(array(effects.completed_prefix_clause_indexes).map(Number)),prefix=array(plan.clauses).slice(0,-1);let completed=null;
  for(const clause of prefix){if(!matched.has(Number(clause?.index)))break;completed=clause;}
  const clauses=array(plan.clauses),completedPosition=completed?clauses.findIndex(clause=>Number(clause?.index)===Number(completed?.index)):-1,next=clauses[completedPosition+1],elapsed=Math.max(0,Number(current)||0),nextLatest=Number(next?.complete_max_minutes);
  if(next&&Number.isFinite(nextLatest)&&elapsed<nextLatest)return elapsed;
...
function turnBeforePlayerChoice(turn){
  const scene=array(turn?.scene),questionIndexes=scene.map((item,index)=>/[?？]/.test(String(item?.text||''))?index:-1).filter(index=>index>=0),dialogueIndexes=scene.map((item,index)=>String(item?.kind||'')==='dialogue'?index:-1).filter(index=>index>=0),decisionIndex=questionIndexes.at(-1)??dialogueIndexes.at(-1)??scene.length;
  return{...turn,scene_title:'',scene_summary:'',scene:scene.slice(0,decisionIndex),choices:[]};
}
...
function applySceneMomentumTimeFloor(incoming,turn,mode='game',consequenceLifecycle=null,consequenceVisibleScene=[]){
  ...
  const completionEvidence=timedActionCompletionEvidence(turn,intent,incoming?.action||'',incoming?.saveState?.pc?.name||''),decisionEvidenceTurn=hasMeaningfulStop?turnBeforePlayerChoice(turn):turn,completionBeforeDecision=hasMeaningfulStop?timedActionCompletionEvidence(decisionEvidenceTurn,intent,incoming?.action||'',incoming?.saveState?.pc?.name||''):completionEvidence,completedBeforeChoice=!hasMeaningfulStop||completionBeforeDecision,rawDecisionBoundary=structuredDecisionBoundaryMinutes(decisionEvidenceTurn,intent?.structuredExecutionPlan,current,hasMeaningfulStop,completionBeforeDecision),decisionBoundary=rawDecisionBoundary==null?null:Math.min(profileMax,Math.max(0,Number(rawDecisionBoundary)||0)),earlierInterruptionBeforeConsequence=Boolean(consequenceWithinProfile&&current<consequenceBoundary&&(structuredInterruption||(hasMeaningfulStop&&!completedBeforeChoice)));
...
function reconcileReturnedTimedTurn(turn,{reason='profile-cap',elapsed=0,boundaryTitle='',completedPrefixActionTypes=[]}={}){
  if(!turn||typeof turn!=='object')return false;
  ...
  const completedAtRaisedFloor=reason==='profile-floor',scheduleLabel=String(boundaryTitle||'예정된 일정').trim()||'예정된 일정',detail=reason==='schedule-boundary'?`${scheduleLabel}의 시작 시점에 도달했다.`:reason==='consequence-boundary'?'후속 상황이 발현할 시점에 도달했다.':reason==='decision-boundary'?'플레이어의 판단이 필요한 선택 지점에 도달했다.':reason==='turn-limit'?'한 턴의 진행 한계에 도달했다.':reason==='explicit-zero'?'요청한 지속시간이 0분이므로 행동 결과는 발생하지 않았다.':completedAtRaisedFloor?'요청한 행동이 완료될 수 있는 최소 시간을 채워 행동을 마쳤다.':'요청한 시간 범위의 끝에 도달했다.';
  ...
  const preservedChoices=reason==='decision-boundary'?array(turn.choices).slice(0,3):[],decisionCandidates=reason==='decision-boundary'?array(turn.scene).filter(item=>String(item?.text||'').trim()):[],questionRows=decisionCandidates.filter(item=>/[?？]/.test(String(item?.text||''))),decisionRows=(questionRows.length?questionRows:decisionCandidates.filter(item=>String(item?.kind||'')==='dialogue')).slice(-2);
  turn.scene_summary=text;turn.scene=[{kind:'narration',text},...decisionRows];turn.choices=preservedChoices;turn.emotion_updates=[];turn.director=null;
  return true;
}
