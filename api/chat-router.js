...
function structuredDecisionBoundaryMinutes(turn,plan={},current=0,hasMeaningfulStop=false,terminalCompletion=false,executionAuthority=null,authoritativeBoundaryReached=false){
  if(!hasMeaningfulStop||terminalCompletion||!plan?.eligible||!Array.isArray(plan.clauses)||plan.clauses.length<1)return null;
  if(executionAuthority?.applicable&&executionAuthority?.reason!=='missing-contract')return executionAuthority.valid&&executionAuthority.boundary_kind==='choice'?executionAuthority.boundary_minutes:authoritativeBoundaryReached?null:Math.max(0,Number(current)||0);
...
function applySceneMomentumTimeFloor(incoming,turn,mode='game',consequenceLifecycle=null,consequenceVisibleScene=[],runtimeAuthority={}){
  ...
  const untrustedStructuredDecision=Boolean(appliedDecisionBoundary&&array(decisionPlan?.clauses).length===1&&executionAuthority.applicable&&!executionAuthority.valid&&!preserveChoiceDirectorState);
