...
function applySceneMomentumTimeFloor(incoming,turn,mode='game',consequenceLifecycle=null,consequenceVisibleScene=[]){
  ...
  const completionEvidence=timedActionCompletionEvidence(turn,intent,incoming?.action||'',incoming?.saveState?.pc?.name||''),decisionEvidenceTurn=hasMeaningfulStop?turnBeforePlayerChoice(turn):turn,completionBeforeDecision=hasMeaningfulStop?timedActionCompletionEvidence(decisionEvidenceTurn,intent,incoming?.action||'',incoming?.saveState?.pc?.name||''):completionEvidence,completedBeforeChoice=!hasMeaningfulStop||completionBeforeDecision,decisionPlan=intent?.structuredExecutionPlan||intent?.structuredDecisionPlan,rawDecisionBoundary=structuredDecisionBoundaryMinutes(decisionEvidenceTurn,decisionPlan,current,hasMeaningfulStop,completionBeforeDecision),decisionBoundary=rawDecisionBoundary==null?null:Math.min(profileMax,Math.max(0,Number(rawDecisionBoundary)||0)),earlierInterruptionBeforeConsequence=Boolean(consequenceWithinProfile&&current<consequenceBoundary&&(structuredInterruption||(hasMeaningfulStop&&!completedBeforeChoice)));
