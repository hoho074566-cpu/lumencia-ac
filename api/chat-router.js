...
function structuredPrefixEffectsForShortening(turn,plan={},applied=0){
  ...
  const elapsed=Math.max(0,Number(applied)||0),prefix=array(plan.clauses).slice(0,-1).filter(clause=>Number.isFinite(Number(clause?.complete_min_minutes))&&Number(clause.complete_min_minutes)>Number(clause?.start_min_minutes)&&Number(clause.complete_min_minutes)<=elapsed);
  ...
  const actionCue={training:/(?:훈련|연습|수련|단련)/,'class-attendance':/(?:수업|강의|세미나|실습|오리엔테이션|교육|입학식)/,meal:/(?:식사|아침|점심|저녁|밥|만찬)/,dialogue:/(?:대화|이야기|상담|논의|면담|회의|브리핑|협상)/,sleep:/(?:잠|수면|숙면)/,rest:/(?:휴식|쉬었|쉼)/,wait:/(?:기다림|대기|기다렸)/,travel:/(?:이동|도착|나가|들어가|향했|다다랐|닿았)/},done=String.raw`(?:마쳤|마치고|끝냈|끝내고|완료했|완료하고|마무리했|마무리하고)`,ended=String.raw`(?:끝났|종료되|종료됐|완료되|완료됐)`,completionByKind={training:new RegExp(`(?:훈련|연습|수련|단련)(?:(?:을|를)\\s*${done}|(?:이|가)\\s*${ended})`),'class-attendance':new RegExp(`(?:수업|강의|세미나|실습|오리엔테이션|교육|입학식)(?:(?:을|를)\\s*(?:${done}|수료했)|(?:이|가)\\s*${ended})`),meal:/(?:식사|아침|점심|저녁|밥|만찬)(?:(?:을|를)\s*(?:마쳤|마치고|끝냈|다\s*먹었|비웠|해치웠)|(?:이|가)\s*(?:끝났|완료됐))/,dialogue:new RegExp(`(?:대화|이야기|상담|논의|면담|회의|브리핑|협상)(?:(?:을|를)\\s*${done}|(?:이|가)\\s*${ended})`),sleep:/(?:잠|수면|숙면)(?:(?:을|를)\s*(?:마쳤|완료했)|(?:이|가)\s*(?:끝났|완료됐))|잠에서\s*깨어|눈을\s*떴|푹\s*잤/,rest:/(?:휴식|쉬기)(?:(?:을|를)\s*(?:마쳤|완료했)|(?:이|가)\s*(?:끝났|완료됐))|충분히\s*쉬었/,wait:/(?:기다림|대기)(?:(?:을|를)\s*(?:마쳤|완료했)|(?:이|가)\s*(?:끝났|완료됐))/,travel:/(?:이동|여정)(?:(?:을|를)\s*(?:마쳤|완료했)|(?:이|가)\s*(?:끝났|완료됐))|(?:도착했|다다랐|닿았|나왔|들어왔)/};
...
function applySceneMomentumTimeFloor(incoming,turn,mode='game',consequenceLifecycle=null,consequenceVisibleScene=[]){
  ...
  const completionEvidence=timedActionCompletionEvidence(turn,intent,incoming?.action||'',incoming?.saveState?.pc?.name||''),decisionIndex=hasMeaningfulStop?array(turn.scene).findIndex(item=>/[?？]/.test(String(item?.text||''))||String(item?.kind||'')==='dialogue'):-1,completionBeforeDecision=decisionIndex<0?completionEvidence:timedActionCompletionEvidence({...turn,scene:array(turn.scene).slice(0,decisionIndex),choices:[]},intent,incoming?.action||'',incoming?.saveState?.pc?.name||''),completedBeforeChoice=!hasMeaningfulStop||completionBeforeDecision,rawDecisionBoundary=structuredDecisionBoundaryMinutes(turn,intent?.structuredExecutionPlan,current,hasMeaningfulStop,completionBeforeDecision),decisionBoundary=rawDecisionBoundary==null?null:Math.min(profileMax,Math.max(0,Number(rawDecisionBoundary)||0)),earlierInterruptionBeforeConsequence=Boolean(consequenceWithinProfile&&current<consequenceBoundary&&(structuredInterruption||(hasMeaningfulStop&&!completedBeforeChoice)));
