...
import { applyRelationshipThresholdReceipts } from '../lib/relationship-thresholds.js';
...
function routedRelationshipThresholds(turn,incoming,mode){
  return applyRelationshipThresholdReceipts(turn,{relationships:incoming?.saveState?.relationships,npcInnerStates:incoming?.saveState?.npcInnerStates,registeredNpcKeys:Object.keys(CHARACTER_REGISTRY),mode,turnNumber:Number(incoming?.saveState?.turnNumber||0)+1});
}
...
export default async function handler(req,res){
  ...
  try{
    ...
    if(mode==='continue'){lockContinueTurn(data.turn);applyExtendedExpressions(data.turn,incoming0.saveState||{});const npcSignificance=routedNpcSignificance(data.turn,telemetry,mode),relationshipThresholds=routedRelationshipThresholds(data.turn,incoming0,mode);data.runtime_state=consumeContinuationRuntime({...incoming,saveState:object(incoming0.saveState)},data.turn);data.background_digest=String(incoming.saveState?.backgroundDigest||'').slice(-1800);const pipeline={pipeline:'continue-stable-v156',stages:1,qa_result:'SKIP',rewrite_applied:false,background_sim:false,context_router:telemetry,event_director_v2:telemetry?.event_director_v2||null,event_director_v3:telemetry?.event_director_v3||null,event_director_v3_enabled:true,world_result_surface:data.runtime_state.scene_runtime?.world_result_surface||null,world_result_surfacing_v1:true,adaptive_time_scale_version:ADAPTIVE_TIME_SCALE_VERSION,adaptive_time_scale_v2:true,scene_novelty:data.runtime_state.scene_runtime?.novelty||null,scene_novelty_v1:true,scene_purpose:data.runtime_state.scene_runtime?.purpose||null,scene_purpose_v1:true,scene_exit_condition:data.runtime_state.scene_runtime?.exit_condition||null,scene_exit_condition_v1:true,turn_hook:data.runtime_state.scene_runtime?.turn_hook||null,turn_hook_v1:true,scene_orchestration:data.runtime_state.scene_runtime?.orchestration||telemetry?.scene_orchestration||null,scene_orchestration_v1:true,npc_significance:npcSignificance,npc_significance_v1:true,relationship_thresholds:relationshipThresholds,relationship_thresholds_v1:true,npc_motivation_v1:true,npc_goal_v2:true,relationship_reason_v1:true,faction_social_v1:true,combat_growth_v2:true,skill_learning_v1:true,awakening_talent_v1:true};data.pipeline=pipeline;setAdapterRoute(data,mode,pipeline,telemetry);return res.status(200).json(data);}
    if(mode==='meta'){if(data.turn?.state_delta){data.turn.state_delta.stat_progress=[];data.turn.state_delta.skill_experience=[];data.turn.state_delta.skill_learning=[];data.turn.state_delta.awakening_progress=[];data.turn.state_delta.talent_evolution=[];}const npcSignificance=routedNpcSignificance(data.turn,telemetry,mode),relationshipThresholds=routedRelationshipThresholds(data.turn,incoming0,mode);const pipeline={pipeline:'meta-full-stable-v156',stages:1,qa_result:'SKIP',rewrite_applied:false,background_sim:false,context_router:telemetry,event_director_v2:telemetry?.event_director_v2||null,event_director_v3:telemetry?.event_director_v3||null,event_director_v3_enabled:true,world_result_surface:null,world_result_surfacing_v1:true,adaptive_time_scale_version:ADAPTIVE_TIME_SCALE_VERSION,adaptive_time_scale_v2:true,scene_orchestration:telemetry?.scene_orchestration||null,scene_orchestration_v1:true,npc_significance:npcSignificance,npc_significance_v1:true,relationship_thresholds:relationshipThresholds,relationship_thresholds_v1:true,npc_motivation_v1:true,npc_goal_v2:true,relationship_reason_v1:true,faction_social_v1:true,combat_growth_v2:true,skill_learning_v1:true,awakening_talent_v1:true};data.pipeline=pipeline;setAdapterRoute(data,mode,pipeline,telemetry);return res.status(200).json(data);}
    ...
    const relationshipThresholds=routedRelationshipThresholds(data.turn,incoming0,mode);
    ...
    for(const [key,update] of Object.entries(object(relationshipThresholds.npc_updates))){
      npcUpdates[key]={...object(npcUpdates[key]),...object(update)};
    }
