...
function structuredPrefixEffectsForShortening(turn,plan={},applied=0){
  ...
  const elapsed=Math.max(0,Number(applied)||0),prefix=array(plan.clauses).slice(0,-1).filter(clause=>Number.isFinite(Number(clause?.complete_min_minutes))&&Number(clause.complete_min_minutes)<=elapsed);
  ...
  const actionCue={training:/(?:훈련|연습|수련|단련)/,'class-attendance':/(?:수업|강의|세미나|실습|오리엔테이션|교육|입학식)/,meal:/(?:식사|아침|점심|저녁|밥|만찬)/,dialogue:/(?:대화|이야기|상담|논의|면담|회의|브리핑|협상)/,sleep:/(?:잠|수면|숙면)/,rest:/(?:휴식|쉬었|쉼)/,wait:/(?:기다림|대기|기다렸)/,travel:/(?:이동|도착|나가|들어가|향했|다다랐|닿았)/},completionCue=/(?:마쳤|마치고|끝냈|끝내고|끝났|종료되|종료됐|완료|마무리|수료|다\s*먹었|먹어\s*치웠|비웠|해치웠|잠에서\s*깨어|눈을\s*떴|푹\s*잤|충분히\s*쉬었|도착했|다다랐|닿았|나왔|들어왔|이동을\s*마쳤|기다림을\s*마쳤|대기를\s*마쳤)/;
  const matchedTypes=[],matchedIndexes=[],matchedSegments=[],planClauses=array(plan.clauses);
  for(const clause of prefix){const kind=String(clause?.action_type||''),cue=actionCue[kind],planPosition=planClauses.findIndex(row=>Number(row?.index)===Number(clause?.index));if(!cue||planClauses.filter(row=>String(row?.action_type||'')===kind).length>1)continue;const destination=String(clause?.destination||'').trim().toLowerCase(),laterCues=planClauses.slice(planPosition+1).map(row=>actionCue[String(row?.action_type||'')]).filter(Boolean),rows=[];for(const segment of segments){const actionAt=segment.search(cue);if(actionAt<0)continue;let boundary=segment.length;for(const laterCue of laterCues){const relative=segment.slice(actionAt+1).search(laterCue);if(relative>=0)boundary=Math.min(boundary,actionAt+1+relative);}const fragment=segment.slice(0,boundary).trim();if(!fragment||hypothetical.test(fragment)||!completionCue.test(fragment)||!completionSegmentAttributedToPc(fragment,actionAt,kind,String(plan.actor_name||''))||(kind==='travel'&&destination&&!travelDestinationReachedForReconciliation(fragment,destination)))continue;rows.push(fragment);}if(!rows.length)continue;matchedTypes.push(kind);matchedIndexes.push(Number(clause.index));matchedSegments.push(...rows);}
...
function applySceneMomentumTimeFloor(incoming,turn,mode='game',consequenceLifecycle=null,consequenceVisibleScene=[]){
  ...
  const completionEvidence=timedActionCompletionEvidence(turn,intent,incoming?.action||'',incoming?.saveState?.pc?.name||''),completedBeforeChoice=!hasMeaningfulStop||completionEvidence,rawDecisionBoundary=structuredDecisionBoundaryMinutes(turn,intent?.structuredExecutionPlan,current,hasMeaningfulStop,completionEvidence),decisionBoundary=rawDecisionBoundary==null?null:Math.min(profileMax,Math.max(0,Number(rawDecisionBoundary)||0)),earlierInterruptionBeforeConsequence=Boolean(consequenceWithinProfile&&current<consequenceBoundary&&(structuredInterruption||(hasMeaningfulStop&&!completedBeforeChoice)));
