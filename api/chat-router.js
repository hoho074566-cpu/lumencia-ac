...
import { deriveFactionSocialState } from '../lib/faction-social-consequence.js';
...
function localSceneRuntime(incoming,turn,directorTelemetry=null,mode='game'){
  ...
  return {
    ...
    tone:clampText(turn?.importance||previous.tone||'routine',80),remaining_beats:hasDecision?[]:array(previous.remaining_beats).slice(0,1),purpose,exit_condition:exitCondition,turn_hook:turnHook,goal_tick:goalTick,momentum,novelty,scene_delta:sceneDelta,...(factionSocial?{faction_social:factionSocial}:{}),...progressState,
  };
}
