...
function precedingActivityEffectsForShortening(turn,action,intent={},applied=0){
  ...
  for(const [field,value] of Object.entries(delta)){if(!Array.isArray(value)||blockedArrays.has(field)||field==='npc_state_updates')continue;const kept=value.filter(row=>consequenceEffectMatches(row,prefixSegments));if(kept.length)preservedDelta[field]=kept;}
  if(Number(delta.fatigue_delta||0)!==0&&prefixSegments.some(segment=>/(?:피로|지쳤|지침|탈진|기력|힘이\s*빠)/.test(segment)))preservedDelta.fatigue_delta=Number(delta.fatigue_delta);
  if(Number(delta.gold_delta||0)!==0&&prefixSegments.some(segment=>/(?:보상|상금|지급|금화|골드|돈)/.test(segment)))preservedDelta.gold_delta=Number(delta.gold_delta);
  const npcStateUpdates=array(delta.npc_state_updates).filter(row=>consequenceEffectMatches(row,prefixSegments)),travelPrefix=prefixTokens.some(token=>['이동','도착','나가','들어가','가서','와서'].includes(token)),newLocation=travelPrefix&&consequenceEffectMatches(delta.new_location,prefixSegments)?String(delta.new_location||'').trim():'',pcStatus=consequenceEffectMatches(delta.pc_status,prefixSegments)?String(delta.pc_status||'').trim():'';
  return{preserved_delta:preservedDelta,npc_state_updates:npcStateUpdates,new_location:newLocation,pc_status:pcStatus};
}
...
function timedActionCompletionEvidence(turn,intent={},action='',actorName=''){
  ...
  const patterns={
    ...
    wait:/(?:기다림을\s*마쳤|대기를\s*마쳤|요청한\s*시간이\s*(?:흘렀|지났)(?!다고|다는|는지|을지|을까)|(?:\d+(?:\.\d+)?|한|두|세|네|다섯|여섯|일곱|여덟|아홉|열)\s*(?:분|시간)(?:이|가)?\s*(?:흘렀|지났)(?!다고|다는|는지|을지|을까)|시간을\s*보낸\s*뒤)/,
  ...
  return segments.some(segment=>{if(hypothetical.test(segment)||kind==='wait'&&uncertainWait.test(segment))return false;if(patterns[kind].test(segment))return true;return Boolean(kind==='meal'&&foodCompletion?.test(segment.toLowerCase()));});
}
...
function applySceneMomentumTimeFloor(incoming,turn,mode='game',consequenceLifecycle=null,consequenceVisibleScene=[]){
  ...
  const completionEvidence=timedActionCompletionEvidence(turn,intent,incoming?.action||'',incoming?.saveState?.pc?.name||''),completedBeforeChoice=!hasMeaningfulStop||completionEvidence,earlierInterruptionBeforeConsequence=Boolean(consequenceWithinProfile&&current<consequenceBoundary&&(structuredInterruption||(hasMeaningfulStop&&!completedBeforeChoice)));
  ...
  const boundaryTruncatesAction=Boolean(reachedBoundary!=null&&reachedBoundary<requestedFloor),preserveSurfacedScheduleScene=Boolean(appliedScheduleBoundary&&boundaryTruncatesAction&&surfacedScheduledBoundary&&!completionEvidence&&!boundaryProgressTerminal),unsurfacedScheduleCapsFloor=Boolean(scheduleBoundary!=null&&scheduleBoundary===floorBoundary&&!reachedScheduledBoundary&&scheduleBoundary<requestedFloor&&applied===scheduleBoundary),unresolvedConsequenceCapsFloor=Boolean(consequenceBoundary!=null&&consequenceBoundary===floorBoundary&&!reachedConsequenceBoundary&&consequenceBoundary<requestedFloor&&applied===consequenceBoundary),overrunStartBoundary=Boolean(intent.scheduledStartOverrun&&Number(intent.scheduledStartOffsetMinutes||0)>0&&applied===Number(intent.scheduledStartOffsetMinutes)),startOnlyBoundary=Boolean(intent.scheduledStartBoundaryOnly&&Number(intent.scheduledStartOffsetMinutes||0)>0&&applied===Number(intent.scheduledStartOffsetMinutes)),reconcileTruncatedTurn=applied<current||boundaryTruncatesAction||unsurfacedScheduleCapsFloor||unresolvedConsequenceCapsFloor||overrunStartBoundary||startOnlyBoundary;
...
function reconcileReturnedTimedTurn(turn,{reason='profile-cap',elapsed=0}={}){
  ...
  const completedAtRaisedFloor=reason==='profile-floor',detail=reason==='schedule-boundary'?'예정된 일정의 시작 시점에 도달했다.':reason==='consequence-boundary'?'후속 상황이 발현할 시점에 도달했다.':reason==='turn-limit'?'한 턴의 진행 한계에 도달했다.':reason==='explicit-zero'?'요청한 지속시간이 0분이므로 행동 결과는 발생하지 않았다.':completedAtRaisedFloor?'요청한 행동이 완료될 수 있는 최소 시간을 채워 행동을 마쳤다.':'요청한 시간 범위의 끝에 도달했다.';
