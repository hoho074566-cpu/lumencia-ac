...
function applySceneMomentumTimeFloor(incoming,turn,mode='game',consequenceLifecycle=null,consequenceVisibleScene=[],runtimeAuthority={}){
  ...
  if(returnedSceneReconciled&&!trimmedSurfacedScheduleScene){if(rejectedClaimedExecution)reconcileRejectedStructuredExecutionTurn(turn);else if(appliedDecisionBoundary)reconcileReturnedTimedTurn(turn,{reason:'decision-boundary',elapsed:applied,completedPrefixActionTypes,decisionPromptText:decisionEvidenceTurn?._choice_prompt_text});else if(preserveRaisedFloorScene)reconcileReturnedRaisedFloorContinuation(turn,{elapsed:applied});else if(preserveAttributedConsequence&&array(consequenceVisibleScene).length)reconcileReturnedConsequenceTurn(turn,{elapsed:applied,scene:consequenceVisibleScene});else reconcileReturnedTimedTurn(turn,{reason:reconciliationReason,elapsed:applied,boundaryTitle:coincidentScheduleBoundary?boundaryRows[0]?.title:'',completedPrefixActionTypes});}
...
function reconcileRejectedStructuredExecutionTurn(turn){
  if(!turn)return turn;
  turn.scene_title='';turn.scene_summary='';turn.scene=[];turn.choices=[];turn.emotion_updates=[];turn.cg_id=null;turn.director=null;turn.runtime_incomplete_boundary=true;
  return turn;
}
...
export default async function handler(req,res){
  ...
  try{
    ...
    if(sceneIntent?.reconciliationReason==='invalid-structured-execution')return res.status(409).json({error:'이번 응답은 안전하게 확정되지 않아 저장하지 않았습니다. 같은 행동을 다시 시도해 주세요.',code:'UNCOMMITTED_TURN',server_version:ADAPTER_VERSION,adapter_version:ADAPTER_VERSION});
