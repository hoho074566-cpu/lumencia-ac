...
function deriveTimedActionRuntime(previousRuntime={},intent={},action='',turn={},mode='game'){
  ...
  const resumed=intent?.resumedTimedAction===true,range=array(intent?.explicitDurationRangeMinutes),prefixMinimum=Math.max(0,Math.trunc(Number(array(intent?.precedingActivityRangeMinutes)[0]??intent?.precedingActivityMinutes??0))),terminalMinimum=intent?.explicitDurationMinutes!=null?Number(intent.explicitDurationMinutes):intent?.strictDurationLowerBoundMinutes!=null?Number(intent.strictDurationLowerBoundMinutes)+1:range.length===2?Number(range[0]):0,declared=Math.max(0,Math.trunc(prefixMinimum+terminalMinimum)),eligibleNew=!resumed&&declared>1440&&!intent?.dateQualifiedStart&&intent?.scheduledStartOffsetMinutes==null;
