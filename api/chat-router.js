...
function structuredChoicePromptContradictsExecution(turn,plan={},executionAuthority=null,intent={},action='',actorName=''){
  if(!executionAuthority?.valid||executionAuthority.boundary_kind!=='choice')return false;
  const prompt=String(turn?._choice_prompt_text||'').trim(),completed=executionAuthority.completed_clause_set instanceof Set?executionAuthority.completed_clause_set:new Set(array(executionAuthority.completed_clause_ids).map(String));if(!prompt)return false;
  const sourceRow=object(array(turn?.scene)[Number(executionAuthority.decision_scene_index)]),promptTurn={scene:[{...sourceRow,text:prompt}],choices:[]};
  return array(plan?.clauses).some((clause,position)=>{const id=String(clause?.clause_id||`action_${Number(clause?.index||position+1)}`);return!completed.has(id)&&timedActionCompletionEvidence(promptTurn,{...intent,completionActionType:String(clause?.action_type||'')},action,actorName);});
}
...
function applySceneMomentumTimeFloor(incoming,turn,mode='game',consequenceLifecycle=null,consequenceVisibleScene=[],runtimeAuthority={}){
  ...
  const reachedScheduledBoundary=surfacedScheduledBoundary||crossedScheduledBoundary||modelHasMeaningfulStop&&scheduleBoundary!=null&&scheduleBoundary<=profileMax&&current===scheduleBoundary;
  ...
  const authoritativeBoundaryAtReturnedMinute=Boolean(reachedScheduledBoundary||turnLimitPreemptsChoice||consequenceWithinProfile&&current>=consequenceBoundary);
  ...
  const untrustedStructuredDecision=Boolean(appliedDecisionBoundary&&(array(decisionPlan?.clauses).length===1&&executionAuthority.applicable&&!executionAuthority.valid&&!preserveChoiceDirectorState||contradictoryStructuredChoicePrompt));
  const scheduleChoiceAtBoundary=Boolean(current===scheduleBoundary&&(visibleBoundary||structuredBoundary));
  if(returnedSceneReconciled&&!trimmedSurfacedScheduleScene){if(appliedDecisionBoundary)reconcileReturnedTimedTurn(turn,{reason:'decision-boundary',elapsed:applied,completedPrefixActionTypes,decisionPromptText:decisionEvidenceTurn?._choice_prompt_text,preserveDecisionRows:!untrustedStructuredDecision});else if(preserveRaisedFloorScene)reconcileReturnedRaisedFloorContinuation(turn,{elapsed:applied});else if(preserveAttributedConsequence&&array(consequenceVisibleScene).length)reconcileReturnedConsequenceTurn(turn,{elapsed:applied,scene:consequenceVisibleScene,preserveChoices:hasMeaningfulStop&&current===consequenceBoundary});else reconcileReturnedTimedTurn(turn,{reason:reconciliationReason,elapsed:applied,boundaryTitle:boundaryRows[0]?.title||'',completedPrefixActionTypes,preserveChoices:unverifiedStructuredScheduleBoundary&&hasMeaningfulStop&&scheduleChoiceAtBoundary});}
  ...
  const runtimeChoicesTrusted=Boolean(hasMeaningfulStop&&array(turn.choices).length&&(appliedDecisionBoundary||unverifiedStructuredScheduleBoundary&&scheduleChoiceAtBoundary||preserveAttributedConsequence&&appliedConsequenceTimeBoundary&&current===consequenceBoundary));
