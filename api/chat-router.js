...
입력에 STRUCTURED_TIME_PLAN이 있으면 plan_used=true로 하고, 제공된 action_N ID만 사용한다. 없으면 plan_used=false, boundary_kind=none, boundary_minutes=state_delta.advance_minutes, completed_clause_ids/decision_completion_clause_ids/effect_owners/scalar_contributions는 빈 배열, 나머지는 null로 둔다.
...
choices가 있으면 실제 선택 질문을 scene의 마지막 항목에 두고 decision_scene_index를 그 0-based index로 보고한다. 그 선택 질문 자체가 PC의 특정 action 완료를 주장하면 해당 action_N만 decision_completion_clause_ids에 넣는다. NPC가 자기 행동을 1인칭으로 말하거나 질문이 완료를 주장하지 않으면 넣지 않는다. choices가 없으면 decision_scene_index=null, decision_completion_clause_ids=[]이다.
...
function timeExecutionFieldSchema(){
  return {
    ...
    properties:{
      ...
      decision_completion_clause_ids:{type:'array',maxItems:8,items:{type:'string',pattern:'^action_[1-8]$'}},
    ...
    required:['version','plan_used','boundary_kind','boundary_minutes','completed_clause_ids','interrupted_clause_id','decision_scene_index','decision_completion_clause_ids','boundary_event_id','effect_owners','scalar_contributions'],
  };
}
...
function structuredDecisionBoundaryMinutes(turn,plan={},current=0,hasMeaningfulStop=false,terminalCompletion=false,executionAuthority=null,authoritativeBoundaryReached=false,choiceOwnsAuthoritativeBoundary=false){
  if(!hasMeaningfulStop||terminalCompletion||!plan?.eligible||!Array.isArray(plan.clauses)||plan.clauses.length<1)return null;
  if(executionAuthority?.applicable&&executionAuthority?.reason!=='missing-contract')return executionAuthority.valid&&executionAuthority.boundary_kind==='choice'&&(!authoritativeBoundaryReached||choiceOwnsAuthoritativeBoundary)?executionAuthority.boundary_minutes:authoritativeBoundaryReached?null:Math.max(0,Number(current)||0);
  if(plan.clauses.length<2)return executionAuthority?.applicable&&!authoritativeBoundaryReached?Math.max(0,Number(current)||0):null;
...
function structuredTextClaimsIncompleteCompletion(text='',sourceRow={},plan={},executionAuthority=null,intent={},action='',actorName=''){
  ...
  const source=object(sourceRow),actorIdentity=String(actorName||'').trim().toLowerCase(),speakerIdentities=[source.speaker_key,source.speaker_name].map(value=>String(value||'').trim().toLowerCase()).filter(Boolean),speakerOwnsFirstPerson=String(source.kind||'')==='dialogue'&&speakerIdentities.length>0&&(!actorIdentity||!speakerIdentities.includes(actorIdentity)),clauses=array(plan?.clauses).map((clause,position)=>({...object(clause),_id:String(clause?.clause_id||`action_${Number(clause?.index||position+1)}`)})),segments=String(prompt).split(/(?<=[.!?。！？])|\n+/).map(value=>value.trim()).filter(Boolean),candidates=[...new Set(segments.flatMap(value=>[value,value.replace(/[?？]+$/u,'').trim()]).filter(Boolean))],promptTurn=(value)=>({scene:[{...source,text:value}],choices:[]}),claimCounts=new Map(),completedCounts=new Map();
  for(const clause of clauses){const kind=String(clause.action_type||'');if(completed.has(clause._id))completedCounts.set(kind,Number(completedCounts.get(kind)||0)+1);if(claimCounts.has(kind))continue;claimCounts.set(kind,candidates.filter(candidate=>timedActionCompletionEvidence(promptTurn(candidate),{...intent,completionActionType:kind},action,actorName,{includeDialogue:true,speakerOwnsFirstPerson})).length);}
  return clauses.some(clause=>!completed.has(clause._id)&&Number(claimCounts.get(String(clause.action_type||''))||0)>Number(completedCounts.get(String(clause.action_type||''))||0));
}
function structuredChoicePromptContradictsExecution(turn,plan={},executionAuthority=null,intent={},action='',actorName=''){
  const completed=executionAuthority?.completed_clause_set instanceof Set?executionAuthority.completed_clause_set:new Set(array(executionAuthority?.completed_clause_ids).map(String)),claimed=array(executionAuthority?.decision_completion_clause_ids).map(String);
  if(executionAuthority?.valid&&claimed.some(clauseId=>!completed.has(clauseId)))return true;
...
function applySceneMomentumTimeFloor(incoming,turn,mode='game',consequenceLifecycle=null,consequenceVisibleScene=[],runtimeAuthority={}){
  ...
  const turnLimitReached=Boolean(intent.turnLimitTruncated&&current>=1440),turnLimitPreemptsChoice=Boolean(turnLimitReached&&modelHasMeaningfulStop),hasMeaningfulStop=Boolean(modelHasMeaningfulStop&&!turnLimitPreemptsChoice);
  ...
  const scheduleTiedAtTurnLimit=Boolean(turnLimitReached&&scheduleBoundary===1440),consequenceTiedAtTurnLimit=Boolean(turnLimitReached&&consequenceWithinProfile&&Number(consequenceBoundary)===1440),reachedScheduledBoundary=!scheduleTiedAtTurnLimit&&(surfacedScheduledBoundary||crossedScheduledBoundary||hasMeaningfulStop&&scheduleBoundary!=null&&scheduleBoundary<=profileMax&&current===scheduleBoundary);
  ...
  const authoritativeBoundaryAtReturnedMinute=Boolean(reachedScheduledBoundary||turnLimitReached||consequenceWithinProfile&&current>=consequenceBoundary);
  const decisionPlan=intent?.structuredExecutionPlan||intent?.structuredDecisionPlan,boundaryRuntime={required_boundary_kind:turnLimitReached?'turn-limit':'',boundaries:{schedule:scheduleBoundary==null?{}:{minutes:scheduleBoundary,event_ids:[...boundaryIds]},consequence:consequenceWithinProfile?{minutes:Number(consequenceBoundary),event_ids:[consequenceLifecycle?.selected_id].filter(Boolean)}:{},choice:modelHasMeaningfulStop?{minutes:current,event_ids:[...trustedChoiceEventIds]}:{},'turn-limit':intent.turnLimitTruncated?{minutes:1440,event_ids:[]}:{}}},executionAuthority=validateStructuredTimeExecution(turn,decisionPlan,boundaryRuntime),executionChoiceEventId=String(executionAuthority?.boundary_event_id||'').trim().toLowerCase(),choiceOwnsAuthoritativeBoundary=Boolean(executionAuthority?.valid&&executionAuthority?.boundary_kind==='choice'&&executionChoiceEventId&&(current===scheduleBoundary&&boundaryIds.has(executionChoiceEventId)||current===consequenceBoundary&&executionChoiceEventId===String(consequenceLifecycle?.selected_id||'').trim().toLowerCase())),terminalClause=array(decisionPlan?.clauses).at(-1),terminalClauseId=String(terminalClause?.clause_id||`action_${Number(terminalClause?.index||0)}`),completionIntent=terminalClause?{...intent,completionActionType:String(terminalClause.action_type||'')}:intent,completionEvidence=timedActionCompletionEvidence(turn,completionIntent,incoming?.action||'',incoming?.saveState?.pc?.name||''),decisionEvidenceTurn=hasMeaningfulStop?turnBeforePlayerChoice(turn,executionAuthority):turn,legacyCompletionBeforeDecision=hasMeaningfulStop?timedActionCompletionEvidence(decisionEvidenceTurn,completionIntent,incoming?.action||'',incoming?.saveState?.pc?.name||''):completionEvidence,completionBeforeDecision=executionAuthority.applicable?Boolean(executionAuthority.valid&&executionAuthority.completed_clause_set.has(terminalClauseId)):legacyCompletionBeforeDecision,completedBeforeChoice=!hasMeaningfulStop||completionBeforeDecision,rawDecisionBoundary=structuredDecisionBoundaryMinutes(decisionEvidenceTurn,decisionPlan,current,hasMeaningfulStop,completionBeforeDecision,executionAuthority,authoritativeBoundaryAtReturnedMinute,choiceOwnsAuthoritativeBoundary),decisionBoundary=rawDecisionBoundary==null||Number(rawDecisionBoundary)>profileMax?null:Math.max(0,Number(rawDecisionBoundary)||0),earlierInterruptionBeforeConsequence=Boolean(consequenceWithinProfile&&current<consequenceBoundary&&(structuredInterruption||(hasMeaningfulStop&&!completedBeforeChoice)));
  const decisionPromptSourceRow=executionAuthority.valid&&Number.isInteger(executionAuthority.decision_scene_index)?array(turn.scene)[executionAuthority.decision_scene_index]:null;
  ...
  const reachedConsequenceBoundary=!consequenceTiedAtTurnLimit&&consequenceWithinProfile&&!earlierInterruptionBeforeConsequence&&(current>=consequenceBoundary||consequenceLifecycle?.status==='resolved'),manifestedConsequenceBoundary=Boolean(reachedConsequenceBoundary&&consequenceLifecycle?.status==='resolved'),manifestedConsequenceAtTurnLimit=Boolean(consequenceTiedAtTurnLimit&&consequenceLifecycle?.status==='resolved');
  ...
  const receiptOwnsScheduleChoices=Boolean(appliedScheduleBoundary&&hasMeaningfulStop&&executionAuthority.valid&&executionAuthority.boundary_kind==='schedule'&&executionChoiceEventId&&boundaryIds.has(executionChoiceEventId)&&Number.isInteger(executionAuthority.decision_scene_index)),receiptOwnsConsequenceChoices=Boolean(appliedConsequenceTimeBoundary&&hasMeaningfulStop&&executionAuthority.valid&&executionAuthority.boundary_kind==='consequence'&&executionChoiceEventId===String(consequenceLifecycle?.selected_id||'').trim().toLowerCase()&&Number.isInteger(executionAuthority.decision_scene_index)),receiptOwnedExternalChoices=receiptOwnsScheduleChoices||receiptOwnsConsequenceChoices?array(turn.choices).slice(0,3):[];
  const contradictoryStructuredChoicePrompt=Boolean((appliedDecisionBoundary||receiptOwnsScheduleChoices||receiptOwnsConsequenceChoices)&&structuredChoicePromptContradictsExecution(decisionEvidenceTurn,decisionPlan,executionAuthority,intent,incoming?.action||'',incoming?.saveState?.pc?.name||'')),untrustedStructuredDecision=Boolean((appliedDecisionBoundary||receiptOwnsScheduleChoices||receiptOwnsConsequenceChoices)&&contradictoryStructuredChoicePrompt);
  ...
  const boundaryTruncatesAction=Boolean(reachedBoundary!=null&&reachedBoundary<requestedFloor),unverifiedStructuredExternalBoundary=Boolean(executionAuthority.applicable&&!executionAuthority.valid&&(appliedScheduleBoundary||appliedConsequenceTimeBoundary)),unverifiedStructuredScheduleBoundary=Boolean(unverifiedStructuredExternalBoundary&&appliedScheduleBoundary),receiptLeavesActionIncomplete=Boolean(executionAuthority.valid&&executionAuthority.interrupted_clause_id),preserveStartedScheduleState=Boolean(appliedScheduleBoundary&&boundaryTruncatesAction&&surfacedScheduledBoundary&&(!completionEvidence||receiptLeavesActionIncomplete)&&!boundaryProgressTerminal),preserveVerifiedScheduleState=Boolean(preserveStartedScheduleState||unverifiedStructuredScheduleBoundary&&surfacedScheduledBoundary&&!boundaryProgressTerminal),preserveSurfacedScheduleScene=Boolean(preserveStartedScheduleState&&visibleBoundary&&!untrustedStructuredDecision),unsurfacedScheduleCapsFloor=Boolean(scheduleBoundary!=null&&scheduleBoundary===floorBoundary&&!reachedScheduledBoundary&&scheduleBoundary<requestedFloor&&applied===scheduleBoundary),unresolvedConsequenceCapsFloor=Boolean(consequenceBoundary!=null&&consequenceBoundary===floorBoundary&&!reachedConsequenceBoundary&&consequenceBoundary<requestedFloor&&applied===consequenceBoundary),coincidentScheduleBoundary=Boolean(!scheduleTiedAtTurnLimit&&scheduleBoundary!=null&&!reachedScheduledBoundary&&scheduleBoundary===requestedFloor&&current>=scheduleBoundary&&applied===scheduleBoundary),coincidentConsequenceBoundary=Boolean(!consequenceTiedAtTurnLimit&&consequenceBoundary!=null&&!manifestedConsequenceBoundary&&consequenceBoundary===requestedFloor&&current>=consequenceBoundary&&applied===consequenceBoundary),overrunStartBoundary=Boolean(intent.scheduledStartOverrun&&Number(intent.scheduledStartOffsetMinutes||0)>0&&applied===Number(intent.scheduledStartOffsetMinutes)),startOnlyBoundary=Boolean(intent.scheduledStartBoundaryOnly&&Number(intent.scheduledStartOffsetMinutes||0)>0&&applied===Number(intent.scheduledStartOffsetMinutes)),reconcileTruncatedTurn=rejectedClaimedExecution||applied<current||boundaryTruncatesAction||unsurfacedScheduleCapsFloor||unresolvedConsequenceCapsFloor||overrunStartBoundary||startOnlyBoundary;
  const ambiguousAppliedConsequence=Boolean(appliedConsequenceBoundary&&!consequenceAttributionSafe);
  if((reconcileTruncatedTurn||ambiguousAppliedConsequence)&&consequenceLifecycle?.status==='resolved'&&(!appliedConsequenceBoundary&&!manifestedConsequenceAtTurnLimit||ambiguousAppliedConsequence)){
  ...
  const preserveAttributedConsequence=Boolean((appliedConsequenceBoundary||manifestedConsequenceAtTurnLimit)&&consequenceAttributionSafe),turnLimitCompletion=Boolean(intent.turnLimitTruncated&&(applied>=1440||completionEvidence||(intent.kind==='travel'&&travelDestinationReachedForReconciliation(turn?.state_delta?.new_location,intent.semanticTarget)))),preserveIntermediateLocation=intent.turnLimitTruncated&&intent.kind==='travel'&&applied>=1440&&!completionEvidence&&!travelDestinationReachedForReconciliation(turn?.state_delta?.new_location,intent.semanticTarget)?turn?.state_delta?.new_location:'',reconcileTimedTurn=reconcileTruncatedTurn||appliedDecisionBoundary||unverifiedStructuredExternalBoundary||ambiguousAppliedConsequence||turnLimitCompletion;
  ...
  const safeConsequenceVisibleScene=sanitizeStructuredBoundaryScene(consequenceVisibleScene,decisionPlan,executionAuthority,intent,incoming?.action||'',incoming?.saveState?.pc?.name||'').filter(row=>!contradictoryStructuredChoicePrompt||row!==decisionPromptSourceRow),choicePrompt=String(decisionEvidenceTurn?._choice_prompt_text||'').trim(),consequenceSceneOwnsDecisionPrompt=Boolean(choicePrompt&&safeConsequenceVisibleScene.some(row=>{const text=String(row?.text||'').trim();return text&&(text.includes(choicePrompt)||choicePrompt.includes(text));})),choiceOwnsConsequenceBoundary=Boolean(executionAuthority.valid&&['choice','consequence'].includes(executionAuthority.boundary_kind)&&executionChoiceEventId===String(consequenceLifecycle?.selected_id||'').trim().toLowerCase()),preserveConsequenceChoices=Boolean(hasMeaningfulStop&&current===consequenceBoundary&&(choiceOwnsConsequenceBoundary||consequenceSceneOwnsDecisionPrompt)),runtimeTrustedConsequenceScene=returnedSceneReconciled&&preserveAttributedConsequence?safeConsequenceVisibleScene:[];
  const scheduleChoiceAtBoundary=Boolean(current===scheduleBoundary&&(visibleBoundary||structuredBoundary));
  if(returnedSceneReconciled&&!trimmedSurfacedScheduleScene){if(appliedDecisionBoundary)reconcileReturnedTimedTurn(turn,{reason:'decision-boundary',elapsed:applied,completedPrefixActionTypes,decisionPromptText:decisionEvidenceTurn?._choice_prompt_text,preserveDecisionRows:!untrustedStructuredDecision});else if(preserveRaisedFloorScene)reconcileReturnedRaisedFloorContinuation(turn,{elapsed:applied});else if(preserveAttributedConsequence)reconcileReturnedConsequenceTurn(turn,{elapsed:applied,scene:safeConsequenceVisibleScene,preserveChoices:preserveConsequenceChoices});else reconcileReturnedTimedTurn(turn,{reason:reconciliationReason,elapsed:applied,boundaryTitle:boundaryRows[0]?.title||'',completedPrefixActionTypes,preserveChoices:unverifiedStructuredScheduleBoundary&&hasMeaningfulStop&&scheduleChoiceAtBoundary});}
  if(receiptOwnedExternalChoices.length)turn.choices=receiptOwnedExternalChoices;
  ...
  const runtimeChoicesTrusted=Boolean(hasMeaningfulStop&&array(turn.choices).length&&(appliedDecisionBoundary||receiptOwnsScheduleChoices||receiptOwnsConsequenceChoices||unverifiedStructuredScheduleBoundary&&scheduleChoiceAtBoundary||preserveAttributedConsequence&&appliedConsequenceTimeBoundary&&preserveConsequenceChoices));
...
function reconcileReturnedConsequenceTurn(turn,{elapsed=0,scene=[],preserveChoices=false}={}){
  if(!turn||typeof turn!=='object')return false;
  const minutes=Math.max(0,Math.trunc(Number(elapsed)||0)),rows=array(scene).filter(item=>String(item?.text||'').trim()).slice(0,4),preservedChoices=preserveChoices?array(turn.choices).slice(0,3):[],boundaryText=`${minutes}분 지점에 후속 상황이 발현했다.`;
  if(!rows.length)return reconcileReturnedTimedTurn(turn,{reason:'consequence-boundary',elapsed:minutes,preserveChoices});
  turn.scene_title='후속 상황';turn.scene_summary=boundaryText;turn.scene=[{kind:'narration',text:boundaryText},...rows];turn.choices=preservedChoices;turn.emotion_updates=[];turn.director=null;
  return true;
}
