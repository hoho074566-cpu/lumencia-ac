...
function applySceneMomentumTimeFloor(incoming,turn,mode='game',consequenceLifecycle=null,consequenceVisibleScene=[]){
  ...
  const completionEvidence=timedActionCompletionEvidence(turn,intent,incoming?.action||'',incoming?.saveState?.pc?.name||''),completedBeforeChoice=!hasMeaningfulStop||completionEvidence,decisionBoundary=structuredDecisionBoundaryMinutes(turn,intent?.structuredExecutionPlan,current,hasMeaningfulStop,completionEvidence),earlierInterruptionBeforeConsequence=Boolean(consequenceWithinProfile&&current<consequenceBoundary&&(structuredInterruption||(hasMeaningfulStop&&!completedBeforeChoice)));
...
function reconcileReturnedTimedTurn(turn,{reason='profile-cap',elapsed=0,boundaryTitle='',completedPrefixActionTypes=[]}={}){
  ...
  const preservedChoices=reason==='decision-boundary'?array(turn.choices).slice(0,3):[];
  turn.scene_summary=text;turn.scene=[{kind:'narration',text}];turn.choices=preservedChoices;turn.emotion_updates=[];turn.director=null;
  return true;
}
