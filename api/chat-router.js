...
function localSceneRuntime(incoming,turn,directorTelemetry=null){
  ...
  const turnHook=deriveTurnHook({turn,sceneDelta,purpose,exitCondition,eventProgress:progressState.eventProgress,turnNumber});
  return {
    ...
    tone:clampText(turn?.importance||previous.tone||'routine',80),remaining_beats:hasDecision?[]:array(previous.remaining_beats).slice(0,1),purpose,exit_condition:exitCondition,turn_hook:turnHook,momentum,scene_delta:sceneDelta,...progressState,
  };
}
