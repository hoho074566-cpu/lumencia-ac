...
function applySceneMomentumTimeFloor(incoming,turn,mode='game',consequenceLifecycle=null,consequenceVisibleScene=[],runtimeAuthority={}){
  ...
  const contradictoryStructuredChoicePrompt=Boolean(appliedDecisionBoundary&&structuredChoicePromptContradictsExecution(decisionEvidenceTurn,decisionPlan,executionAuthority,intent,incoming?.action||'',incoming?.saveState?.pc?.name||''));
  const untrustedStructuredDecision=Boolean(appliedDecisionBoundary&&contradictoryStructuredChoicePrompt);
  const safeConsequenceVisibleScene=sanitizeStructuredBoundaryScene(consequenceVisibleScene,decisionPlan,executionAuthority,intent,incoming?.action||'',incoming?.saveState?.pc?.name||''),choicePrompt=String(decisionEvidenceTurn?._choice_prompt_text||'').trim(),consequenceSceneOwnsDecisionPrompt=Boolean(choicePrompt&&safeConsequenceVisibleScene.some(row=>{const text=String(row?.text||'').trim();return text&&(text.includes(choicePrompt)||choicePrompt.includes(text));})),choiceOwnsConsequenceBoundary=Boolean(executionAuthority.valid&&executionAuthority.boundary_kind==='choice'&&executionChoiceEventId===String(consequenceLifecycle?.selected_id||'').trim().toLowerCase()),preserveConsequenceChoices=Boolean(hasMeaningfulStop&&current===consequenceBoundary&&(choiceOwnsConsequenceBoundary||consequenceSceneOwnsDecisionPrompt)),runtimeTrustedConsequenceScene=returnedSceneReconciled&&preserveAttributedConsequence&&!contradictoryStructuredChoicePrompt?safeConsequenceVisibleScene:[];
