...
function compactSave(incoming,keys,registry,profile,keywords,text='',recentTexts=[]){
  ...
  const fateBackground=compactFateBackgroundForModel(save?.creation,save?.pc),pc=compactPc(save?.pc||{},profile.name.includes('important')||profile.name.includes('critical'),text);
...
function buildInput(incoming,originalInput,profile,routed,mode='game'){
  ...
  const fateBackground=compactFateBackgroundForModel(incoming.saveState?.creation,incoming.saveState?.pc),fateBackgroundDirective=clampText(buildFateBackgroundDirective({creation:incoming.saveState?.creation,pc:incoming.saveState?.pc}),1800);
  ...
  return{text:composeRoutedInput({saveState,optionalContext,reservedContext,authorityTail,actionBlock,inputChars:profile.inputChars}),orchestration,activeThreads,npcSignificance};
}
