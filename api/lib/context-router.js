...
function buildEventDirectorV2(incoming,originalInput,registry,mode='game'){
  ...
  const consequenceLookahead=['downtime','wait'].includes(sceneIntent.kind)?scheduleBoundaryLimitMinutes(sceneIntent):0;
...
function compactAbilityMap(value={},detail=true){return Object.fromEntries(abilityEntries(value).filter(([key])=>/^[^{}<>\r\n]{2,64}$/.test(key)&&!['__proto__','prototype','constructor'].includes(key)).slice(0,8).map(([key,row])=>{if(!detail)return[key,true];const description=clampText(row?.description||'',180),limitation=clampText(row?.limitation||'',180);return[key,{...(description?{description}:{}),...(limitation?{limitation}: {})}];}));}
...
function compactPc(pc={},important=false){const out={...object(pc)};if('characterSetting'in out)out.characterSetting=clampText(out.characterSetting||'',important?1700:1100);if('appearance'in out)out.appearance=clampText(out.appearance||'',350);if(Array.isArray(out.inventory))out.inventory=out.inventory.slice(0,18);if('talents'in out)out.talents=compactTalents(out.talents);out.skills=compactSkills(out.skills);out.skillCandidates=compactSkillCandidates(out.skillCandidates,2);if('traits'in out)out.traits=compactAbilityMap(out.traits,true);if('authorities'in out)out.authorities=compactAbilityMap(out.authorities,true);if('awakeningCandidates'in out)out.awakeningCandidates=compactAwakeningCandidates(out.awakeningCandidates,2);if('talentEvolutionHistory'in out)out.talentEvolutionHistory=compactTalentEvolutionHistory(out.talentEvolutionHistory);return out;}
...
function buildInput(incoming,originalInput,profile,routed,mode='game'){
  ...
  const essentialTalents=compactTalents(pc.talents),essentialTraits=compactAbilityMap(pc.traits,false),essentialAuthorities=compactAbilityMap(pc.authorities,false),essentialAwakening=compactMandatoryAwakeningCandidates(pc.awakeningCandidates);
  ...
  const essentialSave={version:save.version,turnNumber:save.turnNumber,world:{date:world.date||null,time:world.time||null,location:clampText(world.location||'',140)},pc:essentialPc,relevantNpcKeys:array(save.relevantNpcKeys).slice(0,4),npcStates:Object.fromEntries(Object.entries(object(save.npcStates)).slice(0,4).map(([key,row])=>[key,{location:clampText(row?.location||'',100),status:clampText(row?.status||row?.state||'',120)}])),sceneRuntime:{participants:array(scene.participants).slice(0,6),purpose:normalizeScenePurpose(scene.purpose),exit_condition:normalizeSceneExitCondition(scene.exit_condition),turn_hook:turnHook?{kind:turnHook.kind,status:turnHook.status,anchor:clampText(turnHook.anchor,140)}:null,momentum:{stall_streak:Number(momentum.stall_streak||0),last_intent:clampText(momentum.last_intent||'',60)},eventProgress:scene.eventProgress==null?null:{eventInstanceId:clampText(eventProgress.eventInstanceId||'',100),activeBeat:clampText(eventProgress.activeBeat||'',100)},faction_social:Object.keys(essentialFactionSocial.reputations).length?essentialFactionSocial:null}};
  ...
  return{text:composeRoutedInput({saveState,optionalContext,reservedContext,authorityTail,actionBlock,inputChars:profile.inputChars}),orchestration};
}
