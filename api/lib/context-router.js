...
import { buildFateBookDirective, compactFateBookForModel } from '../../lib/fate-book.js';
...
23) 사용자에게 보이는 narration/dialogue에서 내부 명칭 'PC' 또는 자리표시자 'Aaa'를 주어로 출력하지 않는다. 실제 플레이어 이름을 쓰거나 자연스럽게 주어를 생략한다.
 ...
 33) ending_discoveries는 AUTHORITATIVE SAVE_STATE의 ELIGIBLE Ending 중 이번 장면에서 실제 회차 결말을 보여 준 정확한 ID 한 건에만 쓴다. 일반 실패는 Fail Forward이며 Dead Ending이 아니다. 이미 발견한 Ending의 반복 보상을 만들지 않는다.`;
...
function buildInput(incoming,originalInput,profile,routed,mode='game'){
  ...
  const fateBackground=compactFateBackgroundForModel(incoming.saveState?.creation,incoming.saveState?.pc),fateBackgroundDirective=clampText(buildFateBackgroundDirective({creation:incoming.saveState?.creation,pc:incoming.saveState?.pc}),1800),personalStory=compactFatePersonalStoryForModel(incoming.saveState?.creation,{existingHooks:incoming.saveState?.hooks}),personalStoryDirective=clampText(buildFatePersonalStoryDirective({creation:incoming.saveState?.creation,existingHooks:incoming.saveState?.hooks}),1800),fateBook=compactFateBookForModel({saveState:incoming.saveState||{},action,mode}),fateBookDirective=clampText(buildFateBookDirective({saveState:incoming.saveState||{},action,mode}),1100);
  ...
  return{text:composeRoutedInput({saveState,optionalContext,reservedContext,authorityTail,actionBlock,inputChars:profile.inputChars}),orchestration,activeThreads,npcSignificance,npcCharacterBehavior,personalStory,fateBook};
}
