...
import { deriveRelationshipThresholdContext, RELATIONSHIP_THRESHOLDS_VERSION } from '../../lib/relationship-thresholds.js';
...
23) 사용자에게 보이는 narration/dialogue에서 내부 명칭 'PC' 또는 자리표시자 'Aaa'를 주어로 출력하지 않는다. 실제 플레이어 이름을 쓰거나 자연스럽게 주어를 생략한다.
 ...
 27) relationship threshold는 작은 수치 변화마다 태도를 뒤집는 단계표가 아니다. ROUTED MINIMUM의 eligible_signals는 hard eligibility일 뿐 자동 발동이 아니며, 누적 관계와 이번 사건이 행동 가능성을 실제로 바꾼 경우에만 relationship_changes.threshold_signal을 trust_opened/trust_withdrawn/hostility_opened/hostility_eased로 쓰고 구체적 followup을 남긴다. 아니면 none이다. 신호는 현재 장면·목표·significance 안에서 자연스럽게 사용하고 즉시 사건이나 PC 선택을 강제하지 않는다. AUTO/META/CONTINUE에서는 none이다.`;
...
function compactInnerNpc(row={},relevantKeys=[],registry={},sourceKey=''){
  const src=object(row),goal=object(src.active_goal),reason=object(src.relationship_reason),thresholds=object(src.relationship_thresholds);
  const out={
  ...
  if(thresholds.version===RELATIONSHIP_THRESHOLDS_VERSION)out.relationship_thresholds={version:RELATIONSHIP_THRESHOLDS_VERSION,trust_active:thresholds.trust_active===true,hostility_active:thresholds.hostility_active===true,last_transition:clampText(thresholds.last_transition||'none',40),cause:clampText(thresholds.cause||'',150),followup:clampText(thresholds.followup||'',150),updated_turn:boundedNumber(thresholds.updated_turn,0,1e9,0)};
...
function compactSave(incoming,keys,registry,profile,keywords,text='',recentTexts=[]){
  ...
  for(const k of keys){if(save?.relationships?.[k]!=null)rel[k]={...object(save.relationships[k]),threshold_context:deriveRelationshipThresholdContext({relationship:save.relationships[k],innerState:save?.npcInnerStates?.[k]})};if(save?.intimacyStates?.[k]!=null)intimacy[k]=save.intimacyStates[k];if(save?.npcStates?.[k]!=null)npcStates[k]=save.npcStates[k];if(save?.emotionStates?.[k]!=null)emotions[k]=save.emotionStates[k];if(save?.npcInnerStates?.[k]!=null)inner[k]=compactInnerNpc(save.npcInnerStates[k],keys,registry,k);if(save?.memories?.npc?.[k])npcMem[k]=selectMemories(save.memories.npc[k],keywords,names,profile.memoriesPerNpc);}
...
function buildInput(incoming,originalInput,profile,routed,mode='game'){
  ...
  const essentialRelationshipThresholds=Object.fromEntries(Object.entries(object(save.relationships)).map(([key,row])=>[key,object(row).threshold_context]).filter(([,row])=>Object.keys(object(row)).length).slice(0,4));
  ...
  return{text:composeRoutedInput({saveState,optionalContext,reservedContext,authorityTail,actionBlock,inputChars:profile.inputChars}),orchestration,activeThreads,npcSignificance,relationshipThresholds:{version:RELATIONSHIP_THRESHOLDS_VERSION,count:Object.keys(essentialRelationshipThresholds).length}};
}
