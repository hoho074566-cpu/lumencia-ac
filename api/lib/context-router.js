...
function failClosedWriterPacket(reason){
  const error=new Error('AI Writer context could not be reduced to the canonical Thin Scene Packet.');
  error.statusCode=409;
  error.code='WRITER_CONTEXT_UNAVAILABLE';
...
export function routeOpenAIParams(params,{incoming={},mode='game'}={}){
  ...
  if(!required.every(m=>originalInstructions.includes(m)))failClosedWriterPacket('core prompt markers changed');
  const routed=buildInstructions(originalInstructions,incoming,profile,originalInput,mode);if(!Object.keys(routed.registry||{}).length)failClosedWriterPacket('registry parse failed');
