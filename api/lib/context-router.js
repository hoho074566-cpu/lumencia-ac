...
import { buildFateInheritanceDirective, compactFateInheritanceForModel } from '../../lib/fate-inheritance.js';
...
function compactSave(incoming,keys,registry,profile,keywords,text='',recentTexts=[]){
  ...
  const fateBackground=compactFateBackgroundForModel(save?.creation,save?.pc),personalStory=compactFatePersonalStoryForModel(save?.creation,{existingHooks:save?.hooks}),inheritance=compactFateInheritanceForModel(save?.creation),pc=compactPc(save?.pc||{},profile.name.includes('important')||profile.name.includes('critical'),text);
...
function buildInput(incoming,originalInput,profile,routed,mode='game'){
  ...
  const fateBackground=compactFateBackgroundForModel(incoming.saveState?.creation,incoming.saveState?.pc),fateBackgroundDirective=clampText(buildFateBackgroundDirective({creation:incoming.saveState?.creation,pc:incoming.saveState?.pc}),1800),personalStory=compactFatePersonalStoryForModel(incoming.saveState?.creation,{existingHooks:incoming.saveState?.hooks}),personalStoryDirective=clampText(buildFatePersonalStoryDirective({creation:incoming.saveState?.creation,existingHooks:incoming.saveState?.hooks}),1800),inheritance=compactFateInheritanceForModel(incoming.saveState?.creation),inheritanceDirective=clampText(buildFateInheritanceDirective({creation:incoming.saveState?.creation}),1600);
  ...
  return{text:composeRoutedInput({saveState,optionalContext,reservedContext,authorityTail,actionBlock,inputChars:profile.inputChars}),orchestration,activeThreads,npcSignificance,personalStory,inheritance};
}
