...
import { buildLivingWorldDirective, compactLivingWorldContext } from '../../lib/living-world-consequences.js';
...
23) 사용자에게 보이는 narration/dialogue에서 내부 명칭 'PC' 또는 자리표시자 'Aaa'를 주어로 출력하지 않는다. 실제 플레이어 이름을 쓰거나 자연스럽게 주어를 생략한다.
 ...
 33) 같은 장면의 NPC는 PC에게만 차례로 반응하지 않고 서로 말·행동·침묵·경쟁할 수 있다. 명시적으로 퇴장하거나 권위 있는 위치 변경이 확인되기 전까지 장면 참여를 유지하며 단순 동석만으로 관계를 바꾸지 않는다.
 34) NPC 목표끼리 실제로 충돌하는지는 의미와 상황으로 판단한다. 충돌 가능성만으로 사건을 확정하지 않으며, 구체화된 갈등만 기존 사건·hook·delayed consequence로 최소 기록한다. 화면 밖 세계는 Active Thread·주요 관계·공개 faction 증거·현재 사건 관련 NPC만 우선하고 모든 NPC를 전면 시뮬레이션하지 않는다.
 35) 공개적인 큰 행동의 후폭풍은 실제 목격/공식 기록에서 시작해 개별 반응·출처 있는 소문·등록 faction의 해석·평판·미래 사건으로 제한적으로 이어간다. severity·visibility·duration·decay·affected 범위를 먼저 판단하고 실제 변화만 기존 memory/faction/consequence 필드에 기록한다.
 36) 과거 사실·말·약속·물건·장소·관계·선택은 현재 사건과 실제 인과가 있을 때만 setup/payoff로 재사용한다. 실패는 같은 입력 재시도 대신 근거 있는 새 Story State로 전진시키되, 새 generic quest lifecycle·universal event sourcing·full off-screen simulation을 만들지 않는다.`;
...
function buildInput(incoming,originalInput,profile,routed,mode='game'){
  ...
  const livingWorld=compactLivingWorldContext({saveState:incoming.saveState||{},candidateKeys:routed.keys,registry:routed.registry,activeThreads:activeThreads.threads,mode,maxNpcs:4,allowSecrets:routed.secretAllowed});
  const livingWorldDirective=buildLivingWorldDirective({context:livingWorld,maxChars:3600});
  ...
  return{text:composeRoutedInput({saveState,optionalContext,reservedContext,authorityTail,actionBlock,inputChars:profile.inputChars}),orchestration,activeThreads,npcSignificance,npcCharacterBehavior,livingWorld,personalStory};
}
