...
function compactSceneRuntime(sceneRuntime={},keywords=[],text='',registeredNpcKeys=null,maxFactions=3,historyLimit=2,recentText=''){
  const runtime={...object(sceneRuntime)},factionSocial=compactFactionSocialForContext(runtime.faction_social,{text,recentText,keywords,maxFactions,historyLimit,registeredNpcKeys});
  if(Object.keys(factionSocial.reputations).length)runtime.faction_social=factionSocial;
  else delete runtime.faction_social;
  return runtime;
}
function compactSave(incoming,keys,registry,profile,keywords,text='',recentText=''){
...
function buildInput(incoming,originalInput,profile,routed){
  ...
  const essentialFactionSocial=compactFactionSocialForContext(incoming.saveState?.sceneRuntime?.faction_social,{text:action,recentText:recentFactionText,keywords:routed.keywords,maxFactions:2,historyLimit:1,registeredNpcKeys});
