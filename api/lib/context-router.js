...
function compactSkills(value={}){return Object.fromEntries(Object.entries(object(value)).map(([key,row])=>[clampText(key,80),row]).filter(([key])=>key&&!['__proto__','prototype','constructor'].includes(key)).slice(0,24));}
function compactSkillCandidates(value={},historyLimit=2){return Object.fromEntries(Object.entries(object(value)).filter(([key])=>/^[^{}<>\r\n]{2,48}$/.test(key)&&!['__proto__','prototype','constructor'].includes(key)).sort((a,b)=>Number(b[1]?.updated_turn||b[1]?.updatedTurn||0)-Number(a[1]?.updated_turn||a[1]?.updatedTurn||0)||a[0].localeCompare(b[0])).slice(0,8).map(([key,row])=>[key,{progress:boundedNumber(row?.progress,0,99,0),basis:clampText(row?.basis||'',120)||null,updated_turn:Number(row?.updated_turn||row?.updatedTurn||0),history:array(row?.history).slice(-historyLimit).map(item=>({turn:Number(item?.turn||0),amount:boundedNumber(item?.amount,1,15,1),basis:clampText(item?.basis||'',120),reason:clampText(item?.reason||'',180)}))}]));}
function compactPc(pc={},important=false){const out={...object(pc)};if('characterSetting'in out)out.characterSetting=clampText(out.characterSetting||'',important?1700:1100);if('appearance'in out)out.appearance=clampText(out.appearance||'',350);if(Array.isArray(out.inventory))out.inventory=out.inventory.slice(0,18);out.skills=compactSkills(out.skills);out.skillCandidates=compactSkillCandidates(out.skillCandidates,2);return out;}
