// LUMENSIA MOBILE V1.5.3 — Context Router
// External API version: 0.7.0
...
import { routeOpenAIParams, routerVersion, array, object, clampText } from './lib/context-router-v153.js';
...
const PATCH_SYMBOL = Symbol.for('lumensia.v153.responses.parse.router');

const AUTO_DIRECTIVE = String.raw`[LUMENSIA V1.5.3 AUTO FLOW]
...
const CONTINUE_DIRECTIVE = String.raw`[LUMENSIA V1.5.3 CONTINUE]
...
function lockContinueTurn(turn) {
  ...
  turn.director = {
    ...
    reason:'V1.5.3 CONTINUE hard freeze',
  };
  return turn;
}
...
function applyExtendedExpressions(turn,saveState){if(!turn||!Array.isArray(turn.scene))return turn;turn.scene=turn.scene.map(item=>item?.kind==='dialogue'?{...item,display_expression:classifyExtendedExpression(item,saveState),v153_extended_expression:true}:item);return turn;}
...
export default async function handler(req,res){
  ...
  try{
    ...
    if(mode==='continue'){
      ...
      const pipeline={pipeline:'continue-router-v153',stages:1,qa_result:'SKIP',rewrite_applied:false,background_sim:false,context_router:telemetry};
      data.pipeline=pipeline; setAdapterRoute(data,mode,pipeline,telemetry); return res.status(200).json(data);
    }

    if(mode==='meta'){
      const pipeline={pipeline:'meta-full-v153',stages:1,qa_result:'SKIP',rewrite_applied:false,background_sim:false,context_router:telemetry};
      data.pipeline=pipeline; setAdapterRoute(data,mode,pipeline,telemetry); return res.status(200).json(data);
    }
    ...
    const pipeline={
      pipeline:incoming0.qualityPipeline===false?'single-writer-router-v153':'single-pass-q3-router-v153',
      ...
      note:'V1.5.3 keeps one core model call and routes CANON/save/history to a relevance budget before Responses.parse sends it.',
    };
    data.pipeline=pipeline; setAdapterRoute(data,mode,pipeline,telemetry);
    return res.status(200).json(data);
  }catch(error){
    console.error('[V1.5.3]',error);
    return res.status(Number.isInteger(error?.status)?error.status:500).json({error:error?.message||String(error),code:error?.code||'V153_CONTEXT_ROUTER_ERROR',server_version:ADAPTER_VERSION});
  }
}
