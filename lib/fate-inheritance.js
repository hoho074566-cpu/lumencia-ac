import { buildFateBackgroundState } from './fate-background.js';
...
export function quoteInheritanceAllocations(rawAllocations,meta,options={}){
  ...
  const affinityRows=allocations.filter((row)=>row.kind==='affinity');
  if(new Set(affinityRows.map((row)=>row.target)).size!==affinityRows.length)throw new Error('같은 Fate Affinity를 한 purchase에서 두 번 구매할 수 없음.');
...
export function applyInheritanceReceipt(generated,receipt){
  ...
  if(origin){
    origin.baseStats=Object.fromEntries(Object.entries(statLabels).map(([key,label])=>[key,Math.max(1,gradeOrder.indexOf(String(pc.stats?.[label]?.grade||'F').replace(/[+-]/g,''))+1)]));
    origin.talents={...origin.talents,...pc.talents};origin.realm=pc.realm;
    next.creation.fateStart.background=buildFateBackgroundState(origin);
  }
  return next;
}
