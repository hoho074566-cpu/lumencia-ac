...
export const FATE_INHERITANCE_PURCHASE_LOCK = 'lumensia.inheritance.purchase.v1';
...
export function reconcileInheritanceImport(current,incoming,{fateBook,...options}={}){
  const live=normalizeInheritanceState(current,options),imported=normalizeInheritanceState(incoming,options);
  const rawIncomingCount=Object.keys(object(incoming).purchases||{}).length;
  if(rawIncomingCount!==imported.purchaseOrder.length)throw new Error('가져온 계승 원장에 유효하지 않은 구매 영수증이 있음.');
  const liveIds=new Set(live.purchaseOrder),importedIds=new Set(imported.purchaseOrder);
  for(const lifeId of live.purchaseOrder){
    if(importedIds.has(lifeId)&&JSON.stringify(live.purchases[lifeId])!==JSON.stringify(imported.purchases[lifeId])){
      throw new Error('동일한 Next Life의 계승 구매 영수증이 서로 충돌함.');
    }
  }
  const liveOnly=live.purchaseOrder.filter((lifeId)=>!importedIds.has(lifeId));
  const importedOnly=imported.purchaseOrder.filter((lifeId)=>!liveIds.has(lifeId));
  if(liveOnly.length&&importedOnly.length)throw new Error('서로 분기된 계승 원장은 자동 병합할 수 없음. 현재 메타 진행은 보존됨.');
  const next=importedOnly.length?imported:live;
  const summary=inheritancePointSummary(next,fateBook,options);
  if(summary.overspent>0||summary.spent>summary.earned)throw new Error('계승 원장 충돌: 사용 포인트가 획득 포인트를 초과함.');
  return next;
}
...
export async function serializeInheritancePurchase(locks,task){
  if(!locks||typeof locks.request!=='function')throw new Error('이 브라우저에서는 계승 구매를 안전하게 직렬화할 수 없음. 계승 포인트는 사용되지 않음.');
  if(typeof task!=='function')throw new Error('계승 구매 작업이 필요함.');
  return locks.request(FATE_INHERITANCE_PURCHASE_LOCK,{mode:'exclusive'},task);
}
