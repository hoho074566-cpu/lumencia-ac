...
export function buildNpcCharacterBehaviorDirective({ context = null, maxChars = 2800 } = {}) {
  ...
  const profiles = source.profiles
    .filter((row) => row.goal || row.relationship || row.prior_judgment || row.social_stance || row.wants_from_pc || row.concern || row.internal_emotion || array(row.pc_evidence).length)
    .map((row) => ({ ...row, pc_evidence: array(row.pc_evidence).map((entry) => ({ ...entry })) }));
