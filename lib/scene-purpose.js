...
function eventStepSignature(progress={}) { const row=object(progress),completed=array(row.completedBeats||row.completed_beats).map((beat)=>clampText(beat,100)).filter(Boolean).slice(-24),omitted=Math.max(0,Math.floor(Number(row.omittedCompletedCount??row.omitted_completed_count)||0)); return JSON.stringify([activeBeat(row),completed,omitted]); }
...
export function deriveScenePurpose({ previousRuntime = {}, turn = {}, sceneDelta = {}, eventProgress = null, action = '', sceneKey = '', turnNumber = 0 } = {}) {
  ...
  const sameEventAdvanced=Boolean(currentEventId&&currentEventId===previousEventId&&eventStepSignature(progress)!==eventStepSignature(previousRuntime?.eventProgress));
  ...
  if(currentEventId&&previous?.kind==='event'&&currentEventId===previousEventId&&!sceneChanged){
    if(sameEventAdvanced)return makePurpose({kind:'event',focus:summary||activeBeat(progress)||`진행 중인 사건 ${currentEventId}`,source:'event-progress',turnNumber,eventInstanceId:currentEventId});
    return previous;
  }
...
export function buildScenePurposeDirective({ action = '', saveState = {} } = {}) {
  ...
  const intent=classifySceneIntent(rawAction,{location:saveState?.world?.location||'',currentTime:saveState?.world?.time||'',currentDate:saveState?.world?.date||'',currentWeekday:saveState?.world?.weekday||'',actorName:saveState?.pc?.name||''}).kind;
  ...
  if(currentActionFirst)lines.push('- CURRENT ACTION PRIORITY: 이번 요청의 USER ACTION이 저장된 PURPOSE_FOCUS보다 우선한다. 이전 목적을 계속 수행하도록 유도하거나 현재 행동의 목표를 바꾸지 않는다.','- 저장된 PURPOSE_FOCUS는 현재 행동과 호환되는 NPC·세계·이벤트 반응 맥락으로만 사용한다.','- CURRENT ACTION SOVEREIGNTY: USER ACTION에 명시되지 않은 PC의 추가 행동·대사·감정·생각·수락·거절·선택을 대신 만들지 않는다.');
  else lines.push('- Scene Purpose를 세계 변화·NPC 행동·사건의 결과로 진전시키되, PC의 새로운 행동·대사·감정·생각·수락·거절·선택을 대신 만들지 않는다.');
  if(currentActionFirst&&intent==='decision-sensitive')lines.push('- DIRECT QUESTION PRIORITY: 현재 질문에 먼저 직접 답하고, 저장된 PURPOSE_FOCUS를 계속하기 위해 답을 미루거나 질문을 다른 선택의 실행으로 바꾸지 않는다.');
  lines.push('- 충족된 목적을 반복하지 않는다. 비용 있는 PC 답이 실제 필요한 HARD_DECISION만 choices를 열며 종료용 질문은 금지한다.');
