...
export function deriveScenePurpose({ previousRuntime = {}, turn = {}, sceneDelta = {}, eventProgress = null, action = '', sceneKey = '', turnNumber = 0 } = {}) {
  ...
  const intent=String(sceneDelta?.intent||'generic'),npcAction=Boolean(sceneDelta?.flags?.npcAction),rawAction=String(action||'').trim(),playerAction=AUTO_ACTION_RE.test(rawAction)?'':clampText(rawAction,180);
