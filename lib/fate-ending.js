...
export function resolveEndingDefinition(rawId,{allowedCharacterKeys=[]}={}){
  ...
  if(ENDING_REGISTRY[endingId])return{...ENDING_REGISTRY[endingId],discoveryId:endingId,characterKeys:[]};
  const split=endingId.lastIndexOf(':');
  if(split<=0)return null;
  const baseId=endingId.slice(0,split),characterKey=endingId.slice(split+1);
  const base=ENDING_REGISTRY[baseId];
...
function normalizedRecord(raw,options={}){
  ...
  const discoveredAt=iso(source.discoveredAt||source.firstDiscoveredAt)||new Date(0).toISOString();
  return{
...
export function normalizeFateBook(raw,options={}){
  ...
  for(const rawRow of rows){
    ...
    if(!previous||row.discoveredAt<previous.discoveredAt)discoveries[row.discoveryId]=row;
  }
  const rewardLedger={};
...
export function reconcileFateBooks(current,incoming,options={}){
  ...
  return normalizeFateBook({discoveries:{...b.discoveries,...a.discoveries}},options);
}
