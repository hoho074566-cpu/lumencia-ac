...
function historicalId(value){const text=clean(value,120);return /^[a-z0-9_.:-]{1,120}$/i.test(text)&&!FORBIDDEN_KEYS.has(text)?text:'';}
...
function normalizedEarnedReceipt(rawId,raw){
  const id=historicalId(rawId),source=object(raw);if(!id)return null;
  const discoveryId=historicalId(source.discoveryId||id),amount=Number(source.amount),grantedAt=iso(source.grantedAt);
  if(discoveryId!==id||source.source!==`ending:first-discovery:${id}`||!Number.isSafeInteger(amount)||amount<=0||amount>100||!grantedAt)return null;
  const receiptId=clean(source.receiptId||`earned:${id}`,160);
  if(receiptId!==`earned:${id}`)return null;
  return{receiptId,discoveryId:id,source:`ending:first-discovery:${id}`,amount,grantedAt};
}

function historicalRecord(raw,id,receipt){
  const source=object(raw),definition=resolveEndingDefinition(id,{allowedCharacterKeys:[]});
  return{
    discoveryId:id,
    endingId:clean(source.endingId,120)||definition?.endingId||id.split(':')[0],
    category:clean(source.category,40)||definition?.category||'historical',
    title:clean(source.title,160)||definition?.title||id,
    characters:array(source.characters).map((value)=>clean(value,64)).filter(Boolean).slice(0,4),
    worldState:clean(source.worldState,240)||definition?.worldState||'historical',
    reason:clean(source.reason,500)||definition?.conditions||'과거에 확정된 Ending first-discovery',
    reward:receipt.amount,
    discoveredAt:iso(source.discoveredAt||source.firstDiscoveredAt)||receipt.grantedAt,
    runId:clean(source.runId,100)||null,
    turnNumber:Math.max(0,Math.trunc(Number(source.turnNumber)||0)),
  };
}
...
export function normalizeFateBook(raw,options={}){
  const source=object(raw),rows=[],rawLedger=object(source.rewardLedger),rewardLedger={};
  for(const [id,value] of Object.entries(rawLedger)){
    const receipt=normalizedEarnedReceipt(id,value);if(receipt)rewardLedger[id]=receipt;
  }
  ...
  for(const rawRow of rows){
    const id=historicalId(object(rawRow).discoveryId||object(rawRow).endingId||object(rawRow).id);
    const row=normalizedRecord(rawRow,options)||(id&&rewardLedger[id]?historicalRecord(rawRow,id,rewardLedger[id]):null);if(!row)continue;
  ...
  for(const [id,row] of Object.entries(discoveries)){
    if(!rewardLedger[id])rewardLedger[id]={receiptId:`earned:${id}`,discoveryId:id,source:`ending:first-discovery:${id}`,amount:row.reward,grantedAt:row.discoveredAt};
    row.reward=rewardLedger[id].amount;
    if(row.discoveredAt===UNKNOWN_DISCOVERY_AT)row.discoveredAt=rewardLedger[id].grantedAt;
  }
  for(const [id,receipt] of Object.entries(rewardLedger))if(!discoveries[id])discoveries[id]=historicalRecord({},id,receipt);
...
export function inspectFateBook(raw,options={}){
  if(raw==null)return{valid:true,book:normalizeFateBook(null,options),errors:[]};
  const source=object(raw),errors=[],ledger=object(source.rewardLedger),rows=Array.isArray(source.discoveries)?source.discoveries:Object.values(object(source.discoveries));
  for(const [id,value] of Object.entries(ledger))if(!normalizedEarnedReceipt(id,value))errors.push(`earned receipt 손상: ${id}`);
  for(const rawRow of rows){
    const row=object(rawRow),id=historicalId(row.discoveryId||row.endingId||row.id),receipt=id?normalizedEarnedReceipt(id,ledger[id]):null;
    if(!id||(!normalizedRecord(row,options)&&!receipt))errors.push(`Ending discovery 손상: ${id||'unknown'}`);
    if(receipt&&row.reward!=null&&Number(row.reward)!==receipt.amount)errors.push(`earned/discovery amount 불일치: ${id}`);
  }
  const book=normalizeFateBook(source,options);
  if(Object.hasOwn(source,'rewardTotal')&&Number(source.rewardTotal)!==book.rewardTotal)errors.push('Fate Book rewardTotal 불일치');
  return{valid:errors.length===0,book,errors};
}
...
export function reconcileFateBooks(current,incoming,options={}){
  ...
  return normalizeFateBook({discoveries:[...Object.values(a.discoveries),...Object.values(b.discoveries)],rewardLedger:{...b.rewardLedger,...a.rewardLedger}},options);
}
