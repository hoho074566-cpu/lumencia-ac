...
const NON_DEPARTURE = /(?:\b(?:does|did|will|would|should|could|might|may)\s+not\b|\bnot\s+(?:leave|exit|depart)\b|\b(?:might|may|could|should|would|will)\s+(?:leave|exit|depart)\b|\b(?:plans?|intends?|is\s+going)\s+to\s+(?:leave|exit|depart)\b|\b(?:should|could|would)\s+i\s+(?:leave|go)\b|\bif\s+[^.!?]*(?:leave|exit|depart)|떠나지\s*않|나가지\s*않|떠날(?:까|지도|지\s*모르|수|\s*예정)|나갈(?:까|지도|지\s*모르|수|\s*예정)|떠나기로|나가기로|떠나려\s*한다|나가려\s*한다|떠나야\s*할까|나가야\s*할까|떠나고\s*싶|나가고\s*싶)/iu;
...
function carriedParticipants(previous, action, turn, recentTurns, registry) {
  ...
  const staying = /(?:\bstays?\s+behind\b|\bremains?\b|남아|남는|남았|머무|따라가지\s*않|동행하지\s*않)/iu;
  const known = identities(turn, recentTurns, previous, registry);
  const clauses=text.split(/[.!?;]|,?\s+(?:but|while|whereas|and)\s+|(?:하)?지만|그러나|반면/iu).filter(Boolean);
  return array(previous).map(String).filter(key => [...(known.get(key) || [])].some(name => clauses.some(clause=>identityMatch(clause,name)&&accompaniment.test(clause)&&!staying.test(clause))));
}

function returnedAfterDeparture(key, turn, recentTurns, registry) {
  const known = identities(turn, recentTurns, Object.keys(registry||{}), registry);
  if(!known.has(key))known.set(key,new Set([key,registry?.[key]].filter(Boolean)));
  const names = [...(known.get(key) || [])];
  let departed = false;
  for (const item of array(turn?.scene)) {
    const text = String(item?.text || '');
    const itemTurn = {scene:[item]};
    const wasDeparted=departed;
    if (explicitDepartures(itemTurn, recentTurns, {activeParticipants:[key],registry}).has(key)) departed = true;
    const departureIndexes=[...text.matchAll(DEPARTURE)].map(match=>match.index);
    const occurrences=identityOccurrences(text,known);
    const entries=[...text.matchAll(/(?:\b(?:returns?|re-enters?|enters?|arrives?|joins?|comes?\s+back)\b|돌아(?:오|왔)|다시\s*(?:들어오|나타나|합류)|도착하|입장하)/giu)].map(match=>({index:match.index,subject:occurrences.filter(row=>row.end<=match.index).at(-1)?.key}));
    const boundReturn=entries.some(entry=>entry.subject===key&&(wasDeparted||departureIndexes.some(index=>index<entry.index)));
    if ((wasDeparted && item?.speaker_key === key) || boundReturn) departed = false;
  }
  return !departed;
}

export function reconcileParticipants({ previous = [], action = '', turn = {}, recentTurns = [], scheduledEntries = [], registry = {}, currentLocation = '' } = {}) {
  ...
  const carried = changedLocation ? carriedParticipants(previous, action, turn, recentTurns, registry) : array(previous).map(String);
  const participants = new Set([...speakers, ...array(scheduledEntries).map(String), ...carried]);
  const targetLocation=compact(turn?.state_delta?.new_location||currentLocation);
  for(const row of array(turn?.state_delta?.npc_state_updates))if(row?.npc_key&&row?.location&&targetLocation&&compact(row.location)!==targetLocation)participants.delete(String(row.npc_key));
  for (const key of explicitDepartures(turn, recentTurns, {activeParticipants:previous,registry})) if(!returnedAfterDeparture(key,turn,recentTurns,registry))participants.delete(key);
  return [...participants].slice(0, 8);
}

export function actualScheduledEntrants({ due = [], turn = {}, recentTurns = [], currentLocation = '', registry = {} } = {}) {
  ...
  const known = identities(turn, recentTurns,[...candidates],registry);
  ...
  for(const text of entryText){if(!location||!compact(text).includes(location))continue;const occurrences=identityOccurrences(text,known);for(const entry of text.matchAll(/(?:\b(?:arrives?|enters?|joins?|comes?\s+in)\b|도착하|들어(?:오|왔)|입장하|합류하|나타나)/giu)){const subject=occurrences.filter(row=>row.end<=entry.index).at(-1);if(subject&&candidates.has(subject.key))structured.add(subject.key);}}
  return [...structured];
}
...
export function freshChoices(action, turn) {
  ...
  if (!action || failedAttempt(action, turn)) return choices;
  return choices.filter(choice => repeatableAttack(action)&&repeatableAttack(choice) || !isStaleChoice(choice, action, turn));
}
