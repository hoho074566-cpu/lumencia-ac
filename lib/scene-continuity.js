...
function returnedAfterDeparture(key, turn, recentTurns, registry) {
  const known = identities(turn, recentTurns, [key], registry);
  ...
  for (const item of array(turn?.scene)) {
    ...
    const entered = /(?:\b(?:returns?|re-enters?|enters?|arrives?|joins?|comes?\s+back)\b|돌아(?:오|왔)|다시\s*(?:들어오|나타나|합류)|도착하|입장하)/iu.test(text);
    const departureIndexes=[...text.matchAll(DEPARTURE)].map(match=>match.index);
    const entryIndexes=[...text.matchAll(/(?:\b(?:returns?|re-enters?|enters?|arrives?|joins?|comes?\s+back)\b|돌아(?:오|왔)|다시\s*(?:들어오|나타나|합류)|도착하|입장하)/giu)].map(match=>match.index);
    const returnedLaterInItem=departureIndexes.length&&entryIndexes.some(index=>index>departureIndexes.at(-1))&&names.some(name=>identityMatch(text,name));
    if ((wasDeparted && ((item?.speaker_key === key) || (entered && names.some(name=>identityMatch(text,name))))) || returnedLaterInItem) departed = false;
  }
  return !departed;
}
...
export function actualScheduledEntrants({ due = [], turn = {}, recentTurns = [], currentLocation = '', registry = {} } = {}) {
  ...
  for(const text of entryText){const occurrences=identityOccurrences(text,known);for(const entry of text.matchAll(/(?:\b(?:arrives?|enters?|joins?|comes?\s+in)\b|도착하|들어(?:오|왔)|입장하|합류하|나타나)/giu)){const subject=occurrences.filter(row=>row.end<=entry.index).at(-1);if(subject&&candidates.has(subject.key))structured.add(subject.key);}}
  return [...structured];
}
