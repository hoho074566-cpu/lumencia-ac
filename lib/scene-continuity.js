...
const DEPARTURE = /(?:\bleav(?:e|es|ing|eft)\b|\bexits?\b|\bdepart(?:s|ed|ing)?\b|\bwalk(?:s|ed)?\s+away\b|\bgo(?:es|ne)?\s+(?:out|away)\b|떠나(?:다|간다|갔다|갔고|며|서|버린다|버렸다)?|나가(?:다|ㄴ다|고|며|서|버린다|버렸다)?|나갔(?:다|고|으며)?|퇴장(?:한다|했다|하며)?|자리를\s*떠나|작별(?:을)?\s*(?:하고|한\s*뒤)|복도를\s*따라\s*멀어)/iu;
...
export function explicitDepartures(turn, recentTurns = []) {
  ...
  for (const item of array(turn?.scene)) {
    ...
    for (const { key, name } of candidates) {
      if (identityMatch(text, name)) found.add(key);
    }
  }
  return found;
}
...
export function reconcileParticipants({ previous = [], action = '', turn = {}, recentTurns = [], scheduledEntries = [] } = {}) {
  ...
  const participants = new Set(changedLocation ? carriedParticipants(previous, action, turn, recentTurns) : array(previous).map(String));
...
function isStaleChoice(choice, action, turn) {
  ...
  const extraDestination = choiceTokens.some(word => !tokens.includes(word) && !/^(?:건물|회관|강당|방|실|내부|안|hall|building|room|inside|간다|가다|가기|들어간다|들어가다|입장한다)$/.test(word));
