...
const DEPARTURE = /(?:\bleav(?:e|es|ing)\b|\bleft\b|\bexits?\b|\bdepart(?:s|ed|ing)?\b|\bwalk(?:s|ed)?\s+away\b|\bgo(?:es|ne)?\s+(?:out|away)\b|떠나(?:다|간다|갔다|갔고|며|서|버린다|버렸다)?|나가(?:다|ㄴ다|고|며|서|버린다|버렸다)?|나갔(?:다|고|으며)?|퇴장(?:한다|했다|하며)?|자리를\s*떠나|작별(?:을)?\s*(?:하고|한\s*뒤)|복도를\s*따라\s*멀어)/giu;
...
const FAILED_DEPARTURE = /(?:\b(?:tried|attempted|intended)\s+to\s+(?:leave|exit|depart)\b|\b(?:leave|exit|departure)\s+(?:was\s+)?(?:interrupted|prevented|blocked)\b|(?:떠나|나가|퇴장하)려(?:고|\s*했)(?:지만|으나)?.{0,50}(?:막|잠|저지|실패|못|중단|방해)|(?:떠나|나가)려다.{0,40}(?:막|잠|저지|실패|못|중단|방해))/iu;
...
export function explicitDepartures(turn, recentTurns = []) {
  ...
  for (const item of array(turn?.scene)) {
    ...
    if (NON_DEPARTURE.test(text) || FAILED_DEPARTURE.test(text) || text.trim().endsWith('?')) continue;
    const occurrences = identityOccurrences(text, known);
    for (const departure of text.matchAll(DEPARTURE)) {
      const subject = departureSubject(text, departure.index, occurrences);
...
export function reconcileParticipants({ previous = [], action = '', turn = {}, recentTurns = [], scheduledEntries = [] } = {}) {
