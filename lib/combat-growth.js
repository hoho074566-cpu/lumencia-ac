...
const CORRECTED_OUTCOME_RE = /(?:마지막|다음|이어진|재차|세\s*번째)[^\n]{0,220}(?:반복|시도|연습|동작|자세)[^\n]{0,220}(?:성공|해내|완료|깔끔|매끄럽|끊기지|겹치지|교차하지\s*않|흐트러지지|무너지지|안정)/i;
const EXPLICIT_OUTCOME_SUBJECT_RE = /^(?!(?:내가|나는|제가|저는|PC(?:가|는)|플레이어(?:가|는)|주인공(?:이|은)|Aaa(?:가|는))\s)[가-힣A-Za-z0-9_-]{2,32}(?:은|는|이|가)\s+[^\n]{0,120}(?:반복|시도|연습|동작|자세)[^\n]{0,120}(?:성공|해내|완료|깔끔|매끄럽|끊기지|겹치지|교차하지\s*않|흐트러지지|무너지지|안정)/i;
...
function sceneEvidence(scene = [], pcName = '') {
  ...
  for (const row of rows) {
    ...
    const correctedOutcome = !dialogue
      ...
      && CORRECTED_OUTCOME_RE.test(rowText)
      && (pcSubjectInSegment(rowText, pcName) || !EXPLICIT_OUTCOME_SUBJECT_RE.test(rowText))
      && !hasNonPlayerThirdPartyAction(rowText, pcName)
      && !hasNonPlayerThirdPartyStimulus(rowText, pcName);
    if (correctedOutcome) {
      tier = Math.max(tier, 1);
      evidenceSegments.push({ text:rowText, tier:1 });
    }
    for (const rawSegment of rowText.split(/[.!?\n,;]+/)) {
      ...
      if (!playerSubject && !playerAddressed && EXPLICIT_OUTCOME_SUBJECT_RE.test(segment)) continue;
