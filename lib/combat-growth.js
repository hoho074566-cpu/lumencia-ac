...
function playerOwnsGrowthAction(action = '', pcName = '') {
  ...
  for (const rawSegment of text.split(/[.!?\n,;]+/)) {
    ...
    if (OBSERVATION_RE.test(segment)) {
      ...
      if (tail && PLAYER_GROWTH_ACTION_RE.test(tail) && !THIRD_PARTY_ACTION_RE.test(tail)) return true;
    }
    ...
    if (THIRD_PARTY_ACTION_RE.test(segment) && !playerNamed) continue;
...
function sceneEvidence(scene = [], pcName = '') {
  ...
  for (const row of (Array.isArray(scene) ? scene : []).slice(0, 24)) {
    ...
    for (const rawSegment of cleanText(row?.text, 900).split(/[.!?\n,;]+/)) {
      ...
      if (THIRD_PARTY_STIMULUS_RE.test(segment) && !playerSubject && !playerAddressed) continue;
      let segmentTier = 0;
...
function skillIsRelevant(name, action, resolution, deliberateTraining, combatAction) {
  ...
  const directlyNamed = identity(action).includes(key);
  if (deliberateTraining && directlyNamed) return true;
  if (combatAction && resolution.triggered) return false;
  return !combatAction && deliberateTraining && directlyNamed;
}
...
function skillEvidenceTier(name, action, evidence, resolution, deliberateTraining) {
  const key = identity(name);
  const direct = (Array.isArray(evidence?.segments) ? evidence.segments : []).filter((row) => identity(row?.text).includes(key));
  ...
  if (deliberateTraining && identity(action).includes(key)) return Number(evidence?.tier || 0);
...
export function deriveCombatGrowthState({
  ...
  if (allowProgress && actionOwned && evidenceTier > 0) {
    ...
    for (const raw of proposedSkills) {
      ...
      const reason = cleanText(raw?.reason, 240);
      const amount = positiveProgressAmount(raw?.amount);
      const cap = gradeAwardCap(canonical.grade, skillEvidenceTier(canonical.name, action, evidence, resolution, deliberateTraining));
