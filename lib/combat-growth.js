...
const QUESTION_RE = /[?？]|(?:어떻게|왜|얼마나|무엇|뭐가|가능한가|할까|해도\s*될까|하면|한다면|할\s*경우|\b(?:how|why|what|can|could|would|should|if)\b)/i;
const COMMITTED_ACTION_RE = /(?:한다|시작한다|계속한다|이어간다|시도한다|해본다|맞선다|싸운다|공격한다|방어한다|회피한다|연습한다|훈련한다|수련한다|연구한다|분석한다|기도한다|명상한다|\bI\s+(?:train|practice|study|fight|attack|defend|dodge|cast|will)\b)/i;
const OBSERVATION_RE = /(?:지켜보|관찰|구경|목격|보기만|본다|보았다|\b(?:watch|observe|spectat)\w*\b)/i;
...
const THIRD_PARTY_STIMULUS_RE = /(?:^|\s)(?!(?:내가|나는|제가|저는|PC(?:가|는)?|플레이어(?:가|는)?|주인공(?:이|은)?|Aaa(?:가|는)?)(?:\s|$))[가-힣A-Za-z0-9_-]{2,32}(?:만|이|가|은|는)\s*[^.!?\n,;]{0,72}(?:훈련|연습|수련|단련|교정|통찰|깨달|적응|한계|압박|\b(?:train|practice|insight|adapt|pressure)\w*\b)/i;
const ENGLISH_THIRD_PARTY_STIMULUS_RE = /\b(?!(?:I|We|we|You|you)\b)(?:[A-Z][A-Za-z0-9_-]{1,31}|He|She|They|he|she|they|(?:The|the)\s+(?:NPC|npc|student|teacher|instructor|opponent|enemy))\s+[^.!?\n,;]{0,80}\b(?:trains?|trained|training|practices?|practiced|practicing|drills?|drilled|drilling|learns?|learned|learning|corrects?|corrected|correction|adapts?|adapted|adaptation|insight|breakthrough|pressure)\b/;
const NEGATED_STIMULUS_RE = /(?:훈련|연습|수련|단련|반복|교정|지도|분석|통찰|깨달|적응|응용|돌파|한계)[^.!?\n]{0,48}(?:하지\s*않|하지\s*못|되지\s*않|안\s*되|못했|못하|얻지\s*못|깨닫지\s*못|파악하지\s*못|없었|일어나지\s*않|실패했)|\b(?:(?:no|not|without)\s+(?:training|practice|insight|adaptation|breakthrough)|fail(?:ed|s|ing)?\s+to\s+(?:learn|adapt|understand|correct))\b/gi;

const STAT_RELEVANCE = Object.freeze({
  신체: /(?:신체|육체|근력|체력|지구력|반응|민첩|균형|자세|호흡|대검술?|검술|검기|(?:검|창|도끼|활)(?:을|를|으로|로)|격투|회피|방어|달리|physical|strength|stamina|reflex|agility)/i,
...
function playerOwnsGrowthAction(action = '', pcName = '') {
  ...
  for (const rawSegment of text.split(/[.!?\n,;]+/)) {
    ...
    if (OBSERVATION_RE.test(segment)) {
      ...
      if (tail && PLAYER_GROWTH_ACTION_RE.test(tail) && !hasThirdPartyAction(tail)) return true;
    }
    const playerNamed = pcNamedInSegment(segment, pcName) || SELF_SUBJECT_RE.test(segment);
    if (THIRD_PARTY_CAUSATIVE_RE.test(segment)) continue;
    if (hasThirdPartyAction(segment) && !playerNamed) continue;
    if (OBSERVATION_RE.test(segment) && !playerNamed) continue;
    return true;
  }
  return false;
}
...
function pcNamedInSegment(segment, pcName) {
  ...
  return Boolean(name) && new RegExp(`${name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}(?:이|가|은|는|도|에게|의)?`).test(segment);
}
...
function sceneEvidence(scene = [], pcName = '') {
  ...
  for (const row of (Array.isArray(scene) ? scene : []).slice(0, 24)) {
    ...
    for (const rawSegment of cleanText(row?.text, 900).split(/[.!?\n,;]+/)) {
      ...
      const playerSubject = playerNamed || (!dialogue && SELF_SUBJECT_RE.test(segment));
      const playerAddressed = playerNamed || /(?:너(?:는|가|의|에게|도|를)?|네(?:가|게|것|자세|동작|검|마나|오러)|당신(?:은|이|의|에게|도|을|를)?|PC(?:가|는|의|에게|도|를)?|플레이어(?:가|는|의|에게|도|를)?|주인공(?:이|은|의|에게|도|을|를)?)/i.test(segment);
      if (dialogue && !playerAddressed) continue;
      if (hasThirdPartyStimulus(segment) && !playerSubject && !playerAddressed) continue;
      let segmentTier = 0;
...
function skillMentionedExactly(text, name, skills) {
  const haystack = identity(text);
  const target = identity(name);
  if (!haystack || !target) return false;
  const coveringSkills = [...skills.keys()].filter((key) => key !== target && key.length > target.length && key.includes(target));
  for (let targetIndex = haystack.indexOf(target); targetIndex >= 0; targetIndex = haystack.indexOf(target, targetIndex + 1)) {
    let covered = false;
    for (const covering of coveringSkills) {
      for (let coveringIndex = haystack.indexOf(covering); coveringIndex >= 0; coveringIndex = haystack.indexOf(covering, coveringIndex + 1)) {
        if (targetIndex >= coveringIndex && targetIndex + target.length <= coveringIndex + covering.length) {
          covered = true;
          break;
        }
      }
...
function skillIsRelevant(name, action, resolution, deliberateTraining, combatAction, skills) {
  const key = identity(name);
  if (resolution.skill.has(key)) return true;
  if (combatAction) return false;
  const directlyNamed = skillMentionedExactly(action, name, skills);
...
function skillEvidenceTier(name, action, evidence, resolution, deliberateTraining, skills) {
  const key = identity(name);
  const direct = (Array.isArray(evidence?.segments) ? evidence.segments : []).filter((row) => skillMentionedExactly(row?.text, name, skills));
  if (direct.length) return Math.max(...direct.map((row) => Number(row?.tier || 0)));
  if (resolution.skill.size === 1 && resolution.skill.has(key)) return Number(evidence?.tier || 0);
  if (deliberateTraining && skillMentionedExactly(action, name, skills)) return Number(evidence?.tier || 0);
  return Math.min(1, Number(evidence?.tier || 0));
}

function statEvidenceTier(stat, evidence, resolution) {
  const direct = (Array.isArray(evidence?.segments) ? evidence.segments : []).filter((row) => STAT_RELEVANCE[stat]?.test(row?.text || ''));
  if (direct.length) return Math.max(...direct.map((row) => Number(row?.tier || 0)));
  if (resolution.stat.size === 1 && resolution.stat.has(identity(stat))) return Number(evidence?.tier || 0);
  return 0;
}
...
export function deriveCombatGrowthState({
  ...
  if (allowProgress && actionOwned && evidenceTier > 0) {
    for (const raw of proposedStats) {
      ...
      const cap = gradeAwardCap(grade, statEvidenceTier(stat, evidence, resolution));
    ...
    for (const raw of proposedSkills) {
      ...
      const reason = cleanText(raw?.reason, 240);
      const amount = positiveProgressAmount(raw?.amount);
      const cap = gradeAwardCap(canonical.grade, skillEvidenceTier(canonical.name, action, evidence, resolution, deliberateTraining, skills));
      if (!reason || !amount || !cap || !skillIsRelevant(canonical.name, action, resolution, deliberateTraining, combatAction, skills)) continue;
