...
function splitClauses(text = '') {
  ...
  anchors.forEach((anchor, index) => {
    const prelude = source.slice(cursor, anchor.index), sentenceBreaks = [...prelude.matchAll(/[.!?。！？]\s*/gu)], lastBreak = sentenceBreaks.at(-1);
...
function clockExpression(text = '') {
  ...
  for (const match of source.matchAll(/(?:(?<period>오전|오후|저녁|밤|새벽)\s*)?(?<hour>\d{1,2}|한|두|세|네|다섯|여섯|일곱|여덟|아홉|열|열한|열두)\s*시(?!간)(?:\s*(?<minute>\d{1,2}|한|두|세|네|다섯|여섯|일곱|여덟|아홉|열|열한|열두)\s*분)?|(?<colon>\d{1,2}:\d{2})|(?<named>정오|자정)/gu)) {
    ...
    if (minutes != null) {
      const index = match.index ?? 0, end = index + match[0].length, trailing = source.slice(end);
      const complete = !/^\s*[A-Za-z가-힣0-9]/u.test(trailing) || /^\s*(?:에|부터|까지|후|뒤|이후|전|이전|마다|씩|동안|간)(?:\s|$)/u.test(trailing);
...
export function parseTimePlan(action = '', context = {}) {
  ...
  clauses.forEach((clause, index) => {
    ...
    const actionType = clause.anchor?.type || 'unknown', destination = cleanDestination(clause.anchor?.destination), hypothetical = isHypothetical(clause.source), negated = isNegatedAction(clause.source, localAnchor), thirdParty = actor.kind === 'npc', committed = actionType !== 'unknown' && actor.kind === 'pc' && !quoted && !hypothetical && !negated && !completed && !unparsedSuffix;
...
export function deriveStructuredTimingCandidate(plan = {}) {
  ...
  const terminal = clauses.at(-1), eligible = Boolean(plan.mode === 'shadow' && diagnostics.length === 0 && terminal?.committed && !terminal?.third_party && terminal?.action_type !== 'unknown');
  ...
  const relativeDateStart = eligible && terminal.start?.absolute_date == null && terminal.start?.clock_parse_complete !== false && terminal.start?.clock_context_available !== false && !/^(?:밤|새벽)$/u.test(String(terminal.start?.clock_period || '')) && Number.isFinite(terminal.start?.date_offset_days)
  ...
  return {
    ...
    elapsed_relative_date_start: Boolean(Number.isFinite(relativeDateStart) && relativeDateStart < 0),
