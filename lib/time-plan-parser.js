...
function hasUnresolvedCondition(text = '') {
  return /(?:^|[\s,.;!?。！？])(?:만약\s*)?(?:[A-Za-z가-힣0-9_:]{1,32}(?:으면|면)|[A-Za-z가-힣0-9_:]{1,32}\s+때)(?=$|[\s,.;!?。！？])/u.test(String(text || ''));
}
...
function parseStart(text = '', context = {}, sequenceRelation = 'root', deadline = null, actionAnchorIndex = -1) {
  ...
  if (!effectiveDate && !effectiveClock && !relativeOffset) {
    return sequenceRelation === 'root'
      ? { relation: 'immediate', raw: null, date_offset_days: null, date_owned_by_action: date ? dateOwnedByAction : null, clock_minutes: null, offset_minutes: 0, elapsed: false }
      : { relation: sequenceRelation, raw: null, date_offset_days: null, date_owned_by_action: date ? dateOwnedByAction : null, clock_minutes: null, offset_minutes: null, elapsed: false };
  }
  const dateOffset = effectiveDate?.date_offset_days ?? null;
  let offset = dateOffset == null ? relativeOffset?.minutes ?? null : dateOffset * 1440;
  ...
  return {
    relation: elapsed ? 'elapsed' : effectiveDate && effectiveClock ? 'date-and-clock' : effectiveDate ? effectiveDate.kind : relativeOffset ? 'relative-offset' : 'clock',
    raw: [effectiveDate?.raw, relativeOffset?.raw, effectiveClock?.raw].filter(Boolean).join(' ') || null,
    ...
    relative_offset_minutes: relativeOffset?.minutes ?? null,
...
export function parseTimePlan(action = '', context = {}) {
  ...
  clauses.forEach((clause, index) => {
    ...
    const actionType = clause.anchor?.type || 'unknown', destination = cleanDestination(clause.anchor?.destination), hypothetical = isHypothetical(clause.source), conditional = hasUnresolvedCondition(actorScope), negated = isNegatedAction(clause.source, localAnchor), incompleteConnector = index === clauses.length - 1 && isConnectorAnchor(clause.anchor?.text), thirdParty = actor.kind === 'npc', committed = actionType !== 'unknown' && actor.kind === 'pc' && !quoted && !hypothetical && !conditional && !negated && !completed && !unparsedSuffix && !incompleteConnector;
...
export function deriveStructuredTimingCandidate(plan = {}) {
  ...
  const terminal = clauses.at(-1), eligible = Boolean(plan.mode === 'shadow' && diagnostics.length === 0 && terminal?.committed && !terminal?.third_party && terminal?.action_type !== 'unknown' && terminal?.explicit_deadline == null && terminal?.duration_ownership_complete !== false);
  ...
  return {
    ...
    terminal_action_subtype: eligible ? terminal.action_type : null,
