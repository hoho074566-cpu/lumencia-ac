...
function parseDuration(text = '', actionType = 'unknown', destination = '', location = '') {
  ...
  if (selected) {
    ...
    if (/(?:최소|적어도)\s*$/u.test(prefix) || /^\s*이상/u.test(suffix)) {
      ...
      return { min_minutes: selected.minutes, max_minutes: Number.isFinite(defaultMaximum) ? Math.max(selected.minutes, defaultMaximum) : selected.minutes, explicit: true, source: selected.raw };
    }
