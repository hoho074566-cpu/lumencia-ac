...
export function parseTimePlan(action = '', context = {}) {
  ...
  clauses.forEach((clause, index) => {
    ...
    const actionType = clause.anchor?.type || 'unknown', destination = cleanDestination(clause.anchor?.destination), hypothetical = isHypothetical(clause.source), negated = isNegatedAction(clause.source, localAnchor), thirdParty = actor.kind === 'npc', committed = actionType !== 'unknown' && actor.kind === 'pc' && !quoted && !hypothetical && !negated;
...
export function deriveStructuredTimingCandidate(plan = {}) {
  ...
  const relativeDateStart = eligible && terminal.start?.absolute_date == null && terminal.start?.clock_parse_complete !== false && terminal.start?.clock_context_available !== false && !/^(?:밤|새벽)$/u.test(String(terminal.start?.clock_period || '')) && Number.isFinite(terminal.start?.date_offset_days)
    ...
    : null;
  const prefix = clauses.slice(0, -1), regionalTravelPrefix = eligible && prefix.length === 1 && prefix[0]?.committed && !prefix[0]?.third_party && prefix[0]?.action_type === 'travel' && prefix[0]?.duration?.profile === 'regional'
