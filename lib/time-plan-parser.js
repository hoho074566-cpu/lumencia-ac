...
function explicitActor(clause = '', actorName = '') {
  ...
  const additiveSubjects = [...text.matchAll(/(?:^|[\s,"'“‘])(?<name>[A-Za-z가-힣0-9_:]{1,32})도(?=\s|[,.;!?。！？…]|$)/gu)]
    .filter((match) => {
      ...
      return stem && !SUBJECT_STOP_WORDS.has(stem) && !ADDITIVE_ADVERBIAL_STEMS.has(stem);
    });
...
function hasUnparsedPrefixAction(text = '') {
  ...
  return /(?:^|[\s,.;!?。！？])[A-Za-z가-힣0-9_:]{1,48}(?:하고|하면서|하며|면서|으며|가서|와서|해서|되어서|돼서)(?=$|[\s,.;!?。！？])/u.test(source)
...
function parseDuration(text = '', actionType = 'unknown', destination = '', location = '') {
  ...
  if (selected) {
    ...
    if (/(?:최소|적어도)\s*$/u.test(prefix) || /^\s*이상/u.test(suffix)) {
...
export function deriveStructuredDecisionPlan(plan = {}) {
  ...
  for (let position = 0; position < clauses.length; position += 1) {
    ...
    timeline.push({ index: Number(clause.index || position + 1), action_type: String(clause.action_type), action_subtype: String(clause.action_subtype || clause.action_type), destination: clause.destination || null, start_min_minutes: startMin, start_max_minutes: startMax, complete_min_minutes: completeMin, complete_max_minutes: completeMax });
