...
export function deriveStructuredTimingCandidate(plan = {}) {
  ...
  const terminal = clauses.at(-1), eligible = Boolean(plan.mode === 'shadow' && diagnostics.length === 0 && terminal?.committed && !terminal?.third_party && terminal?.action_type !== 'unknown' && terminal?.explicit_deadline == null && terminal?.start?.ownership_complete !== false && terminal?.duration_ownership_complete !== false);
