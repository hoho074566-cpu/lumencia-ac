...
const EXTERIOR_LOCATION_RE = /(?:외부|바깥|정문\s*앞|출입구\s*밖|현관\s*밖|건물\s*앞|기숙사\s*앞|광장|거리|도로|뜰|마당|\boutside\b|\bexterior\b|\boutdoors?\b)/iu;
const INTERMEDIATE_LOCATION_RE = /(?:복도|계단|현관(?:\s*안)?$|문\s*(?:앞|안)|로비|\bcorridor\b|\bhallway\b|\bstair(?:s|well)?\b|\blobby\b)/iu;
...
function compactLocation(value) { return String(value||'').toLowerCase().replace(/[\s\p{P}\p{S}]+/gu,''); }
function destinationReached(destination,afterLocation) {
  const expected=clampText(destination,120),actual=clampText(afterLocation,180);
  if(!expected||!actual)return false;
  if(expected==='current-building-exterior')return EXTERIOR_LOCATION_RE.test(actual)&&!INTERMEDIATE_LOCATION_RE.test(actual);
  const compactExpected=compactLocation(expected),compactActual=compactLocation(actual);
  if(compactExpected.length>=2&&compactActual.includes(compactExpected))return true;
  const tokens=expected.split(/[\s/·,()_-]+/).map((token)=>compactLocation(token).replace(/(?:으로|에게|에서|까지|부터|안으로|내부)$/u,'')).filter((token)=>token.length>=2);
  return tokens.length>1&&tokens.every((token)=>compactActual.includes(token));
}
...
export function normalizeSceneExitCondition(value={}) {
  ...
  const destination=clampText(row.destination,120);
  if(kind==='semantic-destination'&&destination)normalized.destination=destination;
  const instanceId=eventId(row);
  if(kind==='event-step'&&instanceId)normalized.event_instance_id=instanceId;
  return normalized;
}

function makeCondition({kind,target,source,status='open',turnNumber=0,purposeTurn=0,eventInstanceId='',destination=''}) {
  return normalizeSceneExitCondition({kind,target,source,status,established_turn:turnNumber,purpose_established_turn:purposeTurn,event_instance_id:eventInstanceId,destination});
}
...
function conditionForIntent(intent,turnNumber,purposeTurn=0) {
  ...
  if(intent.kind==='exit-exterior')return makeCondition({...common,kind:'semantic-destination',target:'실제 방해가 없다면 사소한 실내 동선을 압축해 현재 건물 외부에 도착한 때',destination:'current-building-exterior'});
  if(intent.kind==='travel')return makeCondition({...common,kind:'semantic-destination',target:`실제 방해가 없다면 선언한 목적지(${clampText(intent.semanticTarget||'목적지',100)})에 도착한 때`,destination:intent.semanticTarget||''});
...
export function deriveSceneExitCondition({ action = '', saveState = {}, purpose = null, turnNumber = 0 } = {}) {
  ...
  if(rawAction&&!isAutoAction(rawAction))return conditionForIntent(classifySceneIntent(rawAction,{location:saveState?.world?.location||'',currentTime:saveState?.world?.time||'',currentDate:saveState?.world?.date||'',currentWeekday:saveState?.world?.weekday||'',actorName:saveState?.pc?.name||''}),turnNumber,currentPurpose?.established_turn||0);
...
export function evaluateSceneExitCondition(condition,{ turn = {}, sceneDelta = {}, previousRuntime = {}, eventProgress = null, incompleteBoundary = false } = {}) {
  ...
  if(incompleteBoundary)return normalizeSceneExitCondition({...current,status:'open'});
  ...
  else if(current.kind==='semantic-destination')reached=Boolean(flags.locationChanged)&&destinationReached(current.destination,sceneDelta?.afterLocation);
...
export function buildSceneExitDirective({ action = '', saveState = {} } = {}) {
  ...
  else lines.push('- 실제 방해나 중요한 판단점 전에는 문·복도·평범한 이동·정적 재묘사에서 조기 STOP하지 않는다.','- EXIT_TARGET 뒤에는 반복·후속 schedule을 시작하지 않는다. 비용 있는 PC 답이 실제 필요한 HARD_DECISION만 choices 3개; 종료용 NPC 질문은 금지하며 아니면 choices=[]로 물리적 beat에서 닫는다.');
