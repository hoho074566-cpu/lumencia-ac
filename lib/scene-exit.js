...
export function deriveSceneExitCondition({ action = '', saveState = {}, purpose = null, turnNumber = 0 } = {}) {
  ...
  if(rawAction&&!isAutoAction(rawAction))return conditionForIntent(classifySceneIntent(rawAction,{location:saveState?.world?.location||'',currentTime:saveState?.world?.time||'',actorName:saveState?.pc?.name||''}),turnNumber,currentPurpose?.established_turn||0);
