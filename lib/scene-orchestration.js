...
function reachableSchedule(saveState, intent, action = '', registry = {}) {
  // Due/overdue rows are current world context, not a new 0-minute hard stop.
  // Only a strictly future occurrence may interrupt the action being planned.
  const minutes = nextScheduleBoundaryMinutes(saveState, { futureOnly: true, action, intent, registry });
  ...
  const interruptible=Boolean(intent?.compression||Number(intent?.boundaryLookaheadMinutes||0)>0);
  return { reached: Boolean(interruptible && limit > 0 && minutes <= limit), minutes };
}
...
function directorDriver(telemetry = {}) {
  ...
  if (result === 'NPC_EVENT' || result === 'WORLD_RESULT_SURFACE') return 'director-event';
  return 'none';
}

export function sceneOrchestrationSuppressesDirectorResult(value = {}, telemetry = {}) {
  const row = normalizeSceneOrchestrationPlan(value);
  const selected = directorDriver(telemetry);
  return selected !== 'none' && row.suppressed.includes(selected);
}
...
export function deriveSceneOrchestrationPlan({ action = '', saveState = {}, mode = 'game', directorTelemetry = null, registry = {} } = {}) {
  const normalizedMode = ['game', 'auto', 'continue', 'meta'].includes(mode) ? mode : 'game';
  const intent = classifySceneIntent(action, { location: saveState?.world?.location || '', currentTime: saveState?.world?.time || '', currentDate: saveState?.world?.date || '', currentWeekday: saveState?.world?.weekday || '', actorName: saveState?.pc?.name || '', resumeTimedAction: saveState?.sceneRuntime?.timed_action });
  ...
  const schedule = reachableSchedule(saveState, intent, action, registry);
  ...
  if (directAction) {
    ...
    const secondary = selectedSystem === 'none' ? 'none' : selectedSystem;
    ...
    return normalizeSceneOrchestrationPlan({
      ...
      order: interrupting ? 'action-until-interruption' : 'user-action-first', max_drivers: secondary === 'none' ? 1 : 2,
...
export function buildSceneOrchestrationDirective({ plan = null, action = '', saveState = {}, mode = 'game', directorTelemetry = null, registry = {} } = {}) {
  const row = normalizeSceneOrchestrationPlan(plan || deriveSceneOrchestrationPlan({ action, saveState, mode, directorTelemetry, registry }));
  return [
    ...
    'EFFECT_ONLY=relationship|faction|growth|offscreen|novelty',
    '- 이 블록은 Purpose·Exit·Hook·Event Director 사이의 최종 우선순위 조정자다. 충돌하면 PRIMARY/SECONDARY/ORDER를 따른다.',
    '- action-until-interruption은 TRIGGER_MINUTES까지만 진행한다. 그 외에는 PRIMARY의 현재 Scene을 깊게 완결하고 명시된 SECONDARY만 인과적으로 한 번 잇는다.',
    '- MAX_DRIVERS는 Scene 안의 system 수지 schedule 단계 수가 아니다. NPC·환경·군중 반응은 PRIMARY 안에서 전개한다.',
    '- SUPPRESSED에 든 하위 지시의 SELECTED NPC·후보·사건을 PRIMARY/SECONDARY에 연결해 재해석하거나 등장시키지 않는다.',
    '- 무관한 세 번째 사건·카메오·목표를 병렬로 시작하지 않는다.',
    '- EFFECT_ONLY 시스템은 보이는 결과의 근거가 있을 때만 상태 효과를 기록하며, 그 자체가 새 장면 비트나 별도 사건을 요구하지 않는다.',
    '- 뒤 schedule/event를 소비하지 않는다. 비용 있는 PC 답이 실제 필요한 HARD_DECISION만 yield하며 종료용 NPC 질문은 금지한다.',
  ].join('\n');
}
...
export function sceneOrchestrationActionFrame(value = {}) {
  ...
  return `TURN_PLAN=${chain}; ORDER=${row.order}; MAX_DRIVERS=${row.max_drivers}; BLOCK=${row.suppressed.join('|') || '-'}; EFFECT_ONLY`;
}
