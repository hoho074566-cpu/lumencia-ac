...
function reachableSchedule(saveState, intent, action = '') {
  ...
  const minutes = nextScheduleBoundaryMinutes(saveState, { futureOnly: true, action, intent });
...
export function deriveSceneOrchestrationPlan({ action = '', saveState = {}, mode = 'game', directorTelemetry = null } = {}) {
  ...
  const schedule = reachableSchedule(saveState, intent, action);
