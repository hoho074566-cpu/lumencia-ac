...
const FULL_PORTRAIT_KEYS = Object.freeze(['beelzebub','bellian','chloe','delpirem','isabel','laris','lena','levian','lillia','lily_lumina','mirabelle','nemesis','sera','serena','veradin']);
const DEFAULT_ONLY_KEYS = Object.freeze(['aria','artemis','elise','lucia','sia']);
...
function validateRequiredImages(assets) {
  ...
  for (const key of PHYSICAL_KEYS) {
    ...
    } else if (DEFAULT_ONLY_KEYS.includes(key)) {
      if (!character.default) missing.push(`${key}: default`);
      if (Object.keys(character.expressions || {}).length) missing.push(`${key}: unexpected non-default portrait`);
...
export function runImageContractRegression(assets = ASSETS) {
  return [
    result('image-count','21명 V2 asset manifest',()=>assets.liveFolders?.length===21&&PHYSICAL_KEYS.every(key=>assets.liveFolders.includes(key)),`${assets.liveFolders?.length ?? 0}명`),
    ...
    result('urls','물리 URL contract',()=>{const urls=manifestUrls(assets);return urls.length===233&&new Set(urls).size===urls.length&&urls.every(url=>url.startsWith(`${assets.base}/`));},`${manifestUrls(assets).length}개 manifest URL (현재 contract: 233)`),
