...
export function runImageContractRegression(assets = ASSETS) {
  return [
    ...
    result('urls','물리 URL contract',()=>{const urls=manifestUrls(assets);return urls.length===448&&new Set(urls).size===urls.length&&urls.every(url=>url.startsWith(`${assets.base}/`));},`${manifestUrls(assets).length}개 manifest URL (현재 contract: 448)`),
