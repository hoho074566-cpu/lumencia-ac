...
const PHYSICAL_KEYS = Object.freeze(['anastasia','aria','artemis','beelzebub','bellian','chloe','delpirem','elise','isabel','laris','lena','levian','lillia','lily_lumina','lucia','mirabelle','nemesis','sera','serena','sia','veradin']);
...
export function runImageContractRegression() {
  const urls = PHYSICAL_KEYS.flatMap(key => { const c=ASSETS.characters[key]; return [c.default,...Object.values(c.expressions||{}),c.fullbody].filter(Boolean); });
  return [
    result('image-count','21명 V2 asset manifest',()=>ASSETS.liveFolders.length===21&&PHYSICAL_KEYS.every(key=>ASSETS.liveFolders.includes(key)),`${ASSETS.liveFolders.length}명`),
    ...
    result('partials','부분 asset fallback',()=>{const ana=ASSETS.characters.anastasia;return !ana.default&&Object.keys(ana.expressions).length===12&&ana.fullbody&&['aria','artemis','elise','lucia','sia'].every(key=>Object.keys(ASSETS.characters[key].expressions).length===0);},'manifest-declared default/fullbody만 사용'),
    result('unknown','알 수 없는 표정',()=>!resolveManifestPortrait('nemesis','not-a-state').url.endsWith('/not-a-state.webp'),'임의 URL 합성 안 함'),
    result('urls','물리 URL contract',()=>new Set(urls).size===urls.length&&urls.every(url=>url.startsWith(`${ASSETS.base}/`)),`${urls.length}개 manifest URL (현재 contract)`),
