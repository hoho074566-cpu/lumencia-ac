...
export function deriveAwakeningTalentState({
  ...
  if (allowProgress) {
    for (const raw of (Array.isArray(awakeningChanges) ? awakeningChanges : []).slice(0, 4)) {
      ...
      const accepted = { kind, name, amount, milestone:countsMilestone, description:canonicalDescription, limitation:canonicalLimitation, reason };
  ...
  if (allowProgress) {
    for (const raw of (Array.isArray(talentEvolutionChanges) ? talentEvolutionChanges : []).slice(0, 4)) {
      ...
      acceptedTalentEvolution.push({ talent, amount:1, cause, reason });
