...
export function buildSceneNoveltyDirective({ action = '', saveState = {}, recentTurns = [] } = {}) {
  ...
  const intent=classifySceneIntent(rawAction,{location:saveState?.world?.location||'',actorName:saveState?.pc?.name||''}),decisionSensitive=intent.kind==='decision-sensitive';
