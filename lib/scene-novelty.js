...
const KOREAN_RECAP_REQUEST_RE = /(?:(?:다시|재차|한\s*번\s*더).{0,32}(?:설명|요약|정리|재설명|알려|읽어|보여|말해|되짚어)(?:\s*해)?\s*(?:줘|주세요|줄래(?:요)?|달라|주겠어|봐(?:요)?)|(?:요약|정리|재설명|되짚어|복습)(?:\s*해)?\s*(?:줘|주세요|줄래(?:요)?|달라|주겠어|봐(?:요)?))(?:[.!?…]|\s)*$/u;
const ENGLISH_RECAP_REQUEST_RE = /(?:\b(?:please\s+)?(?:recap|summari[sz]e)(?:\s+(?:this|that|it|what\s+happened|the\s+(?:scene|situation|story|last\s+response)))?(?:\s+(?:again|please))?|\b(?:please\s+)?(?:repeat|explain)\s+(?:that|it|what\s+you\s+(?:just\s+)?said|the\s+(?:last|previous)\s+(?:part|scene|response))(?:\s+again)?|\b(?:please\s+)?explain\s+again|\b(?:please\s+)?repeat(?:\s+please)?)[.!?…\s]*$/iu;
...
function requestedRecap(action='') {
  const text=String(action||'').trim();
  return KOREAN_RECAP_REQUEST_RE.test(text)||ENGLISH_RECAP_REQUEST_RE.test(text);
}
...
export function buildSceneNoveltyDirective({ action = '', saveState = {}, recentTurns = [] } = {}) {
  ...
  const intent=classifySceneIntent(rawAction,{location:saveState?.world?.location||'',currentTime:saveState?.world?.time||'',currentDate:saveState?.world?.date||'',currentWeekday:saveState?.world?.weekday||'',actorName:saveState?.pc?.name||''}),decisionSensitive=intent.kind==='decision-sensitive';
