...
export function buildSceneNoveltyDirective({ action = '', saveState = {}, recentTurns = [] } = {}) {
  ...
  const intent=classifySceneIntent(rawAction,{location:saveState?.world?.location||'',currentTime:saveState?.world?.time||'',currentDate:saveState?.world?.date||'',actorName:saveState?.pc?.name||''}),decisionSensitive=intent.kind==='decision-sensitive';
