...
export function buildSceneNoveltyDirective({ action = '', saveState = {}, recentTurns = [] } = {}) {
  ...
  const intent=classifySceneIntent(rawAction,{location:saveState?.world?.location||'',currentTime:saveState?.world?.time||'',actorName:saveState?.pc?.name||''}),decisionSensitive=intent.kind==='decision-sensitive';
