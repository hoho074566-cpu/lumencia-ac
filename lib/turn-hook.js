...
export function filterTurnHookChoices(action='',turn={}) {
  ...
  const contextualDecision=DECISION_CONTEXT_RE.test(sceneContext),combat=COMBAT_RE.test(visible);
  if(failed||eventBoundary||directNpcBoundary||contextualDecision||combat)return choices;
