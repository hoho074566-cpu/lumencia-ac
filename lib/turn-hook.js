...
export function filterTurnHookChoices(action='',turn={}) {
  ...
  if(failed||directNpcBoundary||contextualDecision||groundedTravelAlternatives||combat)return choices;
...
export function buildTurnHookDirective({ action = '', saveState = {} } = {}) {
  ...
  const lines=[`HOOK_MODE=${currentActionFirst?'current-action-first':'world-continuity'}`,'- soft hook·남은 시간·다음 일정은 질문/choices가 아니며 종료용 NPC 질문은 금지한다.'];
