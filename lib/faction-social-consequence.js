...
function normalizeHistory(value) {
  return array(value).slice(-8).map((raw)=>{
    const row=object(raw),evidenceType=EVIDENCE_TYPE_SET.has(String(row.evidence_type))?String(row.evidence_type):'public_event';
    return {
      ...
      observer_npc_keys:uniqueNpcKeys(row.observer_npc_keys),reason:clampText(row.reason,300),
