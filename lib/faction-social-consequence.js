...
function evidenceIsSufficient(type,observerKeys) {
  return type==='public_event'||type==='official_record'||observerKeys.length>0;
}
function normalizeHistory(value) {
  return array(value).slice(-8).flatMap((raw)=>{
    const row=object(raw),evidenceType=String(row.evidence_type||''),observerKeys=uniqueNpcKeys(row.observer_npc_keys);
    if(!EVIDENCE_TYPE_SET.has(evidenceType)||!evidenceIsSufficient(evidenceType,observerKeys))return [];
    return {
      ...
      observer_npc_keys:observerKeys,reason:clampText(row.reason,300),
      source_event:clampText(row.source_event,120)||null,
    };
  }).filter((row)=>row.reason);
}

export function normalizeFactionSocial(value={}) {
  ...
  for(const [factionKey,raw] of Object.entries(source)){
    ...
    rows.push([factionKey,{
      ...
      reason:clampText(row.reason,300),updated_turn:boundedTurn(row.updated_turn),history:normalizeHistory(row.history),
    }]);
  }
...
export function factionReputationChangeIsReal(saveState={},raw={}) {
  ...
  const observers=uniqueNpcKeys(row.observer_npc_keys);
  if(!evidenceIsSufficient(evidenceType,observers))return false;
...
export function deriveFactionSocialState({ previous = {}, changes = [], turnNumber = 0, sourceEvent = '', registeredNpcKeys = null } = {}) {
  const normalized=normalizeFactionSocial(previous),reputations={...normalized.reputations},safeTurn=boundedTurn(turnNumber);
  for(const raw of array(changes).slice(0,4)){
    ...
    const observers=uniqueNpcKeys(row.observer_npc_keys,registeredNpcKeys);
    if(!evidenceIsSufficient(evidenceType,observers))continue;
    ...
    const historyRow={turn:safeTurn,reputation_delta:delta,stance,evidence_type:evidenceType,observer_npc_keys:observers,reason,source_event:clampText(sourceEvent,120)||null};
  ...
  return normalizeFactionSocial({reputations});
}

export function compactFactionSocialForContext(value={}, { text = '', keywords = [], maxFactions = 3, historyLimit = 2 } = {}) {
  const normalized=normalizeFactionSocial(value),haystack=`${String(text||'')} ${array(keywords).join(' ')}`.toLowerCase();
  const ranked=Object.entries(normalized.reputations).map(([key,row])=>{
    const faction=FACTION_REGISTRY[key],matched=[key,faction.name,...faction.aliases].some((alias)=>haystack.includes(String(alias).toLowerCase()));
    return {key,row,score:(matched?1_000_000:0)+Number(row.updated_turn||0)*10+Math.abs(Number(row.reputation||0))};
