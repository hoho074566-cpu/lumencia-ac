...
export function compactFactionSocialForContext(value={}, { text = '', recentText = '', keywords = [], maxFactions = 3, historyLimit = 2, registeredNpcKeys = null } = {}) {
  const normalized=normalizeFactionSocial(value,{registeredNpcKeys}),directText=String(text||'').toLowerCase(),recentContext=String(recentText||'').toLowerCase(),keywordText=array(keywords).join(' ').toLowerCase();
  const ranked=Object.entries(normalized.reputations).map(([key,row])=>{
    ...
    const directMatch=aliases.some((alias)=>directText.includes(alias)),recentMatch=aliases.some((alias)=>recentContext.includes(alias)),keywordMatch=aliases.some((alias)=>keywordText.includes(alias));
    return {key,row,score:(directMatch?3_000_000:recentMatch?2_000_000:keywordMatch?1_000_000:0)+Number(row.updated_turn||0)*10+Math.abs(Number(row.reputation||0))};
