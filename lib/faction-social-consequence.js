...
export function compactFactionSocialForContext(value={}, { text = '', keywords = [], maxFactions = 3, historyLimit = 2, registeredNpcKeys = null } = {}) {
  const normalized=normalizeFactionSocial(value,{registeredNpcKeys}),directText=String(text||'').toLowerCase(),keywordText=array(keywords).join(' ').toLowerCase();
  const ranked=Object.entries(normalized.reputations).map(([key,row])=>{
    ...
    return {key,row,score:(directMatch?2_000_000:keywordMatch?1_000_000:0)+Number(row.updated_turn||0)*10+Math.abs(Number(row.reputation||0))};
...
export function compactFactionSocialTelemetry(value={},changes=[]) {
  const normalized=normalizeFactionSocial(value),factionKeys=Object.keys(normalized.reputations).slice(0,6);
  if(!factionKeys.length)return null;
  const changedFactionKeys=[...new Set(array(changes).map((row)=>String(object(row).faction_key||'').trim()).filter((key)=>FACTION_KEY_SET.has(key)&&factionKeys.includes(key)))].slice(0,4);
