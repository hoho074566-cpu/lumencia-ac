...
function normalizeAppliedInheritance(raw){
  if(!raw||typeof raw!=='object')return null;
  try{
    const quote=quoteInheritanceAllocation(raw.allocation),lifeId=cleanText(raw.lifeId,100);
...
export function normalizeCharacterCreation(value) {
  ...
  try {
    ...
    const background=normalizeFateBackgroundState(value.fateStart?.background,origin),inheritance=normalizeAppliedInheritance(value.fateStart?.inheritance);
