...
import { applyFateInheritance, inheritanceGenerationOptions, normalizeFateInheritanceReceipt } from './fate-inheritance.js';
...
function buildStructuredOrigin({gender,socialClass,department,seed,originLocks={},rerollIndex=0}){
  const seedTag=cleanText(`${cleanText(seed||randomSeed(),64)}|r:${clampInt(rerollIndex,0,3)}`,80),rng=seededRandom(seedTag),lockedRegion=cleanText(originLocks.regionKey,40),regionKey=Object.hasOwn(REGION_PROFILES,lockedRegion)?lockedRegion:pick(Object.keys(REGION_PROFILES),rng),region=REGION_PROFILES[regionKey];
  const occupations=socialClass==='commoner'?region.commonerOccupations:FALLEN_NOBLE_OCCUPATIONS,lockedOccupation=cleanText(originLocks.occupationKey,60),occupationRow=occupations.find((row)=>row[0]===lockedOccupation)||pick(occupations,rng),mentor=pick(region.mentors,rng),connection=pick(region.connections,rng);
...
export function generateFateStartingCharacter({gender,socialClass,department,seed,inheritance=null,rerollIndex=0}={}){
  const base=createFateCharacterCreation({gender,socialClass,department}),generation=inheritanceGenerationOptions(inheritance,{rerollIndex}),origin=buildStructuredOrigin({gender,socialClass,department,seed,originLocks:generation.originLocks,rerollIndex:generation.rerollIndex});
  const background=buildFateBackgroundState(origin);
  return applyFateInheritance({
...
export function normalizeCharacterCreation(value) {
  ...
  try {
    ...
    const inheritance=normalizeFateInheritanceReceipt(value.fateStart?.inheritance);
    const background=normalizeFateBackgroundState(value.fateStart?.background,origin,inheritance?.finalEvaluation);
    return{mode:'fate',fateStart:{...base.fateStart,version:2,origin,background,personalStory:normalizeFatePersonalStoryState(value.fateStart?.personalStory,origin,background),...(inheritance?{inheritance}:{})}};
  } catch {
