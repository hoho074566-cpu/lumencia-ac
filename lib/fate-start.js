...
function buildStructuredOrigin({gender,socialClass,department,seed,originLocks,originDraw=0}){
  const draw=clampInt(originDraw,0,2),baseSeed=cleanText(seed||randomSeed(),80),seedTag=cleanText(`${baseSeed}:draw:${draw}`,80),rng=seededRandom(`${baseSeed}:draw:${draw}`),locks=resolveOriginLocks({socialClass,...originLocks});
...
export function generateFateStartingCharacter({gender,socialClass,department,seed,inheritance,inheritancePurchase,allowedNpcKeys=[]}={}){
  ...
  let origin=buildStructuredOrigin({gender,socialClass,department,seed,originLocks:allocation.originLocks,originDraw:allocation.originDraw});
  ...
  const affinity=allocation.fateAffinity,initialRelationships=affinity.level>0?{[affinity.npcKey]:{affinity:affinity.level*2,trust:affinity.level,status:'낯선 운명의 울림',history:['Fate Affinity가 첫인상과 신뢰 형성의 출발점에만 약하게 작용함.']}}:{};
  return {
