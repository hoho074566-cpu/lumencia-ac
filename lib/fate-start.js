...
function buildStructuredOrigin({gender,socialClass,department,seed}){
  const seedTag=cleanText(seed||randomSeed(),80),rng=seededRandom(seedTag),regionKey=pick(Object.keys(REGION_PROFILES),rng),region=REGION_PROFILES[regionKey];
  const occupationRow=pick(socialClass==='commoner'?region.commonerOccupations:FALLEN_NOBLE_OCCUPATIONS,rng),mentor=pick(region.mentors,rng),connection=pick(region.connections,rng);
...
export function generateFateStartingCharacter({gender,socialClass,department,seed}={}){
  const base=createFateCharacterCreation({gender,socialClass,department}),origin=buildStructuredOrigin({gender,socialClass,department,seed});
  return {
    creation:{mode:'fate',fateStart:{...base.fateStart,version:2,origin,background:buildFateBackgroundState(origin)}},
...
export function normalizeCharacterCreation(value) {
  ...
  try {
    ...
    return consistent?{mode:'fate',fateStart:{...base.fateStart,version:2,origin,background:normalizeFateBackgroundState(value.fateStart?.background,origin)}}:base;
  } catch {
