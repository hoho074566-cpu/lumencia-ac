...
import { normalizeInheritanceAllocation, quoteInheritanceAllocation } from './fate-inheritance.js';
...
export function fateOriginLockOptions({socialClass,regionKey=''}={}){
  if(!Object.hasOwn(FATE_START_SOCIAL_CLASSES,socialClass))return{regions:[],occupations:[]};
  const regions=Object.entries(REGION_PROFILES).map(([key,value])=>({key,label:value.label}));
  if(socialClass==='fallen_noble')return{regions,occupations:FALLEN_NOBLE_OCCUPATIONS.map(([key,label])=>({key,label,regionKey:''}))};
  const regionEntries=Object.entries(REGION_PROFILES).filter(([key])=>!regionKey||key===regionKey);
  return{regions,occupations:regionEntries.flatMap(([key,value])=>value.commonerOccupations.map(([occupationKey,label])=>({key:occupationKey,label,regionKey:key})))};
}

function resolveOriginLocks({socialClass,regionKey='',occupationKey=''}={}){
  let resolvedRegion=cleanText(regionKey,32),resolvedOccupation=cleanText(occupationKey,64);
  if(resolvedRegion&&!Object.hasOwn(REGION_PROFILES,resolvedRegion))throw new Error('고정하려는 Origin 지역이 canonical 목록에 없음.');
  if(!resolvedOccupation)return{regionKey:resolvedRegion,occupationKey:''};
  if(socialClass==='fallen_noble'){
    if(!FALLEN_NOBLE_OCCUPATIONS.some(([key])=>key===resolvedOccupation))throw new Error('고정하려는 몰락귀족 직업이 canonical 목록에 없음.');
    return{regionKey:resolvedRegion,occupationKey:resolvedOccupation};
  }
  const matches=Object.entries(REGION_PROFILES).filter(([,value])=>value.commonerOccupations.some(([key])=>key===resolvedOccupation));
  if(matches.length!==1)throw new Error('고정하려는 평민 직업이 canonical 목록에 없음.');
  const requiredRegion=matches[0][0];
  if(resolvedRegion&&resolvedRegion!==requiredRegion)throw new Error('선택한 지역과 직업 Origin lock이 양립할 수 없음.');
  resolvedRegion=requiredRegion;
  return{regionKey:resolvedRegion,occupationKey:resolvedOccupation};
}
...
function normalizeAxes(value){
  ...
  if(!value||keys.some(key=>!Number.isInteger(value[key])||value[key]<1||value[key]>5))return null;
...
function buildStructuredOrigin({gender,socialClass,department,seed,originLocks,originRerolls=0}){
  const rerolls=clampInt(originRerolls,0,2),baseSeed=cleanText(seed||randomSeed(),80),seedTag=cleanText(`${baseSeed}:reroll:${rerolls}`,80),rng=seededRandom(`${baseSeed}:reroll:${rerolls}`),locks=resolveOriginLocks({socialClass,...originLocks});
  const regionKey=locks.regionKey||pick(Object.keys(REGION_PROFILES),rng),region=REGION_PROFILES[regionKey];
  const occupationPool=socialClass==='commoner'?region.commonerOccupations:FALLEN_NOBLE_OCCUPATIONS;
  const occupationRow=locks.occupationKey?occupationPool.find(([key])=>key===locks.occupationKey):pick(occupationPool,rng);
  if(!occupationRow)throw new Error('Origin lock과 양립 가능한 직업을 생성할 수 없음.');
...
export function evaluateFateStartingRealm({department,baseStats={},talents={}}={}){
  if(department==='마법과 1학년'){
    const score=Number(baseStats.mana||0)+Number(talents.magic||0);
    return score>=9?'3서클':score>=7?'2서클':'1서클';
  }
  const score=Number(baseStats.body||0)+Number(talents.martial||0);
  return score>=9?'익스퍼트 중급':score>=7?'익스퍼트 초급':'비기너';
}

function applyInheritanceToOrigin(origin,allocation){
  const next={...origin,baseStats:{...origin.baseStats},talents:{...origin.talents},backgroundFlags:[...origin.backgroundFlags]};
  for(const [key,boost] of Object.entries(allocation.stats))next.baseStats[key]=clampInt(Number(next.baseStats[key]||1)+boost,1,5);
  for(const [key,boost] of Object.entries(allocation.talents))next.talents[key]=clampInt(Number(next.talents[key]||1)+boost,1,5);
  if(Object.values(allocation.stats).some(Boolean))next.backgroundFlags.push('inheritance:stats');
  if(Object.values(allocation.talents).some(Boolean))next.backgroundFlags.push('inheritance:talents');
  next.backgroundFlags=uniqueStrings(next.backgroundFlags,12,100);
  next.realm=evaluateFateStartingRealm(next);
  next.originStory=renderFateOriginStory({...next,originStory:[]});
  return normalizeFateOrigin(next);
}

function originPcFields(origin,gender,department,allocation){
  const grade={1:'F',2:'E',3:'D',4:'C',5:'B'},stats=origin.baseStats,resourceLevel=allocation?.startingResources||0;
  const resourceGold=[0,25,60,120][resourceLevel]||0;
  const inventory=[[],['기초 여행 보급품'],['기초 여행 보급품','초급 회복 물약'],['기초 여행 보급품','초급 회복 물약','학과 실습 장비 교환권']][resourceLevel]||[];
  return {
    ...
    skills:Object.fromEntries(origin.skillsLearned.map(name=>[name,{grade:'E',hiddenXp:0}])),gold:resourceGold,inventory,
  };
}

function hasInheritanceAllocation(allocation){
  return Object.values(allocation.stats).some(Boolean)||Object.values(allocation.talents).some(Boolean)||allocation.startingResources>0||allocation.fateAffinity.level>0||allocation.originRerolls>0||Boolean(allocation.originLocks.regionKey||allocation.originLocks.occupationKey);
}

function normalizeAppliedInheritance(raw,{allowedNpcKeys=[]}={}){
  if(!raw||typeof raw!=='object')return null;
  try{
    const quote=quoteInheritanceAllocation(raw.allocation,{allowedNpcKeys}),lifeId=cleanText(raw.lifeId,100);
    if(!lifeId||!hasInheritanceAllocation(quote.allocation))return null;
    return{version:1,lifeId,cost:quote.cost,allocation:quote.allocation};
  }catch{return null;}
}
...
export function generateFateStartingCharacter({gender,socialClass,department,seed,inheritance,inheritancePurchase,allowedNpcKeys=[]}={}){
  const base=createFateCharacterCreation({gender,socialClass,department}),allocation=normalizeInheritanceAllocation(inheritance,{allowedNpcKeys,strict:true}),applied=hasInheritanceAllocation(allocation);
  let origin=buildStructuredOrigin({gender,socialClass,department,seed,originLocks:allocation.originLocks,originRerolls:allocation.originRerolls});
  if(applied)origin=applyInheritanceToOrigin(origin,allocation);
  const background=buildFateBackgroundState(origin);
  const lifeId=cleanText(inheritancePurchase?.lifeId,100);
  if(applied&&!lifeId)throw new Error('계승을 적용할 Next Life 식별자가 필요함.');
  const receipt=applied?{version:1,lifeId,cost:quoteInheritanceAllocation(allocation,{allowedNpcKeys}).cost,allocation}:null;
  const affinity=allocation.fateAffinity,initialRelationships=affinity.level>0?{[affinity.npcKey]:{affinity:affinity.level*2,trust:affinity.level,status:'낯선 운명의 울림',history:[{turn:0,reason:'Fate Affinity가 첫인상과 신뢰 형성의 출발점에만 약하게 작용함.'}]}}:{};
  return {
    creation:{mode:'fate',fateStart:{...base.fateStart,version:applied?3:2,origin,background,personalStory:buildFatePersonalStoryState(origin,background),...(receipt?{inheritance:receipt}:{})}},
    pc:originPcFields(origin,gender,department,allocation),
    initialRelationships,
  };
}

export function normalizeCharacterCreation(value,{allowedNpcKeys=[]}={}) {
  ...
  try {
    ...
    const background=normalizeFateBackgroundState(value.fateStart?.background,origin),inheritance=normalizeAppliedInheritance(value.fateStart?.inheritance,{allowedNpcKeys});
    return{mode:'fate',fateStart:{...base.fateStart,version:inheritance?3:2,origin,background,personalStory:normalizeFatePersonalStoryState(value.fateStart?.personalStory,origin,background),...(inheritance?{inheritance}:{})}};
  } catch {
