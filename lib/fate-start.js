...
export function materializeFateStartingCharacter(originValue){
  const normalized=normalizeFateOrigin(originValue);if(!normalized)throw new Error('운명 시작 Origin 구조가 올바르지 않음.');
  const origin={...normalized,realm:evaluatedRealm(normalized)};
