...
export const FATE_ORIGIN_OPTIONS = Object.freeze({
  regions:Object.freeze(Object.fromEntries(Object.entries(REGION_PROFILES).map(([key,row])=>[key,row.label]))),
  commonerOccupations:Object.freeze(Object.fromEntries(Object.entries(REGION_PROFILES).flatMap(([regionKey,row])=>row.commonerOccupations.map(([key,label])=>[key,Object.freeze({label,regionKey})])))),
  fallenNobleOccupations:Object.freeze(Object.fromEntries(FALLEN_NOBLE_OCCUPATIONS.map(([key,label])=>[key,Object.freeze({label,regionKey:null})]))),
});
...
export function validateFateOriginLocks({socialClass,region,occupation}={}){
  if(!Object.hasOwn(FATE_START_SOCIAL_CLASSES,socialClass))return false;
  const regionKey=cleanText(region,40),occupationKey=cleanText(occupation,60);
  if(regionKey&&!Object.hasOwn(REGION_PROFILES,regionKey))return false;
  const occupations=socialClass==='commoner'?FATE_ORIGIN_OPTIONS.commonerOccupations:FATE_ORIGIN_OPTIONS.fallenNobleOccupations;
  if(occupationKey&&!Object.hasOwn(occupations,occupationKey))return false;
  if(socialClass==='commoner'&&regionKey&&occupationKey&&occupations[occupationKey].regionKey!==regionKey)return false;
  return true;
}

function normalizeAxes(value,max=3){
  const keys=['body','mana','intelligence','divinity'];
  if(!value||keys.some(key=>!Number.isInteger(value[key])||value[key]<1||value[key]>max))return null;
...
export function normalizeFateOrigin(value){
  ...
  const baseStats=normalizeAxes(value.baseStats,9),talents=normalizeTalents(value.talents),originStory=uniqueStrings(value.originStory,7,360),skillsLearned=uniqueStrings(value.skillsLearned,6,80),socialConnections=uniqueStrings(value.socialConnections,4,160),backgroundFlags=uniqueStrings(value.backgroundFlags,12,100);
...
function buildStructuredOrigin({gender,socialClass,department,seed,originLocks={}}){
  if(!validateFateOriginLocks({socialClass,region:originLocks.region,occupation:originLocks.occupation}))throw new Error('지역과 직업 Origin lock이 양립하지 않음.');
  const seedTag=cleanText(seed||randomSeed(),80),rng=seededRandom(seedTag),lockedOccupation=cleanText(originLocks.occupation,60);
  const occupationOptions=socialClass==='commoner'?FATE_ORIGIN_OPTIONS.commonerOccupations:FATE_ORIGIN_OPTIONS.fallenNobleOccupations;
  const derivedRegion=socialClass==='commoner'&&lockedOccupation?occupationOptions[lockedOccupation]?.regionKey:'';
  const regionKey=cleanText(originLocks.region,40)||derivedRegion||pick(Object.keys(REGION_PROFILES),rng),region=REGION_PROFILES[regionKey];
  const eligibleOccupations=socialClass==='commoner'?region.commonerOccupations:FALLEN_NOBLE_OCCUPATIONS;
  const occupationRow=lockedOccupation?eligibleOccupations.find(([key])=>key===lockedOccupation):pick(eligibleOccupations,rng);
  if(!occupationRow)throw new Error('선택한 지역에서는 잠근 직업을 사용할 수 없음.');
...
function originPcFields(origin,gender,department){
  const grade={1:'F',2:'E',3:'D'},stats=origin.baseStats;
  return {
...
export function generateFateStartingCharacter({gender,socialClass,department,seed,originLocks}={}){
  const base=createFateCharacterCreation({gender,socialClass,department}),origin=buildStructuredOrigin({gender,socialClass,department,seed,originLocks});
  const background=buildFateBackgroundState(origin);
  return {
    creation:{mode:'fate',fateStart:{...base.fateStart,version:2,origin,background,personalStory:buildFatePersonalStoryState(origin,background)}},
    pc:originPcFields(origin,gender,department),
  };
}
