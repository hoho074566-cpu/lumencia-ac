...
function hasPlayerOwnedBoundary(saveState = {}) {
  ...
  return runtime?.turn_hook?.status === 'awaiting-player'
    ...
    || Boolean(clampText(runtime.unresolved_question || '', 240));
}
...
export function deriveSetupPayoffPlan({ saveState = {}, mode = 'game', reachableNpcKeys = [] } = {}) {
  ...
  const playerOwnedBoundary = hasPlayerOwnedBoundary(saveState);
  const candidates = rows
    .filter((row) => ACTIVE_STATUSES.has(row.status))
    .filter((row) => row.status === 'opportunity' || !playerOwnedBoundary)
  ...
  return {
    ...
    reason: selected ? (selected.status === 'opportunity' ? 'awaiting-player-payoff' : 'mature-setup') : playerOwnedBoundary ? 'player-boundary' : 'no-eligible-setup',
  };
}
...
function neutralizeDirectorCallback(turn, reason) {
  ...
  return { version:SETUP_PAYOFF_MEMORY_VERSION, status:'rejected', reason };
}
...
export function reconcileSetupPayoffTurn({ saveState = {}, turn = {}, mode = 'game', plan = null } = {}) {
  ...
  if (['meta', 'auto', 'continue'].includes(normalizedMode)) return neutralizeDirectorCallback(turn, `${normalizedMode}-freeze`);
  if (!key) return neutralizeDirectorCallback(turn, 'invalid-callback-id');
  ...
  if (SETUP_PHASES.has(phase)) {
    ...
    if (!sceneVisible || !clampText(director.callback_note || '', 280) || !['friction', 'encounter'].includes(String(director.beat || ''))) return neutralizeDirectorCallback(turn, 'setup-not-visible');
  ...
  if (phase === 'payoff_opportunity') {
    if (!existing || !ACTIVE_STATUSES.has(existing.status) || selected !== key) return neutralizeDirectorCallback(turn, 'unselected-payoff-opportunity');
    if (!['choice', 'payoff_opportunity'].includes(String(director.beat || ''))) return neutralizeDirectorCallback(turn, 'payoff-opportunity-beat-mismatch');
    if (!array(turn?.choices).length) return neutralizeDirectorCallback(turn, 'payoff-opportunity-requires-choice');
  ...
  if (RESOLUTION_PHASES.has(phase)) {
    if (!existing || existing.status !== 'opportunity' || selected !== key) return neutralizeDirectorCallback(turn, 'payoff-requires-owned-opportunity');
    if (String(director.beat || '') !== phase) return neutralizeDirectorCallback(turn, 'payoff-beat-mismatch');
    if (!sceneVisible) return neutralizeDirectorCallback(turn, 'payoff-not-visible');
    turn.director.callback_key = existing.persisted_key;
