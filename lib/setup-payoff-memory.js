import { CANONICAL_RESOLUTION_OUTCOMES } from './resolution-log.js';
...
export function reconcileSetupPayoffTurn({ saveState = {}, turn = {}, mode = 'game', plan = null } = {}) {
  ...
  if (RESOLUTION_PHASES.has(phase)) {
    ...
    const outcome = String(turn?.resolution_log?.outcome || '');
    if (!CANONICAL_RESOLUTION_OUTCOME_SET.has(outcome)) return neutralizeDirectorCallback(turn, 'payoff-outcome-invalid', { rejectTurn:true });
    if (outcome === 'none') return neutralizeDirectorCallback(turn, 'payoff-outcome-unconfirmed', { rejectTurn:true });
    if (outcome === 'failure') {
      if (!array(turn?.choices).length) {
        if (phase === 'aftermath') {
          turn.director.callback_key = existing.persisted_key;
          return { version:SETUP_PAYOFF_MEMORY_VERSION, status:'resolved', reason:'owned-payoff-terminal-failure', callback_key:key };
        }
        return neutralizeDirectorCallback(turn, 'payoff-failure-requires-retry-choice', { rejectTurn:true });
      }
      turn.director = {
        ...director,
