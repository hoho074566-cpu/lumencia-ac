...
export function deriveSetupPayoffPlan({ saveState = {}, mode = 'game', reachableNpcKeys = [] } = {}) {
  ...
  const candidates = rows
    ...
    .filter((row) => row.status === 'opportunity'
      ? playerBoundary.established_turn != null && row.last_turn === playerBoundary.established_turn
  ...
  return {
    ...
    reason: selected ? (selected.status === 'opportunity' ? 'owned-player-payoff' : 'mature-setup') : playerBoundary.present ? 'player-boundary' : 'no-eligible-setup',
  };
}
...
export function reconcileSetupPayoffTurn({ saveState = {}, turn = {}, mode = 'game', plan = null } = {}) {
  ...
  if (phase === 'payoff_opportunity') {
    ...
    if (!['choice', 'payoff_opportunity'].includes(String(director.beat || ''))) return neutralizeDirectorCallback(turn, 'payoff-opportunity-beat-mismatch', { rejectTurn:true });
  ...
  if (RESOLUTION_PHASES.has(phase)) {
    ...
    if (String(director.beat || '') !== phase) return neutralizeDirectorCallback(turn, 'payoff-beat-mismatch', { rejectTurn:true });
