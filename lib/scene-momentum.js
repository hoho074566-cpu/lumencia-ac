...
const SHARED_UNIT_DURATION_RANGE = String.raw`${DURATION_NUMBER}\s*(?:에서|~|〜|부터)\s*${DURATION_NUMBER}\s*(?:주(?:일)?(?!\s*(?:에|차|째))|일(?!\s*(?:에(?!서)|차|째|자|날))|시간|분)(?:\s*까지)?${DURATION_NOUN_MODIFIER_GUARD}`;
...
function explicitDurationRangeMatch(text='') {
  ...
  const sharedPattern=new RegExp(`(${DURATION_NUMBER})\\s*(?:에서|~|〜|부터)\\s*(${DURATION_NUMBER})\\s*(주(?:일)?(?!\\s*(?:에|차|째))|일(?!\\s*(?:에(?!서)|차|째|자|날))|시간|분)(?:\\s*까지)?${DURATION_NOUN_MODIFIER_GUARD}(?!\\s*${nonActivitySuffix})`),sharedMatch=source.match(sharedPattern);if(!sharedMatch)return null;
