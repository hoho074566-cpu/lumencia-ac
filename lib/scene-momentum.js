...
const DOWNTIME_RE = /^\s*(?:(?:\d+(?:\.\d+)?\s*(?:시간|분)|몇\s*분|잠시|잠깐|좀)\s*(?:동안|만)?\s*)?(?:쉰다|휴식한다|쉬어간다|잠을\s*잔다|잔다|눈을\s*붙인다|수면한다|sleep(?:s|ed|ing)?|rest(?:s|ed|ing)?)\s*[.!?。！？]*$/i;
...
const DOWNTIME_DELIBERATION_RE = /^\s*(?:(?:\d+(?:\.\d+)?\s*(?:시간|분)|몇\s*분|잠시|잠깐|좀)\s*(?:동안|만)?\s*)?(?:쉴까|쉴지|휴식할까|휴식할지|쉬어볼까|잠(?:을)?\s*잘까|잠(?:을)?\s*잘지|수면할까|수면할지|(?:should\s+i|whether\s+to)\s+(?:rest|sleep))\s*[?？.!。！？]*$/i;
const WAIT_RE = /^\s*(?:(?:\d+(?:\.\d+)?\s*(?:시간|분)|몇\s*분|잠시|잠깐)\s*(?:동안|만)?\s*)?(?:기다린다|기다려본다|대기한다|시간을\s*보낸다|가만히\s*(?:기다린다|있는다)|wait(?:s|ed|ing)?)\s*[.!?。！？]*$/i;
const WAIT_NEGATION_RE = /(?:기다리지\s*않|대기(?:하)?지\s*않|안\s*기다|\b(?:do\s*not|don't|without)\s+wait(?:ing)?\b)/i;
const WAIT_DELIBERATION_RE = /^\s*(?:(?:\d+(?:\.\d+)?\s*(?:시간|분)|몇\s*분|잠시|잠깐)\s*(?:동안|만)?\s*)?(?:기다릴까|기다릴지|대기할까|대기할지|(?:should\s+i|whether\s+to)\s+wait)\s*[?？.!。！？]*$/i;
