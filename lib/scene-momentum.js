...
function precedingActivitySubjectProbe(segment='') {
  return String(segment||'').replace(/듣고\s*$/u,'듣는다').replace(/먹고\s*$/u,'먹는다').replace(/자고\s*$/u,'잔다').replace(/쉬고\s*$/u,'쉰다').replace(/기다리고\s*$/u,'기다린다').replace(/마치고\s*$/u,'마친다').replace(/([가-힣]+)하고\s*$/u,'$1한다');
}
function parsePrecedingExplicitDurationBounds(text='',currentTime='',actorName='') {
...
export function classifySceneIntent(action='', { location='', currentTime='', actorName='' } = {}) {
  ...
  const precedingActivityBounds=parsePrecedingExplicitDurationBounds(text,currentTime,actorName),precedingActivityMinimum=precedingActivityBounds?.[0]??0,precedingActivityMinutes=precedingActivityBounds?.[1]??0,precedingActivityRangeMinutes=precedingActivityBounds&&precedingActivityBounds[0]!==precedingActivityBounds[1]?precedingActivityBounds:null;
...
function scheduleActivityTokens(value='') {
  ...
  DATE_QUALIFIER_RE.lastIndex=0;source=source.replace(DATE_QUALIFIER_RE,' ');DATE_QUALIFIER_RE.lastIndex=0;
...
function precedingCommittedActivityClauses(action='') {
  const source=terminalCommittedActionScope(action),clauses=[],connectorRe=/(듣고|참석하고|참여하고|수강하고|받고|훈련하고|연습하고|수련하고|단련하고|먹고|식사하고|대화하고|이야기하고|면담하고|회의하고|자고|쉬고|휴식하고|기다리고|대기하고)(?=\s+)/g,terminalize=(value)=>value==='듣고'?'듣는다':value==='먹고'?'먹는다':value==='자고'?'잔다':value==='쉬고'?'쉰다':value==='기다리고'?'기다린다':value.endsWith('하고')?`${value.slice(0,-2)}한다`:value;
