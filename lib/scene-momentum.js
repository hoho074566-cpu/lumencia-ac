...
const DURATION_PREFIX = String.raw`(?:\d+(?:\.\d+)?\s*시간(?:\s*\d+(?:\.\d+)?\s*분)?|\d+(?:\.\d+)?\s*분|몇\s*분|잠시|잠깐|좀)`;
...
function parseExplicitDurationMinutes(text='') {
  ...
  const hour=String(text).match(/(\d+(?:\.\d+)?)\s*시간/); if(hour){total+=Number(hour[1])*60;found=true;}
  const minute=String(text).match(/(\d+(?:\.\d+)?)\s*분/); if(minute){total+=Number(minute[1]);found=true;}
...
export function classifySceneIntent(action='', { location='' } = {}) {
  ...
  const exteriorMatch=EXTERIOR_RE.test(text),lowValueDeliberation=isLowValueDeliberation(text);
  const explicitDurationMinutes=(hasCommittedDowntime(text)||hasCommittedWait(text)||lowValueDeliberation)?parseExplicitDurationMinutes(text):null;
  const deliberating=(consequential&&DELIBERATION_RE.test(text))||lowValueDeliberation||(exteriorMatch&&EXTERIOR_DELIBERATION_RE.test(text));
  ...
  if(hasCommittedDowntime(text)){const minutes=explicitDurationMinutes??30;return{kind:'downtime',semanticTarget:'rest-complete',compression:true,minAdvanceMinutes:minutes,suggestedAdvanceMinutes:explicitDurationMinutes!=null?[minutes,minutes]:[30,240],deltaTarget:2,requiresNovelty:true,stopPolicy:'post-rest-meaningful-state',location,explicitDurationMinutes};}
  if(hasCommittedWait(text)){const minutes=explicitDurationMinutes??10;return{kind:'wait',semanticTarget:'time-advanced',compression:true,minAdvanceMinutes:minutes,suggestedAdvanceMinutes:explicitDurationMinutes!=null?[minutes,minutes]:[10,60],deltaTarget:1,requiresNovelty:true,stopPolicy:'changed-world-or-important-interruption',location,explicitDurationMinutes};}
  if(EXPLORE_RE.test(text))return{kind:'explore',semanticTarget:'several-nearby-points',compression:true,minAdvanceMinutes:8,suggestedAdvanceMinutes:[8,25],deltaTarget:2,requiresNovelty:true,stopPolicy:'meaningful-discovery-or-choice',location,explicitDurationMinutes};
  if(OBSERVE_RE.test(text))return{kind:'observe',semanticTarget:'new-or-changed-relevant-detail',compression:true,minAdvanceMinutes:1,suggestedAdvanceMinutes:[1,3],deltaTarget:1,requiresNovelty:true,stopPolicy:'new-information-or-world-advance',location,explicitDurationMinutes};
...
export function deriveSceneDelta({ saveState = {}, previousRuntime = {}, turn = {}, nextParticipants = null, action = '' } = {}) {
  ...
  const growthChanged=deltaRowsChange(delta.stat_progress)||deltaRowsChange(delta.skill_experience)||deltaRowsChange(delta.skill_learning)||deltaRowsChange(delta.awakening_progress);
...
export function buildSceneMomentumDirective({ action = '', saveState = {} } = {}) {
  if(CONTINUE_ACTION_RE.test(String(action||'').trim()))return ['[SCENE MOMENTUM V1 — CONTINUE HARD FREEZE]','INTENT=continue-freeze','- CONTINUE는 직전 응답의 같은 순간/같은 장면을 문학적으로 이어 쓰는 전용 흐름이다. 시간·위치·NPC 출입/행동·관계·기억·성장·일정·훅·이벤트 진행 같은 새 상태 변화를 요구하거나 암시하지 않는다.','- 새로운 우연 사건/NPC 개입/Scene Stall 복구를 실행하지 않는다. 직전 장면의 이미 발생한 내용만 확장하고 PC의 새 선택·대사·감정도 만들지 않는다.'].join('\n');
  ...
  if(intent.explicitDurationMinutes!=null)lines.push(`- 명시 시간 규칙: 사용자가 ${intent.explicitDurationMinutes}분을 직접 지정했다. 일반적인 ${intent.kind} 시간 floor로 더 늘리지 말고 이 시간 범위를 우선한다.`);
