...
const DIALOGUE_ACTION_RE = new RegExp(String.raw`(?:말을\s*건다|(?:대화|이야기|질문|답변|설명|상담|논의|면담|회의|브리핑)(?:을|를)?\s*(?:${DECLARED_DURATION_PREFIX}\s*(?:동안|만)?\s*)?한다|묻는다|답한다)\s*[.!。！]*$`,'i');
...
const CLASS_ACTION_RE = new RegExp(String.raw`(?:수업|강의|세미나|실습|오리엔테이션|교육|입학식)(?:에|을|를)?\s*(?:${DECLARED_DURATION_PREFIX}\s*(?:동안|만)?\s*)?(?:참석한다|참여한다|듣는다|수강한다|받는다|시작한다|계속한다|마친다)\s*[.!。！]*$`,'i');
...
function hasThirdPartyTimedSubject(text='',actorName='') {
  ...
  const subjectFrom=(value)=>{const source=stripScheduledStartExpression(value).replace(DATE_QUALIFIER_RE,' ').trim();DATE_QUALIFIER_RE.lastIndex=0;const subject=norm(source.match(/^(.{1,48}?)(?:가|이|은|는)\s+/u)?.[1]||'');return activityTopics.has(subject)?'':subject;};
