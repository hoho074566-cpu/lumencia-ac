...
const FUTURE_DATE_QUALIFIER_RE = /(?:^|[\s,])(?:내일|모레|다음\s*날|익일)(?=\s|[,.!?。！？]|$)/i;
...
export function activityRangeLimitMinutes(intent={}) {
  const row=object(intent),minimum=Math.max(0,Number(row.minAdvanceMinutes||0)),maximum=Math.max(minimum,Number(array(row.suggestedAdvanceMinutes)[1]||0));
...
function parseFutureClockOffsetMinutes(text='',currentTime='') {
  ...
  const raw=String(text),source=activityDurationScope(raw);if(FUTURE_DATE_QUALIFIER_RE.test(raw))return null;
  const named=source.match(/(?:^|\s)(정오|자정)\s*(?=에|부터|경|쯤|시작|개시)/);
  if(named){const target=named[1]==='정오'?720:0,rawOffset=target-now;if(rawOffset===0)return null;return rawOffset>0?rawOffset:rawOffset+1440;}
  const match=source.match(/(?:^|\s)(오전|오후)?\s*(\d{1,2})(?:\s*시(?:\s*(\d{1,2})\s*분)?|:(\d{2}))\s*(?=에|부터|경|쯤|시작|개시)/);
  if(!match)return null;
...
export function classifySceneIntent(action='', { location='', currentTime='' } = {}) {
  ...
  const dateQualifiedStart=Boolean((timedActivity||travel)&&FUTURE_DATE_QUALIFIER_RE.test(text));
  ...
  if(travel){const target=cleanTravelTarget(stripScheduledStartExpression(travel[1]).replace(FUTURE_DATE_QUALIFIER_RE,' ')),time=travelTimeProfile(location,target),activityTiming=explicitDurationMinutes!=null?{...time,minAdvanceMinutes:explicitDurationMinutes,suggestedAdvanceMinutes:[explicitDurationMinutes,explicitDurationMinutes]}:time,offset=scheduledStartOffsetMinutes??0,timing=dateQualifiedStart?{...time,timeProfile:'date-qualified-travel',minAdvanceMinutes:0,suggestedAdvanceMinutes:[0,0]}:scheduledStartOffsetMinutes!=null?{...activityTiming,minAdvanceMinutes:offset+activityTiming.minAdvanceMinutes,suggestedAdvanceMinutes:activityTiming.suggestedAdvanceMinutes.map(value=>offset+value)}:activityTiming;return{kind:'travel',...timing,semanticTarget:target||'declared-destination',compression:!dateQualifiedStart,deltaTarget:dateQualifiedStart?0:2,requiresNovelty:false,stopPolicy:'declared-destination-or-meaningful-interruption',location,explicitDurationMinutes,scheduledStartOffsetMinutes,dateQualifiedStart};}
...
export function buildSceneMomentumDirective({ action = '', saveState = {}, registry = {} } = {}) {
  ...
  const scheduleBoundary=intent.compression&&intent.minAdvanceMinutes>0?nextScheduleBoundaryMinutes(saveState,{futureOnly:true,action,intent,registry}):null;
  ...
  if(intent.dateQualifiedStart)lines.push(`- 날짜 지정 시작 규칙: 내일/모레/다음 날처럼 현재 날짜를 넘는 활동에는 즉시 활동용 시간 floor를 적용하지 않는다. 요청한 날짜를 오늘로 당기거나 일반 활동 최대치로 잘라 완료하지 말고, 권위 있는 일정·날짜 맥락 안에서만 처리한다.${intent.explicitDurationMinutes!=null?` 지정한 ${intent.explicitDurationMinutes}분은 미래 시작 뒤의 활동 지속시간이다.`:''}`);
