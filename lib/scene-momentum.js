...
function explicitDurationRangeMatch(text='') {
  const source=maskCalendarDayExpressions(text),fullPattern=new RegExp(`(${DURATION_ATOM})\\s*(?:에서|~|〜|부터)\\s*(${DURATION_ATOM})(?:\\s*까지)?(?!\\s*${TEMPORAL_OFFSET_QUALIFIER}(?:전(?:에)?|이전|후(?:에)?|뒤(?:에)?))`),fullMatch=source.match(fullPattern);
  ...
  const sharedPattern=new RegExp(`(${DURATION_NUMBER})\\s*(?:에서|~|〜|부터)\\s*(${DURATION_NUMBER})\\s*(일(?!\\s*(?:에(?!서)|차|째|자|날))|시간|분)(?:\\s*까지)?(?!\\s*${TEMPORAL_OFFSET_QUALIFIER}(?:전(?:에)?|이전|후(?:에)?|뒤(?:에)?))`),sharedMatch=source.match(sharedPattern);if(!sharedMatch)return null;
...
function parseExplicitDurationBounds(text='') {
  ...
  let unqualified=maskCalendarDayExpressions(text).replace(temporalRangeQualifier,'').replace(temporalQualifier,'').replace(clockMinute,' '),minimum=0,maximum=0,found=false,rangeMatch;
