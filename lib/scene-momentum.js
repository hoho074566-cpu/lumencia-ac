...
export function buildNarrativeTimePolicyDirective({ mode = 'game' } = {}) {
  if(mode==='continue')return ['[NARRATIVE TIME POLICY V1 — CONTINUE FREEZE]',`POLICY_VERSION=${NARRATIVE_TIME_POLICY_VERSION}`,'- CONTINUE는 같은 순간의 표현 확장이다. 내부 clock, 날짜, 일정, consequence, 행동 진행을 새로 움직이지 않고 exact time도 새로 만들어내지 않는다.'].join('\n');
  return ['[NARRATIVE TIME POLICY V1 — NARRATIVE-FIRST / CLOCK-SECOND]',`POLICY_VERSION=${NARRATIVE_TIME_POLICY_VERSION}`,'- TIME_GUIDE·advance_minutes·내부 minute 값은 시간 역행, 날짜/요일, 일정, deadline, consequence, 명시 duration을 검증하는 내부 authority다. 일반 narration/dialogue에 절대 minute counter나 “N분 경과/소요” 형식의 디버그 보고로 옮기지 않는다.','- 같은 canonical 응답 안에서 행동·장면에 자연스러운 경과와 압축을 판단하고, prose는 반복 과정 대신 변화·NPC/world 반응·사건·관계·성장에 집중한다. 서버가 계산과 hard boundary를 최종 검증한다. 추가 model call은 없다.','- 정확한 시각/남은 시간은 필수 일정·약속·deadline·위험 제한시간·중요 이벤트·사용자의 직접 시간 질문/명시 시각처럼 실제 gameplay relevance가 있을 때만 자연스럽게 드러낸다. 내부 absolute minute 값은 출력하지 않는다.','- 단순 clock tick은 STOP 사유가 아니다. 이동·식사·준비·대기·반복 훈련과 긴 downtime은 권위 있는 한 턴 상한 안에서 의미 있는 beat 중심으로 압축하고, required schedule·due consequence·전투/위험·되돌리기 어려운 결정·중요 NPC 질문 같은 첫 판단점에서만 멈춘다.','- 장기 압축 중에도 일정, consequence, NPC initiative, 관계/성장, world event를 건너뛰지 않는다. 시간 정밀도 때문에 Scene Momentum을 미세 단계나 반복 장면으로 분절하지 않는다.'].join('\n');
}
