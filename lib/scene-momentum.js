...
const DURATION_ATOM = String.raw`(?:${WEEK_DURATION}|${DAY_DURATION}|${DURATION_NUMBER}\s*시간(?:(?:\s*${DURATION_NUMBER}\s*분)|(?:\s*반))?|${DURATION_NUMBER}\s*분)`;
const TEMPORAL_OFFSET_QUALIFIER = String.raw`(?:(?:정도|쯤|가량)\s*)?`;
const DURATION_PREFIX = String.raw`(?:${DURATION_ATOM}|몇\s*분|잠시|잠깐|좀)`;
const SHARED_UNIT_DURATION_RANGE = String.raw`${DURATION_NUMBER}\s*(?:에서|~|〜|부터)\s*${DURATION_NUMBER}\s*(?:주(?:일)?(?!\s*(?:차|째))|일(?!\s*(?:에(?!서)|차|째|자|날))|시간|분)(?:\s*까지)?`;
...
const CLOCK_PERIOD = String.raw`(?:오전|오후|아침|새벽|저녁|밤)`;
...
function explicitDurationRangeMatch(text='') {
  ...
  const sharedPattern=new RegExp(`(${DURATION_NUMBER})\\s*(?:에서|~|〜|부터)\\s*(${DURATION_NUMBER})\\s*(주(?:일)?(?!\\s*(?:차|째))|일(?!\\s*(?:에(?!서)|차|째|자|날))|시간|분)(?:\\s*까지)?(?!\\s*${nonActivitySuffix})`),sharedMatch=source.match(sharedPattern);if(!sharedMatch)return null;
...
function clockExpressionMinutes(marker='',hourValue='',minuteValue='0') {
  const rawHour=Number(hourValue),minute=Number(minuteValue||0),period=String(marker||''),normalizedPeriod=/^(?:오전|아침|새벽)$/.test(period)?'am':/^(?:오후|저녁|밤)$/.test(period)?'pm':'';
...
function clockTokenParts(token='') {
  ...
  const match=String(token||'').trim().match(/^(오전|오후|아침|새벽|저녁|밤)?\s*(\d{1,2})(?:\s*시(?:\s*(?:(\d{1,2})\s*분|(반)))?|:(\d{2}))$/);if(!match)return null;
...
function parseExplicitClockIntervalTiming(text='',currentTime='', { scopeTerminal = true, currentDate = '' } = {}) {
  ...
  if(!match){
    ...
    const dateOffset=selectedActivityDateOffsetDays(source,currentDate),rollover=Boolean((parts.named&&parts.minutes===0)||parts.marker==='새벽'||(parts.marker==='밤'&&(Number(parts.hour)===12||Number(parts.hour)<=5))),duration=dateOffset>0?dateOffset*1440+end-now:end-now+(rollover&&end<=now?1440:0);
...
function selectedActivityClock(text='', { scopeTerminal = true } = {}) {
  ...
  const rollover=parts.marker==='새벽'||parts.marker==='밤'&&(Number(parts.hour)===12||Number(parts.hour)<=5);
