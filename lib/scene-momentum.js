...
const ATTRIBUTED_SPEECH_RE = /(?:^|[\s,])(?:누군가|그가|그녀가|(?!(?:나|내)(?:가|는|은)(?=\s))[가-힣]{1,16}(?:가|이|은|는))(?=\s)[^.!?。！？]{0,40}?(?:겠다고|한다고|라고)\s*(?:외치|말하|소리치)[가-힣]{0,8}/gi;
...
function selectedActivityDateIsFuture(text='') {
  ...
  return Boolean(selected&&selected!=='오늘');
}
...
function parseExplicitDurationMinutes(text='') {
  ...
  const unqualified=activityDurationScope(text).replace(temporalQualifier,'').replace(clockMinute,' ');
  let total=0,found=false;
  const hour=unqualified.match(new RegExp(`(${DURATION_NUMBER})\\s*시간`));
  const hourValue=durationNumber(hour?.[1]);if(hourValue!=null){total+=hourValue*60;found=true;}
  const minute=unqualified.match(new RegExp(`(${DURATION_NUMBER})\\s*분`));
...
function parseFutureClockOffsetMinutes(text='',currentTime='') {
  const now=clockMinutes(currentTime);if(now==null)return null;
  const raw=String(text),source=activityDurationScope(raw);if(selectedActivityDateIsFuture(raw))return null;
  ...
  if(selected.named){const target=selected.named==='정오'?720:0,rawOffset=target-now;if(rawOffset===0)return null;return rawOffset>0?rawOffset:rawOffset+1440;}
  const match=selected.match;
  const marker=String(match[1]||''),rawHour=Number(match[2]),minute=Number(match[3]??match[4]??0);if(!Number.isInteger(rawHour)||!Number.isInteger(minute)||minute<0||minute>59)return null;
  let hour=rawHour;
  if(marker){if(hour<1||hour>12)return null;hour=hour%12+(marker==='오후'?12:0);}
  else if(hour<0||hour>23)return null;
  const target=hour*60+minute,offset=target-now;
...
export function isRequestedScheduledActivity(saveState={},event={},action='',intent=null,registry={}) {
  ...
  const offset=Number(sceneIntent.scheduledStartOffsetMinutes);
  if(!Number.isFinite(offset)||offset<=0||!['class-attendance','training','meal','dialogue'].includes(String(sceneIntent.kind||'')))return false;
  ...
  if(now==null||at==null||at-now!==offset)return false;
...
export function buildSceneMomentumDirective({ action = '', saveState = {}, registry = {} } = {}) {
  ...
  if(intent.explicitDurationMinutes!=null&&!intent.dateQualifiedStart)lines.push(`- 명시 시간 규칙: 사용자가 ${intent.explicitDurationMinutes}분을 직접 지정했다. 일반적인 ${intent.kind} 시간 floor로 더 늘리지 말고 이 시간 범위를 우선한다. 단 SCHEDULE_BOUNDARY가 더 짧으면 그 일정 경계가 최우선이다.`);
