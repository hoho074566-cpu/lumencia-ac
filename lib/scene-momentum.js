...
export function classifySceneIntent(action='', { location='' } = {}) {
  ...
  const exteriorMatch=EXTERIOR_RE.test(text),lowValueDeliberation=isLowValueDeliberation(text),committedDowntime=hasCommittedDowntime(text),committedWait=hasCommittedWait(text),exploreMatch=EXPLORE_RE.test(text),observeMatch=OBSERVE_RE.test(text),travel=text.match(TRAVEL_RE);
  ...
  const questionForm=/[?？]\s*$/.test(text),compressedQuestion=questionForm&&Boolean(exteriorMatch||committedDowntime||committedWait||exploreMatch||observeMatch||travel);
  const deliberating=(consequential&&DELIBERATION_RE.test(text))||lowValueDeliberation||(exteriorMatch&&EXTERIOR_DELIBERATION_RE.test(text))||compressedQuestion;
