...
import { deriveStructuredDecisionPlan, deriveStructuredExecutionPlan, deriveStructuredTimingCandidate, parseTimePlan } from './time-plan-parser.js';

export const SCENE_MOMENTUM_VERSION = '1.0';
export const ADAPTIVE_TIME_SCALE_VERSION = '2.0';
export const NARRATIVE_TIME_POLICY_VERSION = '1.0';

const CONSEQUENTIAL_ACTION_RE = /(?:전투(?:를|에)?\s*(?:시작|돌입|계속|참가|한다|하겠다|할지|할까)|공격(?:을|를)?\s*(?:한다|시도|시작|하겠다|하려|할지|할까|해)|결투(?!장)(?:를|을)?\s*(?:받아들|수락|거절|신청|시작|한다|하겠다|할지|할까)|대련(?:을|를)?\s*(?:한다|시작|신청|수락|거절|할지|할까)|죽이|살해|기습(?:을|를)?\s*(?:한다|시도|하겠다|할지|할까)?|협상(?:을|를)?\s*(?:한다|하고|시작|수락|거절|할지|할까)|고백(?:을|를)?\s*(?:한다|하겠다|할지|할까)|배신(?:을|를)?\s*(?:한다|하겠다|할지|할까)|계약(?:에|을|를)?\s*(?:맺|체결|수락|거절|서명|한다|할지|할까)|서명(?:한다|하겠다|할지|할까)|맹세(?:한다|하겠다|할지|할까)|(?:능력|스킬|권능|마법)(?:을|를)?\s*(?:사용|발동|시전|쓴|쓸|쓸지|쓸까)|도망치|싸우|\b(?:duel|attack|fight|kill|contract|confess)\b)/i;
...
const KOREAN_ONES_NUMBER = String.raw`(?:한|두|세|네|다섯|여섯|일곱|여덟|아홉)`;
const KOREAN_TENS_NUMBER = String.raw`(?:(?:열|스물|서른|마흔|쉰|예순|일흔|여든|아흔)(?:${KOREAN_ONES_NUMBER})?|스무)`;
const KOREAN_DURATION_NUMBER = String.raw`(?<![가-힣])(?:${KOREAN_TENS_NUMBER}|${KOREAN_ONES_NUMBER})`;
...
const WEEK_DURATION = String.raw`(?:일주일|${DURATION_NUMBER}\s*주(?:일)?)(?!\s*(?:에|차|째))`;
const DAY_DURATION = String.raw`(?:(?:${NATIVE_DAY_DURATION}|${DURATION_NUMBER}\s*일)(?:\s*반)?)(?!\s*(?:에(?!서)|차|째|자|날))`;
const MONTH_DURATION = String.raw`${DURATION_NUMBER}\s*(?:개월|달)`;
const YEAR_DURATION = String.raw`${DURATION_NUMBER}\s*년(?!\s*(?:\d{1,2}\s*월|차|째|생))`;
const DURATION_NOUN_MODIFIER_GUARD = String.raw`(?!\s*(?:치|분(?:량)?))`;
const DURATION_ATOM = String.raw`(?:${YEAR_DURATION}|${MONTH_DURATION}|${WEEK_DURATION}|${DAY_DURATION}|${DURATION_NUMBER}\s*시간(?:(?:\s*${DURATION_NUMBER}\s*분)|(?:\s*반))?|${DURATION_NUMBER}\s*분)${DURATION_NOUN_MODIFIER_GUARD}`;
const TEMPORAL_OFFSET_QUALIFIER = String.raw`(?:(?:정도|쯤|가량)\s*)?`;
const DURATION_PREFIX = String.raw`(?:${DURATION_ATOM}|몇\s*분|잠시|잠깐|좀)`;
const SHARED_UNIT_DURATION_RANGE = String.raw`${DURATION_NUMBER}\s*(?:에서|~|〜|부터)\s*${DURATION_NUMBER}\s*(?:년|개월|달|주(?:일)?(?!\s*(?:에|차|째))|일(?!\s*(?:에(?!서)|차|째|자|날))|시간|분)(?:\s*까지)?${DURATION_NOUN_MODIFIER_GUARD}`;
const ALTERNATIVE_DURATION_RANGE = String.raw`(?:${DURATION_ATOM})\s*(?:이나|나|내지|또는|혹은)\s*(?:${DURATION_ATOM})`;
const DURATION_RANGE_PREFIX = String.raw`(?:${ALTERNATIVE_DURATION_RANGE}|(?:${DURATION_ATOM})\s*(?:에서|~|〜|부터)\s*(?:${DURATION_ATOM})(?:\s*까지)?|${SHARED_UNIT_DURATION_RANGE})`;
...
const UPPER_BOUND_ACTIVITY_TIMING = String.raw`(?:최대\s*${DURATION_ATOM}(?:\s*${DURATION_ACTIVITY_SUFFIX})?|${DURATION_ATOM}\s*(?:미만|이내|이하))`;
const DIRECT_TIMED_ACTIVITY_NOUN = String.raw`(?:훈련|연습|수련|단련|수업|강의|세미나|실습|오리엔테이션|교육|입학식|아침|점심|저녁|밥|식사|대화|이야기|질문|답변|설명|상담|논의|면담|회의|브리핑|협상|수면|휴식|대기)`;
const CLOCK_PERIOD = String.raw`(?:오전|오후|아침|새벽|낮|저녁|밤)`;
const CLOCK_HOUR_NUMBER = String.raw`(?:\d{1,2}|${KOREAN_DURATION_NUMBER})`;
const NUMERIC_CLOCK_EXPRESSION = String.raw`(?:(?:${CLOCK_PERIOD})\s*)?${CLOCK_HOUR_NUMBER}(?:\s*시(?:\s*(?:\d{1,2}\s*분|반))?|:\d{2})`;
...
const ACTIVITY_TIMING_PREFIX = String.raw`(?:(?:${SCHEDULED_CLOCK_WINDOW}|${RELATIVE_START_WINDOW})\s*)?(?:(?:${UPPER_BOUND_ACTIVITY_TIMING}|${DECLARED_DURATION_PREFIX}\s*(?:${DURATION_ACTIVITY_SUFFIX})?)\s*)?`;
...
const DIALOGUE_ACTION_RE = new RegExp(String.raw`(?:말을\s*(?:건다|걸겠다|걸게)|(?:대화|이야기|질문|답변|설명|상담|논의|면담|회의|브리핑)(?:을|를)?\s*${ACTIVITY_TIMING_PREFIX}(?:한다|하겠다|할게|계속한다|계속하겠다|계속할게)|묻는다|묻겠다|물을게|답한다|답하겠다|답할게)\s*[.!。！]*$`,'i');
...
const WEEKDAY_QUALIFIER_RE = /(?:^|[\s,])(?:(이번\s*주|다음\s*주|차주|이번|다음)(?:의)?\s*)?(월요일|화요일|수요일|목요일|금요일|토요일|일요일)(?:은|는|엔|에는|에)?(?=\s|[,.!?。！？]|$)/gi;
...
export function isPcRelevantScheduleEvent(saveState={},event={}) {
  ...
  const ownerIdentities=new Set([owner,...owner.split(/[\s,;|/]+/u)].filter(Boolean));
  if([...ownerIdentities].some(identity=>[...pcTokens].some(value=>value.length>=2&&(identity===value||identity.startsWith(`${value}:`)))))return true;
...
function compoundCommittedActionIntent(plan=null,{observeMatch=false,location=''}={}) {
  const clauses=array(plan?.clauses);
  if(!observeMatch||!clauses.length)return null;
  if(clauses.some(clause=>clause?.hypothetical||clause?.conditional||clause?.negated||clause?.quoted||clause?.third_party))return null;
  const dependencyChain=clauses.some(clause=>clause?.unparsed_prefix_action||clause?.incomplete_connector||clause?.unparsed_suffix);
  if(!dependencyChain)return null;
  const travelClause=clauses.find(clause=>clause?.action_type==='travel'),travelMinimum=Math.max(0,Number(travelClause?.duration?.min_minutes||0)),travelMaximum=Math.max(travelMinimum,Number(travelClause?.duration?.max_minutes||20));
  return{kind:'compound-routine',timeProfile:travelClause?.duration?.profile?`compound-${travelClause.duration.profile}`:'compound-contextual',semanticTarget:travelClause?.destination?`complete-chain-at:${travelClause.destination}`:'complete-declared-action-chain',compression:true,minAdvanceMinutes:travelMinimum,suggestedAdvanceMinutes:[travelMinimum,travelMaximum],deltaTarget:2,requiresNovelty:false,stopPolicy:'complete-chain-or-meaningful-interruption',location,explicitDurationMinutes:null,compoundActionChain:true};
}
...
const KOREAN_TENS_VALUES = Object.freeze({열:10,스물:20,스무:20,서른:30,마흔:40,쉰:50,예순:60,일흔:70,여든:80,아흔:90});
function durationNumber(value='') {
  ...
  if(Object.prototype.hasOwnProperty.call(KOREAN_TENS_VALUES,text))return KOREAN_TENS_VALUES[text];
  const compound=text.match(/^(열|스물|서른|마흔|쉰|예순|일흔|여든|아흔)(한|두|세|네|다섯|여섯|일곱|여덟|아홉)$/u);
  if(compound)return KOREAN_TENS_VALUES[compound[1]]+KOREAN_DURATION_VALUES[compound[2]];
...
function terminalCommittedActionScope(text='') {
  ...
  const body=source.trim(),segments=body.split(/(?<!\d)[.!?。！？](?!\d)\s*/gu).map(value=>value.trim()).filter(Boolean);if(!segments.length)return'';
  const stripLead=(value)=>String(value||'').replace(/^(?:그리고|그\s*후|그다음|이어서|다음으로)(?=\s|$)\s*/u,'').replace(/[\s.!?。！？]+$/gu,'').trim();
  const connectorForm=(value)=>{const raw=stripLead(value),forms=[[/한다$/u,'하고'],[/듣는다$/u,'듣고'],[/참석한다$/u,'참석하고'],[/참여한다$/u,'참여하고'],[/수강한다$/u,'수강하고'],[/받는다$/u,'받고'],[/먹는다$/u,'먹고'],[/잔다$/u,'자고'],[/쉰다$/u,'쉬고'],[/기다린다$/u,'기다리고'],[/간다$/u,'가서'],[/이동한다$/u,'이동하고']];for(const [pattern,replacement] of forms){const normalized=raw.replace(pattern,replacement);if(normalized!==raw&&PRIOR_ACTIVITY_CONNECTOR_RE.test(normalized)){PRIOR_ACTIVITY_CONNECTOR_RE.lastIndex=0;return normalized;}PRIOR_ACTIVITY_CONNECTOR_RE.lastIndex=0;}return'';};
  let combined=stripLead(segments.at(-1));
  for(let index=segments.length-2;index>=0;index--){if(!/^(?:그리고|그\s*후|그다음|이어서|다음으로)(?=\s|$)/u.test(segments[index+1]))break;const prior=connectorForm(segments[index]);if(!prior)break;combined=`${prior} ${combined}`;}
  return combined;
}
...
function maskHistoricalTimedClauses(text='') {
  const source=String(text||''),historicalLead=String.raw`(?:어제|그제|그저께|아까|예전에|이전에|앞서|과거(?:에)?|지난\s*(?:날|주|달|해|학기|번))`;
  const beforeCurrent=new RegExp(String.raw`(?:^|[\s,])${historicalLead}[^.!?。！？]{0,120}?(?=(?:오늘|지금|이제|현재)(?:은|는|엔|에는|에)?(?:\s|[,;]))`,'gi');
  const completedContext=new RegExp(String.raw`(?:^|[\s,])${historicalLead}[^.!?。！？]{0,120}?(?:았|었|했|였)(?:고|으며|지만|는데|더니|다가)(?=\s|[,;]|$)`,'gi');
  return source.replace(beforeCurrent,match=>' '.repeat(match.length)).replace(completedContext,match=>' '.repeat(match.length));
}
...
function selectedActivityWeekday(text='',currentDate='',currentWeekday='') {
  ...
  const offset=scope==='다음주'||scope==='차주'||scope==='다음'?7-current+target:scope==='이번주'||scope==='이번'?sameWeek:(sameWeek+7)%7;
...
function durationExpressionMinutes(text='') {
  ...
  const yearMatch=source.match(new RegExp(`^(${DURATION_NUMBER})\\s*년$`));if(yearMatch){const years=durationNumber(yearMatch[1]),total=Math.round(Number(years||0)*525600);return Number.isFinite(total)?Math.max(0,total):null;}
  const monthMatch=source.match(new RegExp(`^(${DURATION_NUMBER})\\s*(?:개월|달)$`));if(monthMatch){const months=durationNumber(monthMatch[1]),total=Math.round(Number(months||0)*43200);return Number.isFinite(total)?Math.max(0,total):null;}
...
function maskCalendarDayExpressions(text='') {
  return String(text||'').replace(new RegExp(ISO_ABSOLUTE_DATE_PATTERN,'g'),' ').replace(new RegExp(ABSOLUTE_DATE_PATTERN,'g'),' ');
}
function explicitDurationRangeMatch(text='') {
  const source=maskCalendarDayExpressions(text),nonActivitySuffix=String.raw`(?:${TEMPORAL_OFFSET_QUALIFIER}(?:전(?:에)?|이전|후(?:에)?|뒤(?:에)?)|마다|간격(?:으로)?|씩|짜리(?!\s*${DIRECT_TIMED_ACTIVITY_NOUN}(?:에|을|를)?(?=\s|$)))`,compactMatch=source.match(new RegExp(`(한두|두세|서너)\\s*(시간|분)(?!\\s*${nonActivitySuffix})`));
  ...
  const alternativePattern=new RegExp(`(${DURATION_ATOM})\\s*(?:이나|나|내지|또는|혹은)\\s*(${DURATION_ATOM})(?!\\s*${nonActivitySuffix})`),alternativeMatch=source.match(alternativePattern);
  if(alternativeMatch){const first=durationExpressionMinutes(alternativeMatch[1]),second=durationExpressionMinutes(alternativeMatch[2]);if(first!=null&&second!=null)return{match:alternativeMatch[0],range:[Math.min(first,second),Math.max(first,second)]};}
  ...
  const sharedPattern=new RegExp(`(${DURATION_NUMBER})\\s*(?:에서|~|〜|부터)\\s*(${DURATION_NUMBER})\\s*(년|개월|달|주(?:일)?(?!\\s*(?:에|차|째))|일(?!\\s*(?:에(?!서)|차|째|자|날))|시간|분)(?:\\s*까지)?${DURATION_NOUN_MODIFIER_GUARD}(?!\\s*${nonActivitySuffix})`),sharedMatch=source.match(sharedPattern);if(!sharedMatch)return null;
  const multiplier=sharedMatch[3]==='년'?525600:['개월','달'].includes(sharedMatch[3])?43200:sharedMatch[3].startsWith('주')?10080:sharedMatch[3]==='일'?1440:sharedMatch[3]==='시간'?60:1,first=durationNumber(sharedMatch[1]),second=durationNumber(sharedMatch[2]);if(first==null||second==null)return null;
...
function parseExplicitDurationRangeMinutes(text='') { const scope=maskHistoricalTimedClauses(maskNegatedTimedClauses(activityDurationScope(text)));return durationUpperBoundMatches(scope).length?parseExplicitDurationBounds(scope):explicitDurationRangeMatch(scope)?.range||null; }
function parseExplicitDurationUpperBoundMinutes(text='') { const scope=maskHistoricalTimedClauses(maskNegatedTimedClauses(activityDurationScope(text)));return durationUpperBoundMatches(scope).length?parseExplicitDurationBounds(scope)?.[1]??null:null; }
...
function parseLinearExplicitDurationBounds(text='') {
  ...
  const nonActivityObject=String.raw`짜리(?!\s*${DIRECT_TIMED_ACTIVITY_NOUN}(?:에|을|를)?(?=\s|$))`;
  const objectModifierRangeQualifier=new RegExp(`(?:${DURATION_RANGE_PREFIX})(?=\\s*${nonActivityObject})`,'g');
  const objectModifierQualifier=new RegExp(`(?:${DURATION_ATOM})(?=\\s*${nonActivityObject})`,'g');
  const clockMinute=new RegExp(`(?:^|\\s)(?:${CLOCK_PERIOD})?\\s*${CLOCK_HOUR_NUMBER}\\s*시\\s*${DURATION_NUMBER}\\s*분(?=\\s*(?:에|부터|까지|경|쯤|시작|개시))`,'g');
  let unqualified=maskCalendarDayExpressions(maskHistoricalTimedClauses(maskNegatedTimedClauses(text))).replace(temporalRangeQualifier,'').replace(temporalQualifier,'').replace(cadenceRangeQualifier,'').replace(cadenceQualifier,'').replace(objectModifierRangeQualifier,'').replace(objectModifierQualifier,'').replace(clockMinute,' '),minimum=0,maximum=0,found=false,rangeMatch;
...
function parseDurationLowerBound(text='', { scopeTerminal = true } = {}) {
  const durationScope=scopeTerminal?activityDurationScope(text):String(text||''),source=maskCalendarDayExpressions(maskHistoricalTimedClauses(maskNegatedTimedClauses(durationScope))),patterns=[
...
function clockExpressionMinutes(marker='',hourValue='',minuteValue='0') {
  const rawHour=durationNumber(hourValue),minute=Number(minuteValue||0),period=String(marker||''),normalizedPeriod=/^(?:오전|아침|새벽)$/.test(period)?'am':/^(?:오후|낮|저녁|밤)$/.test(period)?'pm':'';
...
function clockTokenParts(token='') {
  ...
  const match=String(token||'').trim().match(new RegExp(`^(오전|오후|아침|새벽|낮|저녁|밤)?\\s*(${CLOCK_HOUR_NUMBER})(?:\\s*시(?:\\s*(?:(\\d{1,2})\\s*분|(반)))?|:(\\d{2}))$`));if(!match)return null;
...
function selectedActivityClock(text='', { scopeTerminal = true } = {}) {
  ...
  const hour=durationNumber(parts.hour),rollover=/^(?:오전|아침|새벽)$/.test(parts.marker)||parts.marker==='밤'&&(hour===12||hour<=5);
...
function parseDateQualifiedStartOffsetMinutes(text='',currentTime='', { scopeTerminal = true, currentDate = '', currentWeekday = '' } = {}) {
  ...
  if(dateOffsetDays<=0)return null;
  if(!selected)return dateOffsetDays*1440;
...
function explicitContinuationActionKind(text='') {
  const scope=activityDurationScope(text),patterns=[
    ['downtime',/(?:잠|수면|휴식|쉬)/gu],['wait',/(?:기다|대기)/gu],['meal',/(?:식사|아침|점심|저녁|먹)/gu],
    ['training',/(?:훈련|연습|수련|단련)/gu],['class-attendance',/(?:수업|강의|세미나|실습|오리엔테이션|교육|입학식|수강)/gu],
    ['dialogue',/(?:대화|이야기|질문|답변|설명|상담|논의|면담|회의|브리핑|협상)/gu],['travel',/(?:이동|출발|여행|여정|향하)/gu],
  ];
  let selectedKind='',selectedIndex=-1;
  for(const [candidate,pattern] of patterns)for(const match of scope.matchAll(pattern))if((match.index??-1)>=selectedIndex){selectedKind=candidate;selectedIndex=match.index??-1;}
  return selectedKind;
}
function resumedTimedActionIntent(text='',resumeTimedAction={},location='',actorName='') {
  const record=object(resumeTimedAction),kind=String(record.kind||''),storedRemaining=Math.max(0,Math.trunc(Number(record.remaining_minutes)||0)),hasSplitRemaining=Object.prototype.hasOwnProperty.call(record,'remaining_wait_minutes')||Object.prototype.hasOwnProperty.call(record,'remaining_activity_minutes'),remainingWait=hasSplitRemaining?Math.max(0,Math.trunc(Number(record.remaining_wait_minutes)||0)):0,remainingActivity=hasSplitRemaining?Math.max(0,Math.trunc(Number(record.remaining_activity_minutes)||0)):storedRemaining,remaining=hasSplitRemaining?remainingWait+remainingActivity:storedRemaining,allowedKinds=new Set(['downtime','wait','meal','training','class-attendance','dialogue','travel']);
  ...
  const explicitKind=explicitContinuationActionKind(text);if(explicitKind&&explicitKind!==kind)return null;
  ...
  return{kind,timeProfile:`resumed-${kind}`,semanticTarget:targets[kind],compression:true,minAdvanceMinutes:bounded,suggestedAdvanceMinutes:[bounded,bounded],deltaTarget:kind==='downtime'||kind==='travel'?2:1,requiresNovelty:false,stopPolicy:stops[kind],location,explicitDurationMinutes:remainingActivity,explicitDurationRangeMinutes:null,activityMinimumMinutes:remainingActivity,precedingActivityMinutes:0,precedingActivityRangeMinutes:null,scheduledStartOffsetMinutes:remainingWait>0?remainingWait:null,dateQualifiedStart:false,resumedTimedAction:true,resumeRemainingMinutes:remaining,resumeRemainingWaitMinutes:remainingWait,resumeRemainingActivityMinutes:remainingActivity,turnLimitTruncated:remaining>1440};
}
...
export function classifySceneIntent(action='', { location='', currentTime='', currentDate='', currentWeekday='', actorName='', resumeTimedAction=null } = {}) {
  ...
  let structuredTimePlan=null,structuredTiming={eligible:false},structuredExecution={eligible:false},structuredDecision={eligible:false};try{structuredTimePlan=parseTimePlan(text,{location,currentTime,currentDate,currentWeekday,actorName});structuredTiming=deriveStructuredTimingCandidate(structuredTimePlan);structuredExecution=deriveStructuredExecutionPlan(structuredTimePlan);structuredDecision=deriveStructuredDecisionPlan(structuredTimePlan);}catch{}
  const thirdPartyTimedSubject=hasThirdPartyTimedSubject(text,actorName);
  const consequential=!thirdPartyTimedSubject&&(hasCommittedConsequentialAction(text)||structuredTiming.eligible&&structuredTiming.terminal_action_subtype==='negotiation');
  const structuredKind=structuredTiming.eligible?structuredTiming.terminal_action_type:null;
  const structuredSubtype=structuredTiming.eligible?structuredTiming.terminal_action_subtype:null;
  const exteriorMatch=EXTERIOR_RE.test(text),namedAreaExit=namedAreaExteriorTarget(text),lowValueDeliberation=isLowValueDeliberation(text),travelDeliberation=TRAVEL_DELIBERATION_RE.test(text),committedDowntime=!thirdPartyTimedSubject&&!consequential&&(hasCommittedDowntime(text)||structuredKind==='downtime'),committedWait=!thirdPartyTimedSubject&&(hasCommittedWait(text)||structuredKind==='wait'),exploreMatch=EXPLORE_RE.test(text),observeMatch=OBSERVE_RE.test(text),compoundActionIntent=compoundCommittedActionIntent(structuredTimePlan,{observeMatch,location}),dialogueMatch=!thirdPartyTimedSubject&&(DIALOGUE_ACTION_RE.test(text)||structuredKind==='dialogue'),mealMatch=!thirdPartyTimedSubject&&(MEAL_ACTION_RE.test(text)||(EATING_ACTION_RE.test(text)&&(parseExplicitDurationBounds(activityDurationScope(text))!==null||SCHEDULED_ACTIVITY_CLOCK_RE.test(activityDurationScope(text))))||structuredKind==='meal'),trainingMatch=!thirdPartyTimedSubject&&(TRAINING_ACTION_RE.test(text)||structuredKind==='training'),classMatch=!thirdPartyTimedSubject&&(CLASS_ACTION_RE.test(text)||structuredKind==='class-attendance'),legacyTravel=thirdPartyTimedSubject?null:text.match(TRAVEL_RE),travel=legacyTravel||(structuredKind==='travel'&&structuredTiming.terminal_destination?[null,structuredTiming.terminal_destination]:null);
  ...
  const selectedActivityScope=activityDurationScope(text),clockIntervalTiming=schedulableActivity?parseExplicitClockIntervalTiming(text,currentTime,{currentDate,currentWeekday}):null,legacyDateQualifiedStart=Boolean(schedulableActivity&&!clockIntervalTiming?.deadline&&selectedActivityDateIsFuture(selectedActivityScope,currentDate,currentWeekday)),clockIntervalMinutes=clockIntervalTiming?.minutes??null;
  const structuredRelativeDateStart=structuredTiming.eligible?structuredTiming.relative_date_start_offset_minutes:null,structuredFutureDateStart=Number.isFinite(structuredRelativeDateStart)&&Number(structuredTiming.relative_date_start_day_offset)>0,structuredElapsedDateStart=Boolean(structuredTiming.elapsed_relative_date_start);
  const dateQualifiedStart=Boolean(legacyDateQualifiedStart||schedulableActivity&&!clockIntervalTiming?.deadline&&structuredFutureDateStart);
  const elapsedScheduledStart=Boolean(schedulableActivity&&(structuredElapsedDateStart||clockIntervalTiming?.elapsedDeadline||!clockIntervalTiming?.startOffsetSuppressed&&hasElapsedScheduledDateStart(selectedActivityScope,currentTime,currentDate,currentWeekday)));
  ...
  const legacyPrecedingActivityBounds=parsePrecedingExplicitDurationBounds(text,currentTime,actorName,currentDate,currentWeekday),precedingActivityBounds=structuredTiming.regional_travel_prefix_range||legacyPrecedingActivityBounds,precedingActivityMinimum=precedingActivityBounds?.[0]??0,precedingActivityMinutes=precedingActivityBounds?.[1]??0,precedingActivityRangeMinutes=precedingActivityBounds&&precedingActivityBounds[0]!==precedingActivityBounds[1]?precedingActivityBounds:null;
  ...
  const dateQualifiedStartOffsetMinutes=dateQualifiedStart?(structuredFutureDateStart?structuredRelativeDateStart:parseDateQualifiedStartOffsetMinutes(selectedActivityScope,currentTime,{currentDate,currentWeekday})):null;
  const structuredTimePlanApplied=[structuredKind&&((structuredKind==='training'&&!TRAINING_ACTION_RE.test(text))||(structuredKind==='class-attendance'&&!CLASS_ACTION_RE.test(text))||(structuredKind==='dialogue'&&!DIALOGUE_ACTION_RE.test(text))||(structuredKind==='travel'&&!legacyTravel))?'terminal-action':null,structuredKind==='downtime'&&structuredSubtype==='sleep'&&!SLEEP_ACTION_RE.test(text)?'sleep-subtype':null,structuredElapsedDateStart||structuredFutureDateStart?'relative-date-start':null,structuredTiming.regional_travel_prefix_range?'regional-travel-prefix':null].filter(Boolean);
  const capTimedIntent=(value)=>{const guide=array(value?.suggestedAdvanceMinutes),rawMinimum=Math.max(0,Number(value?.minAdvanceMinutes||0)),rawLower=Math.max(0,Number(guide[0]||0)),rawUpper=Math.max(rawLower,Number(guide[1]||0)),turnLimitTruncated=Boolean(value?.turnLimitTruncated)||rawMinimum>1440||rawLower>1440,cappedMinimum=Math.min(1440,rawMinimum),cappedGuide=[Math.min(1440,rawLower),Math.min(1440,rawUpper)],planMinimum=Math.min(1440,Number(structuredExecution?.total_min_minutes)),planMaximum=Math.min(1440,Number(structuredExecution?.total_max_minutes)),structuredExecutionPlan=structuredExecution?.eligible&&Number.isFinite(planMinimum)&&Number.isFinite(planMaximum)&&planMinimum===cappedMinimum&&planMinimum===cappedGuide[0]&&planMaximum===cappedGuide[1]?structuredExecution:null,structuredDecisionPlan=!structuredExecutionPlan&&structuredDecision?.eligible&&structuredDecision?.exact_timeline===false?structuredDecision:null;return{...value,structuredTimePlanApplied,structuredExecutionPlan,structuredDecisionPlan,strictDurationLowerBoundMinutes,strictDurationLowerBoundInclusive,explicitDurationUpperBoundMinutes,minAdvanceMinutes:cappedMinimum,suggestedAdvanceMinutes:cappedGuide,turnLimitTruncated};};
  const timedRange=(defaultMinutes,defaultMaximum)=>{const strictMinimum=strictDurationLowerBoundMinutes==null?null:strictDurationLowerBoundMinutes+(strictDurationLowerBoundInclusive?0:1),activityMinutes=strictMinimum??explicitDurationRangeMinutes?.[0]??explicitDurationMinutes??defaultMinutes,dateStartOffset=dateQualifiedStart&&dateQualifiedStartOffsetMinutes!=null&&dateQualifiedStartOffsetMinutes<=1440?dateQualifiedStartOffsetMinutes:null,dateCompletionReachable=dateStartOffset!=null&&dateStartOffset+activityMinutes<=1440;if(dateQualifiedStart&&dateStartOffset==null)return{minutes:0,guide:[0,1440],activityMinutes,effectiveStartOffset:null,futureReachable:false,scheduledStartOverrun:false,scheduledStartBoundaryOnly:false};if(dateQualifiedStart&&!dateCompletionReachable)return{minutes:dateStartOffset,guide:[dateStartOffset,dateStartOffset],activityMinutes,effectiveStartOffset:dateStartOffset,futureReachable:true,scheduledStartOverrun:false,scheduledStartBoundaryOnly:true};const effectiveStartOffset=dateStartOffset??scheduledStartOffsetMinutes,scheduledStartOverrun=effectiveStartOffset!=null&&precedingActivityMinimum>effectiveStartOffset;if(scheduledStartOverrun)return{minutes:effectiveStartOffset,guide:[effectiveStartOffset,effectiveStartOffset],activityMinutes,effectiveStartOffset,futureReachable:Boolean(dateQualifiedStart&&dateStartOffset!=null),scheduledStartOverrun,scheduledStartBoundaryOnly:false};const offsetMinimum=effectiveStartOffset??precedingActivityMinimum,offsetMaximum=effectiveStartOffset??precedingActivityMinutes,rawGuide=strictMinimum!=null?[offsetMinimum+strictMinimum,offsetMaximum+Math.max(strictMinimum,defaultMaximum)]:explicitDurationRangeMinutes?[offsetMinimum+explicitDurationRangeMinutes[0],offsetMaximum+explicitDurationRangeMinutes[1]]:explicitDurationMinutes!=null?[offsetMinimum+activityMinutes,offsetMaximum+activityMinutes]:[offsetMinimum+defaultMinutes,offsetMaximum+defaultMaximum],guide=dateQualifiedStart?[rawGuide[0],Math.min(1440,rawGuide[1])]:rawGuide;return{minutes:offsetMinimum+activityMinutes,guide,activityMinutes,effectiveStartOffset,futureReachable:Boolean(dateQualifiedStart&&dateStartOffset!=null),scheduledStartOverrun,scheduledStartBoundaryOnly:false};};
  ...
  if(compoundActionIntent)return compoundActionIntent;
...
export function isRequestedScheduledActivity(saveState={},event={},action='',intent=null,registry={}) {
  ...
  const eventPrimaryText=[row.id,row.title].filter(Boolean).join(' '),eventParticipantText=scheduleParticipantIdentityText(save,row,registry),eventText=[eventPrimaryText,row.note,row.location,row.department,eventParticipantText].filter(Boolean).join(' '),eventCategory=scheduleActivityCategory(eventPrimaryText,row.kind);
  const currentWeekday=String(save?.world?.weekday||''),pcIdentityTokens=new Set(scheduleActivityTokens(save?.pc?.name||'')),eventTokens=new Set(scheduleActivityTokens(eventText)),eventParticipantTokens=new Set(scheduleActivityTokens(eventParticipantText)),compactEventIdentity=norm(eventText).replace(/[\s\p{P}\p{S}]+/gu,''),compactParticipantIdentity=norm(eventParticipantText).replace(/[\s\p{P}\p{S}]+/gu,''),dayStart=dateTimeMinutes(currentDate,'00:00'),candidateRows=[{action:String(action||''),intent:sceneIntent},...precedingCommittedActivityClauses(action).map(candidateAction=>({action:candidateAction,intent:classifySceneIntent(candidateAction,{location:save?.world?.location||'',currentTime:save?.world?.time||'',currentDate,currentWeekday,actorName:save?.pc?.name||''})}))];
...
export function buildSceneMomentumDirective({ action = '', saveState = {}, registry = {} } = {}) {
  ...
  const structuredPlan=intent.structuredExecutionPlan||intent.structuredDecisionPlan;
  if(structuredPlan?.eligible&&Array.isArray(structuredPlan.clauses)&&structuredPlan.clauses.length){
    const compactPlan=structuredPlan.clauses.map((clause,index)=>`${clause.clause_id||`action_${index+1}`}:${clause.action_type}@${clause.start_min_minutes}-${clause.complete_min_minutes}/${clause.start_max_minutes}-${clause.complete_max_minutes??'open'}`).join(';');
    lines.push('[TPP PHASE 3 — STRUCTURED TIME PLAN]',`STRUCTURED_TIME_PLAN=${compactPlan}`,'- time_execution에서 위 action ID의 완료/중단 집합과 모든 state effect의 소유권을 보고한다. 선택이 있으면 그 질문은 반드시 scene의 마지막 항목이며 decision_scene_index는 그 인덱스다.','- narration 문구가 아니라 action ID와 effect_owners가 경계 reconciliation의 권위다. 미완료 action의 효과를 완료 action에 귀속하지 않는다.');
  }
  ...
  else if(intent.kind==='compound-routine')lines.push('- COMPOUND ACTION 규칙: 입력 전체를 선언된 action chain으로 취급한다. 마지막 확인·관찰만 떼어 실행하지 말고, 그것에 필요한 앞선 이동과 결정 가치 없는 중간 단계를 같은 턴에 압축한다. 이동이 실제로 완료되면 state_delta.new_location에 도착 장소를 기록하고, hard boundary나 중요한 판단점에서만 중단한다.');
