...
const SCHEDULED_CLOCK_WINDOW = String.raw`(?:(?:${CLOCK_EXPRESSION})\s*부터\s*(?:${CLOCK_EXPRESSION})\s*까지|(?:${CLOCK_EXPRESSION})\s*(?:에|부터|경|쯤))`;
const ACTIVITY_TIMING_PREFIX = String.raw`(?:(?:${SCHEDULED_CLOCK_WINDOW})\s*)?(?:${DECLARED_DURATION_PREFIX}\s*(?:동안|만)?\s*)?`;
...
const PRIOR_ACTIVITY_CONNECTOR_RE = new RegExp(String.raw`(?:(?:훈련|연습|수련|단련)(?:을|를)?\s*${PRIOR_ACTIVITY_TIMING}(?:하고|한\s*(?:뒤|후)|마치고)|(?:수업|강의|세미나|실습|오리엔테이션|교육|입학식)(?:에|을|를)?\s*${PRIOR_ACTIVITY_TIMING}(?:하고|참석하고|참여하고|듣고|수강하고|받고|마치고|참석한\s*(?:뒤|후)|들은\s*(?:뒤|후))|(?:아침|점심|저녁|밥|식사)(?:을|를)?\s*${PRIOR_ACTIVITY_TIMING}(?:하고|먹고|마치고)|(?:대화|이야기|질문|답변|설명|상담|논의|면담|회의|브리핑)(?:을|를)?\s*${PRIOR_ACTIVITY_TIMING}(?:하고|한\s*(?:뒤|후)|마치고)|(?:잠을\s*${PRIOR_ACTIVITY_TIMING}자고|잠을\s*${PRIOR_ACTIVITY_TIMING}잔\s*(?:뒤|후)|(?:휴식|대기)(?:을|를)?\s*${PRIOR_ACTIVITY_TIMING}(?:하고|마치고)|쉬고|쉰\s*(?:뒤|후)|휴식하고|기다리고|대기하고)|(?:이동하고|도착하고|나가서|들어가서|가서|와서|방을\s*잡고))`,'gi');
...
function parsePrecedingExplicitDurationBounds(text='',currentTime='') {
  ...
  for(const match of connectors){
    const end=match.index+match[0].length,segment=source.slice(boundary,end),explicit=parseExplicitDurationBounds(segment),clockInterval=parseExplicitClockIntervalTiming(segment,currentTime,{scopeTerminal:false}),defaults=/(?:훈련|연습|수련|단련)/.test(match[0])?[30,120]:/(?:수업|강의|세미나|실습|오리엔테이션|교육|입학식)/.test(match[0])?[45,120]:/(?:아침|점심|저녁|밥|식사)/.test(match[0])?[20,45]:/(?:대화|이야기|질문|답변|설명|상담|논의|면담|회의|브리핑)/.test(match[0])?[2,10]:/(?:잠을|잔\s*(?:뒤|후))/.test(match[0])?[240,480]:/(?:휴식|쉬고|쉰\s*(?:뒤|후))/.test(match[0])?[30,240]:/(?:대기|기다리고)/.test(match[0])?[10,60]:/(?:(?:으로|로|에)\s*(?:이동하고|도착하고|나가서|들어가서|가서)|방을\s*잡고)/.test(segment)?[3,30]:[0,0],bounds=clockInterval?[clockInterval.minutes,clockInterval.minutes]:explicit||defaults,relativeStart=parseRelativeStartOffsetMinutes(segment,{scopeTerminal:false}),dateStart=selectedActivityDateIsFuture(segment)?parseDateQualifiedStartOffsetMinutes(segment,currentTime,{scopeTerminal:false}):null,clockStart=parseFutureClockOffsetMinutes(segment,currentTime,{scopeTerminal:false}),startOffset=relativeStart??dateStart??clockStart;
...
function parseExplicitClockIntervalTiming(text='',currentTime='', { scopeTerminal = true } = {}) {
