...
const DURATION_ATOM = String.raw`(?:${DURATION_NUMBER}\s*시간(?:\s*${DURATION_NUMBER}\s*분)?|${DURATION_NUMBER}\s*분)`;
const DURATION_PREFIX = String.raw`(?:${DURATION_ATOM}|몇\s*분|잠시|잠깐|좀)`;
const DURATION_RANGE_PREFIX = String.raw`(?:${DURATION_ATOM})\s*(?:에서|~|〜|부터)\s*(?:${DURATION_ATOM})(?:\s*까지)?`;
const DECLARED_DURATION_PREFIX = String.raw`(?:${DURATION_RANGE_PREFIX}|${DURATION_PREFIX})`;
const DOWNTIME_RE = new RegExp(String.raw`^\s*(?:${DECLARED_DURATION_PREFIX}\s*(?:동안|만)?\s*)?(?:쉰다|휴식한다|쉬어간다|잠을\s*잔다|잔다|눈을\s*붙인다|수면한다|sleep(?:s|ed|ing)?|rest(?:s|ed|ing)?)\s*[.!?。！？]*$`,'i');
const DOWNTIME_ACTION_RE = /(?:쉰다|휴식한다|쉬어간다|잠을\s*잔다|잔다|눈을\s*붙인다|수면한다|sleep(?:s|ed|ing)?|rest(?:s|ed|ing)?)\s*[.!。！]*$/i;
...
const DOWNTIME_DELIBERATION_RE = new RegExp(String.raw`^\s*(?:${DECLARED_DURATION_PREFIX}\s*(?:동안|만)?\s*)?(?:쉴까|쉴지|휴식할까|휴식할지|쉬어볼까|잠(?:을)?\s*잘까|잠(?:을)?\s*잘지|수면할까|수면할지|(?:should\s+i|whether\s+to)\s+(?:rest|sleep))\s*[?？.!。！？]*$`,'i');
const WAIT_RE = new RegExp(String.raw`^\s*(?:${DECLARED_DURATION_PREFIX}\s*(?:동안|만)?\s*)?(?:기다린다|기다려본다|대기한다|시간을\s*보낸다|가만히\s*(?:기다린다|있는다)|wait(?:s|ed|ing)?)\s*[.!?。！？]*$`,'i');
const WAIT_ACTION_RE = /(?:기다린다|기다려본다|대기한다|시간을\s*보낸다|가만히\s*(?:기다린다|있는다)|wait(?:s|ed|ing)?)\s*[.!?。！？]*$/i;
const WAIT_NEGATION_RE = /(?:기다리지\s*않|대기(?:하)?지\s*않|(?:^|[\s,.;!?。！？])(?:안|못)\s*기다|\b(?:do\s*not|don't|without)\s+wait(?:ing)?\b)/i;
const WAIT_DELIBERATION_RE = new RegExp(String.raw`^\s*(?:${DECLARED_DURATION_PREFIX}\s*(?:동안|만)?\s*)?(?:기다릴까|기다릴지|대기할까|대기할지|(?:should\s+i|whether\s+to)\s+wait)\s*[?？.!。！？]*$`,'i');
const DIALOGUE_ACTION_RE = /(?:말을\s*건다|(?:대화|이야기|질문|답변|설명|상담|논의|면담|회의|브리핑)(?:을|를)?\s*한다|묻는다|답한다)\s*[.!。！]*$/i;
const MEAL_ACTION_RE = new RegExp(String.raw`(?:(?:아침|점심|저녁|밥|식사)(?:을|를)?\s*(?:${DECLARED_DURATION_PREFIX}\s*(?:동안|만)?\s*)?먹는다|식사(?:를)?\s*(?:${DECLARED_DURATION_PREFIX}\s*(?:동안|만)?\s*)?(?:한다|마친다))\s*[.!。！]*$`,'i');
const TRAINING_ACTION_RE = new RegExp(String.raw`(?:훈련|연습|수련|단련)(?:을|를)?\s*(?:${DECLARED_DURATION_PREFIX}\s*(?:동안|만)?\s*)?(?:한다|시작한다|계속한다|마친다)\s*[.!。！]*$`,'i');
const CLASS_ACTION_RE = new RegExp(String.raw`(?:수업|강의|세미나|실습)(?:에|을|를)?\s*(?:${DECLARED_DURATION_PREFIX}\s*(?:동안|만)?\s*)?(?:참석한다|참여한다|듣는다|수강한다|받는다|시작한다|계속한다|마친다)\s*[.!。！]*$`,'i');
const PRIOR_ACTIVITY_CONNECTOR_RE = /(?:(?:훈련|연습|수련|단련)(?:을|를)?\s*(?:하고|한\s*(?:뒤|후)|마치고)|(?:수업|강의|세미나|실습)(?:에|을|를)?\s*(?:참석하고|참여하고|듣고|수강하고|받고|마치고|참석한\s*(?:뒤|후)|들은\s*(?:뒤|후))|(?:아침|점심|저녁|밥|식사)(?:을|를)?\s*(?:먹고|마치고)|(?:대화|이야기|상담|논의)(?:를)?\s*(?:하고|한\s*(?:뒤|후)|마치고)|(?:잠을\s*자고|잠을\s*잔\s*(?:뒤|후)|쉬고|쉰\s*(?:뒤|후)|휴식하고|기다리고|대기하고)|(?:이동하고|도착하고|나가서|들어가서|가서|와서|방을\s*잡고))/gi;
const SLEEP_ACTION_RE = /(?:잠을\s*잔다|잔다|눈을\s*붙인다|수면한다)\s*[.!。！]*$/i;
const SHORT_REST_CUE_RE = /(?:잠깐|잠시|좀|몇\s*분)/i;
const DATE_QUALIFIER_RE = /(?:^|[\s,])(오늘|내일|모레|다음\s*날|익일)(?:은|는|엔|에는|에)?(?=\s|[,.!?。！？]|$)/gi;
...
function cleanTravelTarget(raw='') {
  ...
  target=target.replace(new RegExp(String.raw`^(?:${DECLARED_DURATION_PREFIX})\s*(?:동안|만)\s*`,'i'),'');
...
function selectedActivityDateOffsetDays(text='') {
  const matches=[...String(text||'').matchAll(DATE_QUALIFIER_RE)];
  DATE_QUALIFIER_RE.lastIndex=0;
  const selected=String(matches.at(-1)?.[1]||'').replace(/\s+/g,'');
  if(['내일','다음날','익일'].includes(selected))return 1;
  if(selected==='모레')return 2;
  return 0;
}
function selectedActivityDateIsFuture(text='') { return selectedActivityDateOffsetDays(text)>0; }
function hasCommittedConsequentialAction(text='') {
  let source=String(text||'').replace(QUOTED_SEGMENT_RE,' ').replace(ATTRIBUTED_SPEECH_RE,' ');
...
function durationExpressionMinutes(text='') {
  const match=String(text||'').trim().match(new RegExp(`^(${DURATION_NUMBER})\\s*시간(?:\\s*(${DURATION_NUMBER})\\s*분)?$|^(${DURATION_NUMBER})\\s*분$`));if(!match)return null;
  const hours=durationNumber(match[1]),minutes=durationNumber(match[2]??match[3]),total=Math.round(Number(hours||0)*60+Number(minutes||0));
  return Number.isFinite(total)?Math.max(0,total):null;
}
function explicitDurationRangeMatch(text='') {
  const pattern=new RegExp(`(${DURATION_ATOM})\\s*(?:에서|~|〜|부터)\\s*(${DURATION_ATOM})(?:\\s*까지)?(?!\\s*(?:전(?:에)?|이전|후(?:에)?|뒤(?:에)?))`),match=String(text||'').match(pattern);if(!match)return null;
  const first=durationExpressionMinutes(match[1]),second=durationExpressionMinutes(match[2]);if(first==null||second==null)return null;
  return{match:match[0],range:[Math.min(first,second),Math.max(first,second)]};
}
function parseExplicitDurationRangeMinutes(text='') { return explicitDurationRangeMatch(activityDurationScope(text))?.range||null; }
function parseExplicitDurationTotal(text='') {
  const temporalQualifier=new RegExp(`(?:${DURATION_NUMBER}\\s*시간(?:\\s*${DURATION_NUMBER}\\s*분)?|${DURATION_NUMBER}\\s*분)(?=\\s*(?:전(?:에)?|이전|후(?:에)?|뒤(?:에)?))`,'g');
  const clockMinute=new RegExp(`(?:^|\\s)(?:오전|오후)?\\s*\\d{1,2}\\s*시\\s*${DURATION_NUMBER}\\s*분(?=\\s*(?:에|부터|까지|경|쯤|시작|개시))`,'g');
  let unqualified=String(text||'').replace(temporalQualifier,'').replace(clockMinute,' '),total=0,found=false,rangeMatch;
  while((rangeMatch=explicitDurationRangeMatch(unqualified))){total+=rangeMatch.range[1];found=true;unqualified=unqualified.replace(rangeMatch.match,' ');}
  const durations=new RegExp(`(${DURATION_NUMBER})\\s*시간(?:\\s*(${DURATION_NUMBER})\\s*분)?|(${DURATION_NUMBER})\\s*분`,'g');
  for(const match of unqualified.matchAll(durations)){
    const hours=durationNumber(match[1]),minutes=durationNumber(match[2]??match[3]);
    if(hours!=null){total+=hours*60;found=true;}if(minutes!=null){total+=minutes;found=true;}
  }
...
function parseRelativeStartOffsetMinutes(text='') {
  const source=activityDurationScope(text),matches=[...source.matchAll(new RegExp(`(?:(${DURATION_NUMBER})\\s*시간(?:\\s*(${DURATION_NUMBER})\\s*분)?|(${DURATION_NUMBER})\\s*분)\\s*(?:후|뒤|이후)(?:에)?`,'g'))],match=matches.at(-1);
  if(!match)return null;
  const hours=durationNumber(match[1]),minutes=durationNumber(match[2]??match[3]),total=Math.round(Number(hours||0)*60+Number(minutes||0));
  return total>0?total:null;
}
function parsePrecedingExplicitDurationMinutes(text='') {
  const source=String(text||'');let boundary=0,match;
  PRIOR_ACTIVITY_CONNECTOR_RE.lastIndex=0;
  while((match=PRIOR_ACTIVITY_CONNECTOR_RE.exec(source)))boundary=match.index+match[0].length;
  PRIOR_ACTIVITY_CONNECTOR_RE.lastIndex=0;
  return boundary>0?Math.max(0,Number(parseExplicitDurationTotal(source.slice(0,boundary))||0)):0;
}
function clockExpressionMinutes(marker='',hourValue='',minuteValue='0') {
  const rawHour=Number(hourValue),minute=Number(minuteValue||0),period=String(marker||'');
  if(!Number.isInteger(rawHour)||!Number.isInteger(minute)||minute<0||minute>59)return null;
  if(period){if(rawHour<1||rawHour>12)return null;return(rawHour%12+(period==='오후'?12:0))*60+minute;}
  if(rawHour<0||rawHour>23)return null;
  return rawHour*60+minute;
}
function parseExplicitClockIntervalMinutes(text='') {
  const source=activityDurationScope(text),match=source.match(/(?:^|\s)(오전|오후)?\s*(\d{1,2})(?:\s*시(?:\s*(\d{1,2})\s*분)?|:(\d{2}))\s*부터\s*(오전|오후)?\s*(\d{1,2})(?:\s*시(?:\s*(\d{1,2})\s*분)?|:(\d{2}))\s*까지/);
  if(!match)return null;
  const startMarker=String(match[1]||''),explicitEndMarker=String(match[5]||''),endMarker=explicitEndMarker||startMarker;
  const start=clockExpressionMinutes(startMarker,match[2],match[3]??match[4]??0),end=clockExpressionMinutes(endMarker,match[6],match[7]??match[8]??0);
  if(start==null||end==null)return null;
  let duration=end-start;
  if(duration<=0&&startMarker&&explicitEndMarker)duration+=1440;
  return duration>0&&duration<=1440?duration:null;
}
function stripScheduledStartExpression(text='') {
  const clock=String.raw`(?:정오|자정|(?:오전|오후)?\s*\d{1,2}(?:\s*시(?:\s*\d{1,2}\s*분)?|:\d{2}))`;
  const relative=String.raw`(?:${DURATION_NUMBER}\s*시간(?:\s*${DURATION_NUMBER}\s*분)?|${DURATION_NUMBER}\s*분)\s*(?:후|뒤|이후)(?:에)?`;
  return String(text||'').replace(new RegExp(String.raw`(?:^|\s)${clock}\s*부터\s*${clock}\s*까지(?=\s|$)`,'g'),' ').replace(new RegExp(String.raw`(?:^|\s)${clock}\s*(?:에|부터|경|쯤)(?=\s|$)`,'g'),' ').replace(new RegExp(String.raw`(?:^|\s)${clock}(?=\s|$)`,'g'),' ').replace(new RegExp(String.raw`(?:^|\s)${relative}(?=\s|$)`,'g'),' ').trim().replace(/(?:으로|로|에)\s*$/,'');
}
function selectedActivityClock(text='') {
  const source=activityDurationScope(String(text||''));
  const named=[...source.matchAll(/(?:^|\s)(정오|자정)\s*(?=에|부터|경|쯤|시작|개시)/g)].map(match=>({index:match.index??-1,named:match[1],match}));
  const numeric=[...source.matchAll(/(?:^|\s)(오전|오후)?\s*(\d{1,2})(?:\s*시(?:\s*(\d{1,2})\s*분)?|:(\d{2}))\s*(?=에|부터|경|쯤|시작|개시)/g)].map(match=>({index:match.index??-1,match}));
  ...
  const minutes=clockExpressionMinutes(String(match[1]||''),match[2],match[3]??match[4]??0);
  return minutes==null?null:{minutes,named:false};
}
function parseFutureClockOffsetMinutes(text='',currentTime='') {
  const now=clockMinutes(currentTime),raw=String(text);if(now==null||selectedActivityDateIsFuture(raw))return null;
  const selected=selectedActivityClock(raw);if(!selected)return null;
  const offset=selected.minutes-now;if(offset===0)return null;
  return offset>0?offset:selected.named?offset+1440:null;
}
...
function hasCommittedDowntime(text='') { return (DOWNTIME_RE.test(text)||DOWNTIME_ACTION_RE.test(text))&&!DOWNTIME_NEGATION_RE.test(text); }
function hasCommittedWait(text='') { return (WAIT_RE.test(text)||WAIT_ACTION_RE.test(text))&&!WAIT_NEGATION_RE.test(text); }
...
export function classifySceneIntent(action='', { location='', currentTime='' } = {}) {
  const text=String(action||'').trim();
  const consequential=hasCommittedConsequentialAction(text);
  const exteriorMatch=EXTERIOR_RE.test(text),lowValueDeliberation=isLowValueDeliberation(text),travelDeliberation=TRAVEL_DELIBERATION_RE.test(text),committedDowntime=!consequential&&hasCommittedDowntime(text),committedWait=hasCommittedWait(text),exploreMatch=EXPLORE_RE.test(text),observeMatch=OBSERVE_RE.test(text),dialogueMatch=DIALOGUE_ACTION_RE.test(text),mealMatch=MEAL_ACTION_RE.test(text),trainingMatch=TRAINING_ACTION_RE.test(text),classMatch=CLASS_ACTION_RE.test(text),travel=text.match(TRAVEL_RE);
  ...
  const dateQualifiedStart=Boolean(schedulableActivity&&selectedActivityDateIsFuture(text));
  const clockIntervalMinutes=schedulableActivity?parseExplicitClockIntervalMinutes(text):null;
  const explicitDurationRangeMinutes=(committedDowntime||committedWait||lowValueDeliberation||timedActivity||travel)&&clockIntervalMinutes==null?parseExplicitDurationRangeMinutes(text):null;
  const explicitDurationMinutes=(committedDowntime||committedWait||lowValueDeliberation||timedActivity||travel)?clockIntervalMinutes??(explicitDurationRangeMinutes?null:parseExplicitDurationMinutes(text)):null;
  const precedingActivityMinutes=parsePrecedingExplicitDurationMinutes(text);
  const relativeStartOffsetMinutes=dateQualifiedStart?null:parseRelativeStartOffsetMinutes(text);
  const scheduledStartOffsetMinutes=schedulableActivity?relativeStartOffsetMinutes??parseFutureClockOffsetMinutes(text,currentTime):null;
  const timedRange=(defaultMinutes,defaultMaximum)=>{if(dateQualifiedStart)return{minutes:0,guide:[0,1440]};const activityMinutes=explicitDurationRangeMinutes?.[0]??explicitDurationMinutes??defaultMinutes,offset=scheduledStartOffsetMinutes??precedingActivityMinutes;return{minutes:offset+activityMinutes,guide:explicitDurationRangeMinutes?[offset+explicitDurationRangeMinutes[0],offset+explicitDurationRangeMinutes[1]]:explicitDurationMinutes!=null?[offset+activityMinutes,offset+activityMinutes]:[offset+defaultMinutes,offset+defaultMaximum]};};
  ...
  if(consequential)return{kind:'committed-consequence',timeProfile:'immediate-consequence',semanticTarget:null,compression:false,minAdvanceMinutes:0,suggestedAdvanceMinutes:[0,10],deltaTarget:1,requiresNovelty:false,stopPolicy:'resolve-committed-action-then-meaningful-choice',location,explicitDurationMinutes};
  if(exteriorMatch&&!EXTERIOR_NEGATION_RE.test(text))return{kind:'exit-exterior',timeProfile:'travel-building-exit',semanticTarget:'current-building-exterior',compression:true,minAdvanceMinutes:2,suggestedAdvanceMinutes:[2,10],deltaTarget:2,requiresNovelty:true,stopPolicy:'semantic-destination-or-meaningful-interruption',location,explicitDurationMinutes};
  if(committedDowntime){const sleepLike=SLEEP_ACTION_RE.test(text)&&!SHORT_REST_CUE_RE.test(text);if(dateQualifiedStart)return{kind:'downtime',timeProfile:sleepLike?'date-qualified-sleep':'date-qualified-rest',semanticTarget:'rest-complete',compression:false,minAdvanceMinutes:0,suggestedAdvanceMinutes:[0,1440],boundaryLookaheadMinutes:1440,deltaTarget:0,requiresNovelty:false,stopPolicy:'post-rest-meaningful-state',location,explicitDurationMinutes,explicitDurationRangeMinutes,precedingActivityMinutes,scheduledStartOffsetMinutes,dateQualifiedStart};const activityMinutes=explicitDurationRangeMinutes?.[0]??explicitDurationMinutes??(sleepLike?240:30),offset=scheduledStartOffsetMinutes??precedingActivityMinutes,minutes=offset+activityMinutes,guide=explicitDurationRangeMinutes?[offset+explicitDurationRangeMinutes[0],offset+explicitDurationRangeMinutes[1]]:explicitDurationMinutes!=null?[minutes,minutes]:sleepLike?[offset+240,offset+480]:[offset+30,offset+240];return{kind:'downtime',timeProfile:sleepLike?'sleep':'rest',semanticTarget:'rest-complete',compression:true,minAdvanceMinutes:minutes,suggestedAdvanceMinutes:guide,deltaTarget:2,requiresNovelty:true,stopPolicy:'post-rest-meaningful-state',location,explicitDurationMinutes,explicitDurationRangeMinutes,precedingActivityMinutes,scheduledStartOffsetMinutes,dateQualifiedStart};}
  if(committedWait){if(dateQualifiedStart)return{kind:'wait',timeProfile:'date-qualified-wait',semanticTarget:'time-advanced',compression:false,minAdvanceMinutes:0,suggestedAdvanceMinutes:[0,1440],boundaryLookaheadMinutes:1440,deltaTarget:0,requiresNovelty:false,stopPolicy:'changed-world-or-important-interruption',location,explicitDurationMinutes,explicitDurationRangeMinutes,precedingActivityMinutes,scheduledStartOffsetMinutes,dateQualifiedStart};const activityMinutes=explicitDurationRangeMinutes?.[0]??explicitDurationMinutes??10,offset=scheduledStartOffsetMinutes??precedingActivityMinutes,minutes=offset+activityMinutes,guide=explicitDurationRangeMinutes?[offset+explicitDurationRangeMinutes[0],offset+explicitDurationRangeMinutes[1]]:explicitDurationMinutes!=null?[minutes,minutes]:[offset+10,offset+60];return{kind:'wait',timeProfile:'wait',semanticTarget:'time-advanced',compression:true,minAdvanceMinutes:minutes,suggestedAdvanceMinutes:guide,deltaTarget:1,requiresNovelty:true,stopPolicy:'changed-world-or-important-interruption',location,explicitDurationMinutes,explicitDurationRangeMinutes,precedingActivityMinutes,scheduledStartOffsetMinutes,dateQualifiedStart};}
  if(mealMatch){const {minutes,guide}=timedRange(20,45);return{kind:'meal',timeProfile:dateQualifiedStart?'date-qualified-meal':'meal',semanticTarget:'meal-complete',compression:!dateQualifiedStart,minAdvanceMinutes:minutes,suggestedAdvanceMinutes:guide,boundaryLookaheadMinutes:dateQualifiedStart?1440:0,deltaTarget:dateQualifiedStart?0:1,requiresNovelty:false,stopPolicy:'meal-complete-or-meaningful-interruption',location,explicitDurationMinutes,explicitDurationRangeMinutes,precedingActivityMinutes,scheduledStartOffsetMinutes,dateQualifiedStart};}
  if(trainingMatch){const {minutes,guide}=timedRange(30,120);return{kind:'training',timeProfile:dateQualifiedStart?'date-qualified-training':'training',semanticTarget:'training-session-complete',compression:!dateQualifiedStart,minAdvanceMinutes:minutes,suggestedAdvanceMinutes:guide,boundaryLookaheadMinutes:dateQualifiedStart?1440:0,deltaTarget:dateQualifiedStart?0:1,requiresNovelty:false,stopPolicy:'training-result-or-meaningful-interruption',location,explicitDurationMinutes,explicitDurationRangeMinutes,precedingActivityMinutes,scheduledStartOffsetMinutes,dateQualifiedStart};}
  if(classMatch){const {minutes,guide}=timedRange(45,120);return{kind:'class-attendance',timeProfile:dateQualifiedStart?'date-qualified-class':'class',semanticTarget:'class-session-complete',compression:!dateQualifiedStart,minAdvanceMinutes:minutes,suggestedAdvanceMinutes:guide,boundaryLookaheadMinutes:dateQualifiedStart?1440:0,deltaTarget:dateQualifiedStart?0:1,requiresNovelty:false,stopPolicy:'class-result-or-meaningful-interruption',location,explicitDurationMinutes,explicitDurationRangeMinutes,precedingActivityMinutes,scheduledStartOffsetMinutes,dateQualifiedStart};}
  if(dialogueMatch){const {minutes,guide}=timedRange(2,10);return{kind:'dialogue',timeProfile:dateQualifiedStart?'date-qualified-dialogue':'dialogue',semanticTarget:'conversation-exchange',compression:!dateQualifiedStart,minAdvanceMinutes:minutes,suggestedAdvanceMinutes:guide,boundaryLookaheadMinutes:dateQualifiedStart?1440:0,deltaTarget:dateQualifiedStart?0:1,requiresNovelty:false,stopPolicy:'exchange-complete-or-important-question',location,explicitDurationMinutes,explicitDurationRangeMinutes,precedingActivityMinutes,scheduledStartOffsetMinutes,dateQualifiedStart};}
  ...
  if(travel){const target=cleanTravelTarget(stripScheduledStartExpression(travel[1]).replace(DATE_QUALIFIER_RE,' ')),time=travelTimeProfile(location,target),activityTiming=explicitDurationRangeMinutes?{...time,minAdvanceMinutes:explicitDurationRangeMinutes[0],suggestedAdvanceMinutes:explicitDurationRangeMinutes}:explicitDurationMinutes!=null?{...time,minAdvanceMinutes:explicitDurationMinutes,suggestedAdvanceMinutes:[explicitDurationMinutes,explicitDurationMinutes]}:time,prefixedTiming=precedingActivityMinutes>0?{...activityTiming,minAdvanceMinutes:precedingActivityMinutes+activityTiming.minAdvanceMinutes,suggestedAdvanceMinutes:activityTiming.suggestedAdvanceMinutes.map(value=>precedingActivityMinutes+value)}:activityTiming,offset=scheduledStartOffsetMinutes??0,timing=dateQualifiedStart?{...time,timeProfile:'date-qualified-travel',minAdvanceMinutes:0,suggestedAdvanceMinutes:[0,1440]}:scheduledStartOffsetMinutes!=null?{...activityTiming,minAdvanceMinutes:offset+activityTiming.minAdvanceMinutes,suggestedAdvanceMinutes:activityTiming.suggestedAdvanceMinutes.map(value=>offset+value)}:prefixedTiming;return{kind:'travel',...timing,semanticTarget:target||'declared-destination',compression:!dateQualifiedStart,boundaryLookaheadMinutes:dateQualifiedStart?1440:0,deltaTarget:dateQualifiedStart?0:2,requiresNovelty:false,stopPolicy:'declared-destination-or-meaningful-interruption',location,explicitDurationMinutes,explicitDurationRangeMinutes,precedingActivityMinutes,scheduledStartOffsetMinutes,dateQualifiedStart};}
...
function scheduleActivityCategory(value='',kind='') {
  const text=`${String(kind||'')} ${String(value||'')}`.toLowerCase();
  if(/academic|수업|강의|세미나|실습|오리엔테이션|교육/.test(text))return'class-attendance';
...
function scheduleActivityTokens(value='') {
  const generic=new Set(['academic','class','event','schedule','필수','일정','예정','시작','참석','참여','수업','강의','세미나','실습','오리엔테이션','교육','훈련','연습','수련','단련','대련','식사','아침','점심','저녁','만찬','면담','상담','회의','대화','브리핑','수면','휴식','잠','대기','기다림','잔다','쉰다','휴식한다','기다린다','대기한다','sleep','rest','wait','기사과','마법과','신학부','연금술과','일반과','학부','훈련장','강의실','교실','대강당','본관','기숙사','중앙광장','오전','오후','정오','자정','오늘','내일','모레','이번','다음']);
  return [...new Set((String(value||'').toLowerCase().match(/[가-힣a-z0-9_]+/g)||[]).map(token=>token.replace(/(?:에게서|에게|한테|께서|으로|에서|까지|부터|처럼|보다|에는|에게는|으로는|에서는|은|는|이|가|을|를|와|과|의|에)$/u,'')).filter(token=>token.length>=2&&!/^\d/.test(token)&&!generic.has(token)&&!/(?:한다|했다|듣는다|먹는다|참석한다|참여한다|시작한다|계속한다|마친다)$/.test(token)))];
}
...
export function isRequestedScheduledActivity(saveState={},event={},action='',intent=null,registry={}) {
  const save=object(saveState),row=object(event),sceneIntent=object(intent||classifySceneIntent(action,{location:save?.world?.location||'',currentTime:save?.world?.time||''}));
  if(!['class-attendance','training','meal','dialogue','downtime','wait'].includes(String(sceneIntent.kind||'')))return false;
  ...
  const offset=Number(sceneIntent.scheduledStartOffsetMinutes),dateOffsetDays=selectedActivityDateOffsetDays(action),selectedClock=selectedActivityClock(action),dayStart=dateTimeMinutes(currentDate,'00:00');
  const requestedAt=Number.isFinite(offset)&&offset>0?now+offset:sceneIntent.dateQualifiedStart&&dateOffsetDays>0&&selectedClock&&dayStart!=null?dayStart+dateOffsetDays*1440+selectedClock.minutes:null;
  if(requestedAt==null||at!==requestedAt)return false;
  const eventPrimaryText=[row.id,row.title,row.note].filter(Boolean).join(' '),eventText=[eventPrimaryText,row.location,row.department,scheduleParticipantIdentityText(save,row,registry)].filter(Boolean).join(' '),eventCategory=scheduleActivityCategory(eventPrimaryText,row.kind);
  if(eventCategory&&eventCategory!==sceneIntent.kind)return false;
  if(!eventCategory&&sceneIntent.kind!=='dialogue')return false;
  const requestedTokens=scheduleActivityTokens(activityDurationScope(action)),eventTokens=new Set(scheduleActivityTokens(eventText));
  if(requestedTokens.length)return requestedTokens.every(token=>eventTokens.has(token));
  const candidates=new Set();
  for(const candidate of [...array(save.scheduledEvents),...array(save?.scheduleContext?.upcoming)]){
    if(!candidate||['completed','cancelled'].includes(String(candidate.status||'').trim().toLowerCase())||!isPcRelevantScheduleEvent(save,candidate))continue;
    if(dateTimeMinutes(candidate.date||currentDate,candidate.time)!==at)continue;
    const candidatePrimaryText=[candidate.id,candidate.title,candidate.note].filter(Boolean).join(' '),candidateText=[candidatePrimaryText,candidate.location,candidate.department,scheduleParticipantIdentityText(save,candidate,registry)].filter(Boolean).join(' '),candidateCategory=scheduleActivityCategory(candidatePrimaryText,candidate.kind);
    if(candidateCategory!==sceneIntent.kind)continue;
    candidates.add(`${String(candidate.id||candidate.title||'').trim().toLowerCase()}|${candidate.date||currentDate}|${candidate.time||''}`);
  }
...
export function deriveSceneDelta({ saveState = {}, previousRuntime = {}, turn = {}, nextParticipants = null, action = '' } = {}) {
  ...
  const intent=classifySceneIntent(action,{location:saveState?.world?.location||'',currentTime:saveState?.world?.time||''}),target=Math.max(0,Number(intent.deltaTarget||0)),metTarget=meaningfulStop||target===0||score>=target;
...
export function buildSceneMomentumDirective({ action = '', saveState = {}, registry = {} } = {}) {
  ...
  const intent=classifySceneIntent(action,{location:saveState?.world?.location||'',currentTime:saveState?.world?.time||''}),momentum=object(saveState?.sceneRuntime?.momentum),stall=Math.max(0,Number(momentum.stall_streak||0)),[minMinutes,maxMinutes]=intent.suggestedAdvanceMinutes;
  const boundaryLookahead=Math.max(0,Number(intent.boundaryLookaheadMinutes||0)),scheduleBoundary=(intent.compression&&intent.minAdvanceMinutes>0)||boundaryLookahead>0?nextScheduleBoundaryMinutes(saveState,{futureOnly:true,action,intent,registry}):null;
  ...
  if(intent.explicitDurationMinutes!=null&&!intent.dateQualifiedStart){const includedPreceding=intent.scheduledStartOffsetMinutes==null?Number(intent.precedingActivityMinutes||0):0,declaredTotal=Number(intent.explicitDurationMinutes||0)+includedPreceding;lines.push(`- 명시 시간 규칙: 사용자가 ${declaredTotal}분을 직접 지정했다${includedPreceding>0?`(앞선 행동 ${includedPreceding}분 포함)`:''}. 일반적인 ${intent.kind} 시간 floor로 더 늘리지 말고 이 ${intent.scheduledStartOffsetMinutes!=null?'활동 지속시간':'총시간'} 범위를 우선한다. 단 SCHEDULE_BOUNDARY가 더 짧으면 그 일정 경계가 최우선이다.`);}
  if(intent.explicitDurationRangeMinutes&&!intent.dateQualifiedStart){const [rangeMin,rangeMax]=intent.explicitDurationRangeMinutes,offset=intent.scheduledStartOffsetMinutes??Number(intent.precedingActivityMinutes||0);lines.push(`- 명시 시간 범위 규칙: 사용자가 활동 지속시간을 ${rangeMin}-${rangeMax}분으로 지정했다. 두 끝값을 더하지 말고 시작 대기/앞선 행동 ${offset}분을 포함한 TIME_GUIDE ${offset+rangeMin}-${offset+rangeMax}분 안에서 완료한다. 더 이른 SCHEDULE_BOUNDARY가 있으면 그 경계가 최우선이다.`);}
  if(intent.scheduledStartOffsetMinutes!=null)lines.push(`- 시작 시각 규칙: 사용자가 지정한 시작 시각은 현재로부터 ${intent.scheduledStartOffsetMinutes}분 뒤다. 이 숫자를 활동 지속시간으로 오인하지 말고, 그때까지의 대기/이동과 활동 자체의 자연 소요시간을 합친 TIME_GUIDE 안에서 완료한다.`);
  if(intent.dateQualifiedStart)lines.push(`- 날짜 지정 시작 규칙: 내일/모레/다음 날처럼 현재 날짜를 넘는 활동에는 즉시 활동용 시간 floor를 적용하지 않는다. 한 턴 최대 1440분 안에서 먼저 만나는 권위 있는 일정·예약된 인과 결과에서 멈추고, 경계가 없더라도 요청한 날짜를 오늘로 당기거나 일반 활동 최대치로 잘라 완료하지 않는다.${intent.explicitDurationMinutes!=null?` 지정한 ${intent.explicitDurationMinutes}분은 미래 시작 뒤의 활동 지속시간이다.`:intent.explicitDurationRangeMinutes?` 지정한 ${intent.explicitDurationRangeMinutes[0]}-${intent.explicitDurationRangeMinutes[1]}분은 미래 시작 뒤의 활동 지속시간 범위다.`:''}`);
  ...
  else if(intent.kind==='downtime')lines.push(`- DOWNTIME 규칙: 앉기→눈감기→잠들기 같은 미세 단계를 여러 턴 요구하지 않는다. 충분한 시간을 넘긴 뒤 변화한 상황에서 재개한다. 실제 중단이 없으면 완료 시간과 advance_minutes를 TIME_GUIDE ${minMinutes}-${maxMinutes} 안에 두며, 후속 선택이나 편의상 아침 시각은 범위 이탈 근거가 아니다.`);
