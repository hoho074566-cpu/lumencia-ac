...
export function isRequestedScheduledActivity(saveState={},event={},action='',intent=null,registry={}) {
  ...
  for(const candidateRow of candidateRows){
    ...
    const requestedTokens=scheduleActivityTokens(activityDurationScope(candidateAction)).filter(token=>!pcIdentityTokens.has(token));if(requestedTokens.length){if(requestedTokens.every(token=>eventTokens.has(token)||compactEventIdentity.includes(norm(token).replace(/[\s\p{P}\p{S}]+/gu,''))))return true;continue;}
