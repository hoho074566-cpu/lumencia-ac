...
const DIALOGUE_ACTION_RE = /(?:말을\s*건다|대화한다|이야기한다|질문한다|묻는다|답한다|설명한다|상담한다|논의한다)\s*[.!。！]*$/i;
const MEAL_ACTION_RE = new RegExp(String.raw`(?:(?:아침|점심|저녁|밥|식사)(?:을|를)?\s*(?:${DURATION_PREFIX}\s*(?:동안|만)?\s*)?먹는다|식사(?:를)?\s*(?:${DURATION_PREFIX}\s*(?:동안|만)?\s*)?(?:한다|마친다))\s*[.!。！]*$`,'i');
const TRAINING_ACTION_RE = new RegExp(String.raw`(?:훈련|연습|수련|단련)(?:을|를)?\s*(?:${DURATION_PREFIX}\s*(?:동안|만)?\s*)?(?:한다|시작한다|계속한다|마친다)\s*[.!。！]*$`,'i');
const CLASS_ACTION_RE = new RegExp(String.raw`(?:수업|강의|세미나|실습)(?:에|을|를)?\s*(?:${DURATION_PREFIX}\s*(?:동안|만)?\s*)?(?:참석한다|참여한다|듣는다|수강한다|받는다|시작한다|계속한다|마친다)\s*[.!。！]*$`,'i');
...
function cleanTravelTarget(raw='') {
  ...
  target=target.replace(new RegExp(String.raw`^(?:${DURATION_PREFIX})\s*(?:동안|만)\s*`,'i'),'');
...
function parseExplicitDurationTotal(text='') {
  ...
  const unqualified=String(text||'').replace(temporalQualifier,'').replace(clockMinute,' ');
...
export function classifySceneIntent(action='', { location='', currentTime='' } = {}) {
  ...
  const explicitDurationMinutes=(committedDowntime||committedWait||lowValueDeliberation||timedActivity||travel)?clockIntervalMinutes??parseExplicitDurationMinutes(text):null;
  ...
  const timedRange=(defaultMinutes,defaultMaximum)=>{if(dateQualifiedStart)return{minutes:0,guide:[0,1440]};const activityMinutes=explicitDurationMinutes??defaultMinutes,offset=scheduledStartOffsetMinutes??precedingActivityMinutes;return{minutes:offset+activityMinutes,guide:explicitDurationMinutes!=null?[offset+activityMinutes,offset+activityMinutes]:[offset+defaultMinutes,offset+defaultMaximum]};};
...
export function buildSceneMomentumDirective({ action = '', saveState = {}, registry = {} } = {}) {
  ...
  if(intent.dateQualifiedStart)lines.push(`- 날짜 지정 시작 규칙: 내일/모레/다음 날처럼 현재 날짜를 넘는 활동에는 즉시 활동용 시간 floor를 적용하지 않는다. 한 턴 최대 1440분 안에서 먼저 만나는 권위 있는 일정·예약된 인과 결과에서 멈추고, 경계가 없더라도 요청한 날짜를 오늘로 당기거나 일반 활동 최대치로 잘라 완료하지 않는다.${intent.explicitDurationMinutes!=null?` 지정한 ${intent.explicitDurationMinutes}분은 미래 시작 뒤의 활동 지속시간이다.`:''}`);
