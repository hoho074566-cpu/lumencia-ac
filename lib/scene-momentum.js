...
const CLOCK_EXPRESSION = String.raw`(?:(?:${CLOCK_PERIOD})\s*)?\d{1,2}(?:\s*시(?:\s*(?:\d{1,2}\s*분|반))?|:\d{2})`;
...
function parseExplicitClockIntervalMinutes(text='') {
  ...
  const start=clockExpressionMinutes(startMarker,startParts.hour,startParts.minute),end=clockExpressionMinutes(endMarker,endParts.hour,endParts.minute);
  if(start==null||end==null)return null;
  let duration=end-start;
  if(duration<=0&&((startMarker&&explicitEndMarker)||(!startMarker&&!explicitEndMarker)))duration+=1440;
...
function selectedActivityClock(text='') {
  ...
  const numeric=[...source.matchAll(new RegExp(`(?:^|\\s)(${CLOCK_EXPRESSION})\\s*(?=에|부터|경|쯤|시작|개시)`,'g'))].map(match=>({index:match.index??-1,match}));
