...
function parseFutureClockOffsetMinutes(text='',currentTime='') {
  ...
  const source=String(text);if(/(?:내일|모레|다음\s*날|익일)\s*(?:오전|오후)?\s*\d{1,2}\s*(?:시|:)/.test(source))return null;
...
function scheduleActivityTokens(value='') {
  const generic=new Set(['academic','class','event','schedule','필수','일정','예정','시작','참석','참여','수업','강의','세미나','실습','오리엔테이션','교육','훈련','연습','수련','단련','대련','식사','아침','점심','저녁','만찬','면담','상담','회의','대화','브리핑','기사과','마법과','신학부','연금술과','일반과','학부','훈련장','강의실','교실','대강당','본관','기숙사','중앙광장']);
  return [...new Set((String(value||'').toLowerCase().match(/[가-힣a-z0-9_]+/g)||[]).map(token=>token.replace(/(?:에게서|에게|한테|께서|으로|에서|까지|부터|에는|은|는|이|가|을|를|에)$/u,'')).filter(token=>token.length>=2&&!/^\d/.test(token)&&!generic.has(token)&&!/(?:한다|했다|듣는다|먹는다|참석한다|참여한다|시작한다|계속한다|마친다)$/.test(token)))];
}
