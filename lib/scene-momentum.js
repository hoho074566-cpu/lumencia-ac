...
const DAY_DURATION = String.raw`(?:하루|이틀|${DURATION_NUMBER}\s*일)`;
...
const SHARED_UNIT_DURATION_RANGE = String.raw`${DURATION_NUMBER}\s*(?:에서|~|〜|부터)\s*${DURATION_NUMBER}\s*(?:일|시간|분)(?:\s*까지)?`;
...
function explicitDurationRangeMatch(text='') {
  ...
  const sharedPattern=new RegExp(`(${DURATION_NUMBER})\\s*(?:에서|~|〜|부터)\\s*(${DURATION_NUMBER})\\s*(일|시간|분)(?:\\s*까지)?(?!\\s*(?:전(?:에)?|이전|후(?:에)?|뒤(?:에)?))`),sharedMatch=source.match(sharedPattern);if(!sharedMatch)return null;
