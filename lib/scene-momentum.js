...
function parseExplicitDurationMinutes(text='') {
  ...
  const clockMinute=new RegExp(`(?:^|\\s)(?:오전|오후)?\\s*\\d{1,2}\\s*시\\s*${DURATION_NUMBER}\\s*분(?=\\s*(?:에|부터|경|쯤|시작|개시))`,'g');
...
export function classifySceneIntent(action='', { location='' } = {}) {
  ...
  if(mealMatch){const minutes=explicitDurationMinutes??20;return{kind:'meal',timeProfile:'meal',semanticTarget:'meal-complete',compression:true,minAdvanceMinutes:minutes,suggestedAdvanceMinutes:explicitDurationMinutes!=null?[minutes,minutes]:[20,45],deltaTarget:1,requiresNovelty:false,stopPolicy:'meal-complete-or-meaningful-interruption',location,explicitDurationMinutes};}
  if(trainingMatch){const minutes=explicitDurationMinutes??30;return{kind:'training',timeProfile:'training',semanticTarget:'training-session-complete',compression:true,minAdvanceMinutes:minutes,suggestedAdvanceMinutes:explicitDurationMinutes!=null?[minutes,minutes]:[30,120],deltaTarget:1,requiresNovelty:false,stopPolicy:'training-result-or-meaningful-interruption',location,explicitDurationMinutes};}
  if(classMatch){const minutes=explicitDurationMinutes??45;return{kind:'class-attendance',timeProfile:'class',semanticTarget:'class-session-complete',compression:true,minAdvanceMinutes:minutes,suggestedAdvanceMinutes:explicitDurationMinutes!=null?[minutes,minutes]:[45,120],deltaTarget:1,requiresNovelty:false,stopPolicy:'class-result-or-meaningful-interruption',location,explicitDurationMinutes};}
  if(dialogueMatch){const minutes=explicitDurationMinutes??2;return{kind:'dialogue',timeProfile:'dialogue',semanticTarget:'conversation-exchange',compression:true,minAdvanceMinutes:minutes,suggestedAdvanceMinutes:explicitDurationMinutes!=null?[minutes,minutes]:[2,10],deltaTarget:1,requiresNovelty:false,stopPolicy:'exchange-complete-or-important-question',location,explicitDurationMinutes};}
