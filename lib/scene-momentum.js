...
function parseExplicitClockIntervalTiming(text='',currentTime='', { scopeTerminal = true, currentDate = '' } = {}) {
  ...
  if(!match){
    ...
    const absoluteOffset=absoluteCalendarDateOffsetDays(source,currentDate),relativeQualifier=selectedActivityDateQualifier(source),dateOffset=selectedActivityDateOffsetDays(source,currentDate),explicitDate=absoluteOffset!=null||Boolean(relativeQualifier),elapsedDeadline=Boolean(absoluteOffset<0||(explicitDate&&dateOffset===0&&end<now)),rollover=Boolean(!explicitDate&&((parts.named&&parts.minutes===0)||parts.marker==='새벽'||/^(?:오전|아침)$/.test(parts.marker)&&end<now||(parts.marker==='밤'&&(Number(parts.hour)===12||Number(parts.hour)<=5)))),duration=dateOffset>0?dateOffset*1440+end-now:elapsedDeadline?0:end-now+(rollover&&end<=now?1440:0);
