...
const DAY_DURATION = String.raw`(?:하루|이틀|${DURATION_NUMBER}\s*일(?!\s*(?:에(?!서)|차|째|자|날)))`;
const DURATION_ATOM = String.raw`(?:${DAY_DURATION}|${DURATION_NUMBER}\s*시간(?:(?:\s*${DURATION_NUMBER}\s*분)|(?:\s*반))?|${DURATION_NUMBER}\s*분)`;
const DURATION_PREFIX = String.raw`(?:${DURATION_ATOM}|몇\s*분|잠시|잠깐|좀)`;
const SHARED_UNIT_DURATION_RANGE = String.raw`${DURATION_NUMBER}\s*(?:에서|~|〜|부터)\s*${DURATION_NUMBER}\s*(?:일(?!\s*(?:에(?!서)|차|째|자|날))|시간|분)(?:\s*까지)?`;
...
function durationExpressionMinutes(text='') {
  ...
  if(source==='하루')return 1440;
  if(source==='이틀')return 2880;
  const dayMatch=source.match(new RegExp(`^(${DURATION_NUMBER})\\s*일$`));
  if(dayMatch){const days=durationNumber(dayMatch[1]),total=Math.round(Number(days||0)*1440);return Number.isFinite(total)?Math.max(0,total):null;}
...
function maskCalendarDayExpressions(text='') {
  return String(text||'').replace(/(?:\d{1,4}\s*년\s*)?\d{1,2}\s*월\s*\d{1,2}\s*일(?=\s*(?:에|에는|엔|동안|$))/g,' ');
}
function explicitDurationRangeMatch(text='') {
  const source=maskCalendarDayExpressions(text),fullPattern=new RegExp(`(${DURATION_ATOM})\\s*(?:에서|~|〜|부터)\\s*(${DURATION_ATOM})(?:\\s*까지)?(?!\\s*(?:전(?:에)?|이전|후(?:에)?|뒤(?:에)?))`),fullMatch=source.match(fullPattern);
  ...
  const sharedPattern=new RegExp(`(${DURATION_NUMBER})\\s*(?:에서|~|〜|부터)\\s*(${DURATION_NUMBER})\\s*(일(?!\\s*(?:에(?!서)|차|째|자|날))|시간|분)(?:\\s*까지)?(?!\\s*(?:전(?:에)?|이전|후(?:에)?|뒤(?:에)?))`),sharedMatch=source.match(sharedPattern);if(!sharedMatch)return null;
  const multiplier=sharedMatch[3]==='일'?1440:sharedMatch[3]==='시간'?60:1,first=durationNumber(sharedMatch[1]),second=durationNumber(sharedMatch[2]);if(first==null||second==null)return null;
...
function parseExplicitDurationBounds(text='') {
  ...
  let unqualified=maskCalendarDayExpressions(text).replace(temporalQualifier,'').replace(clockMinute,' '),minimum=0,maximum=0,found=false,rangeMatch;
...
function parseRelativeStartOffsetMinutes(text='', { scopeTerminal = true } = {}) {
  const source=scopeTerminal?activityDurationScope(text):String(text||''),matches=[...source.matchAll(new RegExp(`(${DURATION_ATOM})\\s*(?:후|뒤|이후)(?:에)?`,'g'))],match=matches.at(-1);
  if(!match)return null;
  const total=durationExpressionMinutes(match[1]);
  return total>0?total:null;
}
function parsePrecedingExplicitDurationBounds(text='',currentTime='') {
  const source=String(text||'');let boundary=0,minimum=0,maximum=0,matched=false;
  PRIOR_ACTIVITY_CONNECTOR_RE.lastIndex=0;
  const connectors=[...source.matchAll(PRIOR_ACTIVITY_CONNECTOR_RE)];
  PRIOR_ACTIVITY_CONNECTOR_RE.lastIndex=0;
  for(const match of connectors){
    const end=match.index+match[0].length,segment=source.slice(boundary,end),explicit=parseExplicitDurationBounds(segment),defaults=/(?:훈련|연습|수련|단련)/.test(match[0])?[30,120]:/(?:수업|강의|세미나|실습|오리엔테이션|교육|입학식)/.test(match[0])?[45,120]:/(?:아침|점심|저녁|밥|식사)/.test(match[0])?[20,45]:/(?:대화|이야기|질문|답변|설명|상담|논의|면담|회의|브리핑)/.test(match[0])?[2,10]:/(?:잠을|잔\s*(?:뒤|후))/.test(match[0])?[240,480]:/(?:휴식|쉬고|쉰\s*(?:뒤|후))/.test(match[0])?[30,240]:/(?:대기|기다리고)/.test(match[0])?[10,60]:/(?:(?:으로|로|에)\s*(?:이동하고|도착하고|나가서|들어가서|가서)|방을\s*잡고)/.test(segment)?[3,30]:[0,0],bounds=explicit||defaults,relativeStart=parseRelativeStartOffsetMinutes(segment,{scopeTerminal:false}),dateStart=selectedActivityDateIsFuture(segment)?parseDateQualifiedStartOffsetMinutes(segment,currentTime,{scopeTerminal:false}):null,clockStart=parseFutureClockOffsetMinutes(segment,currentTime,{scopeTerminal:false}),startOffset=relativeStart??dateStart??clockStart;
...
function parseExplicitClockIntervalTiming(text='',currentTime='') {
  ...
  const now=clockMinutes(currentTime);if(now==null||selectedActivityDateIsFuture(text))return{minutes:duration,startOffsetSuppressed:false};
  const endAbsolute=start+duration;if(now>=start&&now<endAbsolute)return{minutes:endAbsolute-now,startOffsetSuppressed:true};
  if(parseFutureClockOffsetMinutes(text,currentTime)!=null||now<start)return{minutes:duration,startOffsetSuppressed:false};
...
function parseFutureClockOffsetMinutes(text='',currentTime='', { scopeTerminal = true } = {}) {
...
function hasElapsedTodayStart(text='',currentTime='') {
  const now=clockMinutes(currentTime),selected=selectedActivityClock(text);
  return selectedActivityDateQualifier(text)==='오늘'&&now!=null&&selected!=null&&!selected.rollover&&selected.minutes<now;
}
function parseDateQualifiedStartOffsetMinutes(text='',currentTime='', { scopeTerminal = true } = {}) {
  const now=clockMinutes(currentTime),dateOffsetDays=selectedActivityDateOffsetDays(text),selected=selectedActivityClock(text,{scopeTerminal});
...
function hasThirdPartyTimedSubject(text='',actorName='') {
  ...
  const activityScoped=activityDurationScope(raw),causalMatches=[...activityScoped.matchAll(/(?:[가-힣]{1,24}(?:아서|어서|해서|라서|여서|와서)|때문에)\s+/gu)],causalMatch=causalMatches.at(-1),causalScoped=Boolean(causalMatch),scoped=causalMatch?activityScoped.slice((causalMatch.index??0)+causalMatch[0].length):activityScoped,allowed=new Set(['나','난','내','저','제','우리','우린','저희','pc','player','aaa',norm(actorName)].filter(Boolean));
  ...
  const inheritedSubject=scoped===raw||causalScoped?'':subjectFrom(raw);return Boolean(inheritedSubject&&!allowed.has(inheritedSubject));
}
...
export function classifySceneIntent(action='', { location='', currentTime='', actorName='' } = {}) {
  ...
  const exteriorMatch=EXTERIOR_RE.test(text),lowValueDeliberation=isLowValueDeliberation(text),travelDeliberation=TRAVEL_DELIBERATION_RE.test(text),committedDowntime=!thirdPartyTimedSubject&&!consequential&&hasCommittedDowntime(text),committedWait=!thirdPartyTimedSubject&&hasCommittedWait(text),exploreMatch=EXPLORE_RE.test(text),observeMatch=OBSERVE_RE.test(text),dialogueMatch=!thirdPartyTimedSubject&&DIALOGUE_ACTION_RE.test(text),mealMatch=!thirdPartyTimedSubject&&(MEAL_ACTION_RE.test(text)||(EATING_ACTION_RE.test(text)&&parseExplicitDurationBounds(activityDurationScope(text))!==null)),trainingMatch=!thirdPartyTimedSubject&&TRAINING_ACTION_RE.test(text),classMatch=!thirdPartyTimedSubject&&CLASS_ACTION_RE.test(text),travel=thirdPartyTimedSubject?null:text.match(TRAVEL_RE);
  ...
  const clockIntervalTiming=schedulableActivity?parseExplicitClockIntervalTiming(text,currentTime):null,clockIntervalMinutes=clockIntervalTiming?.minutes??null;
  const elapsedTodayStart=Boolean(schedulableActivity&&!clockIntervalTiming?.startOffsetSuppressed&&hasElapsedTodayStart(text,currentTime));
...
export function buildSceneMomentumDirective({ action = '', saveState = {}, registry = {} } = {}) {
  ...
  if(intent.scheduledStartOffsetMinutes!=null)lines.push(intent.scheduledStartOverrun?`- 시작 시각 충돌 규칙: 앞선 행동의 최소 소요시간이 지정 시작 시각 ${intent.scheduledStartOffsetMinutes}분을 넘는다. 시간을 겹쳐 완료하거나 시작 시각을 무시하지 말고, TIME_GUIDE의 해당 시작 경계에서 앞선 행동과 뒤 활동 모두 끝나지 않은 상태로 멈춘다.`:intent.turnLimitTruncated?`- 시작 시각 규칙: 사용자가 지정한 시작 시각은 현재로부터 ${intent.scheduledStartOffsetMinutes}분 뒤다. 이 숫자를 활동 지속시간으로 오인하지 말고 그때까지 대기/이동을 처리하되, TIME_GUIDE의 1440분에 닿으면 활동을 완료하지 않은 채 멈춘다.`:`- 시작 시각 규칙: 사용자가 지정한 시작 시각은 현재로부터 ${intent.scheduledStartOffsetMinutes}분 뒤다. 이 숫자를 활동 지속시간으로 오인하지 말고, 그때까지의 대기/이동과 활동 자체의 자연 소요시간을 합친 TIME_GUIDE 안에서 완료한다.`);
