...
const DURATION_ACTIVITY_SUFFIX = String.raw`(?:동안|만|간|가량|정도|쯤|${STRICT_DURATION_QUALIFIER})`;
...
function parseStrictDurationLowerBoundMinutes(text='') {
  const source=maskCalendarDayExpressions(activityDurationScope(text)),matches=[...source.matchAll(new RegExp(`(${DURATION_ATOM})\\s*${STRICT_DURATION_QUALIFIER}`,'g'))],match=matches.at(-1);
  if(!match)return null;
  const minutes=durationExpressionMinutes(match[1]);
...
function parsePrecedingExplicitDurationBounds(text='',currentTime='',actorName='',currentDate='') {
  ...
  for(const match of connectors){
    const end=match.index+match[0].length,segment=source.slice(boundary,end),explicit=parseExplicitDurationBounds(segment),strictLowerBound=parseStrictDurationLowerBoundMinutes(segment),clockInterval=parseExplicitClockIntervalTiming(segment,currentTime,{scopeTerminal:false,currentDate}),defaults=/(?:훈련|연습|수련|단련)/.test(match[0])?[30,120]:/(?:수업|강의|세미나|실습|오리엔테이션|교육|입학식)/.test(match[0])?[45,120]:/(?:아침|점심|저녁|밥|식사)/.test(match[0])?[20,45]:/(?:대화|이야기|질문|답변|설명|상담|논의|면담|회의|브리핑|협상)/.test(match[0])?[2,10]:/(?:잠을|잔\s*(?:뒤|후))/.test(match[0])?[240,480]:/(?:휴식|쉬고|쉰\s*(?:뒤|후))/.test(match[0])?[30,240]:/(?:대기|기다리고)/.test(match[0])?[10,60]:/(?:(?:으로|로|에)\s*(?:이동하고|도착하고|나가서|들어가서|가서)|방을\s*잡고)/.test(segment)?[3,30]:[0,0],strictMinimum=strictLowerBound==null?null:strictLowerBound+1,bounds=clockInterval?[clockInterval.minutes,clockInterval.minutes]:strictMinimum==null?explicit||defaults:[strictMinimum,Math.max(strictMinimum,defaults[1])],relativeStart=parseRelativeStartOffsetMinutes(segment,{scopeTerminal:false}),dateStart=selectedActivityDateIsFuture(segment,currentDate)?parseDateQualifiedStartOffsetMinutes(segment,currentTime,{scopeTerminal:false,currentDate}):null,clockStart=parseFutureClockOffsetMinutes(segment,currentTime,{scopeTerminal:false,currentDate}),startOffset=relativeStart??dateStart??clockStart;
...
function parseExplicitClockIntervalTiming(text='',currentTime='', { scopeTerminal = true, currentDate = '' } = {}) {
  ...
  if(!match){
    ...
    const absoluteOffset=absoluteCalendarDateOffsetDays(source,currentDate),relativeQualifier=selectedActivityDateQualifier(source),dateOffset=selectedActivityDateOffsetDays(source,currentDate),explicitDate=absoluteOffset!=null||Boolean(relativeQualifier),elapsedDeadline=Boolean(absoluteOffset<0||(explicitDate&&dateOffset===0&&end<now)),rollover=Boolean(!explicitDate&&((parts.named&&parts.minutes===0)||parts.marker==='새벽'||(parts.marker==='밤'&&(Number(parts.hour)===12||Number(parts.hour)<=5)))),duration=dateOffset>0?dateOffset*1440+end-now:elapsedDeadline?0:end-now+(rollover&&end<=now?1440:0);
...
export function classifySceneIntent(action='', { location='', currentTime='', currentDate='', actorName='' } = {}) {
  ...
  const elapsedScheduledStart=Boolean(schedulableActivity&&(clockIntervalTiming?.elapsedDeadline||!clockIntervalTiming?.startOffsetSuppressed&&hasElapsedScheduledDateStart(selectedActivityScope,currentTime,currentDate)));
  const strictDurationLowerBoundMinutes=(committedDowntime||committedWait||lowValueDeliberation||timedActivity||travel)&&clockIntervalMinutes==null?parseStrictDurationLowerBoundMinutes(text):null;
...
export function buildSceneMomentumDirective({ action = '', saveState = {}, registry = {} } = {}) {
  ...
  if(intent.strictDurationLowerBoundMinutes!=null&&!intent.turnLimitTruncated)lines.push(`- 엄격한 최소시간 규칙: 사용자는 ${intent.strictDurationLowerBoundMinutes}분을 초과하는 활동을 요청했다. TIME_GUIDE 안에서 처리하되 정확히 ${intent.strictDurationLowerBoundMinutes}분에 완료시키지 않는다.`);
