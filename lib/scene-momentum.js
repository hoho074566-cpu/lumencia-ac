...
const SHARED_UNIT_DURATION_RANGE = String.raw`${DURATION_NUMBER}\s*(?:에서|~|〜|부터)\s*${DURATION_NUMBER}\s*(?:시간|분)(?:\s*까지)?`;
...
function explicitDurationRangeMatch(text='') {
  const source=String(text||''),fullPattern=new RegExp(`(${DURATION_ATOM})\\s*(?:에서|~|〜|부터)\\s*(${DURATION_ATOM})(?:\\s*까지)?(?!\\s*(?:전(?:에)?|이전|후(?:에)?|뒤(?:에)?))`),fullMatch=source.match(fullPattern);
  ...
  const sharedPattern=new RegExp(`(${DURATION_NUMBER})\\s*(?:에서|~|〜|부터)\\s*(${DURATION_NUMBER})\\s*(시간|분)(?:\\s*까지)?(?!\\s*(?:전(?:에)?|이전|후(?:에)?|뒤(?:에)?))`),sharedMatch=source.match(sharedPattern);if(!sharedMatch)return null;
  const multiplier=sharedMatch[3]==='시간'?60:1,first=durationNumber(sharedMatch[1]),second=durationNumber(sharedMatch[2]);if(first==null||second==null)return null;
...
function hasThirdPartyTimedSubject(text='',actorName='') {
  ...
  const subjectFrom=(value)=>{let source=stripScheduledStartExpression(value).replace(DATE_QUALIFIER_RE,' ').trim();DATE_QUALIFIER_RE.lastIndex=0;source=source.replace(/^.{1,48}?(?:와는|과는|에게는|한테는|께는|에서는|으로는|로는|에는)\s+/u,'');const matches=[...source.matchAll(/(?:^|[\s,])([^\s,]{1,48}?)(가|이|은|는)\s+/gu)].filter(match=>!(match[2]==='는'&&/(?:다|하|되|겠|었|았|했)$/u.test(match[1]))),subject=norm(matches.at(-1)?.[1]||'');return activityTopics.has(subject)?'':subject;};
...
export function classifySceneIntent(action='', { location='', currentTime='', actorName='' } = {}) {
  ...
  const capTimedIntent=(value)=>{const guide=array(value?.suggestedAdvanceMinutes),rawMinimum=Math.max(0,Number(value?.minAdvanceMinutes||0)),rawLower=Math.max(0,Number(guide[0]||0)),rawUpper=Math.max(rawLower,Number(guide[1]||0)),turnLimitTruncated=Boolean(value?.turnLimitTruncated)||rawMinimum>1440||rawLower>1440||rawUpper>1440;return{...value,minAdvanceMinutes:Math.min(1440,rawMinimum),suggestedAdvanceMinutes:[Math.min(1440,rawLower),Math.min(1440,rawUpper)],turnLimitTruncated};};
  const timedRange=(defaultMinutes,defaultMaximum)=>{const activityMinutes=explicitDurationRangeMinutes?.[0]??explicitDurationMinutes??defaultMinutes,reachableDateOffset=dateQualifiedStart&&dateQualifiedStartOffsetMinutes!=null&&dateQualifiedStartOffsetMinutes+activityMinutes<=1440?dateQualifiedStartOffsetMinutes:null;if(dateQualifiedStart&&reachableDateOffset==null)return{minutes:0,guide:[0,1440],effectiveStartOffset:null,futureReachable:false,scheduledStartOverrun:false};const effectiveStartOffset=reachableDateOffset??scheduledStartOffsetMinutes,scheduledStartOverrun=effectiveStartOffset!=null&&precedingActivityMinimum>effectiveStartOffset,offsetMinimum=scheduledStartOverrun?precedingActivityMinimum:effectiveStartOffset??precedingActivityMinimum,offsetMaximum=scheduledStartOverrun?precedingActivityMinutes:effectiveStartOffset??precedingActivityMinutes,rawGuide=explicitDurationRangeMinutes?[offsetMinimum+explicitDurationRangeMinutes[0],offsetMaximum+explicitDurationRangeMinutes[1]]:explicitDurationMinutes!=null?[offsetMinimum+activityMinutes,offsetMaximum+activityMinutes]:[offsetMinimum+defaultMinutes,offsetMaximum+defaultMaximum],guide=dateQualifiedStart?[rawGuide[0],Math.min(1440,rawGuide[1])]:rawGuide;return{minutes:offsetMinimum+activityMinutes,guide,effectiveStartOffset,futureReachable:Boolean(dateQualifiedStart&&reachableDateOffset!=null),scheduledStartOverrun};};
...
function scheduleActivityTokens(value='') {
  const generic=new Set(['academic','class','event','schedule','필수','일정','예정','시작','참석','참여','수업','강의','세미나','실습','오리엔테이션','교육','입학식','훈련','연습','수련','단련','대련','식사','아침','점심','저녁','만찬','면담','상담','회의','대화','브리핑','수면','휴식','잠','대기','기다림','잔다','쉰다','휴식한다','기다린다','대기한다','sleep','rest','wait','기사과','마법과','신학부','연금술과','일반과','학부','훈련장','강의실','교실','대강당','본관','기숙사','중앙광장','오전','오후','정오','자정','오늘','내일','모레','이번','다음','시간','분','동안','정도','가량','쯤','약','만']);
...
function precedingCommittedActivityClauses(action='') {
  const source=String(action||''),clauses=[],connectorRe=/(듣고|참석하고|참여하고|수강하고|받고|훈련하고|연습하고|수련하고|단련하고|먹고|식사하고|대화하고|이야기하고|면담하고|회의하고|자고|쉬고|휴식하고|기다리고|대기하고)(?=\s+)/g,terminalize=(value)=>value==='듣고'?'듣는다':value==='먹고'?'먹는다':value==='자고'?'잔다':value==='쉬고'?'쉰다':value==='기다리고'?'기다린다':value.endsWith('하고')?`${value.slice(0,-2)}한다`:value;
  let cursor=0,match;while((match=connectorRe.exec(source))){const clause=`${source.slice(cursor,match.index)}${terminalize(match[1])}`.trim();if(clause)clauses.push(clause);cursor=match.index+match[0].length;}
  return clauses;
}
export function isRequestedScheduledActivity(saveState={},event={},action='',intent=null,registry={}) {
  ...
  const pcIdentityTokens=new Set(scheduleActivityTokens(save?.pc?.name||'')),eventTokens=new Set(scheduleActivityTokens(eventText)),dayStart=dateTimeMinutes(currentDate,'00:00'),candidateRows=[{action:String(action||''),intent:sceneIntent},...precedingCommittedActivityClauses(action).map(candidateAction=>({action:candidateAction,intent:classifySceneIntent(candidateAction,{location:save?.world?.location||'',currentTime:save?.world?.time||'',actorName:save?.pc?.name||''})}))];
  for(const candidateRow of candidateRows){
    const candidateAction=candidateRow.action,candidateIntent=object(candidateRow.intent);if(candidateIntent.scheduledStartOverrun||!['class-attendance','training','meal','dialogue','downtime','wait'].includes(String(candidateIntent.kind||'')))continue;
    if(eventCategory&&eventCategory!==candidateIntent.kind)continue;if(!eventCategory&&candidateIntent.kind!=='dialogue')continue;
    const offset=Number(candidateIntent.scheduledStartOffsetMinutes),dateOffsetDays=selectedActivityDateOffsetDays(candidateAction),selectedClock=selectedActivityClock(candidateAction),requestedAt=Number.isFinite(offset)&&offset>0?now+offset:candidateIntent.dateQualifiedStart&&dateOffsetDays>0&&selectedClock&&dayStart!=null?dayStart+dateOffsetDays*1440+selectedClock.minutes:null;if(requestedAt==null||at!==requestedAt)continue;
    const requestedTokens=scheduleActivityTokens(activityDurationScope(candidateAction)).filter(token=>!pcIdentityTokens.has(token));if(requestedTokens.length){if(requestedTokens.every(token=>eventTokens.has(token)))return true;continue;}
...
export function buildSceneMomentumDirective({ action = '', saveState = {}, registry = {} } = {}) {
  ...
  if(intent.scheduledStartOverrun)lines.push(`- 시작 시각 충돌 규칙: 앞선 행동의 최소 소요시간이 지정 시작 시각 ${intent.scheduledStartOffsetMinutes}분을 넘는다. 시간을 겹쳐 완료하거나 시작 시각을 무시하지 말고, 앞선 행동이 끝나지 않은 채 해당 일정 시작 경계에서 멈춘다.`);
