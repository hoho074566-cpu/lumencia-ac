...
const DATE_QUALIFIER_RE = /(?:^|[\s,])(오늘|내일|모레|다음\s*날|익일)(?=\s|[,.!?。！？]|$)/gi;
const QUOTED_SEGMENT_RE = /“[^”]*”|‘[^’]*’|"[^"]*"|'[^']*'|「[^」]*」|『[^』]*』/g;
const ATTRIBUTED_SPEECH_RE = /(?:^|[\s,])(?:누군가|그가|그녀가|(?!(?:나|내)(?:가|는|은)(?=\s))[가-힣]{1,16}(?:가|이|은|는))(?=\s)[^.!?。！？]{0,40}?(?:(?:겠다고|한다고|라고)\s*(?:외치|말하|소리치)[가-힣]{0,8}|(?:겠다는|한다는|라는)\s*(?:말|소리|이야기|얘기|소문|전언|보고|발언|위협|주장|사실|내용|계획|의도|것|걸)(?:을|를)?\s*(?:듣|접하|전해\s*듣|확인|알게\s*되|알고)[가-힣]{0,8})/gi;
...
export function classifySceneIntent(action='', { location='', currentTime='' } = {}) {
  ...
  const timedActivity=dialogueMatch||mealMatch||trainingMatch||classMatch;
  const dateQualifiedStart=Boolean((timedActivity||travel)&&selectedActivityDateIsFuture(text));
  const clockIntervalMinutes=(timedActivity||travel)?parseExplicitClockIntervalMinutes(text):null;
  ...
  const scheduledStartOffsetMinutes=(timedActivity||travel)?parseFutureClockOffsetMinutes(text,currentTime):null;
  ...
  if(committedDowntime){const sleepLike=SLEEP_ACTION_RE.test(text)&&!SHORT_REST_CUE_RE.test(text),activityMinutes=explicitDurationMinutes??(sleepLike?240:30),minutes=precedingActivityMinutes+activityMinutes,guide=explicitDurationMinutes!=null?[minutes,minutes]:sleepLike?[precedingActivityMinutes+240,precedingActivityMinutes+480]:[precedingActivityMinutes+30,precedingActivityMinutes+240];return{kind:'downtime',timeProfile:sleepLike?'sleep':'rest',semanticTarget:'rest-complete',compression:true,minAdvanceMinutes:minutes,suggestedAdvanceMinutes:guide,deltaTarget:2,requiresNovelty:true,stopPolicy:'post-rest-meaningful-state',location,explicitDurationMinutes,precedingActivityMinutes};}
  if(committedWait){const activityMinutes=explicitDurationMinutes??10,minutes=precedingActivityMinutes+activityMinutes;return{kind:'wait',timeProfile:'wait',semanticTarget:'time-advanced',compression:true,minAdvanceMinutes:minutes,suggestedAdvanceMinutes:explicitDurationMinutes!=null?[minutes,minutes]:[precedingActivityMinutes+10,precedingActivityMinutes+60],deltaTarget:1,requiresNovelty:true,stopPolicy:'changed-world-or-important-interruption',location,explicitDurationMinutes,precedingActivityMinutes};}
...
function scheduleActivityTokens(value='') {
  const generic=new Set(['academic','class','event','schedule','필수','일정','예정','시작','참석','참여','수업','강의','세미나','실습','오리엔테이션','교육','훈련','연습','수련','단련','대련','식사','아침','점심','저녁','만찬','면담','상담','회의','대화','브리핑','기사과','마법과','신학부','연금술과','일반과','학부','훈련장','강의실','교실','대강당','본관','기숙사','중앙광장','오전','오후','정오','자정','오늘','내일','모레','이번','다음']);
...
export function isRequestedScheduledActivity(saveState={},event={},action='',intent=null,registry={}) {
  ...
  if(!['class-attendance','training','meal','dialogue'].includes(String(sceneIntent.kind||'')))return false;
