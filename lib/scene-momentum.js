...
function parseExplicitDurationRangeMinutes(text='') { const scope=activityDurationScope(text);return durationUpperBoundMatches(scope).length?parseExplicitDurationBounds(scope):explicitDurationRangeMatch(scope)?.range||null; }
function parseExplicitDurationUpperBoundMinutes(text='') { const scope=activityDurationScope(text);return durationUpperBoundMatches(scope).length?parseExplicitDurationBounds(scope)?.[1]??null:null; }
