...
export function classifySceneIntent(action='', { location='', currentTime='' } = {}) {
  ...
  const explicitDurationMinutes=(committedDowntime||committedWait||lowValueDeliberation||timedActivity||travel)?parseExplicitDurationMinutes(text):null;
  const scheduledStartOffsetMinutes=timedActivity?parseFutureClockOffsetMinutes(text,currentTime):null;
  ...
  if(travel){const target=cleanTravelTarget(travel[1]),time=travelTimeProfile(location,target),timing=explicitDurationMinutes!=null?{...time,minAdvanceMinutes:explicitDurationMinutes,suggestedAdvanceMinutes:[explicitDurationMinutes,explicitDurationMinutes]}:time;return{kind:'travel',...timing,semanticTarget:target||'declared-destination',compression:true,deltaTarget:2,requiresNovelty:false,stopPolicy:'declared-destination-or-meaningful-interruption',location,explicitDurationMinutes};}
