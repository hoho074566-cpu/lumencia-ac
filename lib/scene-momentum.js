...
const TRAINING_ACTION_RE = /(?:훈련|연습|수련|단련)(?:을|를)?\s*(?:한다|시작한다|계속한다|마친다)\s*[.!。！]*$/i;
...
function stripScheduledStartExpression(text='') {
  ...
  return String(text||'').replace(new RegExp(String.raw`(?:^|\s)${clock}\s*부터\s*${clock}\s*까지(?=\s|$)`,'g'),' ').replace(new RegExp(String.raw`(?:^|\s)${clock}\s*(?:에|부터|경|쯤)(?=\s|$)`,'g'),' ').replace(new RegExp(String.raw`(?:^|\s)${clock}(?=\s|$)`,'g'),' ').trim().replace(/(?:으로|로|에)\s*$/,'');
}
...
export function classifySceneIntent(action='', { location='', currentTime='' } = {}) {
  ...
  const scheduledStartOffsetMinutes=schedulableActivity?parseFutureClockOffsetMinutes(text,currentTime):null;
...
export function isRequestedScheduledActivity(saveState={},event={},action='',intent=null,registry={}) {
  ...
  const eventText=[row.id,row.title,row.note,row.location,row.department,scheduleParticipantIdentityText(save,row,registry)].filter(Boolean).join(' '),eventCategory=scheduleActivityCategory(eventText,row.kind);
  ...
  for(const candidate of [...array(save.scheduledEvents),...array(save?.scheduleContext?.upcoming)]){
    ...
    const candidateText=[candidate.id,candidate.title,candidate.note,candidate.location,candidate.department,scheduleParticipantIdentityText(save,candidate,registry)].filter(Boolean).join(' '),candidateCategory=scheduleActivityCategory(candidateText,candidate.kind);
