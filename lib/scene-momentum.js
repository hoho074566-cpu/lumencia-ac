...
export function nextScheduleBoundaryMinutes(saveState={}) {
...
export function buildSceneMomentumDirective({ action = '', saveState = {} } = {}) {
  ...
  const scheduleBoundary=intent.compression&&intent.minAdvanceMinutes>0?nextScheduleBoundaryMinutes(saveState):null;
  const boundedBySchedule=scheduleBoundary!=null&&scheduleBoundary<intent.minAdvanceMinutes;
  ...
  if(boundedBySchedule)lines.push(`SCHEDULE_BOUNDARY=${scheduleBoundary}min`,`- 일정 경계 우선: 사용자가 선언한 ${intent.minAdvanceMinutes}분 ${intent.kind==='downtime'?'휴식':intent.kind==='wait'?'대기':'행동'}을 전부 실행하지 말고 ${scheduleBoundary}분 뒤 일정 시작 순간에서 멈춘다. 그 뒤 시간을 자동 진행하거나 일정 불참·완료를 대신 결정하지 않는다. 해당 일정과 현재 상황을 제시한 뒤 플레이어가 반응할 수 있게 한다.`);
