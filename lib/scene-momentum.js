...
export function classifySceneIntent(action='', { location='', currentTime='', actorName='' } = {}) {
  ...
  const selectedActivityScope=activityDurationScope(text),dateQualifiedStart=Boolean(schedulableActivity&&selectedActivityDateIsFuture(selectedActivityScope));
  ...
  const elapsedTodayStart=Boolean(schedulableActivity&&!clockIntervalTiming?.startOffsetSuppressed&&hasElapsedTodayStart(selectedActivityScope,currentTime));
  ...
  const dateQualifiedStartOffsetMinutes=dateQualifiedStart?parseDateQualifiedStartOffsetMinutes(selectedActivityScope,currentTime):null;
