...
export function classifySceneIntent(action='', { location='', currentTime='' } = {}) {
  ...
  const explicitDurationMinutes=(committedDowntime||committedWait||lowValueDeliberation||timedActivity)?parseExplicitDurationMinutes(text):null;
  ...
  if(travel){const target=cleanTravelTarget(travel[1]),time=travelTimeProfile(location,target);return{kind:'travel',...time,semanticTarget:target||'declared-destination',compression:true,deltaTarget:2,requiresNovelty:false,stopPolicy:'declared-destination-or-meaningful-interruption',location,explicitDurationMinutes};}
