...
const PRIOR_ACTIVITY_CONNECTOR_RE = /(?:(?:훈련|연습|수련|단련)(?:을|를)?\s*(?:하고|한\s*(?:뒤|후)|마치고)|(?:수업|강의|세미나|실습)(?:에|을|를)?\s*(?:참석하고|참여하고|듣고|수강하고|받고|마치고|참석한\s*(?:뒤|후)|들은\s*(?:뒤|후))|(?:아침|점심|저녁|밥|식사)(?:을|를)?\s*(?:먹고|마치고)|(?:대화|이야기|질문|답변|설명|상담|논의|면담|회의|브리핑)(?:을|를)?\s*(?:하고|한\s*(?:뒤|후)|마치고)|(?:잠을\s*자고|잠을\s*잔\s*(?:뒤|후)|쉬고|쉰\s*(?:뒤|후)|휴식하고|기다리고|대기하고)|(?:이동하고|도착하고|나가서|들어가서|가서|와서|방을\s*잡고))/gi;
...
function parseExplicitClockIntervalMinutes(text='') {
  const source=activityDurationScope(text),match=source.match(/(?:^|\s)(오전|오후|아침|저녁|밤)?\s*(\d{1,2})(?:\s*시(?:\s*(\d{1,2})\s*분)?|:(\d{2}))\s*부터\s*(오전|오후|아침|저녁|밤)?\s*(\d{1,2})(?:\s*시(?:\s*(\d{1,2})\s*분)?|:(\d{2}))\s*까지/);
  if(!match)return null;
...
function stripScheduledStartExpression(text='') {
  const clock=String.raw`(?:정오|자정|(?:오전|오후|아침|저녁|밤)?\s*\d{1,2}(?:\s*시(?:\s*\d{1,2}\s*분)?|:\d{2}))`;
...
function selectedActivityClock(text='') {
  ...
  const numeric=[...source.matchAll(/(?:^|\s)(오전|오후|아침|저녁|밤)?\s*(\d{1,2})(?:\s*시(?:\s*(\d{1,2})\s*분)?|:(\d{2}))\s*(?=에|부터|경|쯤|시작|개시)/g)].map(match=>({index:match.index??-1,match}));
