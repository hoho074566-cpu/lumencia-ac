...
function explicitDurationRangeMatch(text='') {
  const source=maskCalendarDayExpressions(text),nonActivitySuffix=String.raw`(?:${TEMPORAL_OFFSET_QUALIFIER}(?:전(?:에)?|이전|후(?:에)?|뒤(?:에)?)|마다|간격(?:으로)?|씩)`,fullPattern=new RegExp(`(${DURATION_ATOM})\\s*(?:에서|~|〜|부터)\\s*(${DURATION_ATOM})(?:\\s*까지)?(?!\\s*${nonActivitySuffix})`),fullMatch=source.match(fullPattern);
...
function parseExplicitDurationBounds(text='') {
  ...
  let unqualified=maskCalendarDayExpressions(text).replace(temporalRangeQualifier,'').replace(temporalQualifier,'').replace(cadenceRangeQualifier,'').replace(cadenceQualifier,'').replace(clockMinute,' '),minimum=0,maximum=0,found=false,rangeMatch;
...
function parseExplicitClockIntervalTiming(text='',currentTime='', { scopeTerminal = true } = {}) {
  ...
  if(!match){
    ...
    const dateOffset=selectedActivityDateOffsetDays(source),rollover=Boolean((parts.named&&parts.minutes===0)||(parts.marker==='밤'&&(Number(parts.hour)===12||Number(parts.hour)<=5))),duration=dateOffset>0?dateOffset*1440+end-now:end-now+(rollover&&end<=now?1440:0);
