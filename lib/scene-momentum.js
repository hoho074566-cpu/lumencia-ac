...
const TRAVEL_DELIBERATION_RE = /([^\n,.!?。！？]{1,48}?)(?:으로|로|에)\s*(?:갈까|갈지|가볼까|가볼지|이동할까|이동할지|향할까|향할지)(?:\s*(?:고민한다|망설인다|생각한다))?\s*[.!?。！？]*\s*$/i;
...
export function classifySceneIntent(action='', { location='' } = {}) {
  ...
  const deliberating=(consequential&&DELIBERATION_RE.test(text))||lowValueDeliberation||travelDeliberation||(exteriorMatch&&EXTERIOR_DELIBERATION_RE.test(text))||compressedQuestion;
