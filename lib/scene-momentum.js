...
export function nextScheduleBoundaryMinutes(saveState={}, { futureOnly = false, action = '', intent = null } = {}) {
  ...
  if(now==null){
    ...
    for(const event of array(schedule.upcoming)){
      ...
      if(action&&isRequestedScheduledActivity(save,event,action,intent))continue;
...
function parseFutureClockOffsetMinutes(text='',currentTime='') {
  ...
  const raw=String(text),source=activityDurationScope(raw);if(/(?:내일|모레|다음\s*날|익일)[^.!?。！？]{0,80}(?:(?:오전|오후)?\s*\d{1,2}\s*(?:시|:)|정오|자정)/.test(raw))return null;
...
export function classifySceneIntent(action='', { location='', currentTime='' } = {}) {
  ...
  const exteriorMatch=EXTERIOR_RE.test(text),lowValueDeliberation=isLowValueDeliberation(text),travelDeliberation=TRAVEL_DELIBERATION_RE.test(text),committedDowntime=hasCommittedDowntime(text),committedWait=hasCommittedWait(text),exploreMatch=EXPLORE_RE.test(text),observeMatch=OBSERVE_RE.test(text),dialogueMatch=DIALOGUE_ACTION_RE.test(text),mealMatch=MEAL_ACTION_RE.test(text),trainingMatch=TRAINING_ACTION_RE.test(text),classMatch=CLASS_ACTION_RE.test(text),travel=text.match(TRAVEL_RE);
  ...
  if(mealMatch){const {minutes,guide}=timedRange(20,45);return{kind:'meal',timeProfile:'meal',semanticTarget:'meal-complete',compression:true,minAdvanceMinutes:minutes,suggestedAdvanceMinutes:guide,deltaTarget:1,requiresNovelty:false,stopPolicy:'meal-complete-or-meaningful-interruption',location,explicitDurationMinutes,scheduledStartOffsetMinutes};}
  if(trainingMatch){const {minutes,guide}=timedRange(30,120);return{kind:'training',timeProfile:'training',semanticTarget:'training-session-complete',compression:true,minAdvanceMinutes:minutes,suggestedAdvanceMinutes:guide,deltaTarget:1,requiresNovelty:false,stopPolicy:'training-result-or-meaningful-interruption',location,explicitDurationMinutes,scheduledStartOffsetMinutes};}
  if(classMatch){const {minutes,guide}=timedRange(45,120);return{kind:'class-attendance',timeProfile:'class',semanticTarget:'class-session-complete',compression:true,minAdvanceMinutes:minutes,suggestedAdvanceMinutes:guide,deltaTarget:1,requiresNovelty:false,stopPolicy:'class-result-or-meaningful-interruption',location,explicitDurationMinutes,scheduledStartOffsetMinutes};}
  if(dialogueMatch){const {minutes,guide}=timedRange(2,10);return{kind:'dialogue',timeProfile:'dialogue',semanticTarget:'conversation-exchange',compression:true,minAdvanceMinutes:minutes,suggestedAdvanceMinutes:guide,deltaTarget:1,requiresNovelty:false,stopPolicy:'exchange-complete-or-important-question',location,explicitDurationMinutes,scheduledStartOffsetMinutes};}
...
export function isRequestedScheduledActivity(saveState={},event={},action='',intent=null) {
  ...
  const eventText=[row.id,row.title,row.note,row.location,row.department].filter(Boolean).join(' '),eventCategory=scheduleActivityCategory(eventText,row.kind);
  ...
  for(const candidate of [...array(save.scheduledEvents),...array(save?.scheduleContext?.upcoming)]){
    ...
    const candidateText=[candidate.id,candidate.title,candidate.note,candidate.location,candidate.department].filter(Boolean).join(' '),candidateCategory=scheduleActivityCategory(candidateText,candidate.kind);
...
export function buildSceneMomentumDirective({ action = '', saveState = {} } = {}) {
  ...
  const scheduleBoundary=intent.compression&&intent.minAdvanceMinutes>0?nextScheduleBoundaryMinutes(saveState,{futureOnly:true,action,intent}):null;
