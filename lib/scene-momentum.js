...
const DOWNTIME_DELIBERATION_RE = /(?:휴식|쉬|쉰|잠|수면|\b(?:sleep|rest)\b).{0,16}(?:할지|할까|하면|한다면|어떨까|고민|망설|\?)/i;
...
function hasCommittedDowntime(text='') {
  return DOWNTIME_RE.test(text)&&!DOWNTIME_NEGATION_RE.test(text)&&!DOWNTIME_DELIBERATION_RE.test(text);
}
function hasCommittedWait(text='') {
  return WAIT_RE.test(text)&&!WAIT_NEGATION_RE.test(text)&&!WAIT_DELIBERATION_RE.test(text);
}
function isLowValueDeliberation(text='') {
  return (DOWNTIME_RE.test(text)&&DOWNTIME_DELIBERATION_RE.test(text))||(WAIT_RE.test(text)&&WAIT_DELIBERATION_RE.test(text));
}

export function classifySceneIntent(action='', { location='' } = {}) {
  const text = String(action || '').trim();
  const consequential = CONSEQUENTIAL_ACTION_RE.test(text);
  const deliberating = (consequential && DELIBERATION_RE.test(text)) || isLowValueDeliberation(text);
  if (deliberating) {
    return {
  ...
  if (consequential) {
    return {
      kind: 'committed-consequence', semanticTarget: null, compression: false,
      minAdvanceMinutes: 0, suggestedAdvanceMinutes: [0, 10], deltaTarget: 1,
      requiresNovelty: false, stopPolicy: 'resolve-committed-action-then-meaningful-choice', location,
    };
  }
  return {
...
export function deriveSceneDelta({ saveState = {}, previousRuntime = {}, turn = {}, nextParticipants = null, action = '' } = {}) {
  ...
  const npcStateChanged = goalRows.some((row) => npcStateUpdateChanges(saveState,row));
  ...
  const npcAction = array(turn?.scene).some((row) => row?.speaker_key) || goalObjectiveChanged || relationshipChanged || npcStateChanged;
  ...
  const structuralScore = [
    ...
    flags.npcStateChanged, flags.newInformation, flags.eventProgress, flags.relationshipChanged,
    flags.objectiveChanged, flags.resourceChanged, flags.growthChanged, flags.scheduleChanged,
    flags.worldThreadChanged, flags.dangerChanged, flags.environmentChanged,
...
export function buildSceneMomentumDirective({ action = '', saveState = {} } = {}) {
  ...
  } else if (intent.kind === 'committed-consequence') {
    lines.push('- COMMITTED ACTION 규칙: 사용자가 이미 공격/결투 수락/협상/계약/능력 사용 등을 선언했다면 다시 “할지 말지”를 묻지 않는다. 그 시도와 즉각 결과·상대 반응·세계 반응까지 처리한 뒤, 새로 중요한 판단이 생긴 지점에서만 STOP한다.');
  }
  if (stall >= 2) {
    lines.push('- SCENE_STALL=true: 이번 턴은 문장만 바꾸거나 scene_title만 바꾸는 것으로 통과할 수 없다. 위치/시간/NPC 출입·행동·상태/새 정보/이벤트/관계/목표/자원/성장/일정/소문·월드 스레드/위험/환경 중 최소 하나의 실제 변화가 필요하다. 작은 변화면 충분하며 대형 사건을 강제하지 않는다.');
  }
  return lines.join('\n');
}
