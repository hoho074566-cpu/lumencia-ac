...
function parseStrictDurationLowerBoundMinutes(text='') {
  const source=maskCalendarDayExpressions(maskNegatedTimedClauses(activityDurationScope(text))),matches=[...source.matchAll(new RegExp(`(${DURATION_ATOM})\\s*${STRICT_DURATION_QUALIFIER}`,'g'))],match=matches.at(-1);
  if(!match)return null;
