...
function stripScheduledStartExpression(text='') {
  const clock=String.raw`(?:오전|오후)?\s*\d{1,2}(?:\s*시(?:\s*\d{1,2}\s*분)?|:\d{2})`;
...
function parseFutureClockOffsetMinutes(text='',currentTime='') {
  ...
  const raw=String(text),source=activityDurationScope(raw);if(/(?:내일|모레|다음\s*날|익일)[^.!?。！？]{0,80}(?:오전|오후)?\s*\d{1,2}\s*(?:시|:)/.test(raw))return null;
...
export function isRequestedScheduledActivity(saveState={},event={},action='',intent=null) {
  ...
  return requestedTokens.length===0?eventTokens.size===0:requestedTokens.every(token=>eventTokens.has(token));
}
