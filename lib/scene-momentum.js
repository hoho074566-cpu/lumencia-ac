...
const DAY_DURATION = String.raw`(?:(?:하루|이틀|${DURATION_NUMBER}\s*일)(?:\s*반)?)(?!\s*(?:에(?!서)|차|째|자|날))`;
...
const TRAVEL_RE = new RegExp(String.raw`([^\n,.!?。！？]{1,48}?)(?:으로|로|에)\s*${ACTIVITY_TIMING_PREFIX}(?:간다|가자|이동한다|향한다|가본다|간다니까|go|move|head)\s*[.!?。！？]*\s*$`,'i');
...
function absoluteCalendarDateOffsetDays(text='',currentDate='') {
  const current=String(currentDate||'').trim().match(/^(\d{1,4})-(\d{1,2})-(\d{1,2})$/),matches=[...String(text||'').matchAll(new RegExp(ABSOLUTE_DATE_PATTERN,'g'))],selected=matches.at(-1);
  if(!current||!selected)return null;
  let year=Number(selected[1]||current[1]);const month=Number(selected[2]),day=Number(selected[3]),currentStart=dateTimeMinutes(currentDate,'00:00');
  ...
  if(!selected[1]&&targetStart<currentStart){year+=1;targetStart=dateTimeMinutes(`${year}-${String(month).padStart(2,'0')}-${String(day).padStart(2,'0')}`,'00:00');if(targetStart==null)return null;}
  return Math.trunc((targetStart-currentStart)/1440);
}
function selectedActivityDateOffsetDays(text='',currentDate='') {
...
function selectedActivityDateIsFuture(text='',currentDate='') { return selectedActivityDateOffsetDays(text,currentDate)>0; }
...
function durationExpressionMinutes(text='') {
  ...
  const namedDayMatch=source.match(/^(하루|이틀)(?:\s*(반))?$/);if(namedDayMatch)return(namedDayMatch[1]==='이틀'?2880:1440)+(namedDayMatch[2]?720:0);
...
function maskCalendarDayExpressions(text='') {
  return String(text||'').replace(/(?:\d{1,4}\s*년\s*)?\d{1,2}\s*월\s*\d{1,2}\s*일(?=\s*(?:에|에는|엔|동안|오전|오후|아침|저녁|밤|정오|자정|\d{1,2}\s*(?:시|:)|$))/g,' ');
}
function explicitDurationRangeMatch(text='') {
  ...
  const sharedPattern=new RegExp(`(${DURATION_NUMBER})\\s*(?:에서|~|〜|부터)\\s*(${DURATION_NUMBER})\\s*(일(?!\\s*(?:에(?!서)|차|째|자|날))|시간|분)(?:\\s*까지)?(?!\\s*${nonActivitySuffix})`),sharedMatch=source.match(sharedPattern);if(!sharedMatch)return null;
...
function parseExplicitDurationBounds(text='') {
  ...
  let unqualified=maskCalendarDayExpressions(text).replace(temporalRangeQualifier,'').replace(temporalQualifier,'').replace(cadenceRangeQualifier,'').replace(cadenceQualifier,'').replace(objectModifierRangeQualifier,'').replace(objectModifierQualifier,'').replace(clockMinute,' '),minimum=0,maximum=0,found=false,rangeMatch;
...
function parsePrecedingExplicitDurationBounds(text='',currentTime='',actorName='',currentDate='') {
  ...
  for(const match of connectors){
    const end=match.index+match[0].length,segment=source.slice(boundary,end),explicit=parseExplicitDurationBounds(segment),clockInterval=parseExplicitClockIntervalTiming(segment,currentTime,{scopeTerminal:false,currentDate}),defaults=/(?:훈련|연습|수련|단련)/.test(match[0])?[30,120]:/(?:수업|강의|세미나|실습|오리엔테이션|교육|입학식)/.test(match[0])?[45,120]:/(?:아침|점심|저녁|밥|식사)/.test(match[0])?[20,45]:/(?:대화|이야기|질문|답변|설명|상담|논의|면담|회의|브리핑|협상)/.test(match[0])?[2,10]:/(?:잠을|잔\s*(?:뒤|후))/.test(match[0])?[240,480]:/(?:휴식|쉬고|쉰\s*(?:뒤|후))/.test(match[0])?[30,240]:/(?:대기|기다리고)/.test(match[0])?[10,60]:/(?:(?:으로|로|에)\s*(?:이동하고|도착하고|나가서|들어가서|가서)|방을\s*잡고)/.test(segment)?[3,30]:[0,0],bounds=clockInterval?[clockInterval.minutes,clockInterval.minutes]:explicit||defaults,relativeStart=parseRelativeStartOffsetMinutes(segment,{scopeTerminal:false}),dateStart=selectedActivityDateIsFuture(segment,currentDate)?parseDateQualifiedStartOffsetMinutes(segment,currentTime,{scopeTerminal:false,currentDate}):null,clockStart=parseFutureClockOffsetMinutes(segment,currentTime,{scopeTerminal:false,currentDate}),startOffset=relativeStart??dateStart??clockStart;
...
function parseExplicitClockIntervalTiming(text='',currentTime='', { scopeTerminal = true, currentDate = '' } = {}) {
  ...
  if(!match){
    ...
    const dateOffset=selectedActivityDateOffsetDays(source,currentDate),rollover=Boolean((parts.named&&parts.minutes===0)||(parts.marker==='밤'&&(Number(parts.hour)===12||Number(parts.hour)<=5))),duration=dateOffset>0?dateOffset*1440+end-now:end-now+(rollover&&end<=now?1440:0);
    return{minutes:Math.max(0,duration),startOffsetSuppressed:true,deadline:true};
  }
  ...
  const now=clockMinutes(currentTime);if(now==null||selectedActivityDateIsFuture(text,currentDate))return{minutes:duration,startOffsetSuppressed:false};
  const endAbsolute=start+duration;if(now>=start&&now<endAbsolute)return{minutes:endAbsolute-now,startOffsetSuppressed:true};
  if(parseFutureClockOffsetMinutes(text,currentTime,{currentDate})!=null||now<start)return{minutes:duration,startOffsetSuppressed:false};
...
function parseFutureClockOffsetMinutes(text='',currentTime='', { scopeTerminal = true, currentDate = '' } = {}) {
  const now=clockMinutes(currentTime),raw=String(text);if(now==null||selectedActivityDateIsFuture(raw,currentDate))return null;
...
function hasElapsedScheduledDateStart(text='',currentTime='',currentDate='') {
  const now=clockMinutes(currentTime),selected=selectedActivityClock(text);
  const absoluteOffset=absoluteCalendarDateOffsetDays(text,currentDate);if(absoluteOffset!=null)return absoluteOffset<0||(absoluteOffset===0&&now!=null&&selected!=null&&!selected.rollover&&selected.minutes<now);
  return selectedActivityDateQualifier(text)==='오늘'&&now!=null&&selected!=null&&!selected.rollover&&selected.minutes<now;
}
function parseDateQualifiedStartOffsetMinutes(text='',currentTime='', { scopeTerminal = true, currentDate = '' } = {}) {
  const now=clockMinutes(currentTime),dateOffsetDays=selectedActivityDateOffsetDays(text,currentDate),selected=selectedActivityClock(text,{scopeTerminal});
...
export function classifySceneIntent(action='', { location='', currentTime='', currentDate='', actorName='' } = {}) {
  ...
  const selectedActivityScope=activityDurationScope(text),clockIntervalTiming=schedulableActivity?parseExplicitClockIntervalTiming(text,currentTime,{currentDate}):null,dateQualifiedStart=Boolean(schedulableActivity&&!clockIntervalTiming?.deadline&&selectedActivityDateIsFuture(selectedActivityScope,currentDate)),clockIntervalMinutes=clockIntervalTiming?.minutes??null;
  const elapsedScheduledStart=Boolean(schedulableActivity&&!clockIntervalTiming?.startOffsetSuppressed&&hasElapsedScheduledDateStart(selectedActivityScope,currentTime,currentDate));
  ...
  const precedingActivityBounds=parsePrecedingExplicitDurationBounds(text,currentTime,actorName,currentDate),precedingActivityMinimum=precedingActivityBounds?.[0]??0,precedingActivityMinutes=precedingActivityBounds?.[1]??0,precedingActivityRangeMinutes=precedingActivityBounds&&precedingActivityBounds[0]!==precedingActivityBounds[1]?precedingActivityBounds:null;
  const relativeStartOffsetMinutes=dateQualifiedStart?null:parseRelativeStartOffsetMinutes(text);
  const scheduledStartOffsetMinutes=schedulableActivity&&!dateQualifiedStart?relativeStartOffsetMinutes??(clockIntervalTiming?.startOffsetSuppressed?null:parseFutureClockOffsetMinutes(text,currentTime,{currentDate})):null;
  const dateQualifiedStartOffsetMinutes=dateQualifiedStart?parseDateQualifiedStartOffsetMinutes(selectedActivityScope,currentTime,{currentDate}):null;
...
export function isRequestedScheduledActivity(saveState={},event={},action='',intent=null,registry={}) {
  const save=object(saveState),row=object(event),sceneIntent=object(intent||classifySceneIntent(action,{location:save?.world?.location||'',currentTime:save?.world?.time||'',currentDate:save?.world?.date||'',actorName:save?.pc?.name||''}));
...
export function deriveSceneDelta({ saveState = {}, previousRuntime = {}, turn = {}, nextParticipants = null, action = '' } = {}) {
  ...
  const intent=classifySceneIntent(action,{location:saveState?.world?.location||'',currentTime:saveState?.world?.time||'',currentDate:saveState?.world?.date||'',actorName:saveState?.pc?.name||''}),target=Math.max(0,Number(intent.deltaTarget||0)),metTarget=meaningfulStop||target===0||score>=target;
...
export function buildSceneMomentumDirective({ action = '', saveState = {}, registry = {} } = {}) {
  ...
  const intent=classifySceneIntent(action,{location:saveState?.world?.location||'',currentTime:saveState?.world?.time||'',currentDate:saveState?.world?.date||'',actorName:saveState?.pc?.name||''}),momentum=object(saveState?.sceneRuntime?.momentum),stall=Math.max(0,Number(momentum.stall_streak||0)),[minMinutes,maxMinutes]=intent.suggestedAdvanceMinutes;
