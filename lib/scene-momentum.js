...
const DURATION_ATOM = String.raw`(?:${DURATION_NUMBER}\s*시간(?:(?:\s*${DURATION_NUMBER}\s*분)|(?:\s*반))?|${DURATION_NUMBER}\s*분)`;
...
const DOWNTIME_RE = new RegExp(String.raw`^\s*(?:${DECLARED_DURATION_PREFIX}\s*(?:동안|만)?\s*)?(?:쉰다|쉬겠다|쉴게|휴식한다|휴식하겠다|휴식할게|쉬어간다|쉬어가겠다|쉬어갈게|잠을\s*(?:잔다|자겠다|잘게)|잔다|자겠다|잘게|눈을\s*(?:붙인다|붙이겠다|붙일게)|수면한다|수면하겠다|수면할게|sleep(?:s|ed|ing)?|rest(?:s|ed|ing)?)\s*[.!?。！？]*$`,'i');
...
const WAIT_RE = new RegExp(String.raw`^\s*(?:${DECLARED_DURATION_PREFIX}\s*(?:동안|만)?\s*)?(?:기다린다|기다리겠다|기다릴게|기다려본다|기다려보겠다|기다려볼게|대기한다|대기하겠다|대기할게|시간을\s*(?:보낸다|보내겠다|보낼게)|가만히\s*(?:기다린다|기다리겠다|기다릴게|있는다|있겠다|있을게)|wait(?:s|ed|ing)?)\s*[.!?。！？]*$`,'i');
...
const DIALOGUE_ACTION_RE = new RegExp(String.raw`(?:말을\s*(?:건다|걸겠다|걸게)|(?:대화|이야기|질문|답변|설명|상담|논의|면담|회의|브리핑)(?:을|를)?\s*(?:${DECLARED_DURATION_PREFIX}\s*(?:동안|만)?\s*)?(?:한다|하겠다|할게)|묻는다|묻겠다|물을게|답한다|답하겠다|답할게)\s*[.!。！]*$`,'i');
const MEAL_ACTION_RE = new RegExp(String.raw`(?:(?:아침|점심|저녁|밥|식사)(?:을|를)?\s*(?:${DECLARED_DURATION_PREFIX}\s*(?:동안|만)?\s*)?(?:먹는다|먹겠다|먹을게)|식사(?:를)?\s*(?:${DECLARED_DURATION_PREFIX}\s*(?:동안|만)?\s*)?(?:한다|하겠다|할게|마친다|마치겠다|마칠게))\s*[.!。！]*$`,'i');
const TRAINING_ACTION_RE = new RegExp(String.raw`(?:훈련|연습|수련|단련)(?:을|를)?\s*(?:${DECLARED_DURATION_PREFIX}\s*(?:동안|만)?\s*)?(?:한다|하겠다|할게|시작한다|시작하겠다|시작할게|계속한다|계속하겠다|계속할게|마친다|마치겠다|마칠게)\s*[.!。！]*$`,'i');
const CLASS_ACTION_RE = new RegExp(String.raw`(?:수업|강의|세미나|실습|오리엔테이션|교육|입학식)(?:에|을|를)?\s*(?:${DECLARED_DURATION_PREFIX}\s*(?:동안|만)?\s*)?(?:참석한다|참석하겠다|참석할게|참여한다|참여하겠다|참여할게|듣는다|듣겠다|들을게|수강한다|수강하겠다|수강할게|받는다|받겠다|받을게|시작한다|시작하겠다|시작할게|계속한다|계속하겠다|계속할게|마친다|마치겠다|마칠게)\s*[.!。！]*$`,'i');
const PRIOR_ACTIVITY_DURATION = String.raw`(?:${DECLARED_DURATION_PREFIX}\s*(?:동안|만)?\s*)?`;
const PRIOR_ACTIVITY_CONNECTOR_RE = new RegExp(String.raw`(?:(?:훈련|연습|수련|단련)(?:을|를)?\s*${PRIOR_ACTIVITY_DURATION}(?:하고|한\s*(?:뒤|후)|마치고)|(?:수업|강의|세미나|실습|오리엔테이션|교육|입학식)(?:에|을|를)?\s*${PRIOR_ACTIVITY_DURATION}(?:하고|참석하고|참여하고|듣고|수강하고|받고|마치고|참석한\s*(?:뒤|후)|들은\s*(?:뒤|후))|(?:아침|점심|저녁|밥|식사)(?:을|를)?\s*${PRIOR_ACTIVITY_DURATION}(?:하고|먹고|마치고)|(?:대화|이야기|질문|답변|설명|상담|논의|면담|회의|브리핑)(?:을|를)?\s*${PRIOR_ACTIVITY_DURATION}(?:하고|한\s*(?:뒤|후)|마치고)|(?:잠을\s*${PRIOR_ACTIVITY_DURATION}자고|잠을\s*${PRIOR_ACTIVITY_DURATION}잔\s*(?:뒤|후)|(?:휴식|대기)(?:을|를)?\s*${PRIOR_ACTIVITY_DURATION}(?:하고|마치고)|쉬고|쉰\s*(?:뒤|후)|휴식하고|기다리고|대기하고)|(?:이동하고|도착하고|나가서|들어가서|가서|와서|방을\s*잡고))`,'gi');
...
const TRAVEL_RE = new RegExp(String.raw`([^\n,.!?。！？]{1,48}?)(?:으로|로|에)\s*(?:${DECLARED_DURATION_PREFIX}\s*(?:동안|만)?\s*)?(?:간다|가자|이동한다|향한다|가본다|간다니까|go|move|head)\s*[.!?。！？]*\s*$`,'i');
...
function cleanTravelTarget(raw='',stripTrailingParticle=false) {
...
function durationExpressionMinutes(text='') {
  const match=String(text||'').trim().match(new RegExp(`^(${DURATION_NUMBER})\\s*시간(?:\\s*(?:(${DURATION_NUMBER})\\s*분|(반)))?$|^(${DURATION_NUMBER})\\s*분$`));if(!match)return null;
...
function parseExplicitDurationBounds(text='') {
  const temporalQualifier=new RegExp(`(?:${DURATION_ATOM})(?=\\s*(?:전(?:에)?|이전|후(?:에)?|뒤(?:에)?))`,'g');
  const clockMinute=new RegExp(`(?:^|\\s)(?:오전|오후|아침|저녁|밤)?\\s*\\d{1,2}\\s*시\\s*${DURATION_NUMBER}\\s*분(?=\\s*(?:에|부터|까지|경|쯤|시작|개시))`,'g');
  let unqualified=String(text||'').replace(temporalQualifier,'').replace(clockMinute,' '),minimum=0,maximum=0,found=false,rangeMatch;
...
function parseRelativeStartOffsetMinutes(text='') {
  const source=activityDurationScope(text),matches=[...source.matchAll(new RegExp(`(${DURATION_ATOM})\\s*(?:후|뒤|이후)(?:에)?`,'g'))],match=matches.at(-1);
  if(!match)return null;
  const total=durationExpressionMinutes(match[1]);
  return total>0?total:null;
}
function parsePrecedingExplicitDurationBounds(text='') {
  const source=String(text||'');let boundary=0,match,minimum=0,maximum=0,matched=false;
  PRIOR_ACTIVITY_CONNECTOR_RE.lastIndex=0;
  while((match=PRIOR_ACTIVITY_CONNECTOR_RE.exec(source))){
    const end=match.index+match[0].length,segment=source.slice(boundary,end),explicit=parseExplicitDurationBounds(segment),defaults=/(?:훈련|연습|수련|단련)/.test(match[0])?[30,120]:/(?:수업|강의|세미나|실습|오리엔테이션|교육|입학식)/.test(match[0])?[45,120]:/(?:아침|점심|저녁|밥|식사)/.test(match[0])?[20,45]:/(?:대화|이야기|질문|답변|설명|상담|논의|면담|회의|브리핑)/.test(match[0])?[2,10]:/(?:잠을|잔\s*(?:뒤|후))/.test(match[0])?[240,480]:/(?:휴식|쉬고|쉰\s*(?:뒤|후))/.test(match[0])?[30,240]:/(?:대기|기다리고)/.test(match[0])?[10,60]:/(?:(?:으로|로|에)\s*(?:이동하고|도착하고|나가서|들어가서|가서)|방을\s*잡고)/.test(segment)?[3,30]:[0,0],bounds=explicit||defaults;
...
function clockExpressionMinutes(marker='',hourValue='',minuteValue='0') {
  const rawHour=Number(hourValue),minute=Number(minuteValue||0),period=String(marker||''),normalizedPeriod=/^(?:오전|아침)$/.test(period)?'am':/^(?:오후|저녁|밤)$/.test(period)?'pm':'';
...
const CLOCK_PERIOD = String.raw`(?:오전|오후|아침|저녁|밤)`;
...
function clockTokenParts(token='') {
  ...
  const match=String(token||'').trim().match(/^(오전|오후|아침|저녁|밤)?\s*(\d{1,2})(?:\s*시(?:\s*(?:(\d{1,2})\s*분|(반)))?|:(\d{2}))$/);if(!match)return null;
...
function parseExplicitClockIntervalMinutes(text='') {
  const source=activityDurationScope(text),match=source.match(new RegExp(`(?:^|\\s)(${CLOCK_EXPRESSION})\\s*부터\\s*(${CLOCK_EXPRESSION})\\s*까지`));
  if(!match)return null;
  const startParts=clockTokenParts(match[1]),endParts=clockTokenParts(match[2]);if(!startParts||!endParts)return null;
  const startMarker=startParts.marker,explicitEndMarker=endParts.marker,endMarker=explicitEndMarker||startMarker;
...
function stripScheduledStartExpression(text='') {
  ...
  const source=String(text||'').trim(),stripped=source.replace(new RegExp(String.raw`(?:^|\s)${clock}\s*부터\s*${clock}\s*까지(?=\s|$)`,'g'),' ').replace(new RegExp(String.raw`(?:^|\s)${clock}\s*(?:에|부터|경|쯤)(?=\s|$)`,'g'),' ').replace(new RegExp(String.raw`(?:^|\s)${clock}(?=\s|$)`,'g'),' ').replace(new RegExp(String.raw`(?:^|\s)${relative}(?=\s|$)`,'g'),' ').trim();
...
function selectedActivityClock(text='') {
  ...
  const rollover=parts.marker==='밤'&&(Number(parts.hour)===12||Number(parts.hour)<=5);
...
function hasThirdPartyTimedSubject(text='',actorName='') {
  ...
  const scoped=activityDurationScope(raw),allowed=new Set(['나','난','내','저','제','우리','우린','저희','pc','player','aaa',norm(actorName)].filter(Boolean));
  const activityTopics=new Set(['잠','수면','휴식','대기','기다림','식사','아침','점심','저녁','훈련','연습','수련','단련','수업','강의','세미나','실습','오리엔테이션','교육','입학식','대화','이야기','상담','논의','면담','회의','브리핑']);
  const subjectFrom=(value)=>{let source=stripScheduledStartExpression(value).replace(DATE_QUALIFIER_RE,' ').trim();DATE_QUALIFIER_RE.lastIndex=0;source=source.replace(/^.{1,48}?(?:와는|과는|에게는|한테는|께는|에서는|으로는|로는|에는)\s+/u,'');const subject=norm(source.match(/^(.{1,48}?)(?:가|이|은|는)\s+/u)?.[1]||'');return activityTopics.has(subject)?'':subject;};
  ...
  const inheritedSubject=scoped===raw?'':subjectFrom(raw);return Boolean(inheritedSubject&&!allowed.has(inheritedSubject));
}
...
export function classifySceneIntent(action='', { location='', currentTime='', actorName='' } = {}) {
  ...
  const exteriorMatch=EXTERIOR_RE.test(text),lowValueDeliberation=isLowValueDeliberation(text),travelDeliberation=TRAVEL_DELIBERATION_RE.test(text),committedDowntime=!thirdPartyTimedSubject&&!consequential&&hasCommittedDowntime(text),committedWait=!thirdPartyTimedSubject&&hasCommittedWait(text),exploreMatch=EXPLORE_RE.test(text),observeMatch=OBSERVE_RE.test(text),dialogueMatch=!thirdPartyTimedSubject&&DIALOGUE_ACTION_RE.test(text),mealMatch=!thirdPartyTimedSubject&&MEAL_ACTION_RE.test(text),trainingMatch=!thirdPartyTimedSubject&&TRAINING_ACTION_RE.test(text),classMatch=!thirdPartyTimedSubject&&CLASS_ACTION_RE.test(text),travel=thirdPartyTimedSubject?null:text.match(TRAVEL_RE);
  ...
  const precedingActivityBounds=parsePrecedingExplicitDurationBounds(text),precedingActivityMinimum=precedingActivityBounds?.[0]??0,precedingActivityMinutes=precedingActivityBounds?.[1]??0,precedingActivityRangeMinutes=precedingActivityBounds&&precedingActivityBounds[0]!==precedingActivityBounds[1]?precedingActivityBounds:null;
  ...
  if(travel){const withoutSchedule=stripScheduledStartExpression(travel[1]),withoutDate=withoutSchedule.replace(DATE_QUALIFIER_RE,' '),target=cleanTravelTarget(withoutDate,withoutDate!==withoutSchedule);DATE_QUALIFIER_RE.lastIndex=0;const time=travelTimeProfile(location,target),activityTiming=explicitDurationRangeMinutes?{...time,minAdvanceMinutes:explicitDurationRangeMinutes[0],suggestedAdvanceMinutes:explicitDurationRangeMinutes}:explicitDurationMinutes!=null?{...time,minAdvanceMinutes:explicitDurationMinutes,suggestedAdvanceMinutes:[explicitDurationMinutes,explicitDurationMinutes]}:time,prefixedTiming=precedingActivityBounds?{...activityTiming,minAdvanceMinutes:precedingActivityMinimum+activityTiming.minAdvanceMinutes,suggestedAdvanceMinutes:[precedingActivityMinimum+activityTiming.suggestedAdvanceMinutes[0],precedingActivityMinutes+activityTiming.suggestedAdvanceMinutes[1]]}:activityTiming,offset=scheduledStartOffsetMinutes??0,timing=dateQualifiedStart?{...time,timeProfile:'date-qualified-travel',minAdvanceMinutes:0,suggestedAdvanceMinutes:[0,1440]}:scheduledStartOffsetMinutes!=null?{...activityTiming,minAdvanceMinutes:offset+activityTiming.minAdvanceMinutes,suggestedAdvanceMinutes:activityTiming.suggestedAdvanceMinutes.map(value=>offset+value)}:prefixedTiming;return capTimedIntent({kind:'travel',...timing,semanticTarget:target||'declared-destination',compression:!dateQualifiedStart,boundaryLookaheadMinutes:dateQualifiedStart?1440:0,deltaTarget:dateQualifiedStart?0:2,requiresNovelty:false,stopPolicy:'declared-destination-or-meaningful-interruption',location,explicitDurationMinutes,explicitDurationRangeMinutes,precedingActivityMinutes,precedingActivityRangeMinutes,scheduledStartOffsetMinutes,dateQualifiedStart});}
...
function scheduleActivityTokens(value='') {
  const generic=new Set(['academic','class','event','schedule','필수','일정','예정','시작','참석','참여','수업','강의','세미나','실습','오리엔테이션','교육','입학식','훈련','연습','수련','단련','대련','식사','아침','점심','저녁','만찬','면담','상담','회의','대화','브리핑','수면','휴식','잠','대기','기다림','잔다','쉰다','휴식한다','기다린다','대기한다','sleep','rest','wait','기사과','마법과','신학부','연금술과','일반과','학부','훈련장','강의실','교실','대강당','본관','기숙사','중앙광장','오전','오후','정오','자정','오늘','내일','모레','이번','다음']);
  return [...new Set((String(value||'').toLowerCase().match(/[가-힣a-z0-9_]+/g)||[]).map(token=>token.replace(/(?:에게서|에게|한테|께서|으로|에서|까지|부터|처럼|보다|에는|에게는|으로는|에서는|은|는|이|가|을|를|와|과|의|에)$/u,'')).filter(token=>token.length>=2&&!/^\d/.test(token)&&!generic.has(token)&&!/(?:하게|스럽게|답게|적으로|히)$/.test(token)&&!/(?:한다|했다|듣는다|먹는다|참석한다|참여한다|시작한다|계속한다|마친다)$/.test(token)))];
}
...
export function isRequestedScheduledActivity(saveState={},event={},action='',intent=null,registry={}) {
  const save=object(saveState),row=object(event),sceneIntent=object(intent||classifySceneIntent(action,{location:save?.world?.location||'',currentTime:save?.world?.time||'',actorName:save?.pc?.name||''}));
  ...
  for(const candidate of [...array(save.scheduledEvents),...array(save?.scheduleContext?.upcoming)]){
    ...
    const candidatePrimaryText=[candidate.id,candidate.title].filter(Boolean).join(' '),candidateText=[candidatePrimaryText,candidate.note,candidate.location,candidate.department,scheduleParticipantIdentityText(save,candidate,registry)].filter(Boolean).join(' '),candidateCategory=scheduleActivityCategory(candidatePrimaryText,candidate.kind);
...
export function deriveSceneDelta({ saveState = {}, previousRuntime = {}, turn = {}, nextParticipants = null, action = '' } = {}) {
  ...
  const intent=classifySceneIntent(action,{location:saveState?.world?.location||'',currentTime:saveState?.world?.time||'',actorName:saveState?.pc?.name||''}),target=Math.max(0,Number(intent.deltaTarget||0)),metTarget=meaningfulStop||target===0||score>=target;
...
export function buildSceneMomentumDirective({ action = '', saveState = {}, registry = {} } = {}) {
  ...
  const intent=classifySceneIntent(action,{location:saveState?.world?.location||'',currentTime:saveState?.world?.time||'',actorName:saveState?.pc?.name||''}),momentum=object(saveState?.sceneRuntime?.momentum),stall=Math.max(0,Number(momentum.stall_streak||0)),[minMinutes,maxMinutes]=intent.suggestedAdvanceMinutes;
  ...
  }else if(intent.kind==='travel')lines.push(intent.turnLimitTruncated?`- TRAVEL 규칙: 평범한 이동 과정은 압축하되, 1440분 상한에서는 선언 목적지(${intent.semanticTarget||'목적지'})에 도착했다고 만들지 말고 이동 중인 실제 위치에서 멈춘다.`:`- TRAVEL 규칙: 특별한 방해가 없으면 평범한 이동 과정을 압축해 선언 목적지(${intent.semanticTarget||'목적지'})까지 이동 완료한다.`);
