...
function cleanTravelTarget(raw='',stripTrailingParticle=false,actorName='') {
...
function parseDateQualifiedStartOffsetMinutes(text='',currentTime='') {
  const now=clockMinutes(currentTime),dateOffsetDays=selectedActivityDateOffsetDays(text),selected=selectedActivityClock(text);
...
export function classifySceneIntent(action='', { location='', currentTime='', actorName='' } = {}) {
  ...
  const scheduledStartOffsetMinutes=schedulableActivity&&!dateQualifiedStart?relativeStartOffsetMinutes??parseFutureClockOffsetMinutes(text,currentTime):null;
  const dateQualifiedStartOffsetMinutes=dateQualifiedStart?parseDateQualifiedStartOffsetMinutes(text,currentTime):null;
  ...
  const timedRange=(defaultMinutes,defaultMaximum)=>{const activityMinutes=explicitDurationRangeMinutes?.[0]??explicitDurationMinutes??defaultMinutes,activityMaximum=explicitDurationRangeMinutes?.[1]??explicitDurationMinutes??defaultMaximum,reachableDateOffset=dateQualifiedStart&&dateQualifiedStartOffsetMinutes!=null&&dateQualifiedStartOffsetMinutes+activityMaximum<=1440?dateQualifiedStartOffsetMinutes:null;if(dateQualifiedStart&&reachableDateOffset==null)return{minutes:0,guide:[0,1440],effectiveStartOffset:null,futureReachable:false};const effectiveStartOffset=reachableDateOffset??scheduledStartOffsetMinutes,offsetMinimum=effectiveStartOffset??precedingActivityMinimum,offsetMaximum=effectiveStartOffset??precedingActivityMinutes;return{minutes:offsetMinimum+activityMinutes,guide:explicitDurationRangeMinutes?[offsetMinimum+explicitDurationRangeMinutes[0],offsetMaximum+explicitDurationRangeMinutes[1]]:explicitDurationMinutes!=null?[offsetMinimum+activityMinutes,offsetMaximum+activityMinutes]:[offsetMinimum+defaultMinutes,offsetMaximum+defaultMaximum],effectiveStartOffset,futureReachable:Boolean(dateQualifiedStart&&reachableDateOffset!=null)};};
  ...
  if(travel){const withoutSchedule=stripScheduledStartExpression(travel[1]),withoutDate=withoutSchedule.replace(DATE_QUALIFIER_RE,' '),target=cleanTravelTarget(withoutDate,withoutDate!==withoutSchedule,actorName);DATE_QUALIFIER_RE.lastIndex=0;const time=travelTimeProfile(location,target),activityTiming=explicitDurationRangeMinutes?{...time,minAdvanceMinutes:explicitDurationRangeMinutes[0],suggestedAdvanceMinutes:explicitDurationRangeMinutes}:explicitDurationMinutes!=null?{...time,minAdvanceMinutes:explicitDurationMinutes,suggestedAdvanceMinutes:[explicitDurationMinutes,explicitDurationMinutes]}:time,prefixedTiming=precedingActivityBounds?{...activityTiming,minAdvanceMinutes:precedingActivityMinimum+activityTiming.minAdvanceMinutes,suggestedAdvanceMinutes:[precedingActivityMinimum+activityTiming.suggestedAdvanceMinutes[0],precedingActivityMinutes+activityTiming.suggestedAdvanceMinutes[1]]}:activityTiming,dateOffset=dateQualifiedStart&&dateQualifiedStartOffsetMinutes!=null&&dateQualifiedStartOffsetMinutes+activityTiming.suggestedAdvanceMinutes[1]<=1440?dateQualifiedStartOffsetMinutes:null,effectiveStartOffset=dateQualifiedStart?dateOffset:scheduledStartOffsetMinutes,offset=effectiveStartOffset??0,deferredDate=dateQualifiedStart&&dateOffset==null,timing=deferredDate?{...time,timeProfile:'date-qualified-travel',minAdvanceMinutes:0,suggestedAdvanceMinutes:[0,1440]}:effectiveStartOffset!=null?{...activityTiming,timeProfile:dateQualifiedStart?'date-qualified-travel':activityTiming.timeProfile,minAdvanceMinutes:offset+activityTiming.minAdvanceMinutes,suggestedAdvanceMinutes:activityTiming.suggestedAdvanceMinutes.map(value=>offset+value)}:prefixedTiming;return capTimedIntent({kind:'travel',...timing,semanticTarget:target||'declared-destination',compression:!deferredDate,boundaryLookaheadMinutes:deferredDate?1440:0,deltaTarget:deferredDate?0:2,requiresNovelty:false,stopPolicy:'declared-destination-or-meaningful-interruption',location,explicitDurationMinutes,explicitDurationRangeMinutes,precedingActivityMinutes,precedingActivityRangeMinutes,scheduledStartOffsetMinutes:effectiveStartOffset,dateQualifiedStart});}
...
export function isRequestedScheduledActivity(saveState={},event={},action='',intent=null,registry={}) {
  ...
  const pcIdentityTokens=new Set(scheduleActivityTokens(save?.pc?.name||'')),requestedTokens=scheduleActivityTokens(activityDurationScope(action)).filter(token=>!pcIdentityTokens.has(token)),eventTokens=new Set(scheduleActivityTokens(eventText));
...
export function buildSceneMomentumDirective({ action = '', saveState = {}, registry = {} } = {}) {
  ...
  if(intent.dateQualifiedStart)lines.push(intent.scheduledStartOffsetMinutes!=null?`- 날짜 지정 시작 규칙: 요청한 미래 날짜·시각은 현재로부터 ${intent.scheduledStartOffsetMinutes}분 뒤이며 활동까지 1440분 안에 완료할 수 있다. 요청 날짜를 오늘로 당기거나 시작 전에 완료하지 말고 TIME_GUIDE의 실제 미래 시각까지 진행한다.`:`- 날짜 지정 시작 규칙: 내일/모레/다음 날처럼 현재 날짜를 넘고 활동까지 한 턴 안에 완료할 수 없는 요청에는 즉시 활동용 시간 floor를 적용하지 않는다. 한 턴 최대 1440분 안에서 먼저 만나는 권위 있는 일정·예약된 인과 결과에서 멈추고, 경계가 없더라도 요청한 날짜를 오늘로 당기거나 일반 활동 최대치로 잘라 완료하지 않는다.${intent.explicitDurationMinutes!=null?` 지정한 ${intent.explicitDurationMinutes}분은 미래 시작 뒤의 활동 지속시간이다.`:intent.explicitDurationRangeMinutes?` 지정한 ${intent.explicitDurationRangeMinutes[0]}-${intent.explicitDurationRangeMinutes[1]}분은 미래 시작 뒤의 활동 지속시간 범위다.`:''}`);
