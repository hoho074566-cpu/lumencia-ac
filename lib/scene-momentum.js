...
const EXTERIOR_RE = /(?:^|\s)(?:밖으로|바깥으로|밖에|건물\s*밖(?:으로|에)|기숙사\s*밖(?:으로|에)|외부로)\s*(?:나가|간|가|이동|향)/i;
...
function eventNullMeansPause(saveState,previousRuntime,delta,turn) {
  ...
  const priorId=norm(previous.eventInstanceId||previous.event_instance_id||'');
...
export function classifySceneIntent(action='', { location='' } = {}) {
  ...
  if (exteriorMatch && !EXTERIOR_NEGATION_RE.test(text)) {
    return {
...
export function deriveSceneDelta({ saveState = {}, previousRuntime = {}, turn = {}, nextParticipants = null, action = '' } = {}) {
  ...
  const pausedEventNull=eventNullMeansPause(saveState,previousRuntime,delta,turn);
