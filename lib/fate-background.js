...
function strengthProfile(origin,finalConditions={}){
  const route=DEPARTMENT_ROUTES[origin.department]||DEPARTMENT_ROUTES['일반학부 1학년'];
  const finalGrade=cleanText(finalConditions.stats?.[STAT_LABELS[route.axis]]?.grade??finalConditions.stats?.[STAT_LABELS[route.axis]],12).replace(/[+-]+$/,'');
  const finalGradeIndex=GRADE_LADDER.indexOf(finalGrade);
  const axis=finalGradeIndex>=0?finalGradeIndex+1:Math.max(1,Math.min(3,Number(origin.baseStats?.[route.axis])||1));
  const finalTalent=Number(finalConditions.talents?.[route.talent]);
  const talent=Number.isFinite(finalTalent)?Math.max(1,Math.min(10,Math.trunc(finalTalent))):Math.max(1,Math.min(3,Number(origin.talents?.[route.talent])||1));
...
export function buildFateBackgroundState(origin={},finalConditions={}){
  ...
  return{version:1,sourceSeedTag:cleanText(origin.seedTag,80),facts,startingRoute:startingRoute(origin),strengthProfile:strengthProfile(origin,finalConditions)};
}

export function normalizeFateBackgroundState(value,origin={},finalConditions={}){
  const canonical=buildFateBackgroundState(origin,finalConditions);
...
export function compactFateBackgroundForModel(creation,pc={}){
  ...
  const state=normalizeFateBackgroundState(fate.background,origin,pc);
