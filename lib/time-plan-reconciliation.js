...
export function validateStructuredTimeExecution(turn = {}, plan = {}, runtime = {}) {
  ...
  if (!Array.isArray(claim.decision_completion_clause_ids)) return { applicable: true, valid: false, reason: 'missing-decision-completion-set' };
  const decisionCompletionIds = claim.decision_completion_clause_ids.map(String), decisionCompletionSet = new Set(decisionCompletionIds);
  if (decisionCompletionSet.size !== decisionCompletionIds.length || decisionCompletionIds.some((id) => !ids.has(id))) return { applicable: true, valid: false, reason: 'invalid-decision-completion-set' };
  ...
  if (boundaryKind === 'none' && (completed.size !== rows.length || interruptedId)) return { applicable: true, valid: false, reason: 'incomplete-none-boundary' };
  ...
  else if (decisionCompletionIds.length) return { applicable: true, valid: false, reason: 'unexpected-decision-completion-set' };
  ...
  return {
    ...
    decision_completion_clause_ids: decisionCompletionIds,
    decision_completion_clause_set: decisionCompletionSet,
...
export function replaceStructuredEffectRows(turn = {}, field = '', nextRows = []) {
  ...
  acceptedRows.forEach((row, newIndex) => {
    const hasSource = Boolean(row && typeof row === 'object' && Object.prototype.hasOwnProperty.call(row, TIME_EFFECT_SOURCE));
...
export function rebaseStructuredEffectOwners(turn = {}) {
  const delta = object(turn?.state_delta), claim = object(turn?.time_execution);
  if (!Array.isArray(claim.effect_owners)) return turn;
  const sourceIndexes = new Map();
  for (const [field, rows] of Object.entries(delta)) {
    if (!Array.isArray(rows)) continue;
    const indexes = new Map(), tagged = rows.some((row) => row && typeof row === 'object' && Object.prototype.hasOwnProperty.call(row, TIME_EFFECT_SOURCE));
    rows.forEach((row, index) => {
      const hasSource = Boolean(row && typeof row === 'object' && Object.prototype.hasOwnProperty.call(row, TIME_EFFECT_SOURCE));
      const source = object(row)[TIME_EFFECT_SOURCE];
      if (Number.isInteger(source)) indexes.set(source, indexes.has(source) ? null : index);
      else if (!tagged && !hasSource) indexes.set(index, index);
    });
    sourceIndexes.set(field, indexes);
  }
  claim.effect_owners = claim.effect_owners.flatMap((rawOwner) => {
    const owner = object(rawOwner), field = String(owner.field || ''), oldIndex = Number(owner.effect_index);
    if (owner.scope !== 'state_delta' || owner.effect_index == null || !Number.isInteger(oldIndex) || !sourceIndexes.has(field)) return [rawOwner];
    const nextIndex = sourceIndexes.get(field).get(oldIndex);
    return Number.isInteger(nextIndex) ? [{ ...owner, effect_index:nextIndex }] : [];
  });
  return turn;
}

export function markStructuredEffectUnowned(row = {}) {
  if (!row || typeof row !== 'object' || Object.prototype.hasOwnProperty.call(row, TIME_EFFECT_SOURCE)) return row;
  Object.defineProperty(row, TIME_EFFECT_SOURCE, { value:null, enumerable:true, configurable:false, writable:false });
  return row;
}
...
export function structuredEffectRows(turn = {}, field = '') {
  ...
  rows.forEach((row, index) => {
    if (!row || typeof row !== 'object' || Object.prototype.hasOwnProperty.call(row, TIME_EFFECT_SOURCE)) return;
