...
const INSTRUCTOR_FEEDBACK_RE = /(?:교수|교관|선생|스승|강사)[^.!?\n]{0,32}(?:교정|지도|가르|시범|판정|평가)|(?:교정|지도|가르침)(?:을|를)?\s*받/i;
...
function hasSelfOwnedAction(segment, actionPattern) {
  ...
  return actionPattern.test(selfTail) && !THIRD_PARTY_LEARNING_ACTION_RE.test(selfTail) && !THIRD_PARTY_COMBAT_ACTION_RE.test(selfTail);
}
...
function hasPlayerOwnedLearningAction(value, max = 1800) {
  ...
  for (const rawSegment of text.split(/[.!?\n,;]+/)) {
    ...
    const hasThirdPartyAction = THIRD_PARTY_LEARNING_ACTION_RE.test(segment) || THIRD_PARTY_COMBAT_ACTION_RE.test(segment);
    ...
    if (OBSERVATION_RE.test(segment)) {
      ...
      if ((tail && actionPattern.test(tail) && !THIRD_PARTY_LEARNING_ACTION_RE.test(tail) && !THIRD_PARTY_COMBAT_ACTION_RE.test(tail)) || hasSelfAction || (directTail && actionPattern.test(directTail))) return true;
      continue;
    }
...
function hasPlayerAttributedSceneEvidence(value, max = 21_600) {
  ...
  for (const rawSegment of text.split(/[.!?\n,;]+/)) {
    ...
    if (THIRD_PARTY_LEARNING_ACTION_RE.test(segment) && !INSTRUCTOR_FEEDBACK_RE.test(segment) && !hasSelfAction) continue;
    return true;
  }
  return false;
}
