...
export function normalizeSkillCandidates(value = {}, { existingSkills = {}, maxCandidates = MAX_SKILL_CANDIDATES } = {}) {
  ...
  for (const [rawName, rawValue] of Object.entries(value && typeof value === 'object' && !Array.isArray(value) ? value : {})) {
    ...
    if (!validSkillName(name) || !identity || existing.has(identity) || seen.has(identity)) continue;
  ...
  rows.sort((a, b) => Number(b[1].updated_turn || 0) - Number(a[1].updated_turn || 0) || a[0].localeCompare(b[0], 'ko'));
  return Object.fromEntries(rows.slice(0, Math.max(0, Math.min(MAX_SKILL_CANDIDATES, Number(maxCandidates) || MAX_SKILL_CANDIDATES))));
}
