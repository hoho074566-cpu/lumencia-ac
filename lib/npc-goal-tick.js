...
function manifestedInTurn(turn,key,name,saveState){
  ...
  const previousRelationship=object(saveState?.relationships?.[key]);
  if(relationChanges.some(row=>String(row?.npc_key||row?.key||'')===key&&(Number(row?.affinity_delta||0)!==0||Number(row?.trust_delta||0)!==0||(Boolean(String(row?.status||'').trim())&&scalarDifferent(previousRelationship.status,row.status)))))return true;
