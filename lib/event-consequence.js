...
export function materializeDelayedConsequences({ rows = [], world = {}, advanceMinutes = 0, turnNumber = 0, existingHooks = [], sourceEvent = null } = {}) {
  const start = worldClock(world);
  const startMinutes = clockMinutes(start);
  if (startMinutes == null) return [];
  const endMinutes = startMinutes + Math.trunc(clamp(advanceMinutes, 0, 1440));
  const activeExisting = array(existingHooks).map(normalizeEventConsequenceHook).filter((row) => row && ACTIVE_STATUSES.has(row.status));
  const fingerprints = new Set(activeExisting.map((row) => row.fingerprint));
  const queueRoom = Math.max(0, 12 - activeExisting.length);
  const additions = [];

  for (const input of array(rows).slice(0, 6)) {
    if (additions.length >= Math.min(3, queueRoom)) break;
    ...
    const delayMinutes = Math.trunc(clamp(input?.delay_minutes, 1, 43200));
...
export function eventConsequenceEvidence(turn = {}, consequence = null) {
  ...
  const visible = [turn?.scene_title, turn?.scene_summary, ...array(turn?.scene).map((item) => item?.text), ...array(turn?.choices), ...array(delta.active_events_add), ...array(delta.completed_events_add), ...array(delta.pc_knowledge_add), ...array(delta.memories_add).map((item) => item?.fact)].filter(Boolean).join(' ').toLowerCase();
