...
import { TIME_EFFECT_SOURCE, markStructuredEffectUnowned, replaceStructuredEffectRows, structuredEffectRows } from './time-plan-reconciliation.js';
...
function carryEffectSource(source, target) {
  const index = object(source)[TIME_EFFECT_SOURCE];
  if (Number.isInteger(index)) Object.defineProperty(target, TIME_EFFECT_SOURCE, { value:index, enumerable:true, configurable:false, writable:false });
  return target;
}
...
export function reconcileEventConsequenceLifecycle({ saveState = {}, turn = {}, selectedConsequence = null } = {}) {
  const delta = object(turn?.state_delta);
  const rawUpdates = turn?.state_delta ? structuredEffectRows(turn, 'hooks_update') : [];
  ...
  const selectedSource = rawUpdates.find((patch) => String(patch?.id || '') === selectedId);
  const existingUpdates = rawUpdates.filter((patch) => patch?.id && String(patch.id) !== selectedId);
  const lifecycle = [];
  let status = 'idle';
  ...
  if (selected && ACTIVE_STATUSES.has(selected.status)) {
    ...
    if (evidence.realized) {
      lifecycle.push(carryEffectSource(selectedSource, { id:selected.id, status:'resolved', reason:`예약된 결과 발현: ${selected.event_name}`.slice(0, 300) }));
      status = 'resolved';
    } else {
      if (selected.status !== 'open') lifecycle.push(carryEffectSource(selectedSource, { id:selected.id, status:'open', reason:'발현 시각 도달; 장면 반영 대기' }));
      status = 'open';
    }
  }

  const expired = expiredEventConsequences(saveState).filter((row) => row.id !== selectedId).slice(0, 4);
  for (const row of expired) lifecycle.push(markStructuredEffectUnowned({ id:row.id, status:'expired', reason:'Event Consequence V1 bounded lifetime 종료' }));
  ...
  if (turn?.state_delta && typeof turn.state_delta === 'object') replaceStructuredEffectRows(turn, 'hooks_update', merged);
  return {
    version: EVENT_CONSEQUENCE_VERSION,
