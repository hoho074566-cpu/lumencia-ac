// LUMENSIA MOBILE V1.5.2 runtime patch loader
// Live V1.4.9 base head: hoho074566-cpu/lumencia-ac @ e3271ecbd4b24ab6ea72d129fb0914fcc3c8b393
...
// a small, auditable V1.5.2 delta at boot. If required source markers disappear, boot stops visibly.

const PATCH_VERSION = '1.5.2';
const BASE_APP_SHA = '39187078f0965cec067cc8dad2b38dd95351ef7b';
const LIVE_BASE_HEAD = 'e3271ecbd4b24ab6ea72d129fb0914fcc3c8b393';
const AUTO_GESTURE_PX = 84;

const FLOW_AUTO_ACTION = '[AUTO FLOW: PC 새 행동 없음]';
const FLOW_CONTINUE_ACTION = '[CONTINUE: 직전 GM 응답 이어쓰기]';

function portraitCandidatesV152(key, expression = 'default') {
  ...
  // V1.5.2: portrait/default까지 실패했을 때만 전신 DEFAULT를 마지막 안전망으로 사용.
...
function latestWorldRecordV152() {
  return [...(save.renderedTurns || [])].reverse().find((record) => record?.turn && !record?.meta) || null;
}

function canAutoFlowV152() {
  const latest = latestWorldRecordV152();
...
function canContinueV152() {
  const latest = latestWorldRecordV152();
  return Boolean(latest && !busy && !metaModeOnce);
}

function renderFlowControlsV152() {
  let wrap = $('flowControlsV152');
  const latest = latestWorldRecordV152();

  if (!wrap) {
    wrap = document.createElement('section');
    wrap.id = 'flowControlsV152';
    wrap.className = 'flow-controls-v152';

    const auto = document.createElement('button');
    auto.id = 'autoFlowBtnV152';
    auto.type = 'button';
    auto.className = 'flow-btn-v152 auto';
    auto.textContent = '▶ 자동 진행';
    auto.addEventListener('click', () => sendAction('', 'auto'));

    const cont = document.createElement('button');
    cont.id = 'continueBtnV152';
    cont.type = 'button';
    cont.className = 'flow-btn-v152 continue';
  ...
  const auto = $('autoFlowBtnV152');
  const cont = $('continueBtnV152');
  if (auto) {
    auto.disabled = !canAutoFlowV152();
...
function installAutoFlowGestureV152() {
  if (!$('v152FlowStyle')) {
    const style = document.createElement('style');
    style.id = 'v152FlowStyle';
    style.textContent = `
      .flow-controls-v152{display:flex;gap:8px;padding:8px 12px 14px;align-items:center;justify-content:flex-end}
      .flow-controls-v152.hidden{display:none}
      .flow-btn-v152{appearance:none;border:1px solid rgba(148,163,184,.26);border-radius:999px;background:rgba(30,41,59,.82);color:#e5e7eb;padding:9px 13px;font-size:12px;font-weight:800;letter-spacing:-.01em}
      .flow-btn-v152.continue{border-color:rgba(217,184,108,.38);color:#f4d995}
      .flow-btn-v152:disabled{opacity:.34;filter:saturate(.4)}
      #autoPullHintV152{position:fixed;z-index:98;left:50%;bottom:104px;transform:translate(-50%,18px);opacity:0;pointer-events:none;padding:7px 11px;border-radius:999px;background:rgba(15,23,42,.94);border:1px solid rgba(148,163,184,.24);font-size:11px;font-weight:800;color:#dbeafe;transition:opacity .12s ease,transform .12s ease}
      #autoPullHintV152.show{opacity:1;transform:translate(-50%,0)}
      #autoPullHintV152.ready{color:#fde68a;border-color:rgba(245,158,11,.5)}
    `;
    document.head.append(style);
  }

  let hint = $('autoPullHintV152');
  if (!hint) {
    hint = document.createElement('div');
    hint.id = 'autoPullHintV152';
    document.body.append(hint);
  }

  actionInput.addEventListener('input', renderFlowControlsV152);
  choicesEl.addEventListener('click', () => setTimeout(renderFlowControlsV152, 0));
  ...
  window.addEventListener('touchstart', (event) => {
    ...
    if (!touch || !atBottom() || !canAutoFlowV152()) return reset();
    eligible = true;
  ...
  window.addEventListener('touchend', () => {
    const fire = eligible && pull >= AUTO_GESTURE_PX && canAutoFlowV152();
    reset();
    if (fire) sendAction('', 'auto');
  }, { passive: true });

  window.addEventListener('touchcancel', reset, { passive: true });
  renderFlowControlsV152();
}

function renderChoicesV152(choices) {
  choicesEl.innerHTML = '';
...
function updateStatusV152(route) {
...
function mergeContinuationIntoRecentV152(turn) {
...
function accumulateUsageV152(usage) {
  if (!usage) return;
...
function applyRuntimeStateV152(data, isContinue = false) {
...
async function sendActionV152(action, requestedMode = null) {
  action = String(action || '').trim();
  const requested = ['auto', 'continue'].includes(requestedMode) ? requestedMode : null;
  if (busy) return;
  if (requested === 'auto' && !canAutoFlowV152()) {
    renderFlowControlsV152();
    return;
  }
  if (requested === 'continue' && !canContinueV152()) {
    renderFlowControlsV152();
    return;
  }
  ...
  const apiAction = isMeta
    ...
    : isAuto
      ? FLOW_AUTO_ACTION_V152
      : isContinue
        ? FLOW_CONTINUE_ACTION_V152
        : action;
  ...
  try {
    ...
    if (settings.demoMode) {
      ...
      data.route = { ...(data.route || {}), input_mode: inputMode, adapter_version: 'demo-v152' };
      if (isContinue && data.turn) {
        data.turn.state_delta = zeroStateDeltaV152();
      ...
      const res = await fetch('/api/chat-v152', {
        method: 'POST',
    ...
    if (!isMeta && !isContinue) {
      ...
      save.recentTurns.push({
        action: isAuto ? FLOW_AUTO_ACTION_V152 : apiAction,
      ...
      mergeContinuationIntoRecentV152(data.turn);
    }

    if (!isMeta) applyRuntimeStateV152(data, isContinue);

    save.renderedTurns.push(record);
    save.renderedTurns = save.renderedTurns.slice(-80);
    accumulateUsageV152(data.usage);
...
async function checkHealthV152() {
  try {
    const r = await fetch('/api/health', { cache: 'no-store' });
    const h = await r.json();
    $('apiHealth').textContent = h.apiConfigured
      ? `API ${h.version || '?'} 연결 준비됨 · ${h.luna} / ${h.terra}${h.qualityPipeline ? ' · Q3' : ''}${h.accessTokenRequired ? ' · 접속 토큰 필요' : ''}`
...
function replaceOnce(source, needle, replacement, label) {
  ...
  if (first < 0) throw new Error(`V1.5.2 patch marker missing: ${label}`);
  if (source.indexOf(needle, first + needle.length) >= 0) throw new Error(`V1.5.2 patch marker duplicated: ${label}`);
...
function replaceRegexOnce(source, regex, replacement, label) {
  ...
  if (matches.length !== 1) throw new Error(`V1.5.2 regex marker ${label}: expected 1, got ${matches.length}`);
...
function showBootError(error, phase = 'patch') {
  ...
  box.textContent = `LUMENSIA V1.5.2 ${phase} 실패\n${error?.message || error}\nBase ${BASE_APP_SHA.slice(0, 10)}`;
...
async function boot() {
  ...
  try {
    ...
    source = replaceOnce(
      ...
      `import { ASSETS } from '${location.origin}/assets.js?v=152';`,
      'ASSETS import'
    );
    ...
    source = replaceRegexOnce(
      ...
      `${renameFunction(portraitCandidatesV152, 'portraitCandidatesV152', 'portraitCandidates')}\n\nfunction assetUrl`,
      'portraitCandidates'
    );
    ...
    source = replaceRegexOnce(
      ...
      `${renameFunction(compactStateV152, 'compactStateV152', 'compactState')}\n\nasync function sendAction`,
    ...
    source = replaceRegexOnce(
      ...
      `${renameFunction(renderChoicesV152, 'renderChoicesV152', 'renderChoices')}\n\nfunction updateStatus`,
      'renderChoices'
    );
    ...
    source = replaceRegexOnce(
      ...
      `${renameFunction(updateStatusV152, 'updateStatusV152', 'updateStatus')}\n\nfunction renderInfo`,
      'updateStatus'
    );

    const helperSource = [
      `const AUTO_GESTURE_PX = ${AUTO_GESTURE_PX};`,
      `const FLOW_AUTO_ACTION_V152 = ${JSON.stringify(FLOW_AUTO_ACTION)};`,
      `const FLOW_CONTINUE_ACTION_V152 = ${JSON.stringify(FLOW_CONTINUE_ACTION)};`,
      latestWorldRecordV152.toString(),
      canAutoFlowV152.toString(),
      canContinueV152.toString(),
      renderFlowControlsV152.toString(),
      installAutoFlowGestureV152.toString(),
      mergeContinuationIntoRecentV152.toString(),
      accumulateUsageV152.toString(),
      zeroStateDeltaV152.toString(),
      compactStateV152.toString(),
      applyRuntimeStateV152.toString(),
    ].join('\n\n');
    ...
    source = replaceRegexOnce(
      ...
      `${renameFunction(sendActionV152, 'sendActionV152', 'sendAction')}\n\nfunction demoResponse`,
      'sendAction'
    );
    ...
    source = replaceRegexOnce(
      ...
      `${renameFunction(checkHealthV152, 'checkHealthV152', 'checkHealth')}\n\n// ===== characters-v2 asset audit: BEGIN =====`,
      'checkHealth'
    );
    ...
    source = replaceOnce(
      ...
      'refreshScheduleContext(); updateForceTerraButton(); checkHealth(); installAutoFlowGestureV152(); renderAll(); renderFlowControlsV152();',
      'boot hook'
    );
  } catch (error) {
